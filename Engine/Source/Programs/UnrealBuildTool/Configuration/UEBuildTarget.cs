...
#pragma warning restore IDE0051
	...
	internal class UnrealTargetPlatformJsonConverter : JsonConverter<UnrealTargetPlatform>
	{
		...
		public override void WriteAsPropertyName(Utf8JsonWriter writer, UnrealTargetPlatform value, JsonSerializerOptions options)
		{
	...
	/// Platform groups
	/// </summary>
	public partial struct UnrealPlatformGroup : IEquatable<UnrealPlatformGroup>
	{
		#region Private/boilerplate
		// internal concrete name of the group
		private int Id;
		...
		/// <inheritdoc/>
		public readonly bool Equals(UnrealPlatformGroup other) => this == other;
	...
	public partial struct UnrealArch : ISerializable, IEquatable<UnrealArch>
	{
		...
		private static readonly Dictionary<int, bool> IsX64Map = new();
	...
	public class UnrealArchitectures
	{
		...
		public UnrealArch SingleArchitecture
		{
			get
			{
				...
				{
					throw new Exception($"Asking CppCompileEnvironment for a single Architecture, but it has multiple Architectures ({String.Join(", ", Architectures)}). This indicates logic needs to be updated");
				}
	...
	public enum UnrealIntermediateEnvironment
	{
		...
		/// <summary>
		/// Profiling compilation can modify a number of compilation settings.
		/// </summary>
		ProfilingCompile,
	}
	...
	class UEBuildTarget
	{
		...
		{
			(TargetRules RulesObject, RulesAssembly RulesAssembly) = CreateRulesObject(Descriptor, bSkipRulesCompile, bForceRulesCompile, bUsePrecompiled, IntermediateEnvironment, Logger);
		...
		internal static (TargetRules, RulesAssembly) CreateRulesObject(TargetDescriptor Descriptor, bool bSkipRulesCompile, bool bForceRulesCompile, bool bUsePrecompiled, UnrealIntermediateEnvironment IntermediateEnvironment, ILogger Logger)
		{
			// make sure we are allowed to build this platform
			if (!UEBuildPlatform.IsPlatformAvailable(Descriptor.Platform))
			{
				throw new BuildException("{0}", UEBuildPlatform.GetPlatformUnavailableReason(Descriptor.Platform));
			}
			...
			{
				using ITimelineEvent _ = Timeline.ScopeEvent("CreateTargetRules");
			...
			// Force shared PCHs off with delegate lifetime, as this is tricky to find out otherwise
			if (RulesObject.bCheckDelegateLifetime)
			{
				RulesObject.bUseSharedPCHs = false;
			}
			...
			return (RulesObject, RulesAssembly);
		}
		...
		/// </summary>
		private readonly Dictionary<string, UEBuildModule> Modules = new(StringComparer.InvariantCultureIgnoreCase);

		private readonly Dictionary<string, UEBuildModule> UsedShortNames = new(StringComparer.InvariantCultureIgnoreCase);
		...
		/// </summary>
		/// <returns></returns>
		public static bool BuiltWithOverriddenSDKs()
		{
		...
		internal UEBuildTarget(TargetDescriptor InDescriptor, ReadOnlyTargetRules InRules, RulesAssembly InRulesAssembly)
		{
			...
			if (ProjectFile != null)
			{
				...
				if (Rules.Type != TargetType.Program || !String.Equals(ProjectFile.GetFileNameWithoutAnyExtensions(), Rules.Name, StringComparison.OrdinalIgnoreCase))
				{
			...
			if (!bUseSharedBuildEnvironment)
			{
				...
				{
					EngineIntermediateDirectory = DirectoryReference.Combine(OutputRootDirectory, PlatformIntermediateFolder, GetTargetIntermediateFolderName(TargetName, IntermediateEnvironment), UnrealTargetConfiguration.Development.ToString());
				}
				else
				{
		...
		public static string GetTargetIntermediateFolderName(string TargetName, UnrealIntermediateEnvironment IntermediateEnvironment)
		{
			...
			switch (IntermediateEnvironment)
			{
				...
				case UnrealIntermediateEnvironment.GenerateProjectFiles:
					...
					break;
				case UnrealIntermediateEnvironment.ProfilingCompile:
					TargetFolderName += "Prof";
					break;
			}
		...
		public static string GetAppName(TargetRules Rules)
		{
			if (Rules.BuildEnvironment == TargetBuildEnvironment.Shared && Rules.Type != TargetType.Program)
			{
				return GetAppNameForTargetType(Rules.Type);
			}
			return Rules.Name;
		}
		...
		void WriteDependencyList(FileReference Location, List<RuntimeDependency> RuntimeDependencies, Dictionary<FileReference, FileReference> RuntimeDependencyTargetFileToSourceFile, ILogger Logger)
		{
			...
			foreach (UEBuildModule Module in Modules)
			{
				...
				SystemLibraryPaths.AddRange(Rules.PublicSystemLibraryPaths.Where(x => !x.StartsWith("$(", StringComparison.Ordinal)).Select(x => Path.GetFullPath(x.Replace('/', Path.DirectorySeparatorChar))));
				...
				foreach (string IncludePath in AllIncludePaths.Where(x => !x.StartsWith("$(", StringComparison.Ordinal)))
				{
					if (Directory.Exists(IncludePath))
					{
						foreach (string IncludeFileName in Directory.EnumerateFiles(IncludePath, "*", SearchOption.AllDirectories))
						{
							string Extension = Path.GetExtension(IncludeFileName).ToUpperInvariant();
		...
		private static void ResolveLibraryName(string LibraryPath, string LibraryName, string LibraryExtension, HashSet<FileReference> Files)
		{
		...
		/// <summary>
		/// Combines multiple manifests into one. Target manifest can be included in the merge sources
		/// </summary>
		internal static void CombineManifests(FileReference TargetManifestPath, IEnumerable<FileReference> SourceManifestPaths, ILogger Logger)
		{
			HashSet<string> BuildProducts = [];
			HashSet<string> DeployTargetFiles = [];

			foreach (FileReference SourceManifestPath in SourceManifestPaths)
			{
				BuildManifest SourceManifest = Utils.ReadClass<BuildManifest>(SourceManifestPath.FullName, Logger);
				if (SourceManifest != null)
				{
					BuildProducts.AddRange(SourceManifest.BuildProducts);
					DeployTargetFiles.AddRange(SourceManifest.DeployTargetFiles);
				}
			}

			BuildManifest Manifest = new BuildManifest();
			Manifest.BuildProducts.AddRange(BuildProducts);
			Manifest.DeployTargetFiles.AddRange(DeployTargetFiles);

			Logger.LogInformation("Writing merged manifest to {ManifestPath}", TargetManifestPath);
			Utils.WriteClass<BuildManifest>(Manifest, TargetManifestPath.FullName, "", Logger);
		}
					...
					if (Binary.Type == UEBuildBinaryType.DynamicLinkLibrary)
					{
						...
						string GetFilenameFromLibraryName(string LibraryName)
						{
							...
							IEnumerable<UEBuildBinary> Candidates = Binaries.Where(x =>
								x.Type == UEBuildBinaryType.DynamicLinkLibrary && x.OutputFilePath.GetFileNameWithoutAnyExtensions().Contains(FilenameFragment, StringComparison.Ordinal));
		...
		TargetReceipt PrepareReceipt(UEToolChain ToolChain, List<KeyValuePair<FileReference, BuildProductType>> BuildProducts, List<RuntimeDependency> RuntimeDependencies)
		{
			...
			if (!Rules.bShouldCompileAsDLL)
			{
				...
				foreach (KeyValuePair<FileReference, BuildProductType> Pair in BuildProducts)
				{
					...
					{
						if (System.IO.Path.GetFileNameWithoutExtension(Pair.Key.FullName).EndsWith("-Cmd", System.StringComparison.Ordinal) && Receipt.LaunchCmd == null)
						{
		...
		static TargetReceipt? PrepareAndWritePartialReceipt(TargetDescriptor Descriptor, TargetRules RulesObject, RulesAssembly RulesAssembly, ILogger Logger)
		{
			try
			{
				UEBuildTarget incompleteTarget = new UEBuildTarget(Descriptor, new ReadOnlyTargetRules(RulesObject), RulesAssembly);
		...
		static HashSet<UEBuildModuleCPP> GatherDependencyModules(List<UEBuildBinary> Binaries)
		{
			...
			Parallel2.ForEach(Binaries, Binary =>
			{
				IEnumerable<UEBuildModule> DependencyModules = Binary.GetAllDependencyModules(bIncludeDynamicallyLoaded: false, bForceCircular: false);
		...
		public CppCompileEnvironment CreateCompileEnvironmentForProjectFiles(ILogger Logger, NullActionGraphBuilder? builder = null)
		{
			...
			CreateSharedPCHInstances(Rules, TargetToolChain, Binaries, GlobalCompileEnvironment, builder, Logger);
		...
		public async Task<TargetMakefile> BuildAsync(BuildConfiguration BuildConfiguration, ISourceFileWorkingSet WorkingSet, TargetDescriptor TargetDescriptor, ILogger Logger, bool bInitOnly = false)
		{
			...
			(TargetMakefile Makefile, Task? UHTTimestampTask) = BuildMakefile(BuildConfiguration, WorkingSet, TargetDescriptor, Logger, bInitOnly);
			if (UHTTimestampTask is not null)
			{
				await UHTTimestampTask;
			}
			return Makefile;

		}

		public (TargetMakefile, Task?) BuildMakefile(BuildConfiguration BuildConfiguration, ISourceFileWorkingSet WorkingSet, TargetDescriptor TargetDescriptor, ILogger Logger, bool bInitOnly = false)
		{
			SourceFileMetadataCache cache = SourceFileMetadataCache.CreateHierarchy(ProjectFile, Logger);
			BuildContext context = BuildMakefile1(BuildConfiguration, TargetDescriptor, Logger, cache);

			// UHT mode uses this to create the makefile at least to the point where UHT would have valid manifest information
			if (bInitOnly)
			{
				return (context.Makefile, null);
			}
			...
			CodeGenContext codeGenContext = new CodeGenContext()
			{
				...
				GenerateHeaderFuncs = context.GenerateHeaderFuncs,
			};

			RunCodeGen(BuildConfiguration, [codeGenContext], Logger);

			BuildMakefile2(WorkingSet, Logger, context, new(false, true));

			codeGenContext.uhtRun?.Wait();

			return (context.Makefile, codeGenContext.uhtTimestamp);
		}

		internal struct GenerateHeaderFunc
		{
			internal DirectoryReference Dir;
			internal Action<ILogger, DirectoryReference> Func;
		}

		internal class BuildContext
		{
			internal required SourceFileMetadataCache MetadataCache;
			internal required TargetDescriptor Descriptor;
			internal required TargetMakefile Makefile;
			internal required List<UEBuildBinary> OriginalBinaries;
			internal required HashSet<UEBuildModuleCPP> ModulesToGenerateHeadersFor;
			internal required CppCompileEnvironment GlobalCompileEnvironment;
			internal required UEToolChain TargetToolChain;
			internal required LinkEnvironment GlobalLinkEnvironment;
			internal required List<GenerateHeaderFunc> GenerateHeaderFuncs;
		}

		internal BuildContext BuildMakefile1(BuildConfiguration BuildConfiguration, TargetDescriptor TargetDescriptor, ILogger Logger, SourceFileMetadataCache MetadataCache)
		{
			CppConfiguration CppConfiguration = GetCppConfiguration(Configuration);
			CppCompileEnvironment GlobalCompileEnvironment = new CppCompileEnvironment(Platform, CppConfiguration, Architectures, MetadataCache);
			LinkEnvironment GlobalLinkEnvironment = new LinkEnvironment(GlobalCompileEnvironment.Platform, GlobalCompileEnvironment.Configuration, GlobalCompileEnvironment.Architectures);
			...
			if (ShouldCompileMonolithic())
			{
				...
				IEnumerable<UEBuildModule> AllReferencedModules = ExecutableBinary.GetAllDependencyModules(bIncludeDynamicallyLoaded: true, bForceCircular: true);
			...
			// Cache module dependencies for StableTopologicalSort
			UEBuildModule.CacheDependenciesForStableTopologicalSort(ModulesToGenerateHeadersFor);

			// Prepare cached data for UHT header generation
			using (GlobalTracer.Instance.BuildSpan("UHTExecution.SetupUObjectModules()").StartActive())
			{
				using ITimelineEvent _ = Timeline.ScopeEvent("UHTModules");

				UHTExecution.SetupUObjectModules(
			...
			bool bEnabledUHTInputCache = BuildConfiguration.bEnableUHTInputCache || Unreal.IsBuildMachine();
			Makefile.UHTTargetSettings.EnableInputCacheRead = bEnabledUHTInputCache && !BuildConfiguration.bForceHeaderGeneration;
			Makefile.UHTTargetSettings.EnableInputCacheWrite = bEnabledUHTInputCache;

#if __VPROJECT_AVAILABLE__
			{
				using ITimelineEvent _ = Timeline.ScopeEvent("VNIModules");
			...
			// Collect generate header funcs
			List<GenerateHeaderFunc> GenerateHeaderFuncs = [];
			foreach (UEBuildModuleCPP Module in Modules.Values.OfType<UEBuildModuleCPP>())
			{
				foreach ((string Subdirectory, Action<ILogger, DirectoryReference> Func) in Module.Rules.GenerateHeaderFuncs)
				{
					...
					GenerateHeaderFuncs.Add(new() { Dir = GenDirectory, Func = Func });
				}
			}
			...
			return new BuildContext()
			{
				...
				Descriptor = TargetDescriptor,
				Makefile = Makefile,
				OriginalBinaries = OriginalBinaries,
				ModulesToGenerateHeadersFor = ModulesToGenerateHeadersFor,
				GlobalCompileEnvironment = GlobalCompileEnvironment,
				TargetToolChain = TargetToolChain,
				GlobalLinkEnvironment = GlobalLinkEnvironment,
				GenerateHeaderFuncs = GenerateHeaderFuncs,
			};
		}
		...
		internal class CodeGenContext
		{
			internal required TargetDescriptor Descriptor;
			internal required TargetMakefile Makefile;
			internal required string TargetName;
			internal required List<GenerateHeaderFunc> GenerateHeaderFuncs;
			internal Task? uhtRun;
			internal Task? uhtTimestamp;
			internal Task? vniRun;
		}

		internal static void RunCodeGen(BuildConfiguration BuildConfiguration, List<CodeGenContext> contexts, ILogger Logger)
		{
			if (contexts.Count == 0)
			{
				return;
			...
			CodeGenContext? lastRunContext = null;
			Dictionary<string, UHTModuleInfo>? lastRunUObjectModules = null;

			string GetTargetName(TargetDescriptor desc) => $"{desc.Name}-{desc.Platform}-{desc.Configuration}";

			foreach (CodeGenContext context in contexts)
			{
				TargetMakefile makefile = context.Makefile;

				foreach (GenerateHeaderFunc entry in context.GenerateHeaderFuncs)
				{
					DirectoryReference.CreateDirectory(entry.Dir);
					entry.Func(Logger, entry.Dir);
				}

				if (lastRunContext != null)
				{
					bool rerunCodeGen = false;

					if (context.TargetName != lastRunContext.TargetName)
					{
						Logger.LogDebug("Different targetname!.. will rerun code gen!");
						rerunCodeGen = true;
					}

					if (!rerunCodeGen)
					{
						if (lastRunUObjectModules == null)
						{
							lastRunUObjectModules = lastRunContext.Makefile.UObjectModules.ToDictionary(k => k.ModuleName, k => k);
						}

						foreach (UHTModuleInfo module in makefile.UObjectModules)
						{
							if (lastRunUObjectModules.TryGetValue(module.ModuleName, out UHTModuleInfo? lastModule))
							{
								int CountHeaders(UHTModuleInfo info) => info.PublicUObjectClassesHeaders.Count + info.PublicUObjectHeaders.Count + info.InternalUObjectHeaders.Count + info.PrivateUObjectHeaders.Count;
								if (CountHeaders(module) != CountHeaders(lastModule))
								{
									Logger.LogDebug(
										"Target {CurrentTarget} module {Module} has a different number of headers than previous target {PreviousTarget}, will rerun code gen",
										GetTargetName(context.Descriptor),
										module.ModuleName,
										GetTargetName(lastRunContext.Descriptor));
									rerunCodeGen = true;
									break;
								}
							}
							...
							{
								Logger.LogDebug(
									"Target {CurrentTarget} has uht module {Module} that previous target {PreviousTarget} did not have, will rerun code gen",
									GetTargetName(context.Descriptor),
									module.ModuleName,
									GetTargetName(lastRunContext.Descriptor));
								rerunCodeGen = true;
					...
					if (!rerunCodeGen)
					{
						HashSet<string>? lastRunVNIModules = new(lastRunContext.Makefile.VNIModules.Select(m => m.ModuleName));

						foreach (VNIModuleInfo module in makefile.VNIModules)
						{
							if (!lastRunVNIModules.Contains(module.ModuleName))
							{
								Logger.LogDebug(
									"Target {CurrentTarget} has vni modules that previous target {PreviousTarget} did not have, will rerun code gen",
									GetTargetName(context.Descriptor),
									GetTargetName(lastRunContext.Descriptor));
								rerunCodeGen = true;
								break;
				...
				// This should be very rare to rerun code gen.. can be reproed with having for example both these being built in one go:
				// -Target="UnrealGame Win64 Development"
				// -Target="UnrealGame Win64 DebugGame -allmodules"

				if (lastRunContext != null)
				{
					lastRunContext.uhtRun?.Wait();
					lastRunContext.uhtTimestamp?.Wait();
#if __VPROJECT_AVAILABLE__
					lastRunContext.vniRun?.Wait();
#endif
				}

				TargetDescriptor descriptor = context.Descriptor;

				if (makefile.UObjectModules.Count > 0)
				{
					// Because UHT execution runs alongside building the rest of the makefile, I suspect we need to pre-populate the cache
					// for the makefile generation to correctly pick up source files.
					UHTExecution.PrepopulateDirectoryCache(makefile.UObjectModules);
					(Task? uhtRun, Task? uhtTimestamp) = UHTExecution.ExecuteHeaderToolIfNecessary(BuildConfiguration, descriptor.ProjectFile, makefile, context.TargetName!, Logger);
					context.uhtRun = uhtRun;
					context.uhtTimestamp = uhtTimestamp;
				}

#if __VPROJECT_AVAILABLE__
				if (makefile.VNIModules.Count > 0)
				{
					VNIExecution.PrepopulateDirectoryCache(makefile);
					context.vniRun = Task.Run(() => VNIExecution.ExecuteVNITool(makefile, descriptor, Logger));
				}
#endif
		...
		internal static void WaitCodeGen(List<CodeGenContext> contexts, bool prefetchAfterReset)
		{
			foreach (CodeGenContext context in contexts)
			{
				if (context.uhtRun != null)
				{
					UHTExecution.WaitForExecution(context.uhtRun, context.Makefile);

					if (prefetchAfterReset)
					{
						Task task = FileMetadataPrefetch.QueueDirectoryTrees(context.Makefile.UObjectModules.Select(m => m.GeneratedCodeDirectory));
					}
				}
#if __VPROJECT_AVAILABLE__
				if (context.vniRun != null)
				{
					VNIExecution.WaitForExecution(context.vniRun, context.Makefile);
				}
#endif
			}
		}

		internal static List<Task> GetTimestampTasks(List<CodeGenContext> contexts)
		{
			List<Task> timestampTasks = [];
			foreach (CodeGenContext codeGenContext in contexts)
			{
				if (codeGenContext.uhtTimestamp != null)
				{
					timestampTasks.Add(codeGenContext.uhtTimestamp);
				}
			}
			return timestampTasks;
		}

		internal void BuildMakefile2(ISourceFileWorkingSet WorkingSet, ILogger Logger, BuildContext context, FileHashCache fileHashCache, IEnumerable<FileReference>? customManifestFileNames = null)
		{
			SourceFileMetadataCache MetadataCache = context.MetadataCache;
			TargetDescriptor TargetDescriptor = context.Descriptor;
			TargetMakefile Makefile = context.Makefile;
			List<UEBuildBinary> OriginalBinaries = context.OriginalBinaries;
			HashSet<UEBuildModuleCPP> ModulesToGenerateHeadersFor = context.ModulesToGenerateHeadersFor;
			CppCompileEnvironment GlobalCompileEnvironment = context.GlobalCompileEnvironment;
			UEToolChain TargetToolChain = context.TargetToolChain;
			LinkEnvironment GlobalLinkEnvironment = context.GlobalLinkEnvironment;

			TargetMakefileBuilder MakefileBuilder = new TargetMakefileBuilder(Makefile, fileHashCache, Logger);
			...
			if (Platform.IsInGroup(UnrealPlatformGroup.Windows) && Rules.WindowsPlatform.bSharedResourceFiles)
			{
				...
				{
					CppCompileEnvironment DefaultResourceCompileEnvironment = new(GlobalCompileEnvironment);
					LinkEnvironment DefaultResourceLinkEnvironment = new(GlobalLinkEnvironment);
					DefaultResourceCompileEnvironment.bIsBuildingDLL = true;
					DefaultResourceLinkEnvironment.bIsBuildingDLL = true;

					FileItem DefaultResourceFile = FileItem.GetItemByFileReference(DefaultResourceLocation);

					CPPOutput DefaultResourceOutput = TargetToolChain.CompileRCFiles(DefaultResourceCompileEnvironment, [DefaultResourceFile], EngineIntermediateDirectory, MakefileBuilder);
					GlobalLinkEnvironment.DefaultResourceFiles.AddRange(DefaultResourceOutput.ObjectFiles);
					DefaultResourceOutput = TargetToolChain.ConvertRCFiles(DefaultResourceLinkEnvironment, DefaultResourceOutput.ObjectFiles, EngineIntermediateDirectory, MakefileBuilder);
					GlobalLinkEnvironment.DefaultConvertedResourceFiles.AddRange(DefaultResourceOutput.ObjectFiles);
				}
			}
			...
			{
				IEnumerable<FileItem> Items = Module.CopyDebuggerVisualizers(TargetToolChain, MakefileBuilder);
				MakefileBuilder.AddOutputFiles(Items);
			}

			// Cache module dependencies for StableTopologicalSort
			UEBuildModule.CacheDependenciesForStableTopologicalSort(Modules.Values);
			...
			using (GlobalTracer.Instance.BuildSpan("UEBuildBinary.Build()").StartActive())
			{
				Parallel2.ForEach(Binaries, Binary =>
				{
					...
					MakefileBuilder.AddOutputFiles(BinaryOutputItems);
				});
			}

			if (Rules.bStripExports)
			{
				CreateStripExportsActions(TargetToolChain, MakefileBuilder, GlobalCompileEnvironment);
			}
			...
			TargetToolChain.FinalizeOutput(Rules, MakefileBuilder, ExeDir);
			...
			if (!Rules.bDisableLinking)
			{
				...
				if (VersionFile == null)
				{
					...
					FileReference TargetInfoFile = FileReference.Combine(ProjectIntermediateDirectory, "TargetMetadata.json");

					// Make a copy of this since OutputItems might be added byh post build actions
					List<FileItem> TargetPrereqItems = new (Makefile.OutputItems);

					writeMetadataTasks.Add(Task.Run(() =>
					{
				...
				else
				{
					...
					WriteMetadataTargetInfo EngineInfo = new WriteMetadataTargetInfo(null, VersionFile, Receipt.Version, null, null, EngineFileToManifest, null, Rules.bMergeModules);
						FileReference EngineInfoFile = FileReference.Combine(ProjectIntermediateDirectory, "EngineMetadata.json");
						string EngineInfoName = TargetName.Equals(VersionFile.GetFileNameWithoutExtension(), StringComparison.Ordinal) ? VersionFile.GetFileName() : $"{VersionFile.GetFileName()} ({TargetName})";
			...
			foreach (FileReference ManifestFileName in customManifestFileNames ?? Rules.ManifestFileNames)
			{
			...
			if (!Rules.bDisableLinking)
			{
				...
				{
					List<DirectoryReference> RootDirectories = [Unreal.EngineDirectory];
			...
			// Check for referencing modules prohibited by the EULA
			...
			Makefile.ExternalDependencies.UnionWith(ConfigCache.ReadHierarchy(ConfigHierarchyType.Encryption, Rules.ProjectFile?.Directory, Rules.Platform).FileReferences.Select(FileItem.GetItemByFileReference));
			Makefile.ExternalDependencies.UnionWith(ConfigCache.ReadHierarchy(ConfigHierarchyType.Crypto, Rules.ProjectFile?.Directory, Rules.Platform).FileReferences.Select(FileItem.GetItemByFileReference));
			...
			Task.WaitAll(writeMetadataTasks.ToArray());

			// Wait for makefile tasks to finish (tasks writing files to disk etc)
			{
				using ITimelineEvent _ = Timeline.ScopeEvent("WaitOnWrites");
		...
		void CreateStripExportsActions(UEToolChain TargetToolChain, TargetMakefileBuilder MakefileBuilder, CppCompileEnvironment CompileEnvironment)
		{
...
#pragma warning restore CA1308 // Normalize strings to uppercase
			...
			SortedSet<FileItem> allExis = new(allObjects.Select(x => FileItem.GetItemByFileReference(x.Location.ChangeExtension(".exi"))), FileItem.SortByPath);

			void CreateGenerateExportsAction(string Name, UEBuildBinaryType binaryType, IEnumerable< FileItem> toStrip, DirectoryReference IntermediateDirectory)
			{
				SortedSet<FileItem> toStripExis = new(toStrip.Select(x => FileItem.GetItemByFileReference(x.Location.ChangeExtension(".exi"))), FileItem.SortByPath);
				SortedSet<FileItem> otherExis = new(allExis, FileItem.SortByPath);
				otherExis.ExceptWith(toStripExis);

				string Normalize(FileReference file) => TargetToolChain.NormalizeCommandLinePath(file, CompileEnvironment.RootPaths);
				...
				[
					.. toStripExis.Select(path => $"/S:{Normalize(path.Location)}"),
					.. otherExis.Select(path => $"/D:{Normalize(path.Location)}"),
					$"/O:{Normalize(outputFile)}",
					$"/T:{Platform}",
					$"/M:{Name}",
				...
				exportsAction.CommandArguments = $"@\"{Normalize(exportsRsp.Location)}\"";
				...
				MakefileBuilder.AddOutputFiles(exportsAction.ProducedItems);
			}

			foreach (UEBuildBinary Binary in Binaries.Where(x => x.bStripUnusedExports))
			{
				CreateGenerateExportsAction(Binary.OutputFilePaths.First().GetFileNameWithoutExtension(), Binary.Type, Binary.InputObjects, Binary.IntermediateDirectory);
			}
		}
		...
		void CreateWriteMetadataAction(TargetMakefileBuilder MakefileBuilder, string StatusDescription, FileReference InfoFile, WriteMetadataTargetInfo Info, IEnumerable<FileItem> PrerequisiteItems)
		{
			...
			if (!ProducedItems.Any(x => Unreal.IsFileInstalled(x)))
			{
				...
				byte[] InfoBytes = JsonSerializer.SerializeToUtf8Bytes(Info, WriteMetadataMode.SerializerOptions);
				DirectoryReference.CreateDirectory(InfoFile.Directory);
				FileReference.WriteAllBytesIfDifferent(InfoFile, InfoBytes);

				InfoFileItem.ResetCachedInfo();
				MakefileBuilder.AddInternalDependency(InfoFileItem);
				...
				MakefileBuilder.AddOutputFiles(WriteMetadataAction.ProducedItems);
			}
		}
		...
		public DirectoryReference GetExecutableDir()
		{
			...
			if (Platform == UnrealTargetPlatform.Mac && ExeDir.FullName.EndsWith(".app/Contents/MacOS", System.StringComparison.Ordinal))
			{
		...
		/// <summary>
		/// Cleans any stale modules that have changed output folder.
		/// 
		...
		void CleanStaleModules(ILogger logger)
		{
			...
			HashSet<FileReference> outputFiles = [.. Binaries.SelectMany(x => x.OutputFilePaths)];

			// Build a map of base filenames to their full path
			Dictionary<string, FileReference> outputNameToLocation = new(StringComparer.InvariantCultureIgnoreCase);
			...
			Parallel2.ForEach(outputFiles.Select(x => x.Directory).Distinct().Where(DirectoryReference.Exists), outputDirectory =>
			{
		...
		private void CheckForEULAViolation(ILogger Logger)
		{
				...
				Rules.bCheckLicenseViolations)
			{
				...
				foreach (UEBuildBinary Binary in Binaries)
				{
					IEnumerable<UEBuildModule> AllDependencies = Binary.GetAllDependencyModules(true, false);
		...
		protected void SetupMergeModules(ILogger Logger)
		{
			void SetPerModuleInline(UEBuildBinary binary)
			{
				...
				Queue<(UEBuildModule Module, UEBuildModule Source)> queue = new();
				...
				foreach (UEBuildModule m in binary.Modules)
				{
					if (visited.Add(m))
					{
						queue.Enqueue((m, m));
				...
				while (queue.Count > 0)
				{
					(UEBuildModule Module, UEBuildModule Source) current = queue.Dequeue();
					if (current.Module.RulesFile.IsUnderDirectory(Unreal.EngineDirectory))
					{
						continue;
					}
					if (current.Module.Rules.PublicDefinitions.Exists(x => x.StartsWith("PER_MODULE_INLINE_FILE", StringComparison.Ordinal)))
					{
						((UEBuildModuleCPP)current.Source).bCreatePerModuleFile = true;
						return;
					}

					foreach (UEBuildModule? dep in current.Module.PublicDependencyModules!.Union(current.Module.PrivateDependencyModules!))
					{
						if (visited.Add(dep))
						{
			...
			void GetPluginDependencies(UEBuildPlugin plugin, ref HashSet<UEBuildPlugin> dependencies, bool bShouldOnlyFollowCode = false, bool bHasUsersWithSourceCode = false)
			{
				if (plugin.Modules.Any())
				{
					bHasUsersWithSourceCode = true;
				}
				...
				foreach (PluginReferenceDescriptor dependency in plugin.Info.Descriptor.Plugins ?? new())
				{
					if (!Rules.ShouldIgnorePluginDependency(plugin.Info, dependency))
					{
						...
						if (dependentPluginCandidates.Any())
						{
							...
							bool bShouldFollowPlugin = (CanPluginBeMerged(dependentPlugin) && !bShouldOnlyFollowCode) || bHasUsersWithSourceCode;

							if (!dependencies.Contains(dependentPlugin) && bShouldFollowPlugin)
							{
								...
								GetPluginDependencies(dependentPlugin, ref dependencies, bShouldOnlyFollowCode, bHasUsersWithSourceCode);
							}
						}
					}
				}
			...
			if ((Rules.MergePlugins.Any() || Rules.MergePluginsShared.Any()) && BuildPlugins != null)
			{
				// Dependency rules:
				...
				foreach (KeyValuePair<string, IEnumerable<string>> sharedGroupRule in Rules.MergePluginsShared)
				{
					...
					{
						if (BuildPlugins.Any(x => x.Name == name) && !Rules.MergePlugins.Contains(name))
						{
							rootPluginsToMerge.Add(name);
				...
				foreach (string name in rootPluginsToMerge)
				{
					...
					else if (CanPluginBeMerged(plugin))
					{
						...
						Logger.LogInformation("{Name}: setup for merging with {NumDependencies} total dependencies", name, dependencies.Count);
					}
				}
				...
				foreach (KeyValuePair<string, HashSet<UEBuildPlugin>> group in pluginGroups)
				{
					...
					{
						Logger.LogDebug("{PluginName}: added to plugin group {GroupName}", plugin.Name, groupName);
						...
						foreach (UEBuildPlugin plugin in plugins)
						{
							if (!seenPlugins.Contains(plugin) && CanPluginBeMerged(plugin))
							{
								...
								else
								{
									...
									{
										sharedGroupCandidates = sharedGroupCandidates.OrderByDescending(x => x.Value.Count());
									...
									{
										Logger.LogDebug("{PluginName}: unmergeable", plugin.Name);
									}
								}
							}
						}
				...
				{
					HashSet<UEBuildPlugin> dependencies = new();
					bool bShouldFollowEveryDependency = Rules.MergePluginsExcluded.Contains(plugin.Name);
					GetPluginDependencies(plugin, ref dependencies, !bShouldFollowEveryDependency);
				...
				foreach (KeyValuePair<UEBuildPlugin, HashSet<string>> dependency in unmergeablePluginDependencies)
				{
					if (CanPluginBeMerged(dependency.Key))
					{
						...
						{
							Logger.LogDebug("{PluginName}: unmergeable (dependency of: {DependantPluginNames})", dependency.Key, String.Join(",", dependency.Value));
						}
					}
				}

				Logger.LogInformation("Found a total of {NumPlugins} plugins to merge", pluginGroups.SelectMany(x => x.Value).Count());
				...
				{ 
					...
					foreach (UEBuildModuleCPP module in allLeftoverDependencies)
					{
						...
						if (pluginCandidates.Any())
						{
							...
							void FindRootUsers(UEBuildPlugin plugin, ref Dictionary<string, List<string>> roots, List<string> path, HashSet<string> seen)
							{
								...
								if (globalPluginUserMap.ContainsKey(plugin))
								{
									foreach (UEBuildPlugin user in globalPluginUserMap[plugin])
									{
										...
										{
											FindRootUsers(user, ref roots, newPath, seen);
										}
									}
								}
								else
							...
							FindRootUsers(plugin, ref roots, new(), new());
			...
			// Exclude the Instrumentation module from merging so it stays as its own small DLL.
			// Its symbols are dllimport'd by PerModuleInline.inl in every other DLL, so it needs
			// a standalone import library. Combined with ModuleSymbolVisibility = VisibileForDll
			// in Instrumentation.Build.cs to ensure DLLEXPORT even with bMergeModules.
			allModules.RemoveWhere(m => m.Name == "Instrumentation");
		...
		public void PreBuildSetup(ILogger Logger)
		{
			...
			UEBuildModule CreateModuleDelegate(string Name, string RefChain) => FindOrCreateModuleByName(Name, RefChain, Logger);
			StringBuilder ReferenceChain = new(TargetName);
			foreach (UEBuildBinary Binary in Binaries)
			{
				foreach (UEBuildModule Module in Binary.Modules)
				{
					Module.RecursivelyCreateModules(CreateModuleDelegate, ReferenceChain, Logger);
				}
			}

			// Cache dependencies since we can do this in parallel for all modules
			if (ShouldCompileMonolithic())
			{
				UEBuildModule.CacheDependenciesForStableTopologicalSort(Binaries.SelectMany(b => b.Modules.Take(1)));
			}
			else
			{
				UEBuildModule.CacheDependenciesForStableTopologicalSort(Modules.Values.OfType<UEBuildModuleCPP>());
			}

			// Bind every referenced C++ module to a binary
			for (int Idx = 0; Idx < Binaries.Count; Idx++)
				{
					IEnumerable<UEBuildModule> DependencyModules = Binaries[Idx].GetAllDependencyModules(true, true);
					foreach (UEBuildModule DependencyModule in DependencyModules)
					{
						if (DependencyModule.Binary == null && DependencyModule is UEBuildModuleCPP DependencyModuleCpp) // Do type check here, much faster than in enumerable
						{
							AddModuleToBinary(DependencyModuleCpp);
						}
					}
				}
			...
			// Note, we want to add them sorted since we want binary modules to be sorted
			foreach (UEBuildBinary binary in Binaries)
			{
				for (int i=0, e= binary.Modules.Count; i!=e; ++i)
				{
					if (binary.Modules[i] is UEBuildModuleCPP Module)
					{
						foreach (UEBuildModule ReferencedModule in Module.GetUnboundReferences())
						{
							binary.AddModule(ReferencedModule);
						}
					}
				}
			}

			if (Rules.bCheckDelegateLifetime)
			{
				// Add module name information for code in dynamic libraries.
				if (Rules.bMergeModules)
				{
					foreach (UEBuildModuleCPP Module in Modules.Values.OfType<UEBuildModuleCPP>())
					{
						if (Module.Binary != null && Module.Binary.Type == UEBuildBinaryType.DynamicLinkLibrary)
						{
							Module.Rules.PrivateDefinitions.Add($"UE_DYNAMIC_MODULE_NAME=\"{Module.Name}\"");
						}
					}
				}

				// When merged modules are disabled, simulate the behavior, except we'll do it for all mergeable plugins as we don't know how the merging would have been done.
				else
				{
					foreach (UEBuildPlugin Plugin in BuildPlugins!)
					{
		...
		private static void AddCustomBuildSteps(CustomBuildSteps BuildSteps, UEBuildPlugin? Plugin, List<Tuple<string[], UEBuildPlugin?>> CommandBatches)
		{
			string[]? Commands;
		...
		internal Dictionary<string, string> GetTargetVariables(UEBuildPlugin? Plugin)
		{
		...
		public void FindSharedPCHs(List<UEBuildBinary> OriginalBinaries, CppCompileEnvironment GlobalCompileEnvironment, ILogger Logger)
		{
			...
			SortedDictionary<string, (int, PrecompiledHeaderTemplate)> SharedPCHModuleToPriority = new ();
			Parallel2.ForEach(SharedPCHModules, SharedPCHModule =>
			{
				using Pooled<HashSet<UEBuildModule>> rented = HashSetPool<UEBuildModule>.Rent();
				HashSet<UEBuildModule> Dependencies = rented.Value;
				SharedPCHModule.GetAllDependencyModulesForPCH(Dependencies, false);
				Dependencies.IntersectWith(SharedPCHModules);

				// TODO This is bad.. unfortunately does circular dependency logic not working when getting dependency modules
				// If we have Engine with circular dependency on UnrealEd but then is included via some other module, then we're screwed
				foreach (string CircularDependency in SharedPCHModule.Rules.CircularlyReferencedDependentModules)
				{
					if (Modules.TryGetValue(CircularDependency, out UEBuildModule? Module))
					{
						Dependencies.Remove(Module);
					}
				}
				int DependencyCount = Dependencies.Count;
				PrecompiledHeaderTemplate Template = SharedPCHModule.CreateSharedPCHTemplate(this, GlobalCompileEnvironment, Logger);
				lock (SharedPCHModuleToPriority)
				{
					SharedPCHModuleToPriority.Add(SharedPCHModule.Name, (DependencyCount, Template));
				}
			});
			...
			foreach (PrecompiledHeaderTemplate Template in SharedPCHModuleToPriority.OrderByDescending(x => x.Value.Item1).Select(v => v.Value.Item2))
			{
				OrderedSharedPCHModules.Add(Template);
			}
		...
		public static void CreateSharedPCHInstances(ReadOnlyTargetRules Target, UEToolChain ToolChain, List<UEBuildBinary> OriginalBinaries, CppCompileEnvironment GlobalCompileEnvironment, IActionGraphBuilder Graph, ILogger Logger)
		{
			...
			// Sometimes it is one binary with lots of modules, sometimes it is lots of binaries with one module each
			Parallel2.ForEach(OriginalBinaries, Binary =>
			{
				CppCompileEnvironment BinaryCompileEnvironment = Binary.CreateBinaryCompileEnvironment(GlobalCompileEnvironment);

				Parallel2.ForEach(Binary.Modules.OfType<UEBuildModuleCPP>(), Module =>
				{
					...
					if (SharedPCH.Instances.Any())
					{
							...
							foreach (PrecompiledHeaderInstance UnsafeTypeCastErrorInstance in SharedPCH.Instances.Where(instance => instance.Modules.First().Rules.CppCompileWarningSettings.UnsafeTypeCastWarningLevel == WarningLevel.Error))
							{
								if (UEBuildModuleCPP.IsCompatibleForSharedPCH(UnsafeTypeCastErrorInstance.CompileEnvironment, UpdatedCppCompileEnvironment))
								{
									foreach (UEBuildModuleCPP Module in UnsafeTypeCastWarningInstance.Modules)
									{
										...
										{
											Logger.Log(PCHLoggerLevel, "{Prefix} Module '{Module}': Please set 'UnsafeTypeCastWarningLevel' to 'WarningLevel.Error' instead of 'WarningLevel.Warning'. This creates PCH permutations.", Prefix, Module.Name);
											bFoundIssues = true;
										}
									}
								}
						...
						foreach (PrecompiledHeaderInstance Instance in SharedPCH.Instances.Where(instance => instance.Modules.First().Rules.OptimizeCode == ModuleRules.CodeOptimization.Never))
						{
							foreach (UEBuildModuleCPP Module in Instance.Modules)
							{
								...
								{
									Logger.Log(PCHLoggerLevel, "{Prefix} Module '{Module}': Do not set 'OptimizeCode' to 'CodeOptimization.Never'. This creates PCH permutations.", Prefix, Module.Name);
									bFoundIssues = true;
								}
							}
						}
		...
		protected void AddAllValidModulesToTarget(ILogger Logger)
		{
			...
			HashSet<string> ValidModuleNames = rented.Value;
			...
			using Pooled<HashSet<string>> rented2 = HashSetPool<string>.Rent();
			HashSet<string> FilteredModuleNames = rented2.Value;
			...
			if (TargetType != TargetType.Program)
			{
				...
				foreach (string ModuleName in ModuleNames)
				{
					...
					if (!UEBuildPlatform.GetPlatformFolderNames().Any(Name => RulesType.Name.EndsWith("_" + Name, StringComparison.Ordinal)))
					{
			...
			foreach (string FilteredModuleName in FilteredModuleNames)
			{
				...
				try
				{
					string PrecompileReferenceChain = $"{TargetName} allmodules option";
			...
			using Pooled<HashSet<UEBuildModule>> rented3 = HashSetPool<UEBuildModule>.Rent();
			HashSet<UEBuildModule> AllModules = rented3.Value;

			foreach (string ModuleName in ValidModuleNames)
			{
				StringBuilder PrecompileReferenceChain = new($"{TargetName} allmodules option");
				if (ModulePluginSource.TryGetValue(ModuleName, out PluginInfo? value))
				{
					PrecompileReferenceChain.Append(" -> ").Append(value.File.GetFileName());
				}
				UEBuildModule Module = FindOrCreateModuleByName(ModuleName, PrecompileReferenceChain.ToString(), Logger);
		...
		private DirectoryReference GetModuleIntermediateDirectory(ModuleRules ModuleRules, UnrealArchitectures? Architectures)
		{
			...
			if (Configuration == UnrealTargetConfiguration.DebugGame && !ModuleRules.Context.bCanBuildDebugGame && !ModuleRules.Name.Equals(Rules.LaunchModuleName, StringComparison.OrdinalIgnoreCase))
			{
		...
		private UEBuildBinary CreateDynamicLibraryForModules(string MergedName, IEnumerable<UEBuildModuleCPP> Modules)
		{
			...
			List<FileReference> OutputPaths = MakeBinaryPaths(OutputDirectory, AppName + Rules.DecoratedSeparator + MergedName, Platform, Configuration, UEBuildBinaryType.DynamicLinkLibrary, Rules.Architectures, Rules.UndecoratedConfiguration, Rules.ExeBinariesSubFolder, ProjectFile, Rules);
		...
		public static string MakeBinaryFileName(string BinaryName, string Separator, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, UnrealArchitectures Architectures, UnrealTargetConfiguration UndecoratedConfiguration, UEBuildBinaryType BinaryType)
		{
			...
			// If the binary is a library and is on a platform that expects the
			// "lib" library prefix, add the prefix to the library filename.
			// Platforms that expect this are Android, Apple (Mac & iOS), and Linux.
			bool bBinaryIsLibrary = BinaryType is UEBuildBinaryType.DynamicLinkLibrary or UEBuildBinaryType.StaticLibrary;
			bool bRequiresLibraryFilenamePrefix = UEBuildPlatform.IsPlatformInGroup(Platform, UnrealPlatformGroup.Android) ||
			                                      UEBuildPlatform.IsPlatformInGroup(Platform, UnrealPlatformGroup.Apple) ||
			                                      UEBuildPlatform.IsPlatformInGroup(Platform, UnrealPlatformGroup.Linux);
			if (bBinaryIsLibrary && bRequiresLibraryFilenamePrefix)
			{
				Result.Append("lib");
			}
		...
		public void SetupPlugins(ILogger Logger)
		{
			...
			if (ForeignPlugin != null)
			{
				...
				AddPlugin(PluginReference, $"{TargetName} command line", ExcludeFolders, NameToInstance, NameToInfos, Logger);
			}
			...
			foreach (string PluginName in Rules.EnablePlugins)
			{
				if (ReferencedNames.Add(PluginName))
				{
					...
					AddPlugin(PluginReference, $"{TargetName} EnablePlugins", ExcludeFolders, NameToInstance, NameToInfos, Logger);
				}
			}
			...
			foreach (string PluginName in Rules.DisablePlugins)
			{
				if (ReferencedNames.Add(PluginName))
				{
					...
					AddPlugin(PluginReference, $"{TargetName} DisablePlugins", ExcludeFolders, NameToInstance, NameToInfos, Logger);
				}
			}
			...
			foreach (string PluginName in Rules.OptionalPlugins)
			{
				if (ReferencedNames.Add(PluginName))
				{
					...
					AddPlugin(PluginReference, $"{TargetName} OptionalPlugins", ExcludeFolders, NameToInstance, NameToInfos, Logger);
				}
			}
			...
			if (ProjectDescriptor != null)
			{
				...
				{
					HashSet<string> EnablePlugins = new(Rules.EnablePlugins, StringComparer.InvariantCultureIgnoreCase);
					HashSet<string> DisablePlugins = new(Rules.DisablePlugins, StringComparer.InvariantCultureIgnoreCase);
					...
					{
						if (!EnablePlugins.Contains(PluginReference.Name, StringComparer.InvariantCultureIgnoreCase) && !DisablePlugins.Contains(PluginReference.Name, StringComparer.InvariantCultureIgnoreCase))
						{
			...
			if (Rules.bCompileAgainstEngine || Rules.bCompileWithPluginSupport)
			{
				foreach (PluginSet PluginVersions in NameToInfos.Values)
				{
					...
					if (Plugin != null && Plugin.IsEnabledByDefault(bAllowEnginePluginsEnabledByDefault) && !ReferencedNames.Contains(Plugin.Name))
					{
						...
						AddPlugin(PluginReference, $"{TargetName} default plugins", ExcludeFolders, NameToInstance, NameToInfos, Logger);
					}
				}
			}
			...
			if (TargetType == TargetType.Program)
			{
				...
				if (EngineConfig.GetArray("Plugins", "ProgramEnabledPlugins", out PluginNames))
				{
					foreach (string PluginName in PluginNames)
					{
						...
						{
							PluginReferenceDescriptor PluginReference = new PluginReferenceDescriptor(PluginName, null, true);
							AddPlugin(PluginReference, $"{TargetName} DefaultEngine.ini", ExcludeFolders, NameToInstance, NameToInfos, Logger);
						}
					}
				}
			}
			...
			foreach (string PluginName in Rules.BuildPlugins)
			{
				...
				{
					PluginReferenceDescriptor PluginReference = new PluginReferenceDescriptor(PluginName, null, true);
					AddPlugin(PluginReference, $"{TargetName} BuildPlugins", ExcludeFolders, NameToInstance, NameToInfos, Logger);
				}
			}

			// Set the list of plugins that should be built
			BuildPlugins = new(NameToInstance.Values);
			BuildPlugins.SortBy(x => x.Name);
		...
		private UEBuildPlugin? AddPlugin(PluginReferenceDescriptor Reference, string ReferenceChain, string[] ExcludeFolders, Dictionary<string, UEBuildPlugin> NameToInstance, Dictionary<string, PluginSet> NameToInfos, ILogger Logger)
		{
			...
			else
			{
				...
				HashSet<UEBuildPlugin> Dependencies;
				if (Info.Descriptor.Plugins != null)
				{
					Dependencies = new(Info.Descriptor.Plugins.Count + 1);
					foreach (PluginReferenceDescriptor NextReference in Info.Descriptor.Plugins)
					{
						...
						{
							Logger.LogDebug("Ignoring plugin '{PluginName}' (referenced via {ReferencingPluginName}) due to it being filtered out by the target rules.", NextReference.Name, Info.Name);
							continue;
						}
						...
						{
							if (Dependencies.Add(NextInstance))
							{
		...
		protected void SetupBinaries(ILogger Logger)
		{
			...
			string BinaryName = bCompileMonolithic ? TargetName : AppName;
			if (Platform == UnrealTargetPlatform.Mac && Rules.MacPlatform.bForceTargetNameAsExeName)
			{
				BinaryName = TargetName;
			}
			List<FileReference> OutputPaths = MakeBinaryPaths(OutputDirectory, BinaryName, Platform, Configuration, bCompileAsDLL ? UEBuildBinaryType.DynamicLinkLibrary : UEBuildBinaryType.Executable, Rules.Architectures, Rules.UndecoratedConfiguration, Rules.ExeBinariesSubFolder, ProjectFile, Rules);
		...
		public void SetupGlobalEnvironment(UEToolChain ToolChain, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
		{
			...
			GlobalCompileEnvironment.bMinCpuArchAPX = Rules.bMinCpuArchAPX;
			...
			if (Rules.bEnableConstInitUObject)
			{
				...
				GlobalCompileEnvironment.Definitions.Add("WITH_HOT_RELOAD=0");
			}
			else
			{
			...
			if (Rules.bEnableTrace)
			{
				...
				GlobalCompileEnvironment.Definitions.Add("UE_TRACE_ASSERT_FMT=checkf");
			}
			else
			{
			...
			if(Rules.bMultiViewPerSlateWindowSupport && Rules.bCompileAgainstEditor && UEBuildPlatform.IsPlatformInGroup(Platform, UnrealPlatformGroup.Apple))
			{
				GlobalCompileEnvironment.Definitions.Add("WITH_MULTI_VIEW_SLATE_WINDOW_SUPPORT=1");
			}
			else
			{
				GlobalCompileEnvironment.Definitions.Add("WITH_MULTI_VIEW_SLATE_WINDOW_SUPPORT=0");
			}
			...
			if (Rules.bUseSparseSetStrictMode)
			{
				GlobalCompileEnvironment.Definitions.Add("UE_USE_SPARSE_SET_STRICT_MODE=1");
			}
			else
			{
				GlobalCompileEnvironment.Definitions.Add("UE_USE_SPARSE_SET_STRICT_MODE=0");
			}

			if (Rules.bUseCompactSetAsDefault)
			{
				GlobalCompileEnvironment.Definitions.Add("UE_USE_COMPACT_SET_AS_DEFAULT=1");
			}
			else
			{
				GlobalCompileEnvironment.Definitions.Add("UE_USE_COMPACT_SET_AS_DEFAULT=0");
			}
...
#pragma warning disable CA1308 // Normalize strings to uppercase - preserve casing of compiler define
			GlobalCompileEnvironment.Definitions.Add(String.Format("UBT_COMPILED_ARCHITECTURE=" + (Architectures.bIsMultiArch ? "MULTI" : Architectures.SingleArchitecture.ToString().ToLowerInvariant()) ));
#pragma warning restore CA1308
			...
			if (!bUseSharedBuildEnvironment || bCompileMonolithic)
			{
				...
				if (IsCurrentPlatform)
				{
					...
					if (Platform == UnrealTargetPlatform.Mac && OutputFilePath.Contains(".app/Contents/MacOS", StringComparison.Ordinal))
					{
						OutputFilePath = OutputFilePath.Substring(0, OutputFilePath.LastIndexOf(".app/Contents/MacOS", StringComparison.Ordinal) + 4);
					}
		...
		private ModuleRules CreateModuleRulesAndSetDefaults(string ModuleName, string ReferenceChain, ILogger Logger)
		{
			...
			{
				ModuleDescriptor? Module = ProjectDescriptor.Modules.FirstOrDefault(x => x.Name.Equals(ModuleName, StringComparison.OrdinalIgnoreCase));
			...
			// Exclude Instrumentation to avoid circular dependency (Instrumentation -> AutoRTFM -> Instrumentation).
			if (Rules.bUseAutoRTFMCompiler && ModuleName != "AutoRTFM" && ModuleName != "Instrumentation")
			{
		...
		/// <param name="Logger">Logger for output</param>
		public UEBuildModule CreateModuleByName(string ModuleName, string ReferenceChain, ILogger Logger)
		{
			...
			if (RulesObject.Type == ModuleRules.ModuleType.CPlusPlus && RulesObject.bAddDefaultIncludePaths && (RulesObject.Plugin != null || (ProjectFile != null && RulesObject.File.IsUnderDirectory(ProjectFile.Directory))))
			{
				...
				{
					if (DirectoryItem.GetItemByDirectoryReference(BaseSourceDirectory).Exists)
					{
			...
			UEBuildModule Module = InstantiateModule(RulesObject, GeneratedCodeDirectory, Logger);

			// Check if this module conflicts with the ShortName of any other existing modules
			if (!UsedShortNames.TryAdd(Module.Rules.ShortName ?? Module.Name, Module))
			{
				UEBuildModule? ConflictingModule = UsedShortNames[Module.Rules.ShortName ?? Module.Name];
		...
		public UEBuildModule FindOrCreateModuleByName(string ModuleName, string ReferenceChain, ILogger Logger)
		{
			...
			{
				Module = CreateModuleByName(ModuleName, ReferenceChain, Logger);
				Modules.Add(Module.Name, Module);
			}

			// Warn if the module reference has incorrect text case
			if (!Module.Name.Equals(ModuleName, StringComparison.Ordinal) && Module.Name.Equals(ModuleName, StringComparison.OrdinalIgnoreCase))
			{
