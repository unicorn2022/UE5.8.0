...
namespace UnrealBuildTool
{
	...
	class PrecompiledManifest
	{
		...
		/// <summary>
		/// The MSVC toolchain version used to compile the precompiled objects.
		/// Null for non-MSVC or manifests produced before this field was added.
		/// </summary>
		public VersionNumber? ToolChainVersion = null;
		...
		public static PrecompiledManifest Read(FileReference Location)
		{
			...
			if (RawObject.TryGetStringField("ToolChainVersion", out string? ToolChainVersionStr))
			{
				_ = VersionNumber.TryParse(ToolChainVersionStr, out Manifest.ToolChainVersion);
			}
		...
		public void WriteIfModified(FileReference Location)
		{
			...
			using (JsonWriter Writer = new JsonWriter(new StreamWriter(MemoryStream)))
			{
				...
				{
					OutputFileStrings[Idx] = OutputFiles[Idx].MakeRelativeTo(BaseDir).Replace('\\', '/');
				}
				Writer.WriteStringArrayField("OutputFiles", OutputFileStrings);

				if (ToolChainVersion != null)
				{
					Writer.WriteValue("ToolChainVersion", ToolChainVersion.ToString());
				}
