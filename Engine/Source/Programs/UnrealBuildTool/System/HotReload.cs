...
namespace UnrealBuildTool
{
	...
	class HotReloadState
	{
		...
		/// </summary>
		public Dictionary<FileReference, FileReference> OriginalFileToHotReloadFile { get; set; } = [];

		/// <summary>
		/// Set of all temporary files created for hot reload
		/// </summary>
		public HashSet<FileReference> TemporaryFiles { get; set; } = [];
		...
		public static FileReference GetLocation(FileReference? ProjectFile, string TargetName, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, UnrealArchitectures Architectures)
		{
			...
			return FileReference.Combine(BaseDir, UEBuildTarget.GetPlatformIntermediateFolder(Platform, Architectures, false), TargetName, Configuration.ToString(), "HotReloadState.json");
		}
		...
		{
			using FileStream FileStream = FileReference.Open(Location, FileMode.Open, FileAccess.Read, FileShare.Read);
			HotReloadState? MaybeState = JsonSerializer.Deserialize<HotReloadState>(FileStream);
			return MaybeState ?? throw new BuildLogEventException("Hot reload state file '{FilePath}' was invalid", Location);
		}
	...
	static class HotReload
	{
		...
		public static FileReference ReplaceSuffix(FileReference File, int Suffix)
		{
			...
			int HyphenIdx = FileName.IndexOf('-', StringComparison.Ordinal);
			if (HyphenIdx == -1)
			{
				throw new BuildException("Hot-reloadable files are expected to contain a hyphen, eg. UnrealEditor-Core ({0})", FileName);
			}
						...
						|| Action.CommandPath.GetFileName().Equals("verse-clang-cl.exe", StringComparison.OrdinalIgnoreCase))
					{
						...
						for (int Idx = 0; Idx < ResponseLines.Length; Idx++)
						{
							...
							if (ResponseLine.StartsWith(SourceDependencyPrefix, StringComparison.Ordinal))
							{
								...
								FileReference NewSourceDependencyFile = new(OldSourceDependencyFile.FullName.Substring(0, OldSourceDependencyFile.FullName.Length - extension.Length) + extraExt + extension);
		...
		public static Dictionary<FileReference, FileReference> PatchActionGraph(IEnumerable<LinkedAction> Actions, Dictionary<FileReference, FileReference> OriginalFileToHotReloadFile)
		{
			...
			foreach (LinkedAction Action in Actions.Where((Action) => Action.ActionType == ActionType.Link))
			{
				...
				NewAction.PrerequisiteItems = new (UpdatePrerequisiteItems, FileItem.SortByPath);
				...
				NewAction.ProducedItems = new (UpdateProducedItems, FileItem.SortByPath);
				...
				NewAction.DeleteItems = new (UpdateDeleteItems, FileItem.SortByPath);
			...
			foreach (LinkedAction Action in Actions)
			{
				if (Action.ActionType == ActionType.WriteMetadata)
				{
					...
					int InputIdx = Arguments.IndexOf(InputArgument, StringComparison.Ordinal);
					if (InputIdx == -1)
					{
					...
					WriteMetadataTargetInfo TargetInfo;
					using (FileStream FileStream = FileReference.Open(TargetInfoFile, FileMode.Open, FileAccess.Read, FileShare.Read))
					{
						WriteMetadataTargetInfo? MaybeTargetInfo = JsonSerializer.Deserialize<WriteMetadataTargetInfo>(FileStream, WriteMetadataMode.SerializerOptions);
						TargetInfo = MaybeTargetInfo ?? throw new BuildLogEventException("Target info file '{FilePath}' was invalid", TargetInfoFile);
					}
					...
					{
						FileReference HotReloadTargetInfoFile = FileReference.Combine(TargetInfoFile.Directory, "Metadata-HotReload.json");
						byte[] InfoBytes = JsonSerializer.SerializeToUtf8Bytes(TargetInfo, WriteMetadataMode.SerializerOptions);
						DirectoryReference.CreateDirectory(HotReloadTargetInfoFile.Directory);
						FileReference.WriteAllBytesIfDifferent(HotReloadTargetInfoFile, InfoBytes);
			...
			using (JsonWriter Writer = new JsonWriter(ManifestFile))
			{
				...
				foreach (IExternalAction Action in Actions)
				{
					if (Action.ActionType == ActionType.Link)
					{
						...
						FileItem? OutputFile = Action.ProducedItems.FirstOrDefault(x => (x.HasExtension(".exe") || x.HasExtension(".dll")) && !x.HasExtension(".alt.exe") && !x.HasExtension(".alt.dll"));
						if (OutputFile != null && Action.PrerequisiteItems.Any(x => OriginalFileToPatchedFile.ContainsKey(x.Location)))
						{
							...
							{
								foreach ((uint id, DirectoryReference vfs, DirectoryReference local) RootPath in Action.RootPaths)
								{
				...
				foreach ((DirectoryReference virtualPath, DirectoryReference localPath) kv in vfsPaths)
				{
					Writer.WriteObjectStart();
