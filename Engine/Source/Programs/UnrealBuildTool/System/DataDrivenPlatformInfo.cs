...
namespace UnrealBuildTool
{
	...
	/// </summary>
	public static class DataDrivenPlatformInfo
	{
#pragma warning disable CA1034 // Nested types should not be visible - preserve API of APIs exposed to UAT
		...
		public class ConfigDataDrivenPlatformInfo
		{
			...
			/// <summary>
			/// The ICU data directory name this platform ships (e.g. "icudt64l", "icudt78l"). Null if unset.
			/// </summary>
			public string? ICUDataVersion = null;
					...
					foreach (DirectoryReference EngineConfigDir in Unreal.GetExtensionDirs(Unreal.EngineDirectory, "Config", bIncludeRestrictedDirectories: false))
					{
						...
						foreach (string FilePath in Directory.EnumerateFiles(EngineConfigDir.FullName, "DataDrivenPlatformInfo.ini", SearchOption.AllDirectories))
						{
							...
							if (Config.TryGetSection("DataDrivenPlatformInfo", out Section))
							{
								...
								{
									_ = ConfigHierarchy.TryParse(Temp, out NewInfo.bIsConfidential);
								}
								...
								{
									_ = ConfigHierarchy.TryParse(Temp, out NewInfo.bCanUseCrashReporter);
								}
								...
								{
									_ =ConfigHierarchy.TryParse(Temp, out NewInfo.bIsEnabled);
								}
								...
								if (ParsedSection.TryGetValue("ICUDataVersion", out NewInfo.ICUDataVersion) == false)
								{
									NewInfo.ICUDataVersion = null;
								}

								NewInfo.bNoAccountOverride = false;
								if (ParsedSection.TryGetValue("bNoAccountOverride", out Temp))
								{
									_ = ConfigHierarchy.TryParse(Temp, out NewInfo.bNoAccountOverride);
								}
