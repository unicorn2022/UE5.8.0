...
#pragma warning restore CA1724
	{
		...
		public static string ResolveEnvironmentVariable(string InString)
		{
			...
			while (M.Success)
			{
				...
				{
					EnvironmentVariable = EnvironmentVariable.Substring(1, EnvironmentVariable.Length - 2).Replace("(", "", StringComparison.Ordinal);
				}
				...
				Result = Result.Replace(M.ToString(), Environment.GetEnvironmentVariable(EnvironmentVariable), StringComparison.Ordinal);
		...
		{
			return RunLocalProcessAndReturnStdOut(Command, Args, Logger, out _);
		}
		...
		static readonly Dictionary<UnrealPlatformClass, Lazy<UnrealTargetPlatform[]>> s_PlatformsInClass = new()
		{
		...
		public static List<string> MakeListOfUnsupportedPlatforms(List<UnrealTargetPlatform> SupportedPlatforms, bool bIncludeUnbuildablePlatforms, ILogger Logger)
		{
			...
			List<string> OtherPlatformNameStrings = new List<string>();
			{
				...
				foreach (UnrealTargetPlatform CurPlatform in UnrealTargetPlatform.GetValidPlatforms())
				{
					...
					foreach (UnrealPlatformGroup Group in UnrealPlatformGroup.GetValidGroups())
					{
						if (Group.ToString().Equals(CurPlatformString, StringComparison.Ordinal))
						{
		...
		public static string CollapseRelativeDirectories(string InPath)
		{
			...
			if (LocalString.Contains('\\', StringComparison.Ordinal))
			{
				LocalString = LocalString.Replace("\\", "/", StringComparison.Ordinal);
			...
			for (; ; )
			{
				...
				if (LocalString.StartsWith("..", StringComparison.Ordinal) || LocalString.StartsWith(ParentDir, StringComparison.Ordinal))
				{
					return InPath;
				}

				// If there are no "/.."s left then we're done
				int Index = LocalString.IndexOf(ParentDir, StringComparison.Ordinal);
				if (Index == -1)
				{
					break;
				}
		...
		public static string MakePathRelativeTo(string SourcePath, string RelativeToDirectory, bool AlwaysTreatSourceAsDirectory = false)
		{
			...
			bool SourcePathEndsWithDirectorySeparator = AbsolutePath.EndsWith(Path.DirectorySeparatorChar) || AbsolutePath.EndsWith(Path.AltDirectorySeparatorChar);
			...
			if (!AbsoluteRelativeDirectory.EndsWith(Path.DirectorySeparatorChar) && !AbsoluteRelativeDirectory.EndsWith(Path.AltDirectorySeparatorChar))
			{
			...
			if (!SourcePathEndsWithDirectorySeparator && AlwaysTreatSourceAsDirectory && RelativePath.EndsWith(Path.DirectorySeparatorChar))
			{
		...
		public static T ReadClass<T>(string FileName, ILogger Logger) where T : new()
		{
			...
			try
			{
				...
				Instance = (T)Serialiser.Deserialize(XmlReader)!;
			}
			catch (Exception E)
			{
		...
		public static bool WriteClass<T>(T Data, string FileName, string DefaultNameSpace, ILogger Logger)
		{
			try
			{
				...
				XmlSerializerNamespaces EmptyNameSpace = new();
				EmptyNameSpace.Add("", DefaultNameSpace);

				using StreamWriter XmlStream = new(FileName, false, Encoding.Unicode);
				XmlSerializer Serialiser = new(typeof(T));
		...
		public static bool IsFileUnderDirectory(string FilePath, string Directory)
		{
			...
			if (!DirectoryPathPlusSeparator.EndsWith(Path.DirectorySeparatorChar))
			{
				DirectoryPathPlusSeparator += Path.DirectorySeparatorChar;
			}
			return Path.GetFullPath(FilePath).StartsWith(DirectoryPathPlusSeparator, StringComparison.OrdinalIgnoreCase);
		}
		...
		/// <returns>True if TestType implements TInterface. False otherwise.</returns>
		public static bool ImplementsInterface<TInterface>(Type TestType)
		{
			return Array.IndexOf(TestType.GetInterfaces(), typeof(TInterface)) != -1;
		}
		...
		/// </summary>
		[Obsolete("Deprecated in UE5.8. Replace with Unreal.UserSettingDirectory")]
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetLogicalProcessorCount() from EpicGames.Core instead.")]
		public static int GetLogicalProcessorCount() => SystemUtils.GetLogicalProcessorCount();
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetPhysicalProcessorCount() from EpicGames.Core instead.")]
		public static int GetPhysicalProcessorCount() => SystemUtils.GetPhysicalProcessorCount();

		/// <summary>
		/// Gets if the processor has asymmetrical cores (Windows only)
		/// </summary>
		/// <returns></returns>
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.IsAsymmetricalProcessor() from EpicGames.Core instead.")]
		public static bool IsAsymmetricalProcessor() => SystemUtils.IsAsymmetricalProcessor();
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetAvailableMemoryBytes() from EpicGames.Core instead.")]
		public static long GetAvailableMemoryBytes() => SystemUtils.GetAvailableMemoryBytes();
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetTotalSystemMemoryBytes() from EpicGames.Core instead.")]
		public static long GetTotalSystemMemoryBytes() => SystemUtils.GetTotalSystemMemoryBytes();
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetFreeMemoryBytes() from EpicGames.Core instead.")]
		public static long GetFreeMemoryBytes() => SystemUtils.GetFreeMemoryBytes();
		...
		[Obsolete("Deprecated in UE5.8. Use SystemUtils.GetTotalCpuUtilization() from EpicGames.Core instead.")]
		public static bool GetTotalCpuUtilization(out float Utilization) => SystemUtils.GetTotalCpuUtilization(out Utilization);
		...
		public static int GetMaxActionsToExecuteInParallel(int MaxProcessorCount, double ProcessorCountMultiplier, bool ConsiderLogicalCores, long MemoryPerActionBytes)
		{
			...
			int NumLogicalCores = SystemUtils.GetLogicalProcessorCount();

			// Use WMI to figure out physical cores, excluding hyper threading.
			int NumPhysicalCores = SystemUtils.GetPhysicalProcessorCount();
			...
			if (MemoryPerActionBytes > 0)
			{
				...
				long AvailableMemoryBytes = OperatingSystem.IsMacOS() ? SystemUtils.GetTotalSystemMemoryBytes() : SystemUtils.GetFreeMemoryBytes();
		...
		public static void ExecuteCustomBuildSteps(FileReference[] ScriptFiles, ILogger Logger)
		{
			...
			{
				using ITimelineEvent _ = Timeline.ScopeEvent(ScriptFile.GetFileName());
			...
			{
				using ITimelineEvent _ = Timeline.ScopeEvent("ResetCachedInfo");
		...
		public static bool TryParseProjectFileArgument(CommandLineArguments Arguments, ILogger Logger, [NotNullWhen(true)] out FileReference? ProjectFile, bool MarkArgumentAsUsed = true)
		{
			...
			{
				Logger.LogDebug("No project argument provided while trying to parse Project File. Attempting fallback parsing to attempt to find a Project File.");
			...
			if (!String.IsNullOrEmpty(CandidateProjectPath))
			{
				...
				if (!FileReference.Exists(CandidateProjectFile))
				{
					...
					CandidateProjectFile = NativeProjectFiles.Where(F => F.GetFileName().Equals(ProjectName, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
				}
