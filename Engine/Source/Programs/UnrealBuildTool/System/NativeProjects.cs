	...
	public class NativeProjects : NativeProjectsBase
	{
		...
		public static bool TryGetProjectForTarget(string InTargetName, ILogger Logger, [NotNullWhen(true)] out FileReference? OutProjectFileName)
		{
			if (CachedTargetNameToProjectFile == null)
			{
				lock (LockObject)
				{
#pragma warning disable CA1508 // Avoid dead conditional code - required to check for null again after locking
					if (CachedTargetNameToProjectFile == null)
#pragma warning restore CA1508
					{
						using ITimelineEvent _ = Timeline.ScopeEvent("Find target files");
						Dictionary<string, FileReference> TargetNameToProjectFile = [];

						IEnumerable<DirectoryReference> AllPotentialTargetDirectories = EnumerateProjectFiles(Logger)
							.SelectMany(ProjectFile => Unreal.GetExtensionDirs(ProjectFile.Directory))
							.SelectMany(ExpandSource);
						Rules.Populate(AllPotentialTargetDirectories);

						foreach (FileReference ProjectFile in EnumerateProjectFiles(Logger))
						{
							IEnumerable<DirectoryReference> ProjectTargetDirectories = Unreal.GetExtensionDirs(ProjectFile.Directory).SelectMany(ExpandSource);
							List<FileReference> TargetFiles = Rules.Get(ProjectTargetDirectories, Rules.RulesFileType.Target);
		...
		private static IEnumerable<DirectoryReference> ExpandSource(DirectoryReference Item)
		{
			yield return DirectoryReference.Combine(Item, "Source");
			yield return DirectoryReference.Combine(Item, "Intermediate", "Source");
		}
		...
		private static bool RequiresTempTarget(FileReference UProjectFile, List<UnrealTargetConfiguration> Configurations, [NotNullWhen(true)] out string? Reason, ILogger Logger)
		{
			...
			List<UnrealTargetPlatform> Platforms = DataDrivenPlatformInfo.GetAllPlatformInfos()
				...
				.Select(x => UnrealTargetPlatform.Parse(x.Key.Replace("Windows", "Win64", StringComparison.Ordinal)))
				.ToList();
		...
		static bool GetCodePluginsForTarget(ProjectDescriptor? ProjectDescriptor, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, TargetType TargetType, HashSet<string> CodePluginNames, Dictionary<string, PluginInfo> AllPlugins, out PluginReferenceDescriptor? OutMissingPlugin, ILogger Logger)
		{
			...
			if (ProjectDescriptor != null)
			{
				...
				if (ProjectDescriptor.Plugins != null)
				{
					...
					foreach (PluginReferenceDescriptor PluginReference in ProjectDescriptor.Plugins)
					{
						...
						{
							PluginReferenceDescriptor? MissingPlugin;
							if (!ConfigureEnabledPluginForTarget(PluginReference, null, Platform, Configuration, TargetType, bLoadPluginsForTargetPlatforms, AllPlugins, EnabledPlugins, out MissingPlugin, Logger))
							{
			...
			foreach (KeyValuePair<string, PluginInfo> PluginPair in AllPlugins)
			{
				...
				{
					PluginReferenceDescriptor? MissingPlugin;
					if (!ConfigureEnabledPluginForTarget(new PluginReferenceDescriptor(PluginPair.Key, null, true), null, Platform, Configuration, TargetType, bLoadPluginsForTargetPlatforms, AllPlugins, EnabledPlugins, out MissingPlugin, Logger))
					{
						OutMissingPlugin = MissingPlugin;
						return false;
					}
		...
		/// </summary>
		static bool ConfigureEnabledPluginForTarget(PluginReferenceDescriptor FirstReference, string? TargetName, UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, TargetType TargetType, bool bLoadPluginsForTargetPlatforms, Dictionary<string, PluginInfo> AllPlugins, Dictionary<string, PluginInfo> EnabledPlugins, out PluginReferenceDescriptor? OutMissingPlugin, ILogger Logger)
		{
