...
namespace UnrealBuildTool
{
	...
	public static class CommandLine
	{
		...
		private static readonly SearchValues<char> ArgumentSeparators = SearchValues.Create("=:");
		...
		public static void ParseAndRemoveArguments(List<string> Arguments, object TargetObject, ILogger Logger)
		{
			...
			for (int Idx = 0; Idx < Arguments.Count; Idx++)
			{
				...
				if (Argument.Length > 0 && Argument[0] == '-')
				{
					...
					int EqualsIdx = Argument.AsSpan().IndexOfAny(ArgumentSeparators);
			...
			if (MissingFieldToParameter.Count > 0)
			{
				...
				{
					throw new BuildException("Missing {0} argument", MissingFieldToParameter.First().Value.Prefix.Replace("=", "=...", StringComparison.Ordinal));
				}
				else
				{
					throw new BuildException("Missing {0} arguments", StringUtils.FormatList(MissingFieldToParameter.Values.Select(x => x.Prefix.Replace("=", "=...", StringComparison.Ordinal))));
				}
			}
		}
