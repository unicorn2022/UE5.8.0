...
namespace UnrealBuildTool
{
	 ...
	 * 	
	 *	<!-- optional additions to GameApplication onCreate in GameApplication.java (all processes) -->
	 *	<gameApplicationOnCreateAdditionsAlways> </gameApplicationOnCreateAdditionsAlways>
	 *  
	 *	<!-- optional additions to GameApplication onCreate in GameApplication.java (non-PSO processes) -->
	 *	<gameApplicationOnCreateAdditions> </gameApplicationOnCreateAdditions>
	 *  
	...
	class UnrealPluginLanguage
	{
		...
		/** XML namespace */
		private readonly XNamespace XMLDefaultNameSpace;
		private readonly Dictionary<string, XNamespace> XMLNameSpaceAliases = new Dictionary<string, XNamespace>();
		...
		private class UPLContext
		{
			...
			public UPLContext(string inFilename, (string UEArch, string NativeArch) Arch, string PluginDir)
			{
				...
				StringVariables["PluginDir"] = PluginDir.Replace("\\", "/", StringComparison.Ordinal);
				StringVariables["AbsPluginDir"] = Path.GetFullPath(PluginDir).Replace("\\", "/", StringComparison.Ordinal);
...
#pragma warning disable IDE0044 // Make field readonly - this field appears intended to be mutable.
...
#pragma warning restore IDE0044
		...
		public UnrealPluginLanguage(FileReference? InProjectFile, List<string> InXMLFiles, List<(string UEArch, string NativeArch)> InArchs, List<AliasedXMLNamespace>? InXMLNameSpaceAliases, UnrealTargetPlatform InTargetPlatform, ILogger InLogger)
		{
			...
			string PathPrefix = Path.GetFileName(Directory.GetCurrentDirectory()).Equals("Source", StringComparison.Ordinal) ? ".." : "Engine";

			XDoc = XDocument.Parse("<root " + XMLRootDefinition + "></root>");
			foreach (string Basename in InXMLFiles)
			{
				string Filename = Path.Combine(PathPrefix, Basename.Replace("\\", "/", StringComparison.Ordinal));
		...
		public static bool GetTrace() { return bGlobalTrace; }
		public static void SetTrace() { bGlobalTrace = true; }
		public static void ClearTrace() { bGlobalTrace = false; }
		public static bool GetTraceFilename() { return bGlobalTraceFilename; }
		public static void SetTraceFilename() { bGlobalTraceFilename = true; }
		public static void ClearTraceFilename() { bGlobalTraceFilename = false; }
		...
		private static string DumpContext(UPLContext Context)
		{
			StringBuilder Text = new StringBuilder();
			foreach (KeyValuePair<string, bool> Variable in Context.BoolVariables)
			{
				Text.AppendLine(String.Format("\tbool {0} = {1}", Variable.Key, Variable.Value ? "true" : "false"));
			}
		...
		private string ExpandVariables(UPLContext Context, string InputString)
		{
			...
			for (int Idx = Result.IndexOf("$B(", StringComparison.Ordinal); Idx != -1; Idx = Result.IndexOf("$B(", Idx, StringComparison.Ordinal))
			{
				...
				Result = Result.Substring(0, Idx) + (Value ? "true" : "false") + Result.Substring(EndIdx + 1);
			}
			for (int Idx = Result.IndexOf("$I(", StringComparison.Ordinal); Idx != -1; Idx = Result.IndexOf("$I(", Idx, StringComparison.Ordinal))
			{
				// Find the end of the variable name
				int EndIdx = Result.IndexOf(')', Idx + 3);
				if (EndIdx == -1)
			...
			for (int Idx = Result.IndexOf("$S(", StringComparison.Ordinal); Idx != -1; Idx = Result.IndexOf("$S(", Idx, StringComparison.Ordinal))
			{
				// Find the end of the variable name
				int EndIdx = Result.IndexOf(')', Idx + 3);
				if (EndIdx == -1)
			...
			for (int Idx = Result.IndexOf("$E(", StringComparison.Ordinal); Idx != -1; Idx = Result.IndexOf("$E(", Idx, StringComparison.Ordinal))
			{
				// Find the end of the variable name
				int EndIdx = Result.IndexOf(')', Idx + 3);
				if (EndIdx == -1)
		...
		private static string TraceNodeString(UPLContext Context, XElement Node, bool ShowFilename = true)
		{
			...
			foreach (XAttribute Attrib in Node.Attributes())
			{
				...
				if (AttribStr.StartsWith("__line=", StringComparison.Ordinal))
				{
					LineAttrib = Attrib;
				}
				else
		...
		private static void AddElements(XElement Target, XElement Source)
		{
					...
					case "isArch":
						{
							...
							if (arch != null && arch.Equals(Architecture, StringComparison.Ordinal))
							{
								foreach (XElement instruction in Node.Elements().Reverse())
								{
									ExecutionStack.Push(instruction);
					...
					case "break":
						{
							...
							while (ExecutionStack.Count > 0)
							{
								...
								if (Node.Name.ToString().Equals("while", StringComparison.Ordinal))
								{
									break;
								}
							}
					...
					case "addAttribute":
						{
							...
							if (Tag != null && Name != null && Value != null)
							{
								...
								{
									if (CurrentElement.Name.ToString().Equals(Tag, StringComparison.Ordinal))
									{
					...
					case "insert":
						{
							if (Node.HasElements)
							{
								...
								{
									string Value = Element.ToString().Replace(" " + XMLRootDefinition + " ", "", StringComparison.Ordinal);
					...
					case "replace":
						{
							...
							{
								GlobalContext.StringVariables["Output"] = GlobalContext.StringVariables["Output"].Replace(Find, With, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "deleteFiles":
						{
							...
							if (Filespec != null)
							{
								if (Filespec.Contains(':', StringComparison.Ordinal) || Filespec.Contains("..", StringComparison.Ordinal))
								{
							...
							if (Result != null && Ini != null && Section != null && Property != null)
							{
								...
								if (ConfigIni != null)
								{
									...
									if (ConfigIni.GetArray(Section, Property, out StringList))
									{
										...
										{
											if (Entry.Equals(Contains, StringComparison.Ordinal))
											{
												Value = true;
												break;
											}
					...
					case "setBoolContains":
						{
							...
							if (Result != null)
							{
								CurrentContext.BoolVariables[Result] = Source.Contains(Find, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "setBoolStartsWith":
						{
							...
							if (Result != null)
							{
								CurrentContext.BoolVariables[Result] = Source.StartsWith(Find, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "setBoolEndsWith":
						{
							...
							if (Result != null)
							{
								CurrentContext.BoolVariables[Result] = Source.EndsWith(Find, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "setBoolIsEqual":
						{
							...
							if (Result != null)
							{
								CurrentContext.BoolVariables[Result] = Arg1.Equals(Arg2, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "setIntFindString":
						{
							...
							if (Result != null)
							{
								CurrentContext.IntVariables[Result] = Source.IndexOf(Find, StringComparison.Ordinal);
							}
						}
						break;
					...
					case "setStringReplace":
						{
							...
							{
								string Value = Source.Replace(Find, With, StringComparison.Ordinal);
								if (Result == "Output")
								{
									GlobalContext.StringVariables["Output"] = Value;
								}
					...
					case "setStringToLower":
						{
							...
							if (Result != null)
							{
#pragma warning disable CA1308 // Normalize strings to uppercase - specifically implementing a method of lowercasing a string
...
#pragma warning restore CA1308
			...
			bool bPerArchBuildDir = false, Dictionary<string, string>? ArchRemapping = null)
		{
			...
			GlobalContext.StringVariables["EngineDir"] = EngineDirectory.Replace("\\", "/", StringComparison.Ordinal);
			GlobalContext.StringVariables["BuildDir"] = BuildDirectory.Replace("\\", "/", StringComparison.Ordinal);
			GlobalContext.StringVariables["ProjectDir"] = ProjectDirectory.Replace("\\", "/", StringComparison.Ordinal);
			...
			GlobalContext.StringVariables["AbsEngineDir"] = Path.GetFullPath(GlobalContext.StringVariables["EngineDir"]).Replace("\\", "/", StringComparison.Ordinal);
			GlobalContext.StringVariables["AbsBuildDir"] = Path.GetFullPath(GlobalContext.StringVariables["BuildDir"]).Replace("\\", "/", StringComparison.Ordinal);
			GlobalContext.StringVariables["AbsProjectDir"] = Path.GetFullPath(GlobalContext.StringVariables["ProjectDir"]).Replace("\\", "/", StringComparison.Ordinal);
			...
			foreach (string Arch in Architectures)
			{
				if (bPerArchBuildDir)
				{
					...
					string ArchBuildDirectory = Path.Combine(GlobalContext.StringVariables["BuildDir"], ActiveArch.Replace("-", "_", StringComparison.Ordinal));
					string ArchBuildDir = ArchBuildDirectory.Replace("\\", "/", StringComparison.Ordinal);
					string ArchAbsBuildDir = Path.GetFullPath(ArchBuildDir).Replace("\\", "/", StringComparison.Ordinal);
	...
	class ConfigCacheIni_UPL
	{
		...
		public static ConfigCacheIni_UPL CreateConfigCacheIni_UPL(UnrealTargetPlatform Platform, string BaseIniName, DirectoryReference? ProjectDirectory, ILogger Logger, DirectoryReference? EngineDirectory = null)
		{
			...
			string Key = ConfigHierarchy.GetIniPlatformName(Platform) + BaseIniName + EngineDirectory.FullName + (ProjectDirectory != null ? ProjectDirectory.FullName : "");
			if (!IniCache.ContainsKey(Key))
			{
				IniCache.Add(Key, new ConfigCacheIni_UPL(Platform, BaseIniName, ProjectDirectory, Logger));
			}
			return IniCache[Key];
		}
		...
		/// </summary>
		readonly bool bIsMergingConfigs = false;
		...
		public ConfigCacheIni_UPL(string BaseIniName, string? ProjectDirectory, ILogger Logger)
			: this(BuildHostPlatform.Current.Platform, BaseIniName, ProjectDirectory, Logger)
		{
		}
		...
		public ConfigCacheIni_UPL(string BaseIniName, DirectoryReference ProjectDirectory, ILogger Logger)
			: this(BuildHostPlatform.Current.Platform, BaseIniName, ProjectDirectory, Logger)
		{
		}
		...
		/// <param name="BaseIniName">Ini name (Engine, Editor, etc)</param>
		/// <param name="Logger"></param>
		public ConfigCacheIni_UPL(UnrealTargetPlatform Platform, string BaseIniName, string? ProjectDirectory, ILogger Logger)
			: this(Platform, BaseIniName, ProjectDirectory == null ? null : new DirectoryReference(ProjectDirectory), Logger)
		{
		}
		...
		/// <param name="BaseIniName">Ini name (Engine, Editor, etc)</param>
		/// <param name="Logger"></param>
		public ConfigCacheIni_UPL(UnrealTargetPlatform Platform, string BaseIniName, DirectoryReference? ProjectDirectory, ILogger Logger) : this()
		{
			bIsMergingConfigs = true;

			foreach (FileReference IniFileName in EnumerateConfigFileLocations(Platform, BaseIniName, ProjectDirectory))
			{
		...
		public bool GetBool(string SectionName, string Key, out bool Value)
		{
			...
			if (Result)
			{
				...
				if (String.Equals(TextValue, "true", StringComparison.OrdinalIgnoreCase) || String.Equals(TextValue, "1", StringComparison.Ordinal))
				{
					Value = true;
				}
				else if (String.Equals(TextValue, "false", StringComparison.OrdinalIgnoreCase) || String.Equals(TextValue, "0", StringComparison.Ordinal))
				{
					Value = false;
				}
				else
		...
		public bool GetGUID(string SectionName, string Key, out Guid Value)
		{
			...
			{
				string HexString = "";
				if (TextValue.Contains("A=", StringComparison.Ordinal) && TextValue.Contains("B=", StringComparison.Ordinal) && TextValue.Contains("C=", StringComparison.Ordinal) && TextValue.Contains("D=", StringComparison.Ordinal))
				{
		...
		{
			if (String.IsNullOrEmpty(Line) || Line.StartsWith(';') || Line.StartsWith("//", StringComparison.Ordinal))
			{
		...
		{
			int AssignIndex = TrimmedLine.IndexOf('=', StringComparison.Ordinal);
			if (AssignIndex < 0)
			{
				...
				case ParseAction.Remove:
					{
						...
						{
							int ExistingIndex = Value.FindIndex(X => (String.Equals(SingleValue, X, StringComparison.OrdinalIgnoreCase)));
							if (ExistingIndex >= 0)
							{
		...
		{
			int SectionEndIndex = TrimmedLine.IndexOf(']', StringComparison.Ordinal);
		...
		/// <summary>
		/// Returns the ordered list of INI files that should be merged for the given
		/// platform + project, matching the hierarchy used by C# build code
		/// (ConfigHierarchy / ConfigCache). This keeps UPL and C# readers in agreement
		/// on which files override which — including platform Base*.ini, Generated*.ini,
		/// Custom/{CUSTOMCONFIG}/*.ini, LimitedAccess restricted layers, platform
		/// extensions ({EXTENGINE}/{EXTPROJECT}) and the DataDrivenPlatformInfo
		/// IniParentChain.
		/// </summary>
		static IEnumerable<FileReference> EnumerateConfigFileLocations(UnrealTargetPlatform Platform, string BaseIniName, DirectoryReference? ProjectDirectory)
		{
			ConfigHierarchyType Type = Enum.Parse<ConfigHierarchyType>(BaseIniName, ignoreCase: true);
			return ConfigHierarchy.EnumerateConfigFileLocations(Type, ProjectDirectory, Platform, CustomConfig: String.Empty, IncludePluginsForTargetType: null);
		}
	}
}
