...
namespace UnrealBuildTool
{
	class Unity
	{
		...
		/// <summary>
		/// Constant used to estimate file size for generated files
		/// </summary>
		/// <remarks>
		/// When sampled, the median size for a generated file going into a Unity module was ~7.3KB.
		/// 4KB is rounding down to the closest power of two, as generated code has been observed to
		/// compile faster than non-generated code of equivalent size.
		/// </remarks>
		public const long PlaceholderFileSize = 4 * 1024;

		/// <summary>
		/// Safe way to get file length
		/// </summary>
		static long FileLength(FileItem file)
		{
			// Temp: Currently there is a race between unity generation and UHT file generation that can cause files to move across Unity TUs
			// based on whether they've been generated or not yet, which can cause errors about modified build files if multiple targets
			// generate unity sets for the same intermediates (e.g. DebugGame, Development).
			// Until this is fixed properly, we'll always use the placeholder size to make this deterministic.
			if (file.Location.ContainsName("Intermediate", 0))
			{
				return PlaceholderFileSize;
			}
			// Files outside of an Intermediate directory must never be placeholder.
			return file.Length;
		}
		...
		public class UnityFileBuilder
		{
			...
			public void AddFile(FileItem File)
			{
				if (SplitLength != -1 && FileLength(File) > SplitLength)
				{
					EndCurrentUnityFile();
				}
			...
			int NumIncludedBytesPerUnitySource)
		{
			...
			long TotalBytesInSourceFiles = SourceFiles.Sum(F => FileLength(F));
			...
			GetAdaptiveFiles(Target, SourceFiles, HeaderFiles, CompileEnvironment, WorkingSet, IntermediateDirectory, Graph, out NormalFiles, out AdaptiveFiles);
			...
			List<FileCollection> AllUnityFiles;
			{
				...
				SortedSourceFiles.Sort((A, B) =>
				{
					...
					bool bAIsGenerated = A.AbsolutePath.EndsWith(".gen.cpp", StringComparison.Ordinal) || CompileEnvironment.FileMatchesExtraGeneratedCPPTypes(A.AbsolutePath);
					bool bBIsGenerated = B.AbsolutePath.EndsWith(".gen.cpp", StringComparison.Ordinal) || CompileEnvironment.FileMatchesExtraGeneratedCPPTypes(B.AbsolutePath);
					...
					{
						bool bAIsOversized = FileLength(A) > NumIncludedBytesPerUnitySource;
						bool bBIsOversized = FileLength(B) > NumIncludedBytesPerUnitySource;
...
#pragma warning disable CA1308 // Normalize strings to uppercase - File extensions are safe to lowercase under normal usage
...
#pragma warning restore CA1308
			...
			out List<FileItem> AdaptiveFiles)
		{
			...
			foreach (FileItem CPPFile in CPPFiles)
			{
				...
				if (CPPFile.Name.EndsWith(".gen.cpp", StringComparison.Ordinal))
				{
					// Ignore these files, does not need to be in adapt 
				}
				else if (CompileEnvironment.MetadataCache.GetFirstInclude(CPPFile) is string FirstInclude &&
			...
			if (Target.bAdaptiveUnityCompilesHeaderFiles)
			{
				foreach (FileItem HeaderFile in UnhandledHeaderFilesInWorkingSet)
				{
					...
					if (CompileEnvironment.MetadataCache.GetHeaderUnitType(HeaderFile) != HeaderUnitType.Valid)
					{
						continue;
					}
