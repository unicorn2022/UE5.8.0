...
namespace UnrealBuildTool
{
	...
	/// </summary>
	public static class InstalledPlatformInfo
	{
...
#pragma warning restore CA1810
		...
		private static void ParsePlatformConfiguration(string PlatformConfiguration)//, ILogger Logger)
		{
			...
			{
				_ = Enum.TryParse(ConfigurationName, out Configuration);
			}
			...
			{
				RequiredFile = FileReference.Combine(Unreal.RootDirectory, RequiredFile).ToNormalizedPath();
			}
			...
			{
				_ = Enum.TryParse(ProjectTypeName, out ProjectType);
			}
		...
		{
			Result = String.Empty;
			int MatchIndex = TrimmedLine.IndexOf(Match, StringComparison.Ordinal);
			if (MatchIndex < 0)
			{
				return false;
			}
		...
		public static void WriteConfigFileEntries(List<InstalledPlatformConfiguration> Configs, List<string> OutEntries)
		{
			...
			{
				WriteConfigFileEntry(Config, OutEntries);
			}
		}

		private static void WriteConfigFileEntry(InstalledPlatformConfiguration Config, List<string> OutEntries)
		{
