...
#pragma warning restore CA1707
		...
		private static void MergeWithParent(PluginInfo Parent, PluginInfo Child)
		{
			...
			if (Child.Descriptor.Modules != null)
			{
				...
				else
				{
					...
					{
						ModuleDescriptor? ParentModule = Parent.Descriptor.Modules.FirstOrDefault(x => x.Name.Equals(ChildModule.Name, StringComparison.Ordinal) && x.Type == ChildModule.Type);
			...
			if (Child.Descriptor.Plugins != null)
			{
				...
				else
				{
					...
					{
						PluginReferenceDescriptor? ParentPluginReference = Parent.Descriptor.Plugins.FirstOrDefault(x => x.Name.Equals(ChildPluginReference.Name, StringComparison.Ordinal));
		...
		public static IReadOnlyList<PluginInfo> ReadPluginsFromDirectory(DirectoryReference RootDirectory, string Subdirectory, PluginType Type)
		{
			...
			return PluginDirectoryCache.GetOrAdd(pluginDir, new Lazy<List<PluginInfo>>(() =>
			{
				...
				Parallel2.ForEach(RootDirectories.SelectMany(Dir => PluginsBase.EnumeratePlugins(Dir)), (PluginFileName) =>
				{
					CreatedInfos.TryAdd(PluginFileName, new PluginInfo(PluginFileName, Type));
				});

				// Then add them properly sorted
