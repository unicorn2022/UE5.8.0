...
{
	UE_LOGF( LogInit, Error, "UnrealAtoS - not enough parameters." );
	UE_LOGF( LogInit, Error, " ... usage: UnrealAtoS <binary-image> [-s <binary-signature>] [-d <usymbol-path>] [-l <module-load-address>] <program-counter>" );
}
...
int32 RunUnrealAtoS(int32 ArgC, TCHAR* Argv[])
{
	...
	if(ModulePath && ProgramCounter)
	{
		...
		{
			UE_LOGF( LogInit, Error, "UnrealAtoS - no such symbol." );
			return 1;
		}
	}
	else
	{
		UE_LOGF( LogInit, Error, "UnrealAtoS - invalid arguments." );
		return 1;
	}
}
