...
namespace Gauntlet
{
	...
	internal abstract class WinBasePGOPlatform : IPGOPlatform
	{
		...
		private void GatherResultsMSVC(string PathToPGOArtifacts)
		{
			...
			{
				Log.Error(KnownLogEvents.Gauntlet, "No .pgc files were generated. Make sure that you are using a build with PGO instrumentation.");
		...
		public void GatherResults(string ArtifactPath)
		{
			...
			{
				FileInfo Info = new FileInfo(ExpectedProfrawFileName);
				if (Info.Length == 0)
				{
					throw new AutomationException($"file has zero length, probably crashed - {ExpectedProfrawFileName}");
				}
