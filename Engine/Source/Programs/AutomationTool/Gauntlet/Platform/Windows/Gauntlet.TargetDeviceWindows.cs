...
namespace Gauntlet
{
	...
	public class TargetDeviceWindows : TargetDeviceDesktopCommon
	{
		...
		public override IAppInstance Run(IAppInstall App)
		{
			...
			ILongProcessResult Result = new LongProcessResult(
		...
		protected override IAppInstall CreateStagedInstall(UnrealAppConfig AppConfig, StagedBuild Build)
		{
			...
			string ExecutablePath = Path.IsPathRooted(Build.ExecutablePath)
		...
		protected override IAppInstall InstallStagedBuild(UnrealAppConfig AppConfig, StagedBuild InBuild)
		{
			...
			WinApp.WorkingDirectory = Path.GetDirectoryName(WinApp.ExecutablePath);
			...
			else
			{
				...
				if (CommandUtils.IsBuildMachine)
				{
					...
					if (CDB != null)
					{
						...
						if (Stack.Any())
						{
							Log.Error(KnownLogEvents.Horde_BuildHealth_Ignore, "Stack analysis produced with 'cdb {Args}':\n{StackTrace}", Args, string.Join('\n', Stack));
						}
						else
						{
							Log.Warning(KnownLogEvents.Horde_BuildHealth_Ignore, "Could not find stack analysis from cdb command.");
						}
					}
				}
			}
	...
	public class Win64DeviceFactory : IDeviceFactory
	{
		...
		{
			if (RemoteWinSupport.IsRemoteDevice(InRef, UnrealTargetPlatform.Win64))
			{
				return new TargetDeviceRemoteWin(InRef, InCachePath, UnrealTargetPlatform.Win64);
			}
