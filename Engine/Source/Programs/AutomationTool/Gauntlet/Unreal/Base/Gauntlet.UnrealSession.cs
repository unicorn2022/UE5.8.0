...
namespace Gauntlet
{
	...
	public class UnrealSessionRole
	{
		...
		/// <summary>
		/// Whether to respawn this process automatically if it fails unexpectedly
		/// </summary>
		public bool RespawnProcessOnUnexpectedExit { get; set; }

		/// <summary>
		/// What to do if this role exits unexpectedly.
		/// </summary>
		public OnTryToRestoreProcessDelegate OnTryToRestoreProcess;
	...
	public class UnrealSessionInstance : IDisposable
	{
		...
		{
			return LaunchDeferredRole(Role, out _);
		}
		...
		/// <returns></returns>
		public bool LaunchDeferredRole(UnrealSessionRole Role, out RoleInstance RoleInstance)
		{
			RoleInstance = null;
			...
			try
			{
				...
				if (Instance != null || Globals.CancelSignalled)
				{
					...
					RoleInstance = new RoleInstance(Role, Instance);
					AllRoles[CurrentRoleInstanceIndex] = RoleInstance;
				}
		...
		/// <summary>
		/// Tries to recover a process for a role that exited unexpectedly.
		/// </summary>
		/// <param name="Role"></param>
		/// <returns></returns>
		public bool RecoverProcessForRole(UnrealSessionRole Role, IAppInstall CurInstall)
		{
			int CurrentRoleInstanceIndex = AllRoles.FindIndex(X => X.Role == Role);
			if (CurrentRoleInstanceIndex < 0)
			{
				Log.Error("Cannot restart role {0} because it cannot be found among the 'all roles' list", Role.ToString());
				return false;
			}
			bool Success = false;
			...
			try
			{
				Log.Info("Attempting to restore {0} on {1}", Role, CurInstall.Device);
				IAppInstance Instance = CurInstall.Run();
			
				if (Instance != null || Globals.CancelSignalled)
				{
					// Update to the new wowrking instance.
					AllRoles[CurrentRoleInstanceIndex].AppInstance = Instance;
				}
			...
			{
				Log.Error("Cannot restart role {0} because device {Name} threw an exception during launch. \nException={Exception}", Role.ToString(), CurInstall.Device, Ex.Message);
				Success = false;
			}
	...
	public class UnrealSession : IDisposable
	{
		...
		public UnrealSession(UnrealBuildSource InSource, IEnumerable<UnrealSessionRole> InSessionRoles)
		{
			...
			if (!CheckRolesArePossible(ref ValidationIssues))
			{
				...
				Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.BuildNotFound, "The test was unable to find the necessary executables.");
		...
		{
			if(Globals.Params.ParseParam("LegacyLaunchSession"))
			{
		...
		public void ReleaseSessionDevices(bool bForce = false)
		{
			...
			UnrealDeviceReservation.ReleaseDevices(bForce);
		...
		{
			string DummyOutputString = null;
			return SaveRoleArtifacts(InContext, InRunningRole, DestinationArtifactPath, ref DummyOutputString);
		}

		public UnrealRoleArtifacts SaveRoleArtifacts(UnrealTestContext InContext, UnrealSessionInstance.RoleInstance InRunningRole, string DestinationArtifactPath, ref string DirectoryToCleanUp)
		{
			...
			{
				if (DirectoryToCleanUp != null)
				{
					DirectoryToCleanUp = SourceDirectory.FullName;
				}
				else
				{
		...
		public IEnumerable<UnrealRoleArtifacts> SaveRoleArtifacts(UnrealTestContext Context, UnrealSessionInstance TestInstance, string OutputPath)
		{
			...
			HashSet<string> DirectoriesToDelete = new();
			foreach (UnrealSessionInstance.RoleInstance App in TestInstance.RunningRoles)
			{
				...
				if (!App.Role.IsNullRole() && !App.Role.InstallOnly)
				{
					...
					try
					{
						...
						Artifacts = SaveRoleArtifacts(Context, App, DestPath, ref DirectoryToDelete);
						if (!string.IsNullOrEmpty(DirectoryToDelete))
						{
							DirectoriesToDelete.Add(DirectoryToDelete);
						}
			...
			{
				DirectoryInfo Directory = new DirectoryInfo(DirectoryToDelete);
				try
				{
				...
				{
					Log.Info("Could not remove the read-only attribute from {SourceDirectory}.", Directory);
				}
				try
				{
				...
				{
					Log.Info("Encountered an exception when deleting source artifacts at {SourceDirectory}. Artifacts will remain on the device.\n\tError message: {Exception}", Directory, Exception.Message);
				}
			}
		...
		private UnrealSessionInstance Legacy_LaunchSession()
		{
			...
			while (SessionInstance == null && Globals.CancelSignalled == false)
			{
				...
				foreach (UnrealSessionRole Role in SortedRoles)
				{
					...
					if (RolesToInstalls == null || !RolesToInstalls.ContainsKey(Role) || ReinstallPerPass)
					{
						...
						catch (Exception Ex)
						{
							...
							{
								Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.NotEnoughDriveSpace, ErrorMessage);
				...
				if (InstallSuccess == false)
				{
					...
					if (SessionRetries == 0)
					{
						Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.InvalidPerfSpec, "Unable to install application for session.");
			...
			foreach(UnrealSessionRole Role in SessionRoles)
			{
				...
				catch(Exception Ex)
				{
					...
					if (IsOutOfSpaceException(Ex) && IsHostPlatform)
					{
						...
						Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.NotEnoughDriveSpace, Message);
					...
					else if (IsOverlayException(Ex))
					{
						...
						Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.OverlayException, Message);
		...
		private UnrealSessionInstance LaunchProcesses()
		{
			...
			foreach (KeyValuePair<UnrealSessionRole, IAppInstall> RoleInstall in RolesToInstalls)
			{
				...
				try
				{
					...
					if (AppInstance == null)
					{
						throw new AutomationException("Failed to create an IAppInstance after attempting Run() on {0}", Install);
					}
					else
					{
				...
				catch(Exception Ex)
				{
					...
					string WarningMessage = $"Encountered exception when attempting to run install: {Ex}";
					if (Ex is DeviceException)
					{
