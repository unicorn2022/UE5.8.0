...
namespace Gauntlet
{
		...
		where TConfigClass : UnrealTestConfiguration, new()
	{
		...
		/// <summary>
		/// Test Metadata for audit
		/// </summary>
		public ITestAuditMetadata TestMetadata;

#nullable enable
		/// <summary>
		/// Unique key to identify and cross reference test across test sessions 
		/// </summary>
		protected virtual string ReportTestKey
		{
			get
			{
				return Type;
		...
		protected virtual string HordeReportTestKey => ReportTestKey;

		/// <summary>
		/// The test name used in report
		/// </summary>
		protected virtual string ReportTestName
		{
			get
			{
		...
		protected virtual string HordeReportTestName => ReportTestName;

		/// <summary>
		/// The harness used to execute the test
		/// </summary>
		protected virtual TestHarness? TestHarness => null;

		/// <summary>
		/// The test profile (unit test, functional, performance)
		/// </summary>
		protected virtual TestProfile? TestProfile => null;

		/// <summary>
		/// The team for which the test is for
		/// </summary>
		protected virtual TestTeam? TestTeam => null;

		/// <summary>
		/// Short description of the test intent
		/// </summary>
		protected virtual string? TestIntent => null;

		/// <summary>
		/// The tags associated with this test
		/// </summary>
		protected virtual TestTag[]? TestTags => null;
#nullable disable
		...
		/// <summary>
		/// Report an error - Legacy.  Instead, please consider using ReportInterrupt for functional test problems, or ReportFailure for actual test failures
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public virtual void ReportError(string Message, params object[] Args)
		{
			Reporter.Instance.Failure(Message, Args);
		}

		/// <summary>
		/// Report a test-halting error that is NOT directly related to the behavior that is being tested.
		/// This method is deprecated. Please use Gauntlet.Reporter.Instance.Interrupt instead of this method.
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public virtual void ReportInterrupt(string Message, params object[] Args)
        {
			Reporter.Instance.Interrupt(Message, Args);
		}

		/// <summary>
		/// Report a test failure.  Specifically, the behavior that is targeted by the test case did not function as expected.
		/// This method is deprecated. Please use Gauntlet.Reporter.Instance.Failure instead of this method.
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public virtual void ReportFailure(string Message, params object[] Args)
        {
			Reporter.Instance.Failure(Message, Args);
		}
		...
		/// <summary>
		/// Report a warning.
		/// This method is deprecated. Please use Gauntlet.Reporter.Instance.Warning instead of this method.
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public virtual void ReportWarning(string Message, params object[] Args)
		{
			Reporter.Instance.Warning(Message, Args);
		}

		/// <summary>
		/// Report information.
		/// This method is deprecated. Please use Gauntlet.Reporter.Instance.Info instead of this method.
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public virtual void ReportInfo(string Message, params object[] Args)
		{
			Reporter.Instance.Info(Message, Args);
		}
		...
		/// </summary>
		protected IReadOnlyCollection<UnrealTestEvent> TestNodeEvents => _TestNodeEvents;
		private ConcurrentQueue<UnrealTestEvent> _TestNodeEvents { get; set; } = new();

		public override void AddTestEvent(UnrealTestEvent InEvent)
		{
			_TestNodeEvents.Enqueue(InEvent);
		}
		...
		public UnrealTestNode(UnrealTestContext InContext)
		{
			...
			(
				...
				() => TestVersion,
				() => MaxDuration / 60,
				() => GetRunLocalCommand(Globals.BuildCommand),
				() => TestHarness,
				() => TestProfile,
				() => TestTeam,
				() => TestIntent,
				() => TestTags,
				() => GetDistilledTestParams()
						.ToLookup(A => A.Key, A => A.Value) /* use a lookup here to avoid duplicate param key */
		...
		protected virtual IEnumerable<RoleInstance> FindAnyMissingRoles()
		{
			return TestInstance.RunningRoles.Where(R => R.AppInstance.HasExited);
		}

		protected virtual bool TryRestoreAllMissingRoles(IEnumerable<UnrealSessionInstance.RoleInstance> MissingRoles)
		{
			// We want to make sure we don't start respawning some processses if vital ones are missing
			foreach (UnrealSessionInstance.RoleInstance RoleInstance in MissingRoles)
			{
				if (!RoleInstance.Role.RespawnProcessOnUnexpectedExit)
				{
					return false;
				}
			}
			foreach (UnrealSessionInstance.RoleInstance RoleInstance in MissingRoles)
			{
				if (!UnrealApp.RolesToInstalls.ContainsKey(RoleInstance.Role))
				{
					return false;
				}
				if (RoleInstance.Role.OnTryToRestoreProcess != null)
				{
					RoleInstance.Role.OnTryToRestoreProcess.Invoke(RoleInstance);
				}
				IAppInstall DesiredInstall = UnrealApp.RolesToInstalls[RoleInstance.Role];
				UnrealApp.SessionInstance.RecoverProcessForRole(RoleInstance.Role, DesiredInstall);
			}
		...
		public virtual bool IsTestRunning()
		{
			...
			if (TryRestoreAllMissingRoles(MissingRoles))
			{
				return true;
			}
		...
		protected bool PrepareUnrealApp()
		{
			...
			if (Config == null)
			{
				Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.InvalidConfig, "Null config returned for this test.");
			...
			foreach (var TypesToRoles in Config.RequiredRoles)
			{
				...
				foreach (UnrealTestRole TestRole in TypesToRoles.Value)
				{
					...
					SessionRole.RespawnProcessOnUnexpectedExit = TestRole.RespawnProcessOnUnexpectedExit;
					SessionRole.OnTryToRestoreProcess = TestRole.OnTryToRestoreProcess;
		...
		public override bool StartTest(int Pass, int InNumPasses)
		{
			...
			{
				if (InNumPasses > 1 && DevicePool.DeviceReservationBlock)
				{
					// We throw here because we don't want different devices used for a multi-iteration test when using a reservation block
					throw new AutomationException("Failed to start test iteration {0} when using a device reservation block. Terminating the test.", Pass);
				}
				return false;
			}
		...
		/// <param name="TextLink">The Text you want to show as URL link. Default is "Open link".</param>
		public void AddLinkToReport(ITestReport Report, string ReportName, string URLLink, string TextLink = null)
		{
			if (Report == null)
			{
			...
			Horde.OnStepSummaryFinalization += () => $"* **{ReportName}**: [{TextLink}]({URLLink})";
			Horde.OnJobSummary += () => $"* **{ReportName}**: [{TextLink}]({URLLink})";
		}
		...
		protected virtual IEnumerable<KeyValuePair<string, object>> GetDistilledTestParams()
		{
			HashSet<string> ArgsToNotDisplay =
			new List<string>{
					"test",
					"tests",
					"tempdir",
					...
					"PerfReportServer",
					"sandbox",
			}.Select(I => I.ToLower()).ToHashSet();

			return Context.TestParams.AllArguments.Select(A =>
			{
				var Arg = A.Split('=', 2);
				return new KeyValuePair<string, object>(Arg[0], Arg.Length > 1 ? (Arg[1].Equals("false", StringComparison.OrdinalIgnoreCase)? false : Arg[1]) : true);
			}).Where(A => !ArgsToNotDisplay.Contains(A.Key.ToLower()));
		}

		public override string GetRunLocalCommand(string LaunchingBuildCommand)
		{

			string WrapParameterInQuotes(string ParamStrName, string ParamStrValue)
			{
			...
			var CleanedArgs = GetDistilledTestParams().Select(Arg => Arg.Value.Equals(true)? Arg.Key : WrapParameterInQuotes(Arg.Key, Arg.Value.ToString()));
		...
		{
			if (Context.TestParams.ParseParam("FailurePrtScr") && (GetTestResult() != TestResult.Passed || InReason != StopReason.Completed))
			{
				foreach (ITargetDevice TargetDevice in UnrealApp.UnrealDeviceReservation.ReservedDevices)
				{
					TargetDevice.TakeScreenshot("FailureScreenshot.png");
				}
			}
			...
			if (RoleResults.Any(R => R.ProcessResult == UnrealProcessResult.EncounteredFatalError))
			{
				UnrealRoleResult CrashedRole = RoleResults.FirstOrDefault(R => R.ProcessResult == UnrealProcessResult.EncounteredFatalError);
				if (CrashedRole != null)
				{
					if (CrashedRole.Artifacts.SessionRole.RoleType == UnrealTargetRole.Client)
					{
						Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.ClientCrashDetected);
					}
					else if (CrashedRole.Artifacts.SessionRole.RoleType == UnrealTargetRole.Server)
					{
						Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.ServerCrashDetected);
					}
					else if (CrashedRole.Artifacts.SessionRole.RoleType == UnrealTargetRole.Editor)
					{
						Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.EditorCrashDetected);
					}
				}
			}
			...
			{
				if (RoleResults.Any(R => R.ProcessResult == UnrealProcessResult.TimeOut))
				{
					Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.TestTimeoutExceeded);
					SetTestResult(TestResult.TimedOut);
				}
				else if (RoleResults.Any(R => R.ProcessResult == UnrealProcessResult.EncounteredFatalError))
				{
					SetTestResult(TestResult.Failed);
				}
				else if (!GetCachedConfiguration().IgnoreRoleFailures)
				{
					SetTestResult(TestResult.OperationalException);
				}
			}
		...
		protected virtual ITestReport CreateSimpleReportForHorde(TestResult Result)
		{
			...
			{
				AutomatedTestSessionData TestReport = new AutomatedTestSessionData(TestMetadata);
		...
		public virtual ITestReport CreateUnrealEngineTestPassReport(string UnrealAutomatedTestReportPath, string ReportURL)
		{
			...
			if (!Horde.UseTestDataV1)
			{
				AutomatedTestSessionData SessionReport = new AutomatedTestSessionData(TestMetadata);
				string ArtifactPathLabel = new DirectoryInfo(ArtifactPath).Name;
				...
				if (RoleResults != null)
				{
					foreach (UnrealRoleResult Item in RoleResults)
					{
						// Add the related devices to initialization phase
			...
			if (IsFileReportExist)
			{
				...
				{ // Populate v2 if set before
					...
					if (InitPhase != null)
					{
						...
						{
							bool bAllowZeroTestResults = Globals.Params.ParseParam("AllowZeroTestResults");

							var Stream = InitPhase.GetStream();
							if (bAllowZeroTestResults)
							{
								Stream.AddWarning("No tests were executed, check test filter!");
							}
							else
							{
								Stream.AddError("No tests were executed!");
							}
							if (RoleResults != null)
							{
								// Add any relevant events collected through log parsing
								foreach (UnrealRoleResult RoleResult in RoleResults)
								{
									foreach (UnrealTestEvent Item in RoleResult.Events)
									{
				...
				else if (HordeTestPassResults == null)
				{
					// Legacy report
			...
			else
			{
				...
				if (HordeTestPassResults is AutomatedTestSessionData SessionReport)
				{
					...
					if (RoleResults != null)
					{
						// Check if there is any non-OK exit to add more context
						foreach (UnrealRoleResult RoleResult in RoleResults.Where(R => R.ProcessResult != UnrealProcessResult.ExitOk && R.ProcessResult != UnrealProcessResult.EncounteredEnsure))
						{
							string RoleType = RoleResult.Artifacts.SessionRole.RoleType.ToString();
							var Stream = InitPhase.GetStream();
							Stream.AddError("{Role}: {Summary}", RoleType, RoleResult.Summary);
							foreach (UnrealTestEvent Item in RoleResult.Events)
							{
								Stream.AddEvent(Item);
							}

							// In case of a fatal error or initialization failure, it is an interrupted outcome
							if (RoleResult.ProcessResult == UnrealProcessResult.EncounteredFatalError || RoleResult.ProcessResult == UnrealProcessResult.InitializationFailure)
							{
								if (RoleResult.ProcessResult == UnrealProcessResult.InitializationFailure)
								{
			...
			{ // Populate generic data and artifacts
				...
				if (SessionArtifacts != null)
				{
					foreach (UnrealRoleArtifacts Artifact in SessionArtifacts)
					{
						...
						{
							string LogName = FileUtils.ConvertPathToUnix(Path.GetRelativePath(Path.GetFullPath(Horde.UseTestDataV1 ? Context.Options.LogDir : ArtifactPath), Path.GetFullPath(Artifact.LogPath)));
		...
		protected virtual void SetReportMetadata(ITestReport Report, IEnumerable<UnrealTestRole> Roles)
		{
			...
			// Additional metadata based on test config
			if (GetCachedConfiguration() is UnrealTestConfiguration TestConfig)
			{
				if (!string.IsNullOrEmpty(TestConfig.Map))
				{
					Report.SetMetadata("Map", Path.GetFileName(TestConfig.Map));
				}
			}
		}
		...
		private void CheckHeartbeat()
		{
			...
			if (LastActiveHeartbeatTime == DateTime.MinValue && HeartbeatOptions.TimeoutBeforeFirstActiveHeartbeat > 0)
			{
				...
				if (SecondsSinceSessionStart > HeartbeatOptions.TimeoutBeforeFirstActiveHeartbeat)
				{
					...
					Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.HeartbeatTimeout);
		...
		protected static LogEvent DecomposeAssert(UnrealTestEvent Event, int MaxCallstackLines = 0)
		{
			...
			{
				CallstackString = CallstackString.Replace("{", "{{").Replace("}", "}}");
		...
		protected virtual void LogRoleSummary(UnrealRoleResult InRoleResult)
		{
			...
			if (HasFailed && !GetCachedConfiguration().IgnoreRoleFailures)
			{
			...
			foreach (UnrealTestEvent Event in Asserts)
			{
				...
				if (GetCachedConfiguration().IgnoreRoleFailures)
				{
					Log.Info(AssertData.Id, AssertData.Format ?? AssertData.Message, ((Dictionary<string, object>)AssertData.Properties)?.Values.ToArray());
				}
				else
				{
			...
			foreach (UnrealTestEvent Event in Ensures.Distinct())
			{
				...
				if (GetCachedConfiguration().IgnoreRoleFailures)
				{
					Log.Info(KnownLogEvents.Gauntlet_TestEvent, " * Ensure: {Summary}\n{Callstack}", Event.Summary, string.Join("\n", Callstack.Select(C => "    " + C)));
				}
				else
				{
			...
			if (Errors.Any())
			{
				...
				foreach (var Error in PrintedErrorList)
				{
					if (GetCachedConfiguration().IgnoreRoleFailures)
					{
						Log.Info(KnownLogEvents.Gauntlet_TestEvent, " * {Message}", Error);
					}
					else
					{
        ...
        protected virtual IEnumerable<UnrealRoleResult> GetRolesThatFailed()
        {
            ...
            return RoleResults.Where(R => R.ProcessResult == UnrealProcessResult.TestFailure);
        }

		/// <summary>
		/// Return all artifacts that are considered to have caused a test Interrupt
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerable<UnrealRoleResult> GetRolesThatInterrupted()
		{
			if (RoleResults == null)
			{
				Log.Warning("RoleResults was null, unable to check for Interrupts");
				return Enumerable.Empty<UnrealRoleResult>();
			}

			return RoleResults.Where(R => R.ProcessResult != UnrealProcessResult.ExitOk && R.ProcessResult != UnrealProcessResult.TestFailure);
		}
		...
		protected virtual TestResult GetUnrealTestResult()
		{
            ...
            List<UnrealRoleResult> InterruptRoles = GetRolesThatInterrupted().ToList();
            List<UnrealRoleResult> FailedRoles = GetRolesThatFailed().ToList();

			bool bInterruptFound = false;

			// Prioritizing Failed tests over Interrupted tests, because Failed tests are more likely to be the result of a product bug.
			if (FailedRoles.Any())
			{
			...
			else if (InterruptRoles.Any())
            {
				bInterruptFound = true;
                foreach (var Role in InterruptRoles)
                {
                    Log.Info("Test Interrupt because {Role} exited with {ExitCode}. ({Result})", Role.Artifacts.SessionRole, Role.ExitCode, Role.Summary);
                }
                ExitCode = InterruptRoles.FirstOrDefault().ExitCode;
            }

			// This is a bit of an extrapolation from the original code, which looked for a combination of a good exit code but with a "cancelled" flag, 
			// and then marked that combination as "Failed".  Since we're now splitting "failures" from other problems, we're going a bit more granular.
			// Since we have a "Cancelled" test result available, we'll make use of it, and chalk any other interrupt up as a general "OperationalException".
			if (bInterruptFound)
			{
				return (ExitCode == 0 && WasCancelled) ? TestResult.Cancelled : TestResult.OperationalException;
			}
		...
		protected virtual void LogTestSummaryHeader()
		{
			...
			bool TestHadErrors = GetTestResult() != TestResult.Passed;

			// Good/Bad news upfront
			string Prefix = TestHadErrors ? "Error: " : "";
			string WarningStatement = (HasWarnings && !TestHadErrors) ? " With Warnings" : "";
			...
			if (TestHadErrors && GetRolesThatFailed().Where(R => R.ProcessResult == UnrealProcessResult.Unknown).Any())
			{
				Log.Info("");
