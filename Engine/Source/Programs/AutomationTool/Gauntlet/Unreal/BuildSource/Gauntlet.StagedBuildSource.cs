...
namespace Gauntlet
{
	...
	public class StagedBuild : IBuild
	{
		...
		public virtual IStagedBuildFilter GetInstallFilter(UnrealAppConfig AppConfig)
		{
			return IStagedBuildFilter.GetFilterForPlatform(AppConfig.Platform.Value);
		}
		...
		public static string InstallBuildParallel(UnrealAppConfig AppConfig, StagedBuild InBuild, string BuildPath, string DestPath, string Desc)
		{
			...
			if (Install)
			{
				try
				{
					...
					IStagedBuildFilter Filter = InBuild.GetInstallFilter(AppConfig);
					if (Filter != null)
					{
