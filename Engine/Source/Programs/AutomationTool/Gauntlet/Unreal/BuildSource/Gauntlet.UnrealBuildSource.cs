...
namespace Gauntlet
{
	public class UnrealBuildSource : IBuildSource
	{
		...
		virtual protected bool ResolveBuildReference(string InBuildReference, Func<string, string> ResolutionDelegate, out IEnumerable<string> OutBuildPaths, out string OutBuildName)
		{
			...
			{
				Log.Error("Unable to resolve build argument '{BuildReference}'", InBuildReference);
				Horde.UpdateStepMessage(Horde.GauntletHordeStepMessage.BuildNotFound);
				return false;
			}
		...
		virtual public UnrealAppConfig CreateConfiguration(UnrealSessionRole Role, IEnumerable<UnrealSessionRole> OtherRoles)
		{
			...
			{
				Config.OverlayExecutable = new OverlayExecutable(Role, Config.ProjectName, Config.ProjectFile);
			}
		...
		private string GenerateProcessedCommandLine(string InCommandLine)
		{
			...
			Dictionary<string, List<string>> CommandlineTokens = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
			...
			foreach (Match M in Matches)
			{
				...
				if (CommandlineTokens.ContainsKey(Name))
				{
					...
					else if (Name.ToLower() == "execcmds")
					{
						...
						CommandlineTokens[Name][0] = string.Format("\"{0}, {1}\"", CommandlineTokens[Name][0].Replace("\"", ""), Params.Replace("\"", ""));
					}
					else if (Name.ToLower() == "dpcvars")
					{
						// dpcvars is also appendable but is slightly different in syntax. It doesn't use quotes and cannot contain spaces
						CommandlineTokens[Name][0] = string.Format("{0},{1}", CommandlineTokens[Name][0], Params);
					}
					else
					{
						bool bFoundDuplicateValue = false;
						foreach (string ExistingValue in CommandlineTokens[Name])
						{
							if (ExistingValue == Params)
							{
								Log.Info(string.Format("Duplicate flag {0}={1} found and ignored. Please fix this. ", Name, Params));
								bFoundDuplicateValue = true;
								break;
							}
						}

						if (!bFoundDuplicateValue)
						{
							// dpcvars are also a special parameter because the pattern is to repeat it like: -dpcvars=var1=val1 -dpcvars=var2=val2 etc
							if (Name.ToLower() != "dpcvars")
							{
								...
								CommandlineTokens[Name][0] = Params.Trim();
							}
							else
							{
								CommandlineTokens[Name].Add(Params);
							}
						}
					}
				}
				...
				{
					CommandlineTokens.Add(Name, new List<string>{ Params });
				}
			}
			...
			{
				foreach (string DictValue in CommandlineTokens[DictKey])
				{
					CommandlineToReturn += string.Format("-{0}{1} ",
						DictKey,
						string.IsNullOrWhiteSpace(DictValue) ? "" : string.Format("={0}", DictValue)
						);
				}
			}
