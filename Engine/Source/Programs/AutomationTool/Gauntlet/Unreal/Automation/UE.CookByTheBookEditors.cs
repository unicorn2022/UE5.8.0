...
namespace UE
{
	...
	public class CookByTheBookEditors : CookByTheBookEditor
	{
		...
		protected void RestartEditorRole()
		{
			...
			{
				ReportError($"Couldn't launch the deferred role {UnrealTargetRole.Editor}");
		...
		private void StopRunningRoles()
		{
			...
			{
				ReportError("Couldn't stop the running roles");
				CompleteTest(TestResult.Failed);
				return;
			}

			ReportInfo("Shutting down the running roles");
