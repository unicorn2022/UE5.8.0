...
namespace UE
{
	...
	public class CookOnTheFly : UnrealTestNode<UnrealTestConfiguration>
	{
		...
		public override void TickTest()
		{
			...
			{
				Phases.Start("Start Cook Server");
				return;
			}
			...
			{
				ReportError("No logfile activity observed in last {0} minutes. Ending test", TimeoutDuration);
				MarkTestComplete();
			...
			{
				ReportError("Editor instance shouldn't be null");
				MarkTestComplete();
			...
			if (!ServerStarted)
			{
				...
				{
					ReportInfo("Found '{0}'. Cook Server Started", CompletionString);
					ServerStarted = true;
					Phases.Get("Start Cook Server")?.End();
					Phases.Start("Start Client");
				}
			}
			...
			if (ServerStarted && !DeferredClientStarted)
			{
				...
				{
					ReportError($"Couldn't launch the deferred role {UnrealTargetRole.Client}");
					MarkTestComplete();
				...
				Phases.Get("Start Client")?.End();
				Phases.Start("Connect to Client");
			}
			...
			if (!ClientConnected)
			{
				...
				{
					ReportInfo("Found '{0}'. Client connected", ClientConnectedString);
					ClientConnected = true;
					Phases.Get("Connect to Client")?.End();
					Phases.Start("Change CookOnTheFly Mode");
				}
			}
			...
			if (!CookingRequest)
			{
				...
				{
					ReportInfo("Found '{0}'. Cooking request exists", CookingRequestString);
					CookingRequest = true;
					Phases.Get("Get Cooking Request")?.End();
					Phases.Start("Check for active CookOnTheFly Channel");
				}
			}
			...
			if (CookingProcessStarted && !CookOnTheFlyModeChange && File.Exists(CookedSettingsFilePath))
			{
				...
				{
					ReportInfo("Found '{0}'. The cook mode changed for the project", CookModeString);
					CookOnTheFlyModeChange = true;
					Phases.Get("Change CookOnTheFly Mode")?.End();
					Phases.Start("Get Cooking Request");
				}
			}
			...
			if (CookOnTheFlyModeChange)
			{
				...
				{
					ReportError("There should only be one client instance");
					MarkTestComplete();
					SetUnrealTestResult(Gauntlet.TestResult.Failed);
				}
				...
				{
					ReportError("Client instance shouldn't be null");
					MarkTestComplete();
					SetUnrealTestResult(Gauntlet.TestResult.Failed);
				}
				...
				{
					ReportInfo("Found '{0}', '{1}', '{2}'. The CookOnTheFly log channel is active. The cooking process is taking place.", GetGameStartString(), GetCookingProcessString(), GetReceivedPackagesCookedString());
					Phases.Get("Check for active CookOnTheFly Channel")?.End();
					MarkTestComplete();
