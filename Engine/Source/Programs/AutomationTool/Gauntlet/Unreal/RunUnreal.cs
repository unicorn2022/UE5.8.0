...
namespace Gauntlet
{
	...
	[ParamHelp("CookedEditorFlavor", "Specifies RequiredFlavor for the cooked editor role", ParamType = typeof(string))]
	...
	public class RunUnreal : BuildCommand
	{
		...
		/// <summary>
		/// Constant for environment variable that can be set to trigger the cleanup of Options.TempDir upon completion
		/// </summary>
		private const string CleanEnvironmentVariable = "UE_GauntletCleanTempDir";
		...
		public override ExitCode Execute()
		{
			...
			Globals.BuildCommand = ToString().Split(".").Last();
		...
		public virtual ExitCode RunTests(UnrealTestOptions ContextOptions)
		{
			...
			// Use this flag if iterating in horde to get a test immediately declared invalid.
			if (ParseParam("forcestepfail"))
			{
				string ErrorMessage = "-forcestepfail specified on commandline.";
				Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.TestStepMessage, ErrorMessage);
			...
			{
				string ErrorMessage = "No build specified. Use -build=p:\\path\\to\\build";
				Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.BuildNotSpecified, ErrorMessage);
				throw new AutomationException(ErrorMessage);
			}

			if (typeof(UnrealBuildSource).IsAssignableFrom(ContextOptions.BuildSourceType) == false)
			{
				string ErrorMessage = "Provided BuildSource type does not inherit from UnrealBuildSource";
				Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.InvalidBuildSource, ErrorMessage);
				throw new AutomationException(ErrorMessage);
			}
			...
			Horde.GenerateStepSummary();
			Horde.GenerateJobSummary();
		...
		bool ExecuteTests(UnrealTestOptions Options, IEnumerable<ITestNode> TestList)
		{
			...
			finally
			{
				...
				bool bCleanFromArgument = ParseParam("Clean");
				bool bCleanFromEnvironmentVariable = bool.TryParse(Environment.GetEnvironmentVariable(CleanEnvironmentVariable), out bool bCleanEnvvar) && bCleanEnvvar;
				bool bCleanFromEnvvarAllowed = !DevicePool.DeviceReservationBlock || DevicePool.DeviceReservationBlockFinalStep;
				if (bCleanFromArgument || (bCleanFromEnvironmentVariable && bCleanFromEnvvarAllowed))
				{
					// Flush any undisposed file streams that may be holding onto tempdir files
					GC.Collect();
					GC.WaitForPendingFinalizers();

					Logger.LogInformation("Cleaning TempDir '{GauntletTempDir}' by request...", Options.TempDir);
					DeleteDirectory(Options.TempDir);
				}
		...
		IEnumerable<ITestNode> CreateTestList(UnrealTestContext Context, Params DefaultParams, ArgumentWithParams PlatformParams = null)
		{
			...
			foreach (var Test in Context.Options.TestList)
			{
				...
				{
					string ErrorMessage = string.Format("Unable to convert perfspec '{0}' into an EPerfSpec", PerfSpec);
					Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.InvalidPerfSpec, ErrorMessage);
					throw new AutomationException(ErrorMessage);
				}
