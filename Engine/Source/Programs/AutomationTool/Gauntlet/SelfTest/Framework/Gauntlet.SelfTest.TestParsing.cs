...
namespace Gauntlet.SelfTest
{
	...
	class TestParamParsing : BaseTestNode
	{
		...
		public override void TickTest()
		{
			...
			// check default value
			Params MultiArgs4 = new Params(new[] { "foo=one", "foo=two", "foo=three" });
			List<string> MultiDefaultResult4 = ["four", "five"];
			List<string> MultiArgs4Result = MultiArgs4.ParseValues("dummy", Default: MultiDefaultResult4);
			CheckResult(MultiArgs4Result.Count == 2, "Incorrect result count");
			CheckResult(MultiArgs4Result[0] == "four" && MultiArgs4Result[1] == "five", "Incorrect result count");

			// negative check
			Params MultiArgs5 = new Params(new[] { "foo=one" });
			List<string> MultiArgs5Result = MultiArgs5.ParseValues("dummy");
			CheckResult(MultiArgs5Result != null, "Result is NULL");
			CheckResult(MultiArgs5Result.Count == 0, "Incorrect result count");
