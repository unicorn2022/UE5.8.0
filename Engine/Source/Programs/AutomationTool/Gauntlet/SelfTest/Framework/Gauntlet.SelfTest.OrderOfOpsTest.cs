...
namespace Gauntlet.SelfTest
{
	...
	class OrderOfOpsTest : ITestNode
	{
		...
		{
			Log.Info(" >> Executing {Stage}", NewStage.ToString());
		...
		{
			// this is a stage that can hit multiple times.
			// we can just return Passed because SetNewStage will 
			// throw an exception if something is wrong here
			if (CurrentStage != TestStages.GetResult)
			{
				SetNewStage(TestStages.GetResult);
			}
		...
		{
			SetNewStage(TestStages.SetFinalResult);
		}
