...
namespace Gauntlet
{
	public class TestExecutorOptions
	{
		...
		/// <summary>
		/// Optional delay in seconds to wait between test iterations. Primarily used to allow devices to cool down
		/// when benchmarking to mitigate thermal throttling. Default is 0 (no additional delay beyond standard crash processing time).
		/// Users can tune this value based on their specific device thermal characteristics and observed behavior.
		/// </summary>
		[AutoParam(0)]
		public int TestIterationCooldownSeconds;
	...
	public class TestExecutor
	{
		...
		public bool ExecuteTests(TestExecutorOptions InOptions, IEnumerable<ITestNode> RequiredTests)
		{
			...
			Dictionary<string, int> TestIterationsPassed = new Dictionary<string, int>();
			Dictionary<string, int> TestIterationsInterrupted = new Dictionary<string, int>();
			...
			for (CurrentTestPass = 0; CurrentTestPass < Options.TestIterations; CurrentTestPass++)
			{
				...
				if (CurrentTestPass > 0)
				{
					...
					// processing things.
					const int CrashProcessingDelayTimeMs = 10000;

					// Apply optional device cooldown delay to mitigate thermal throttling between benchmark iterations.
					// The delay is user-configurable based on device thermal characteristics.
					// Clamp to valid range [0, 9999] to prevent negative values and overflow.
					const int MaxCooldownSeconds = 9999;
					int CooldownSeconds = Math.Clamp(Options.TestIterationCooldownSeconds, 0, MaxCooldownSeconds);
					int TestIterationCooldownMs = CooldownSeconds * 1000;
					if (CooldownSeconds > 0)
					{
						Log.Info("Cooling down for {CooldownSeconds} seconds", CooldownSeconds);
					}

					Thread.Sleep(Math.Max(CrashProcessingDelayTimeMs, TestIterationCooldownMs));
				}
						...
						for (int i = 0; i < PendingTests.Count(); i++)
						{
							...
							catch (System.Exception ex)
							{
								...
								else
								{
									...
									Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.FailedToStart, "The test encountered a failure on startup.");
									NodeInfo.Result = TestExecutionInfo.ExecutionResult.Interrupted;
							...
							else
							{
								...
								if (RunningTests.Count() == 0 && StartingTests.Count() == 0)
								{
									...
									if (TimeWaiting >= Options.Wait)
									{
										...
										Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.NotEnoughDevices, "The test was unable to secure enough devices to run.");
										CompletedTests.Add(NodeInfo);
									}
								}
							}
							...
							Thread StartThread = new Thread(() =>
							{
								...
								lock (Globals.MainLock)
								{
									if (Started == false)
									{
										...
										TestToStart.Result = TestExecutionInfo.ExecutionResult.Interrupted;
						...
						else
						{
							...
							switch (Result)
							{
								case TestResult.Passed:
									{
										TestInfo.Result = TestExecutionInfo.ExecutionResult.Passed;
										break;
									}
								case TestResult.Failed:
									{
										TestInfo.Result = TestExecutionInfo.ExecutionResult.Failed;
										break;
									}
								default:
									{
										// Default to "Interrupt"... if the test actually Failed, then the test case should explicitly say so
										TestInfo.Result = TestExecutionInfo.ExecutionResult.Interrupted;
										break;
									}
							}
				...
				else
				{
					...
					CompletedTests.ForEach(T =>
					{
                        ...
                        bool TestHadFailures = T.Result == TestExecutionInfo.ExecutionResult.Failed;
                        bool TestHadInterrupts = !TestHadFailures && T.Result != TestExecutionInfo.ExecutionResult.Passed && T.Result != TestExecutionInfo.ExecutionResult.Skipped;
                        ...
                        if (TestHadInterrupts)
                        {
                            InterruptCount++;
                        }

                        // increment counts for each test
                        if (!TestIterationsPassed.ContainsKey(T.TestNode.Name))
						{
							TestIterationsPassed[T.TestNode.Name] = 0;
							TestIterationsInterrupted[T.TestNode.Name] = 0;
						...
						if (TestHadInterrupts)
						{
							TestIterationsInterrupted[T.TestNode.Name]++;
						}
                        else if (TestHadFailures)
                        {
						...
						{
							TestIterationsPassed[T.TestNode.Name]++;
						}
					...
					if ((FailedCount > 0 || InterruptCount > 0) && Options.StopOnError)
					{
						break;
					}
				}
            ...
            int InterruptTestCount = TestIterationsInterrupted.Values.Sum();
			...
			if (Options.TestIterations > 1)
			{
				...
				MB.Paragraph(string.Format("All iterations completed. {0} of {1} executed tests completed without error", TotalTests - (FailedTestCount + InterruptTestCount), TotalTests));

				List<string> Lines = new List<string>();
				foreach (ITestNode Test in RequiredTests)
				{
					int IterationsWithInterrupts = TestIterationsInterrupted[Test.Name];
                    int IterationsWithFailures = TestIterationsFailed[Test.Name];
                    int IterationsWithWarnings = TestIterationsPassedWithWarnings[Test.Name];
					int IterationsWithPasses = TestIterationsPassed[Test.Name];				

					Lines.Add(string.Format("{0}: {1} Iterations, {2} Passed, {3} Passed with Warnings, {4} Failed, {5} Interrupts", 
						Test.Name, Options.TestIterations, IterationsWithPasses, IterationsWithWarnings, IterationsWithFailures, IterationsWithInterrupts));
				}

				MB.UnorderedList(Lines);

                if (InterruptTestCount > 0)
                {
                    MB.Paragraph(string.Format("Error: {0} of {1} executed tests had Interrupts.", InterruptTestCount, TotalTests));
                }
				...
				// Always report the number of passed tests
				MB.Paragraph(string.Format("{0} total tests passed.", TotalTests - (InterruptTestCount + FailedTestCount)));
			...
			return InterruptTestCount == 0 && FailedTestCount == 0 && !IsCancelled;
		}
