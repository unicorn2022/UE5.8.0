...
namespace Gauntlet
{
	...
	namespace TestTracking
	{
		...
		public class PhaseQueue : IEnumerable<Phase>
		{
			...
			private ConcurrentQueue<Phase> _queuedPhases;
			private Phase? _last;
			private Phase? _current;
			private bool _orderByCompletion = false;
			...
			{
				if (_orderByCompletion)
				{
					return _queuedPhases.OrderBy(P => !P.StartedAt.HasValue).ThenBy(P => P.Outcome == TestPhaseOutcome.Unknown).ThenBy(P => P.StartedAt?.AddSeconds(P.ElapseTimeInSeconds)).GetEnumerator();
				}
			...
			/// <summary>
			/// Whether or not to order the phases by completion date when iterating the object
			/// </summary>
			public void SetOrderByCompletion(bool Value)
			{
				_orderByCompletion = Value;
			}
