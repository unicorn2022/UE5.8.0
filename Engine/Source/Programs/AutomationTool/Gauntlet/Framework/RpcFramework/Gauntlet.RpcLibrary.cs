...
using static Gauntlet.RpcLibrary;
...
namespace Gauntlet
{
	...
	public class RpcLibrary
	{
			...
			int DelayInMilliseconds = 5 * 1000, int TimeoutInMilliseconds = 60 * 1000, bool bSuppressExceptions = false, bool bSuppressTimeoutException = false, OnBeforeTimeoutDelegate OnBeforeTimeout = null)
		{
			...
			while (true)
			{
				...
				catch (Exception Err)
				{
					...
					{
						Log.Info($"{Err.GetType()} encountered during call {CallCount} of {FuncName}");
						throw;
					}
					else
					{
						Log.Verbose($"Suppressed Exception during call {CallCount} of {FuncName}: {Err}");
					}
				}
				...
				if (StopTimer.ElapsedMilliseconds > TimeoutInMilliseconds)
				{
					...
					string TimeoutErrorMessage = $"Hang detected during {FuncName}: call never succeeded after {CallCount} attempts in {TimeoutInMilliseconds}ms";
		...
		public struct ScriptedBehaviorStatusResponse
		{
			public bool IsRunning { get; set; }
			public string BehaviorName { get; set; }
		}

		private static JsonSerializerOptions DefaultSerializationOptions = new() { PropertyNameCaseInsensitive = true };

		/// <summary>
		/// Gets the status of scripted behavior currently running on the pawn
		/// </summary>
		/// <param name="InTarget">RPC Target to check</param>
		/// <returns>ScriptedBehaviorStatusResponse with IsRunning status and BehaviorName</returns>
		public static ScriptedBehaviorStatusResponse GetScriptedBehaviorStatus(RpcTarget InTarget)
		{
			const string RPCName = "ScriptedBehaviorStatus";
			using HttpResponseMessage RpcResponse = RpcExecutor.CallRpc(InTarget, RPCName, 5000);

			if (RpcResponse == null || !RpcResponse.IsSuccessStatusCode)
			{
				Log.Error(KnownLogEvents.Gauntlet_TestEvent, "RPC call {RPCName} failed", RPCName);
				return new ScriptedBehaviorStatusResponse { IsRunning = false, BehaviorName = string.Empty };
			}

			string ResponseContent = RpcResponse.Content.ReadAsStringAsync().Result;
			try
			{
				return System.Text.Json.JsonSerializer.Deserialize<ScriptedBehaviorStatusResponse>(ResponseContent, DefaultSerializationOptions);
			}
			catch(System.Text.Json.JsonException)
			{
				Log.Error(KnownLogEvents.Gauntlet_TestEvent, "RPC call {RPCName} failed, malformed JSON input", RPCName);
			}
			return new ScriptedBehaviorStatusResponse();
		}

		/// <summary>
		/// Checks if the player cosmetic loadout has been set (cosmetic loadout change is done)
		/// </summary>
		/// <param name="InTarget">RPC Target to check</param>
		/// <returns>True if cosmetic loadout is set, false otherwise</returns>
		public static bool IsPlayerCosmeticSet(RpcTarget InTarget)
		{
			const string RPCName = "IsPlayerCosmeticSet";
			using HttpResponseMessage RpcResponse = RpcExecutor.CallRpc(InTarget, RPCName, 5000);
			SimpleResponse Response = GetSimpleResponse(RpcResponse);
			...
			return bool.TryParse(Response.Value, out bool bIsCosmeticSet) && bIsCosmeticSet;
		}
		...
		string MessageContents, int TimeoutInSeconds = 300, int TimeBetweenRetriesInSeconds = 60)
		{
			...
			if (DateTime.UtcNow >= TimeoutEnd)
			{
				...
				string ErrorMsg = $"Hang detected waiting for targets to send requested '{MessageCategory}' message. {TotalMatches} targets reported ready within {TimeoutInSeconds}sec.";
				if (NamesMissing.Length > 0)
				{
					ErrorMsg += $" The following targets never responded: {NamesMissing}";
				}
				Log.Error(ErrorMsg);

				List<string> UnmatchedMessages = RpcExecutor.Instance.GetAllMessageStrings();
				if (UnmatchedMessages.Count > 0)
				{
					Log.Warning($"At the time of failure, the following unhandled RPC messages were queued:\n{String.Join('\n', UnmatchedMessages)}");
				}

				throw new TestException(ErrorMsg);
			}

			Log.Info($"Message '{MessageCategory}' received from all RPC Targets!");
		}
...
#nullable disable
	...
	public class BaseRpcResponse : SimpleResponse
	{
	...
	/// <summary>
	/// Simple base class for building RPC libraries.
	/// </summary>
	public class BaseRpcLibrary
	{
		const string REGISTRATION_FINISHED = "RegistrationFinished";

		/// <summary>
		/// Runs a RPC for a given target.
		/// </summary>
		public BaseRpcResponse RunRpc(RpcTarget InTarget, string InRpcName, Dictionary<string, object> InRequestArgs, int InTimeout = 5000, bool InSilenceErrors = false)
		{
			ValidateRpcTarget(InTarget);

			string MessageContent = string.Empty;

			HttpResponseMessage MessageResponse = RpcExecutor.CallRpc(InTarget, InRpcName, InTimeout, InRequestArgs);
			if (MessageResponse == null)
			{
				if (!InSilenceErrors)
				{
					throw new AutomationException($"An error occurred calling {InRpcName}. Check the logs.");
				}
			}
			else
			{
				var RunValidatorStatus = MessageResponse.StatusCode;
				MessageContent = MessageResponse.Content.ReadAsStringAsync().Result;
				Log.Info($"=============>RPC test attempt is: {RunValidatorStatus} | {MessageContent}");
			}
			SimpleResponse SimpleResponse = GetSimpleResponse(MessageResponse);
			return new BaseRpcResponse
			{
				Succeeded = SimpleResponse.Succeeded,
				Value = SimpleResponse.Value,
				Fatal = SimpleResponse.Fatal,
				HttpResponse = MessageResponse,
				RawContent = MessageContent
			};
		}

		private void ValidateRpcTarget(RpcTarget Target)
		{
			if (null == Target)
			{
				throw new AutomationException("The RpcTarget is null.");
			}

			if ("localhost" != Target.IpAddress.ToLower())
			{
				IPAddress IpAddress = null;
				if (!IPAddress.TryParse(Target.IpAddress, out IpAddress))
				{
					throw new AutomationException($"The rpcTarget has an invalid IPAddress: {Target.IpAddress}");
				}
			}
		}

		/// <summary>
		/// Run the default "RegistrationFinished" RPC that checks if registration has finished.
		/// </summary>
		public BaseRpcResponse RegistrationFinished(RpcTarget InTarget)
		{
			return RunRpc(InTarget, REGISTRATION_FINISHED, null, 0, true);
		}

		public JsonObject GetResponseValueAsJson(SimpleResponse InResponse)
		{
			if (InResponse == null)
			{
				throw new AutomationException("[GetResponseValueAsJson] SimpleResponse cannot be null.");
			}

			if (!InResponse.Succeeded)
			{
				Log.Warning("[GetResponseValueAsJson] SimpleResponse \"succeeded\" field is false.");
			}

			if (InResponse.Succeeded)
			{
				return JsonObject.Parse(InResponse.Value);
			}
			else if (InResponse.Fatal)
			{
				throw new AutomationException("[GetResponseValueAsJson] SimpleResponse \"fatal\" field is true, its value field is: {0}.", InResponse.Value);
			}
			return new JsonObject();
		}

		public bool GetResponseBoolFieldValue(SimpleResponse InResponse, string InBoolFieldName)
		{
			JsonObject ResponseJson = GetResponseValueAsJson(InResponse);
			bool ResponseBoolFieldValue = false;
			if(!ResponseJson.TryGetBoolField(InBoolFieldName, out ResponseBoolFieldValue))
			{
				throw new AutomationException("[GetResponseBoolFieldValue] Failed to get bool field {0} from response json.", InBoolFieldName);
			}
			return ResponseBoolFieldValue;
		}
	}
}