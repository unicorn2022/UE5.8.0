...
namespace Gauntlet
{
	public static class Globals
	{
		...
		public static string BuildCommand = String.Empty;
			...
			get
			{
				if (InnerAbortHandlers == null)
				{
					...
					Console.CancelKeyPress += new ConsoleCancelEventHandler((obj, args) =>
					{
						...
						Horde.ReportInvalidatingIssue(Horde.GauntletHordeStepMessage.TestCancelled, "Test was manually cancelled");
	...
	public class Log
	{
		...
		static public void Critical(string Format, params object[] Args)
		{
			Critical(KnownLogEvents.Gauntlet, Format, Args);
		}
		static public void Critical(Logging.EventId EventId, string Format, params object[] Args)
		{
			OutputMessage(Format, EventId, Logging.LogLevel.Critical, false, Args);
		}
	}

#nullable enable
	/// <summary>
	/// Helper class providing logging methods for test events of various levels.
	/// Used to clearly display test events on the Horde dashboard, eliminates inconsistent reporting.
	/// </summary>
	public class Reporter
	{
		private static volatile Reporter? _Instance;
		...
		private PhaseQueue? _Phases;
		private Action<TestResult>? _SetUnrealTestResult;
		private BaseTest? _TestNode;

		/// <summary>
		/// This method is used to switch context when running tests in parallel.
		/// The method should be called each time TestExecutor switches between test nodes, such as in node loops, etc.
		/// </summary>
		/// <param name="Phases">Reference to Phases variable in test node</param>
		/// <param name="SetUnrealTestResult">Reference to SetUnrealTestResult method from test node</param>
		/// <param name="TestNode">Reference to test node that is calling this method</param>
		public void UpdateReferences(PhaseQueue Phases, Action<TestResult> SetUnrealTestResult, BaseTest TestNode)
		{
			this._Phases = Phases;
			this._SetUnrealTestResult = SetUnrealTestResult;
		...
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public void Interrupt(string Message, params object[] Args)
		{
			Instance?._Phases?.Current?.AddEvent(Logging.LogLevel.Critical, Message, Args);
			Instance?._TestNode?.AddTestEvent(new UnrealTestEvent(EventSeverity.Fatal, Message, Args));
			
			if (_TestNode == null || !_TestNode.LogWarningsAndErrorsAfterSummary)
			{
				Log.Critical(KnownLogEvents.Gauntlet_FatalEvent, Message, Args);
			}

			if (Instance?._TestNode?.GetTestStatus() == TestStatus.Complete && Instance._TestNode.GetTestResult() == TestResult.Passed)
			{
				Instance?._SetUnrealTestResult?.Invoke(TestResult.OperationalException);
			}
		}
		...
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public void Failure(string Message, params object[] Args)
		{
			Instance?._Phases?.Current?.AddEvent(Logging.LogLevel.Error, Message, Args);
			Instance?._TestNode?.AddTestEvent(new UnrealTestEvent(EventSeverity.Error, Message, Args));

			if (_TestNode == null || !_TestNode.LogWarningsAndErrorsAfterSummary)
			{
				Log.Error(KnownLogEvents.Gauntlet_TestEvent, Message, Args);
			}

			if (Instance?._TestNode?.GetTestStatus() == TestStatus.Complete && Instance._TestNode.GetTestResult() == TestResult.Passed)
			{
				Instance?._SetUnrealTestResult?.Invoke(TestResult.Failed);
			}
		}

		/// <summary>
		/// Report a warning with Gauntlet_TestEvent
		/// </summary>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		public void Warning(string Message, params object[] Args)
		{
			Warning(KnownLogEvents.Gauntlet_TestEvent, Message, Args);
		}

		/// <summary>
		/// Report a warning with required Log Event type
		/// </summary>
		/// <param name="EventId"></param>
		/// <param name="Message"></param>
		/// <param name="Args"></param>
		...
		{
			Instance?._Phases?.Current?.AddEvent(Logging.LogLevel.Warning, Message, Args);
			Instance?._TestNode?.AddTestEvent(new UnrealTestEvent(EventSeverity.Warning, Message, Args));

			if (_TestNode == null || !_TestNode.LogWarningsAndErrorsAfterSummary)
			{
				Log.Warning(EventId, Message, Args);
			}
		}
		...
		{
			Instance?._Phases?.Current?.AddEvent(Logging.LogLevel.Information, Message, Args);
			Instance?._TestNode?.AddTestEvent(new UnrealTestEvent(EventSeverity.Info, Message, Args));
	...
	*/
	...
	namespace Utils
	{
		...
		public static class SystemHelpers
		{
			...
			public static void Delete(DirectoryInfo Directory, bool bRecursive, bool bForce = false)
			{
				try
				{
					Log.Info("Deleting directory {Directory}.", Directory);
				...
				catch (Exception Ex)
				{
					...
					else if(bForce)
					{
						...
						{
							Log.Info("Deleting file after attributes change: {FileName}", Info.FullName);
							...
							{
								Log.Warning("Could not set attributes for {File}. Error: {Error}", Info.FullName, ArgEx.Message);
								// Continue to deletion attempt - we'll try using extended-length path
							}

							// Try to delete the file, with retry logic for locked files
							bool bDeleted = false;
							for (int Attempt = 0; Attempt < 3 && !bDeleted; Attempt++)
							{
								try
								{
								...
								catch (System.IO.IOException IoEx) when (IoEx.Message.Contains("because it is being used by another process"))
								{
									// Looking specifically for 
									if (Attempt < 2)
									{
										Log.Info("File {File} is locked. Forcing garbage collection and retrying... (Attempt {Attempt}/3)", Info.FullName, Attempt + 1);
										// Force garbage collection to run finalizers that might be holding file handles
										System.GC.Collect();
										System.GC.WaitForPendingFinalizers();
										System.GC.Collect();
										// Wait a bit for the OS to release the file handle
								...
								{
									// Handle files with invalid Windows filenames using extended-length path
									Log.Warning("Attempting deletion with extended-length path: {File}", Info.FullName);
									if (Info.FullName.StartsWith(@"\\"))
									{
										// Network or already an extended-length path
										throw;
									}
									string ExtendedPath = @"\\?\" + Info.FullName;
	...
	public class LongProcessResult : ILongProcessResult
	{
		...
		public LongProcessResult(string Command, string Args, ERunOptions Options, OutputFilterCallbackType OutputCallback = null, string WorkingDir = null, string LocalCache = null)
		{
			...
			{
				StartInfo = new ProcessStartInfo(Command, GauntletCommandLine.GetArgumentList(Args));
			}
