...
namespace Gauntlet
{
	...
	public class UnrealDeviceReservation
	{
		...
		private static readonly HordeDeviceReservationService.Options ForceReleaseOption = new HordeDeviceReservationService.Options()
		{
			bForceDelete = true
		};
		...
		public bool TryReserveDevices(Dictionary<UnrealDeviceTargetConstraint, int> RequiredDeviceTypes, int ExpectedNumberOfDevices, bool bAllowPartialReservations = false)
		{
			...
			lock (LockObject)
			{
				...
				DevicePool.Instance.ReleaseDevices(SkippedDevices, ForceReleaseOption);
				...
				if (AcquiredDevices.Count < ExpectedNumberOfDevices && AcquiredDevices.Count > 0)
				{
					...
					{
						DevicePool.Instance.ReleaseDevices(AcquiredDevices, ForceReleaseOption);
					}
		...
		public void ReleaseDevices(bool bForceClearServiceReservations = false)
		{
			if ((ReservedDevices != null) && (ReservedDevices.Count() > 0))
			{
				...
				IDeviceReservationService.Options ServiceOptions = bForceClearServiceReservations && HordeDeviceReservationService.Enabled
					? ForceReleaseOption
					: null;
				DevicePool.Instance.ReleaseDevices(ReservedDevices, ServiceOptions);
