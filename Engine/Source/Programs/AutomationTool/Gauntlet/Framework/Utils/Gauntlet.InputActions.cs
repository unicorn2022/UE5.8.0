...
namespace Gauntlet.Utils
{
	...
	public class InputAction(FKey InKey, EInputEvent InInputEvent, double InXDelta = 0, double InYDelta = 0, bool InTapped = false)
	{
		[JsonPropertyName("inputkey")]
		public FKey Key { get; } = InKey;

		[JsonPropertyName("inputevent")]
		public EInputEvent KeyAction { get; } = InInputEvent;

		[JsonPropertyName("xdelta")]
		public double XDelta { get; } = InXDelta;

		[JsonPropertyName("ydelta")]
		public double YDelta { get;  } = InYDelta;

		[JsonPropertyName("tapped")]
		public bool Tapped { get;  } = InTapped;

		/// <summary>
		/// Converts the instance's properties into a dictionary suitable for RPC method arguments.
		/// Properties decorated with <see cref="JsonPropertyNameAttribute"/> are serialized and added to the dictionary.
		/// </summary>
		/// <returns>A dictionary with JSON property names as keys and serialized property values as JSON strings.</returns>
		/// <remarks>
		/// If this pattern is needed across multiple classes, consider creating an extension method
		/// on <see cref="object"/> to avoid code duplication.
		/// </remarks>
		public Dictionary<string, object> AsRpcArgs()
		{
			...
			{
				JsonPropertyNameAttribute JsonAttribute = Property.GetCustomAttribute<JsonPropertyNameAttribute>();
				if (JsonAttribute != null)
				{
					object Value = Property.GetValue(this);
					Args[JsonAttribute.Name] = JsonSerializer.Serialize(Value);
				}
			}
