...
namespace Gauntlet
{
	...
	{
		public static List<Dictionary<string, string>> Load(string File, string DefaultForMissingColumns = null)
		{
			...
			using (StreamReader Reader = new StreamReader(File))
			{
				...
				while (!Reader.EndOfStream)
				{
					...
					if (DefaultForMissingColumns != null)
					{
						if (Values.Length < HeaderValues.Length)
						{
							
							Values = Enumerable.Concat(Values,Enumerable.Repeat(DefaultForMissingColumns, HeaderValues.Length - Values.Length)).ToArray();
						}
					}
