...
namespace Gauntlet
{
	public class Params
	{
		...
		/// <param name="Default">List with default parameter values ​​if the required values ​​are not found in CLI.</param>
		/// <returns>Parameter values or default values or empty list if param is not found and default list is not set</returns>
		public List<string> ParseValues(string Param, bool CommaSeparated=false, List<string> Default = default)
		{
			...
			if ((FoundValues.Count == 0) && (Default != default))
			{
				return Default;
			}
