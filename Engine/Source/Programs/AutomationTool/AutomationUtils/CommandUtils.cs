...
namespace AutomationTool
{
	...
	public partial class CommandUtils
	{
		...
		/// <summary>
		/// Access to scoped services which are created per command
		/// </summary>
		public static IServiceScope ServiceScope { get; set; } = null!;
		...
		{
			DateTime Date = UtcDate ?? DateTime.UtcNow;
		...
		public static string CombinePaths(PathSeparator SeparatorType, params string[] Paths)
		{
			...
			StringBuilder CombinedPath = new StringBuilder(CombinePathMaxLength);
		...
		public static void ThreadedCopyFiles(string SourceDirectory, string DestDirectory, int MaxThreads = 64)
		{
			...
			List<string> SourceFiles = Directory.EnumerateFiles(SourceDirectory, "*", SearchOption.AllDirectories).ToList();
			List<string> DestFiles = SourceFiles.Select(SourceFile => CommandUtils.MakeRerootedFilePath(SourceFile, SourceDirectory, DestDirectory)).ToList();
		...
		{
			List<string> SourceFileNames = RelativePaths.Select(RelativePath => CommandUtils.CombinePaths(SourceDir, RelativePath)).ToList();
			List<string> TargetFileNames = RelativePaths.Select(RelativePath => CommandUtils.CombinePaths(TargetDir, RelativePath)).ToList();
		...
		public static List<string> ThreadedCopyFiles(string SourceDir, string TargetDir, FileFilter Filter, bool bIgnoreSymlinks, int MaxThreads = 64, bool bRetry = false)
		{
			...
			List<string> RelativePaths = Filter.ApplyToDirectory(SourceDirRef, bIgnoreSymlinks).Select(x => x.MakeRelativeTo(SourceDirRef)).ToList();
		...
		public static IEnumerable<string> LegacyUnzipFiles(string ZipFileName, string BaseDirectory, bool OverwriteFiles = true)
		{
			...
			{
				// Manually extract the files. Leaving this code as is in order to preserve behaviour around file permissions.
				...
				foreach (ZipArchiveEntry Entry in Zip.Entries.Where(e => !string.IsNullOrEmpty(e.Name)))
				{
					...
					if (OverwriteFiles || File.Exists(OutputFileName) == false)
					{
						...
						using FileStream OutputStream = new(OutputFileName, OutputFileMode, FileAccess.Write);
		...
		public static ICodeSignHelper FindHelper(string SignToolPath)
		{
			if (HelperClass == null)
			{
				...
				foreach (Assembly Dll in LoadedAssemblies)
				{
					...
					foreach (Type PotentialConfigType in AllTypes.Where(x => !x.IsAbstract && x != typeof(ICodeSignHelper) && typeof(ICodeSignHelper).IsAssignableFrom(x)))
					{
						try
						{
