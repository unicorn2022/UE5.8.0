...
namespace AutomationTool
{
	...
	public partial class CommandUtils
	{
		...
		private static void CheckIfCommandsRequireP4(List<CommandInfo> CommandsToExecute, Dictionary<string, Type> Commands, out bool bRequireP4, out bool bRequireCL)
		{
			...
			foreach (CommandInfo CommandInfo in CommandsToExecute)
			{
				Type Command;
	...
	public partial class P4Connection
	{
		...
		public IProcessResult P4(string ExtraGlobalOptions, string CommandLine, string Input, bool AllowSpew = true, bool WithClient = true, bool SpewIsVerbose = false)
		{
			...
			ProcessResult.SpewFilterCallbackType SpewDelegate = AllowSpew ? null : new ProcessResult.SpewFilterCallbackType(NoSpewFilter);
		...
		public bool P4Output(out string Output, string ExtraGlobalOptions, string CommandLine, string Input = null, bool AllowSpew = true, bool WithClient = true)
		{
			...
			IProcessResult Result = P4(ExtraGlobalOptions, CommandLine, Input, AllowSpew, WithClient);
		...
		public bool LogP4Output(out string Output, string ExtraGlobalOptions, string CommandLine, string Input = null, bool AllowSpew = true, bool WithClient = true, bool SpewIsVerbose = false)
		{
			...
			IProcessResult Result = P4(ExtraGlobalOptions, CommandLine, Input, AllowSpew, WithClient, SpewIsVerbose:SpewIsVerbose);
        ...
        public string UserToEmail(string User)
        {
            ...
            try
            {
				IProcessResult P4Result = P4(String.Format("user -o {0}", User), AllowSpew: false);
			    if (P4Result.ExitCode == 0)
			    {
					Dictionary<string, string> Tags = ParseTaggedP4Output(P4Result.Output);
                    ...
                    if (ChangeAt == 0 && OnAt > ChangeAt && ByAt < OnAt)
                    {
						...
						if( AffectedFilesAt == 0 )
						{
							...
							for( ; LineIndex < Lines.Length; ++LineIndex )
							{
								...
								DescribeRecord.DescribeFile DescribeFile = new DescribeRecord.DescribeFile();
		...
		public  void BatchedCommand(string Command, List<string> Files, bool AllowSpew = true)
		{
			...
			int MaxCommandLineLength = 32699 - Command.Length - GlobalOptions.Length - 2;
		...
		public int GetLatestCLNumber()
		{
			...
			Match Match = ChangesListOutputPattern.Match(Output);
		...
		{
			List<P4Label> LabelList = new List<P4Label>();
		...
		/// <param name="DepotFile">Depot path</param>
		/// <param name="AllowSpew"></param>
		/// <returns>True if the file exists in the depot with a non-deleted head revision.</returns>
		public bool FileExistsInDepot(string DepotFile, bool AllowSpew = false)
		{
			string CommandLine = String.Format("fstat {0}", CommandUtils.MakePathSafeToUseWithCommandLine(DepotFile));

			string Output;
			if (!LogP4Output(out Output, "-z tag", CommandLine, AllowSpew: AllowSpew) || !Output.Contains("headRev") || Output.Contains("headAction delete"))
			{
				return false;
			}
		...
		public void ChangeFileType(string Filename, P4FileAttributes Attributes, string Changelist = null)
		{
			...
			P4FileStat Stat = FStat(Filename);
		...
		public bool DoesClientExist(string ClientName, bool Quiet = false)
		{
			...
			IProcessResult P4Result = P4(String.Format("-c {0}", ClientName), "where //...", Input: null, AllowSpew: false, WithClient: false);
		...
		public P4ClientInfo GetClientInfoInternal(string ClientName)
		{
			...
			IProcessResult P4Result = P4(String.Format("client -o {0}", ClientName), AllowSpew: false, WithClient: false);
			if (P4Result.ExitCode == 0)
			{
				Dictionary<string, string> Tags = ParseTaggedP4Output(P4Result.Output);
                ...
                foreach (KeyValuePair<string, string> Pair in Tags)
                {
                    if (Pair.Key.StartsWith("View", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        {
							KeyValuePair<string, string> ViewPair = new KeyValuePair<string, string>(
		...
		{
			List<P4ClientInfo> ClientList = new List<P4ClientInfo>();
		...
		public P4ClientInfo CreateClient(P4ClientInfo ClientSpec, bool AllowSpew = true)
        {
			...
			else
			{
				...
				foreach (KeyValuePair<string, string> Mapping in ClientSpec.View)
				{
		...
		public List<string> Dirs(string CommandLine)
		{
			...
			IProcessResult P4Result = P4(DirsCmdLine, AllowSpew: false);
			if (P4Result.ExitCode != 0)
			{
		...
		public List<string> Files(string CommandLine)
		{
			...
			foreach (KeyValuePair<string, string> KV in FileActions)
			{
		...
		{
			KeyValuePair<object, string>[] AllFileTypes = GetEnumValuesAndKeywords(typeof(P4FileType));
			KeyValuePair<object, string>[] AllAttributes = GetEnumValuesAndKeywords(typeof(P4FileAttributes));

			Stat.Type = P4FileType.Unknown;
			Stat.Attributes = P4FileAttributes.None;

			// Parse file flags
			KeyValuePair<object, string>[] OldFileFlags = GetEnumValuesAndKeywords(typeof(P4FileAttributes), OldStyleBinaryFlags);
			foreach (KeyValuePair<object, string> FileTypeFlag in OldFileFlags)
			{
			...
			foreach (KeyValuePair<object, string> FileType in AllFileTypes)
			{
			...
			if (TypeAndAttributes.Length > 1 && !String.IsNullOrEmpty(TypeAndAttributes[1]))
			{
				...
				for (int AttributeIndex = 0; AttributeIndex < FileAttributes.Length; ++AttributeIndex)
				{
					...
					foreach (KeyValuePair<object, string> FileAttribute in AllAttributes)
					{
		...
		static P4Action ParseAction(string Action)
		{
			...
			KeyValuePair<object, string>[] AllActions = GetEnumValuesAndKeywords(typeof(P4Action));
			foreach (KeyValuePair<object, string> ActionKeyword in AllActions)
			{
		...
		private static string FileAttributesToString(P4FileAttributes Attributes)
		{
			KeyValuePair<object, string>[] AllAttributes = GetEnumValuesAndKeywords(typeof(P4FileAttributes));
			string Text = "";
			foreach (KeyValuePair<object, string> Attr in AllAttributes)
			{
				P4FileAttributes AttrValue = (P4FileAttributes)Attr.Key;
