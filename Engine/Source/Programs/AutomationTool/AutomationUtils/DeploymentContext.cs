...
public class PackageStoreData
{
	...
	/// <summary>
	/// Cached parsed ChunkAssignment op from the cook oplog, populated by
	/// ReadZenCookedFilesFromZenServer so downstream consumers (e.g. CreatePaksUsingChunkManifests)
	/// can read pakchunk assignment without re-fetching the oplog. Null if the op
	/// was absent or the oplog wasn't read with cooked-files loading enabled.
	/// Stored as <c>object</c> so DeploymentContext doesn't have to take a dependency
	/// on AutomationScripts.Oplog — cast to <c>ChunkAssignmentOp</c> at the use site.
	/// </summary>
	public object ChunkAssignment { get; set; }
}
...
public class DeploymentContext //: ProjectParams
{
		...
		)
	{
		...
		{
			IEnumerable<ReceiptProperty> Results = Target.Receipt.AdditionalProperties.Where(x => x.Name == "CustomConfig");
		...
		Dictionary<string,string> SubstitutionVariables = new()
		{
		...
		if (GameConfig.GetArray("Staging", "RemapDirectories", out RemapDirectoriesList))
		{
			foreach (string RemapDirectory in RemapDirectoriesList)
			{
				...
				FromDir = Utils.ExpandVariables(FromDir, SubstitutionVariables, bUseAdditionalVariablesOnly:true);
				...
				ToDir = Utils.ExpandVariables(ToDir, SubstitutionVariables, bUseAdditionalVariablesOnly:true);
		...
		if (GameConfig.GetArray("Staging", "AllowedDirectories", out DirectoriesAllowListStrings))
		{
			...
			{
				string AllowedDirAdjusted = Utils.ExpandVariables(AllowedDir, SubstitutionVariables, bUseAdditionalVariablesOnly:true);

				DirectoriesAllowList.Add(new StagedDirectoryReference(AllowedDirAdjusted));
			}
		}
