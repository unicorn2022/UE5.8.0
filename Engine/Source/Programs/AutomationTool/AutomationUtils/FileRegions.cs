...
namespace AutomationTool
{
	...
	public struct FileRegion
	{
		...
		public static List<FileRegion> ReadRegionsFromFile(string Filename)
		{
			...
			try
			{
				using (BinaryReader Reader = new BinaryReader(File.OpenRead(Filename)))
				{
					...
					List<FileRegion> Results = new List<FileRegion>(NumRegions);
        ...
        {
			Dictionary<FileRegionType, List<FileRegion>> RegionsByType = (from Region in Regions group Region by Region.Type).ToDictionary(g => g.Key, g => g.ToList());

			Logger.LogInformation(" +-------------------+----------------------------------+---------------+");
			Logger.LogInformation(" |        Type       |           Total Length           |  Num Regions  |");
			Logger.LogInformation(" +-------------------+----------------------------------+---------------+");
			foreach (KeyValuePair<FileRegionType, List<FileRegion>> Pair in RegionsByType)
			{
