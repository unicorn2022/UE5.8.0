	...
	public abstract class BuildCommand : CommandUtils
	{
		...
		public FileReference? ParseProjectString(string originalProjectName)
		{
			...
			else
			{
				...
				BranchInfo.BranchUProject GameProj = Branch.FindGame(originalProjectName);
				if (GameProj != null)
				{
		...
		/// <summary>
		/// Attempts to parse the project param. Returns null if no project is found, and throws if the specified project does not exist on disk.
		/// </summary>
		/// <returns>FileReference to uproject</returns>
