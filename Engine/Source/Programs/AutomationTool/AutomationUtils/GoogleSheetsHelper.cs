...
namespace SheetsHelper
{
	...
	public class SpreadSheetWrapper
	{
        ...
        public SpreadSheetWrapper(string AppTitle, string SecretKeyPath, string CredentialPath, string InSpreadsheetID, EAuthTokenType TokenType = EAuthTokenType.UserToken)
		{
			string[] Scopes = { SheetsService.Scope.Spreadsheets };

			if (!File.Exists(SecretKeyPath))
			{
				throw new FileNotFoundException($"Google auth credential file not found: {SecretKeyPath}");
			}
			...
			try
			{
				Console.Write("Using Google Auth Type: '{0}'\n", TokenType);

				switch (TokenType)
				{
					case EAuthTokenType.ServiceAccountToken:
						GoogleCredential ServiceAccount = GoogleCredential.FromFile(SecretKeyPath).CreateScoped(SheetsService.Scope.SpreadsheetsReadonly);
						// Create Google Sheets API service, using a Service Account token
						...
						break;
					case EAuthTokenType.UserToken:
						UserCredential UserAccount = null;
						using (var stream = new FileStream(SecretKeyPath, FileMode.Open, FileAccess.Read))
						{
							Task<UserCredential> Tsk = GoogleWebAuthorizationBroker.AuthorizeAsync(
							...
							if (!Tsk.Wait(TimeSpan.FromMinutes(1)))
							{
								throw new TimeoutException("Google Sheets authorization timed out after 1 minute");
							}

							UserAccount = Tsk.Result;

							// Create Google Sheets API service, using a User token
							Service = new SheetsService(new BaseClientService.Initializer()
							{
								HttpClientInitializer = UserAccount,
								ApplicationName = AppTitle,
							});
						}
						break;
					default:
						throw new NotImplementedException($"Google Sheets authorization method {TokenType.ToString()} does not have an implementation.");
				}
			}
