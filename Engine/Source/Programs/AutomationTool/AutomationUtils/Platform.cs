...
namespace AutomationTool
{
	...
	public class Platform : CommandUtils
	{
		...
		internal static void InitializePlatforms(HashSet<Assembly> AssembliesWithPlatforms)
		{
			...
			foreach (Assembly ScriptAssembly in AssembliesWithPlatforms)
			{
		...
		private static void CreatePlatformsFromAssembly(Assembly ScriptAssembly)
		{
			...
			foreach (Type PotentialPlatformType in AllTypes)
			{
				if (PotentialPlatformType != typeof(Platform) && typeof(Platform).IsAssignableFrom(PotentialPlatformType) && !PotentialPlatformType.IsAbstract)
				{
					...
					Platform PlatformInstance = Activator.CreateInstance(PotentialPlatformType) as Platform;
					TargetPlatformDescriptor PlatformDesc = PlatformInstance.GetTargetPlatformDescriptor();
		...
		public UnrealTargetPlatform TargetPlatformType { get; protected set; }
		public UnrealTargetPlatform TargetIniPlatformType { get; protected set; }
		...
		public virtual void StripAllSymbols(FileReference SourceFile, FileReference TargetFile)
		{
			StripSymbols(SourceFile, TargetFile);
		}
