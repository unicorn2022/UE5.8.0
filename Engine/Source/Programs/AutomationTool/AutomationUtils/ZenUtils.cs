...
namespace AutomationUtils
{
	...
	public class ZenRunContext
	{
		...
		private void InitializeInstanceData()
		{
			...
			try
			{
				using(FileStream Stream = File.Open(LockFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete))
				{
					...
					Stream.ReadExactly(CbObjectData);
				}
			}
			catch(IOException)
			{
		...
		public static HttpClient CreateUnixSocketHttpClient(string UnixSocketPath)
		{
			SocketsHttpHandler SocketsHandler = new SocketsHttpHandler
			{
				ConnectCallback = async (Context, Token) => {
					...
					UnixDomainSocketEndPoint EndPoint = new UnixDomainSocketEndPoint(UnixSocketPath);
		...
		{
			List<ZenWorkspace> Workspaces = GetWorkspaceList();
			if (Workspaces == null)
			{
				return null;
			}
	...
	public static class ZenUATProcessUtil
	{
		private static readonly ConcurrentDictionary<ManagedProcess, ZenUatProcessStub> ProcessStubs = new ConcurrentDictionary<ManagedProcess, ZenUatProcessStub>();
		public static IProcess GetUATProcessStub(ManagedProcess process)
		{
			return ProcessStubs.GetOrAdd(process, p => new ZenUatProcessStub(p));
		}
	}

	public class ZenUatProcessStub(ManagedProcess process) : IProcess
	{
		public void StopProcess(bool killDescendants = true)
		{
			process.Kill(killDescendants: killDescendants);
		}
		...
		public string GetProcessName()
		{
			try
			{
				return Process.GetProcessById(process.Id).ProcessName;
			}
