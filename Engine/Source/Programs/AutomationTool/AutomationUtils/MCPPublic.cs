...
namespace EpicGames.MCP.Automation
{
	...
	public partial struct MCPPlatform
	{
		...
		/// WindowsMSG Platform.
		/// </summary>
		public static MCPPlatform WindowsMSG = FindOrAddByName("WindowsMSG");
	...
	public abstract class BuildPatchToolBase
	{
		...
		public class ManifestDiffOptions
		{
			...
			// If the set of tags associated with the install has changed, pass the new set here and the old set in CompareTagSets.
			// This will report the patch between those sets.
			public List<HashSet<string>> CompareNewTagSet;
			
			// If an optional optimized delta is required, pass it here.
			public string DeltaFilenameTrailer;
			
			// If we expect an optimized delta in the patch, pass this to ensure a failure to avoid looking at unrepresentative data.
			public bool bRequireOptimizedDelta;
		}
		...
		public class PackageChunksOptions
		{
			...
			// If a custom optimized delta is required, specify it here.
			public string DeltaFilenameTrailer;
			
			// If we are packaging the chunks for an optimized delta, pass this to make sure if the optimized delta is missing the operation fails.
			public bool RequireOptimizedDelta;
		}
		...
		public class ChunkDeltaOptimiseOptions
		{
			...
			// If the patch matcher should find matches from more than one tag, pass the set of tags here.
			// This should be a single entry with the list of tags to match across, e.g.
			// SourceManifestAliases.Add("MyAliasedTag", {"Tag1", "Tag2"})
			// The destination manifest can have a different set of tags associated with the alias.
			public Dictionary<string, List<string>> SourceManifestAliases;
			public Dictionary<string, List<string>> DestinationManifestAliases;

			// Some tags can be ignored during tag matching
			public List<string> SourceTagsToIgnore;
			public List<string> DestinationTagsToIgnore;
			
			// If generating with a custom optimized delta, specify it here.
			public string DeltaFilenameTrailer;

			// Minimum compression benefit to require when writing chunks to accept compression, to avoid spending client decompression time
			// on minimal benefits. (10 is good)
			public int MinCompressionPct = 0;
		}

		public class ManifestMetadataOptions
		{
			public string SourceManifestFilename;
			public string OutputFilename; // This can be empty, a temp path will be used instead.
		}
		public class ManifestMetadataOutput
		{
			public string FeatureLevel;
			public ulong AppId;
			public string AppName;
			...
			public ulong DownloadSize;
			public ulong BuildSize;
			public List<string> FileTagList;

			public class ManifestMetadataOutputFile
			{
			...
			public List<ManifestMetadataOutputFile> Files;
		}
		...
		// Runs BPT to request information about the given metadata.
		public abstract void Execute(ManifestMetadataOptions Opts, out ManifestMetadataOutput Output, ToolVersion Version = ToolVersion.Live);
...
namespace EpicGames.MCP.Config
{
    ...
    public class McpConfigData
    {
		...
		public McpConfigData(string InName, string InAccountBaseUrl, string InFortniteBaseUrl, string InLauncherBaseUrl, string InBuildInfoV2BaseUrl, string InLauncherV2BaseUrl, string InCatalogBaseUrl, string InArtifactBaseUrl, string InClientId, string InClientSecret)
			: this(InName, InAccountBaseUrl, InFortniteBaseUrl, InLauncherBaseUrl, InBuildInfoV2BaseUrl, InLauncherV2BaseUrl, InCatalogBaseUrl, InArtifactBaseUrl, InClientId, () => InClientSecret)
		{
		}

		public McpConfigData(string InName, string InAccountBaseUrl, string InFortniteBaseUrl, string InLauncherBaseUrl, string InBuildInfoV2BaseUrl, string InLauncherV2BaseUrl, string InCatalogBaseUrl, string InArtifactBaseUrl, string InClientId, Func<string> InClientSecret)
        {
            Name = InName;
			...
			ArtifactBaseUrl = InArtifactBaseUrl;
            ClientId = InClientId;
        ...
        public void SpewValues()
        {
			...
			Logger.LogDebug("ArtifactBaseUrl : {ArtifactBaseUrl}", ArtifactBaseUrl);
