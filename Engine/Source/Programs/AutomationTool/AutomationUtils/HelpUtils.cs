	...
	public class ParamHelpAttribute : HelpAttribute
	{
		...
		public string ParamDescription
		{
			get
			{
				...
				string ChoicesStr = "";
				IEnumerable<object> ValidChoices = Choices as IEnumerable<object>;
...
/// </summary>
	public partial class CommandUtils
	{
		...
		public static void LogHelp(Type Command, string Description, List<string> Params)
		{
			...
			foreach (var Param in Params)
			{
				...
				if (!String.IsNullOrWhiteSpace(Param))
				{
					...
					if (SplitPoint > 0)
					{
						// Extract the name and description separately
		...
		public static void GetTypeHelp(Type ObjType, out string ObjectDescription, out List<string> Params)
		{
			...
			{
				HelpAttribute HelpAttribute = CustomAttribute as HelpAttribute;
			...
			List<MemberInfo> AllMembers = GetAllMemebers(ObjType);
		...
		private static List<MemberInfo> GetAllMemebers(Type ObjType)
		{
			...
			{
				MemberInfo[] ObjMembers = ObjType.GetMembers(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
