...
namespace AutomationTool.DeviceReservation
{
	...
	public sealed class DeviceReservationAutoRenew : IDisposable
	{
		...
		/// <summary>
		/// When true, the ActiveReservation will be force deleted upon termination of the auto renewal thread
		/// Force deleted reservations will clear any associated reservation blocks, regardless of the active step
		/// </summary>
		...
		private void DoAutoRenew()
		{
			...
			try
			{
				ActiveReservation.Delete(ReservationBaseUri, ForceDelete);
			}
			catch (Exception Ex)
			{
	...
	public static class Utils
	{
		...
		public static Uri AppendQuery(this Uri BaseUri, string NewQuery)
		{
			UriBuilder Builder = new UriBuilder(BaseUri);
		...
		{
			JsonSerializerOptions SerializeOptions = new JsonSerializerOptions() { PropertyNameCaseInsensitive = true };
			SerializeOptions.Converters.Add(new TimeSpanJsonConverter());

			HttpWebRequest Request = (HttpWebRequest)WebRequest.Create(UriToRequest);
	...
	public sealed class Reservation
	{
		...
		public class Tags : ICloneable
		{
			...
			public Tags Clone()
			{
				Tags Other = (Tags)MemberwiseClone();
				Other.Required = new List<string>(Required);
				Other.Preferred = new List<string>(Preferred);
				Other.Undesired = new List<string>(Undesired);
				Other.Blocked = new List<string>(Blocked);
				return Other;
			}
		...
		public void Delete(Uri BaseUri, bool Force = false)
		{
			string Path = $"api/v1/reservations/{Guid}";
			Uri DeleteUri = BaseUri.AppendPath(Path);
			if (Force)
			{
				DeleteUri = DeleteUri.AppendQuery("force=true");
			}
			Utils.InvokeAPI(DeleteUri, "DELETE");
