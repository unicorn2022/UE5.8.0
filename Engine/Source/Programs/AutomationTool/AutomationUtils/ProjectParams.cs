...
namespace AutomationTool
{
	...
	public class ProjectParams
	{
		...
		public ProjectParams(ProjectParams InParams)
		{
			...
			this.ReferenceContainerGlobalFileName = InParams.ReferenceContainerGlobalFileName;
			this.ReferenceContainerAdditionalPath = InParams.ReferenceContainerAdditionalPath;
			this.ReferenceContainerChangesCSVFileName = InParams.ReferenceContainerChangesCSVFileName;
			this.ReferenceContainerCryptoKeys = InParams.ReferenceContainerCryptoKeys;
			...
			this.RunChunkAssigner = InParams.RunChunkAssigner;
			this.PreStaged = InParams.PreStaged;
			...
			this.OnDemandSettings = InParams.OnDemandSettings;
			...
			this.DeviceLockedTimeout = InParams.DeviceLockedTimeout;
		}
		...
		public ProjectParams(			
			...
			string Upload = null,
			IoStoreOnDemandSettings OnDemandSettings = null,
			...
			)
		{
			...
			this.IncrementalCook = GetParamValueIfNotSpecified(Command, IncrementalCook, this.IncrementalCook, new string[] { "iterativecooking", "iterate", "cookincremental" });
			...
			this.RunChunkAssigner = GetParamValueIfNotSpecified(Command, RunChunkAssigner, this.RunChunkAssigner, "RunChunkAssigner");
			this.PreStaged = GetParamValueIfNotSpecified(Command, PreStaged, this.PreStaged, "prestaged");
			if (this.PreStaged)
			{
				this.Build = false;
				this.Cook = true;
			...
			this.IncrementalCook = this.Snapshot || this.IncrementalCook;
			...
			this.OnDemandSettings = OnDemandSettings ?? new IoStoreOnDemandSettings(); // Make sure we have a default (OnDemand disabled) settings if none were provided.
			this.OnDemandSettings.ReadFromCommandLine(Command);
            ...
            if (Port == null)
            {
                if( Command != null )
                {
                    ...
                    {
						ParamList<string> Ports = new ParamList<string>(PortString.Split(new char[] { '+' }, StringSplitOptions.RemoveEmptyEntries));
            ...
            if (MapsToCook == null)
            {
                if (Command != null)
                {
                    ...
                    {
						ParamList<string> MapNames = new ParamList<string>(MapsString.Split(new char[] { '+' }, StringSplitOptions.RemoveEmptyEntries));
            ...
            if (TitleID == null)
			{
				if (Command != null)
				{
					...
					{
						ParamList<string> TitleIDs = new ParamList<string>(TitleString.Split(new char[] { '+' }, StringSplitOptions.RemoveEmptyEntries));
			...
			if (Command != null)
			{
				this.DeviceLockedTimeout = Command.ParseParamInt("DeviceLockedTimeout", 120);
			}
		...
		/// <summary>
		/// Staging: run OplogChunkAssigner before staging to generate pak chunk manifests from the Zen oplog.
		/// </summary>
		[Help("RunChunkAssigner", "Run OplogChunkAssigner before staging to generate pak chunk manifests from the Zen oplog")]
		public bool RunChunkAssigner { private set; get; }

		/// <summary>
		/// Shared: true if the staged build contains a build that may not have been created locally, command line: -prestaged (experimental)
		/// </summary>
		[Help("prestaged", "the staged directory contains a build that wasn't necessarily created locally, implies -skipbuild -skipcook -skipstage (experimental)")]
		public bool PreStaged { private set; get; }
		...
		/// <summary>
		/// Name of dlc to cook and package (if this parameter is supplied cooks the dlc and packages it into the dlc directory)
		/// </summary>
		...
		/// </summary>
		[Help("IterativeDeploy", "Runs UnrealPak and targetplatform install iteratively, command line: -iterativedeploy or -iterate")]
		...
		public bool NoCleanStage { set { bNoCleanStage = value; } get { return SkipStage || PreStaged || bNoCleanStage; } }
		...
		[Help("DeviceLockedTimeout=n", "Seconds to wait for a locked iOS device before failing deploy (default 120, 0 to disable)")]
		public int DeviceLockedTimeout = 120;
		...
		[Help("ondemandsettings", "The settings for OnDemand data to apply to the project")]
		public IoStoreOnDemandSettings OnDemandSettings { get; }
		...
		private void SelectDefaultTarget(List<string> AvailableTargets, TargetType Type, ref string Target)
		{
			...
			if (EngineConfigs[BuildHostPlatform.Current.Platform].GetString("/Script/BuildSettings.BuildSettings", ConfigKey, out DefaultTarget))
			{
				...
				{
					throw new AutomationException(string.Format($"A default {Type} target '{DefaultTarget}' was specified in engine.ini but does not exist. Available targets: {string.Join(", ", AvailableTargets)}"));
				}
		...
		private void AutodetectSettings(bool bReset)
		{
			...
			ProjectProperties Properties = ProjectUtils.GetProjectProperties(RawProjectPath, ClientTargetPlatformTypes, ClientConfigsToBuild, bRunAssetNativization);
			...
			TargetType ProjectType = TargetType.Game;
			...
			else if (TargetNames.Count > 0)
			{
				...
				{
					if (Target.Rules.Type == TargetType.Game && !ClientCookedTargetsList.Contains(Target.TargetName))
					{
						ClientCookedTargetsList.Add(Target.TargetName);
						bHasGameTarget = true;
					}
					else if (Target.Rules.Type == TargetType.Client && !ClientCookedTargetsList.Contains(Target.TargetName))
					{
						ClientCookedTargetsList.Add(Target.TargetName);
						Client = true;
					}
					else if (Target.Rules.Type == TargetType.Server && !ServerCookedTargetsList.Contains(Target.TargetName))
					{
						ServerCookedTargetsList.Add(Target.TargetName);
						DedicatedServer = true;
					}
					else if (!ProgramTargetsList.Contains(Target.TargetName))
					{
			...
			if (ProjectPlatformBinariesPaths == null || ProjectExePaths == null)
			{
				...
				DirectoryReference ProjectClientBinariesPath = ProjectUtils.GetClientProjectBinariesRootPath(RawProjectPath, ProjectType, Properties.bIsCodeBasedProject);
		...
		public List<Platform> ClientTargetPlatformInstances
		{
			get
			{
				...
				foreach (TargetPlatformDescriptor ClientPlatform in ClientTargetPlatforms )
				{
		...
		public List<Platform> ServerTargetPlatformInstances
		{
			get
			{
				...
				foreach (TargetPlatformDescriptor ServerPlatform in ServerTargetPlatforms)
				{
		...
		public void Validate()
		{
            ...
            if (HasBasedOnReleaseVersion && (IncrementalCook || IterativeDeploy || HasIterateSharedCookedBuild))
            {
		...
		public virtual void ValidateAndLog()
		{
			...
			if (!bLogged)
			{
				...
				Logger.LogDebug("IncrementalCook={IncrementalCook}", IncrementalCook);
