...
namespace AutomationTool
{
	...
	public class HashCollection
	{
		...
		{
			FileInfo Fi = new FileInfo(InPath.FullName);
		...
		public void LogDifferences(HashCollection RHS)
		{
			...
			foreach (HashEntry Entry in Hashes)
			{
		...
		public static HashCollection CreateFromFile(string InPath)
		{
			try
			{
				using (FileStream stream = System.IO.File.OpenRead(InPath))
				{
					XmlSerializer Serializer = new XmlSerializer(typeof(HashCollection));
	...
	public static class CsProjectInfoExtensionMethods
	{
		...
		/// <param name="HashType"></param>
		/// <returns></returns>
		public static bool AddCsProjectInfo(this HashCollection Hasher, IDotnetProjectInfo Project, HashCollection.HashType HashType)
		{
