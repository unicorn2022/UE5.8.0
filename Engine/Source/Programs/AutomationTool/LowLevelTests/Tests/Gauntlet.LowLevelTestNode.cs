...
namespace LowLevelTests
{
	public class LowLevelTests : BaseTest
	{
		...
		public override string Name { get { return Context != null ? Context.Options.TestApp : "LowLevelTest"; } }
		...
		public override void StopTest(StopReason InReason)
		{
			try
			{
				...
				if (ReportCopied.HasValue && !ReportCopied.Value)
				{
					LowLevelTestResult = TestResult.Failed;
				...
				else if (ReportPath != null)
				{
					...
					else if (ReportType == "xml")
					{
						...
						{
							LowLevelTestResult = TestResult.Failed;
							ExitReason = $"Tests failed according to {ReportType} report";
						}
					}
					else if (ReportType == "testdata")
					{
						LowLevelTestsJsonTestDataParser LowLevelTestsJsonTestDataParser = new LowLevelTestsJsonTestDataParser(ReportContents);
						if (LowLevelTestsJsonTestDataParser.HasPassed())
						{
							LowLevelTestResult = TestResult.Passed;
							ExitReason = "Tests passed";
						}
						...
						MinimalHordeReportGenerator ReportGenerator = new MinimalHordeReportGenerator(
							new RoleReportData()
							{
								Configuration = Context.RoleContext.Configuration,
								Platform = Context.RoleContext.Platform,
								RoleType = "Client",
								SessionStartTime = SessionStartTime,
								TargetDevice = TestInstance.Device,
								InitializationEvents = new System.Collections.Generic.List<UnrealTestEvent>(),
								ClosureEvents = new System.Collections.Generic.List<UnrealTestEvent>()
							});
						ITestReport Report = ReportGenerator.GenerateReport(Context.Options.HordeTestDataKey, this, ReportPath, null);
						ReportGenerator.SubmitToHorde(Report, Name, string.Empty, Context.Options.HordeTestDataKey);
					}
					else
					{
						LowLevelTestResult = TestResult.Failed;
						ExitReason = $"Unknown report type {Context.Options.ReportType}";
						Log.Error(ExitReason);
					}
				}
