...
namespace LowLevelTests
{
	...
	public class RunLowLevelTests : BuildCommand
	{
		public override ExitCode Execute()
		{
			...
			var Result = RunTests(ContextOptions);

			Gauntlet.Horde.GenerateStepSummary();
			Gauntlet.Horde.GenerateJobSummary();
	...
	public class LowLevelTestExecutorOptions : TestExecutorOptions, IAutoParamNotifiable
	{
		...
		public virtual void ParametersWereApplied(string[] InParams)
		{
			...
			// Parse all -testnames= parameters (supports multiple test names)
			TestNames = new List<string>();
			foreach (string Arg in Params.AllArguments)
			{
				if (Arg.StartsWith("testnames=", StringComparison.OrdinalIgnoreCase))
				{
					string TestName = Arg.Substring("testnames=".Length);
	...
	public class LowLevelTestsSession : IDisposable
	{
		...
		public LowLevelTestsSession(LowLevelTestsBuildSource InBuildSource, LowLevelTestExecutorOptions InOptions)
		{
			...
			Tags = InOptions.Tags;
			TestNames = new List<string>(InOptions.TestNames);
		...
		public bool TryReserveDevices()
		{
			...
			bool Reserved = UnrealDeviceReservation.TryReserveDevices(RequiredDeviceTypes, 1);
			if (!Reserved)
			{
				Gauntlet.Horde.UpdateStepMessage(Gauntlet.Horde.GauntletHordeStepMessage.NotEnoughDevices);
			}
			return Reserved;
		}
		...
		public IAppInstance InstallAndRunNativeTestApp()
		{
			...
			UnrealAppConfig AppConfig = BuildSource.GetUnrealAppConfig(Tags, Sleep, AttachToDebugger, ReportType, PerTestTimeout, TestExtraArgs, Containerized, TestNames);
			...
			else
			{
				...
				catch (Exception Ex)
				{
					...
					Gauntlet.Horde.ReportInvalidatingIssue(Gauntlet.Horde.GauntletHordeStepMessage.InstallationFailed, "Unable to install application for session.");
	...
	public interface ILowLevelTestsBuildFactory
	{
		...
		protected static string GetExecutable(UnrealTargetPlatform InPlatform, UnrealTargetConfiguration InConfiguration, string InTestApp, string InBuildPath, string FileRegEx)
		{
			...
			foreach (string Executable in Executables)
			{
				...
				// Note: The executable name may legitimately contain a platform name as part of the test app name
				// (e.g., OnlineServicesPS4Tests). The Equals check is sufficient to identify Development builds.
			...
			Gauntlet.Horde.ReportInvalidatingIssue(Gauntlet.Horde.GauntletHordeStepMessage.BuildNotFound, "The test was unable to find the necessary executables.");
	...
	public class LowLevelTestsBuildSource : IBuildSource
	{
		...
		public UnrealAppConfig GetUnrealAppConfig(string InTags, int InSleep, bool InAttachToDebugger, string InReportType, int InPerTestTimeout = 0, string InTestExtraArgs = null, bool InContainerized = false, List<string> InTestNames = null)
		{
			if (CachedConfig == null)
			{
				...
				// Both test names and tags can be provided as positional arguments to Catch2
				// Multiple test names are passed as quoted strings without spaces between them: "Test1","Test2"
				// This allows Catch2 to treat them as comma-separated filters (OR logic)
				// Commas within test names are escaped with backslash (\,) for Catch2
				if (InTestNames != null && InTestNames.Count > 0)
				{
					// Build comma-separated test names as adjacent quoted strings
					StringBuilder TestNamesBuilder = new StringBuilder();
					for (int i = 0; i < InTestNames.Count; ++i)
					{
						// Escape commas in the test name for Catch2 parsing
						string EscapedTestName = InTestNames[i].Replace(",", "\\,");
						TestNamesBuilder.AppendFormat("\"{0}\"", EscapedTestName);

						// Add comma between test names (no space, so shell concatenates them)
						if (i < InTestNames.Count - 1)
						{
							TestNamesBuilder.Append(",");
						}
					}
					// Pass as single argument (e.g., "Test1","Test2" or "Test, with comma\, in name","Test2")
					CachedConfig.CommandLineParams.Add(TestNamesBuilder.ToString(), null, true);
				}
				if (!string.IsNullOrEmpty(InTags))
				{
					// Tags/filters as a positional argument
					// Catch2 applies AND logic when both test names and tags are provided
				...
				CachedConfig.CommandLineParams.AddRawCommandline("--order=decl");
