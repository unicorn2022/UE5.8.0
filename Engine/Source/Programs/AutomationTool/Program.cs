...
namespace AutomationToolDriver
{
	...
	public partial class Program
	{
		...
		private static void ParseParam(string CurrentParam, CommandInfo CurrentCommand, ILogger Logger)
		{
			...
			string Option_ScriptsForProject = "-ScriptsForProject";	// restrict game search directories to the the specified uproject
			string Option_ScriptDir = "-ScriptDir";					// additional search directory for .csproj files
			string Option_AutomationProjects = "-AutomationProject";// filter the found .csproj projects to build and load, specified as name1+name2+...+nameN
			...
			else if (CurrentParam.StartsWith(Option_AutomationProjects + "=", StringComparison.InvariantCultureIgnoreCase))
            {
				string AutomationProjectString = CurrentParam.Substring(CurrentParam.IndexOf('=') + 1);
				List<string> AutomationProjects = AutomationProjectString.Split("+".ToCharArray()).ToList();
				if (AutomationProjects.Count > 0)
				{
					// AutomationUtils is a necessity, see MainProc()
					if (!AutomationProjects.Contains("AutomationUtils"))
					{
						AutomationProjects.Add("AutomationUtils");
					}
					AutomationToolCommandLine.SetUnchecked(Option_AutomationProjects, AutomationProjects);
				}
			}
		...
		private static void ParseProfile(ref string[] CommandLine)
		{
			...
			if (!string.IsNullOrEmpty(Profile))
			{
				if (File.Exists(Profile))
				{
					...
					Dictionary<string, object> RawObject = fastJSON.JSON.Instance.Parse(text) as Dictionary<string, object>;
					List<object> Params = RawObject["scripts"] as List<object>;
		...
		public static void ParseCommandLine(string[] Arguments, ILogger Logger)
		{
			AutomationToolCommandLine = new ParsedCommandLine(
				new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase)
				{
					...
					{"-NoAutoSDK", ""},
					{"-CompileOnly", "Run without a command to compile all out-of-date script modules."},
			...
			// Validate
			var Result =
				AutomationToolCommandLine.CommandsToExecute.Count > 0 ||
				AutomationToolCommandLine.IsSetGlobal("-Help") ||
				AutomationToolCommandLine.IsSetGlobal("-List") ||
				AutomationToolCommandLine.IsSetGlobal("-CompileOnly");
		...
		public static async Task<int> Main(string[] Arguments)
		{
			...
			finally
			{
				...
				if (!Log.HasFileWriter() && ReturnCode != ExitCode.Success)
				{
					...
					Logger.LogInformation("AutomationTool encountered an error before a log file was set, see '{File}' for more details", errorLogFile.FullName);
				}
		...
		static async Task<ExitCode> MainProc()
		{
			...
			List<string> AutomationProjects = (List<string>) AutomationToolCommandLine.GetValueUnchecked("-AutomationProject");
			...
			HashSet<FileReference> ScriptModuleAssemblyPaths = CompileScriptModule.InitializeScriptModules(
					Rules.RulesFileType.AutomationModule,
					ScriptsForProject,
			...
			// Call into AutomationTool.Automation.ProcessAsync()
			...
			Logger.LogInformation("Total script module initialization time: {InitTime:0.00} s.", Timer.Elapsed.TotalSeconds);
			return await (Task<ExitCode>) Automation_Process.Invoke(null, [AutomationToolCommandLine, ScriptModuleAssemblyPaths]);
		}
	}
}
