...
public class AndroidPlatform : Platform
{
	...
	public override bool InstallSDK(BuildCommand BuildCommand, ITurnkeyContext TurnkeyContext, DeviceInfo Device, bool bUnattended, bool bSdkAlreadyInstalled)
	{
		...
		bool InstallCommandlineTools()
		{
			...
			string OutputPath = TurnkeyContext.RetrieveFileSource("AndroidSDKCommandlineTools");
			if (OutputPath != null)
			{
				string SdkManager = Path.Combine(OutputPath, "cmdline-tools", "bin", "sdkmanager");
				if (!OperatingSystem.IsWindows())
				{
					// Mark executable before running
					File.SetUnixFileMode(SdkManager, File.GetUnixFileMode(SdkManager) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
				}
				else
				{
					SdkManager += ".bat";
				...
				// Use the JBR from Android Studio for the initial call to sdkmanager, if JAVA_HOME doesn't exist
				string JavaHome = Environment.GetEnvironmentVariable("JAVA_HOME");
				if (JavaHome == null)
				{
					string StudioExe = GetAndroidStudioExe();
					JavaHome = OperatingSystem.IsMacOS() ? Path.Combine(StudioExe, "Contents", "jbr", "Contents", "Home") : Path.Combine(Path.GetDirectoryName(Path.GetDirectoryName(StudioExe))!, "jbr");
				}
				
				int ExitCode;
				if (!OperatingSystem.IsWindows())
				{
					string BashCmd = $"export JAVA_HOME=\"{JavaHome}\" && \"{SdkManager}\" --sdk_root=\"{SdkDir}\" \"cmdline-tools;latest\"";
					string Command;
					string Arguments;
					if (OperatingSystem.IsMacOS())
					{
						...
						Arguments = 
							...
							"-e \"activate\" " +
							$"-e \"do script \\\"{BashCmd.Replace("\"", "\\\\\\\"")}; exit\\\"\" " +
							"-e \"set theTab to the result\" " +
							"-e \"repeat\" " +
							"-e \"delay 0.5\" " +
							"-e \"if not busy of theTab then exit repeat\" " +
							"-e \"end repeat\" " +
							"-e \"close (every window whose selected tab is theTab)\" " +
							"-e \"end tell\"";
					}
					else
					{
						...
						try
						{
							Which = Process.Start(new ProcessStartInfo { FileName = "which", Arguments = "gnome-terminal", RedirectStandardOutput = true, UseShellExecute = false });
							Which?.WaitForExit();
						}
						catch (Exception)
						{
						...
						if (Which is { ExitCode: 0 })
						{
							...
							Arguments = $"--wait -- bash -c \"{BashCmd.Replace("\"", "\\\"")}\"";
						}
						else
						{
							Command = "x-terminal-emulator";
							Arguments = $"-e bash -c \"{BashCmd.Replace("\"", "\\\"")}\"";
						}
					}
					
					ExitCode = TurnkeyContext.RunExternalCommand(Command, Arguments, false, bUnattended, false);
				}
				else
				{
					string BatchCmd = $"set \"JAVA_HOME={JavaHome}\" && \"{SdkManager}\" --sdk_root=\"{SdkDir}\" \"cmdline-tools;latest\"";
					string Arguments = $"/c {BatchCmd}";
					ExitCode = TurnkeyContext.RunExternalCommand("cmd.exe", Arguments, false, bUnattended, true);
				}
	...
	private static string GetSdkDir()
	{
		...
		return string.IsNullOrEmpty(RegValue) ? DefaultSdkDir : RegValue;
	}
	...
	public override bool UpdateHostPrerequisites(BuildCommand Command, ITurnkeyContext TurnkeyContext, bool bVerifyOnly)
	{
		...
		if (!bHaveAndroidStudio)
		{
			TurnkeyContext.PauseForUser("Android Studio was not found on this machine, which is required to use Android. Download and install Android Studio?");
	...
	{
		if (Params.ClientArchitecture != null)
		{
			return Params.ClientArchitecture;
		}

		// use Receipt architectures if bIsCodeBasedProject (UnrealGame.target used if no code or plugin differences)
		if (SC.StageTargets[0].Receipt.TargetName != "UnrealGame")
		{
			Logger.LogInformation("GetDeploymentArchitectures: not UnrealGame");
			return SC.StageTargets[0].Receipt.Architectures;
		}
		
		// otherwise figure it out from project ini
		return UnrealArchitectureConfig.ForPlatform(UnrealTargetPlatform.Android).ActiveArchitectures(Params.RawProjectPath, null);
	}
	...
	private bool CreateOBBFile(DeploymentContext SC, string OutputFilename, List<FileReference> FilesForObb)
	{
		...
		try
		{
			using ZipArchive ObbFile = ZipFile.Open(OutputFilename, ZipArchiveMode.Create);
			ObbFile.Comment = $"{"1",10}";
			int ObbFileCount = 0;

			foreach (FileReference FileRef in FilesForObb)
			{
				string RelativePath = FileRef.MakeRelativeTo(SC.StageDirectory).Replace("\\", "/");
				ObbFile.CreateEntryFromFile_CrossPlatform(FileRef.FullName, RelativePath, CompressionLevel.NoCompression);
				ObbFileCount += 1;
				Logger.LogInformation("[{Count}/{Total}] Adding {File} to OBB", ObbFileCount, FilesForObb.Count, RelativePath);
			}
		}
		catch (Exception)
		{
	...
	private bool UpdateObbStoreVersion(string Filename)
	{
		...
		try
		{
			using ZipArchive ObbFile = ZipFile.Open(Filename, ZipArchiveMode.Update);
			// Add the store version from the filename as a comment
			ObbFile.Comment = $"{Version,10}";
		}
		catch (Exception)
		{
			return false;
	...
	public override void Package(ProjectParams Params, DeploymentContext SC, int WorkingCL)
	{
		...
		foreach (UnrealArch Architecture in Architectures.Architectures)
		{
			...
			if (!Deploy.bNoObbs && !bPackageDataInsideApk)
			{
			...
			void CreateInstallFilesAction(UnrealTargetPlatform Target)
			{
				...
				string[] UninstallBatchLines = GenerateUninstallBatchFile(bPackageDataInsideApk, PackageName, ApkName, Params, bIsPC, Target);
				...
				if (!OperatingSystem.IsWindows())
				{
					if (!Deploy.bNoObbs && bHaveAPK)
					{
						File.SetUnixFileMode(BatchName, File.GetUnixFileMode(BatchName) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					}
					if (bHaveAPK || bHaveUniversal)
					{
						File.SetUnixFileMode(UninstallBatchName, File.GetUnixFileMode(UninstallBatchName) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					}
					if (bHaveUniversal)
					{
						File.SetUnixFileMode(UniversalBatchName, File.GetUnixFileMode(UniversalBatchName) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					}
					// Deprecated check of bBuildWithHiddenSymbolVisibility. Remove in 5.8+
					if (bBuildWithHiddenSymbolVisibility || bSaveSymbols)
					{
						File.SetUnixFileMode(SymbolizeBatchName, File.GetUnixFileMode(SymbolizeBatchName) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					}
					//if(File.Exists(NoInstallBatchName)) 
					//{
					//    File.SetUnixFileMode(NoInstallBatchName, File.GetUnixFileMode(NoInstallBatchName) | UnixFileMode.UserExecute | UnixFileMode.GroupExecute | UnixFileMode.OtherExecute);
					//}
				}
			}
			...
			if (bNeedsPCInstall)
			{
				CreateInstallFilesAction(UnrealTargetPlatform.Win64);
			}
			if (bNeedsMacInstall)
			{
				CreateInstallFilesAction(UnrealTargetPlatform.Mac);
			}
			if (bNeedsLinuxInstall)
			{
				CreateInstallFilesAction(UnrealTargetPlatform.Linux);
			}
		...
		bool bIsPC, bool bIsDistribution, bool bRequireRuntimeStoragePermission, bool bDisablePerfHarden, bool bUseAFS, bool bUseAFSProject, string AFSToken, UnrealTargetPlatform Target)
	{
		...
		if (!bIsPC)
		{
			...
			BatchLines.AddRange(new string[] {
						...
						"\t\tSTUDIO_SDK_PATH=" + (Target == UnrealTargetPlatform.Mac ? "~/Library/Android/sdk" : "$HOME/Android/Sdk"),
						"\t\tif [ ! -d \"$STUDIO_SDK_PATH\" ]; then",
						"\t\t\techo Unable to locate local Android SDK location.",
						"\t\t\texit 1",
						"\t\tfi",
						"\tfi",
						"\tADB=$STUDIO_SDK_PATH/platform-tools/adb",
						"fi",
						"DEVICE=",
	...
	private string[] GenerateUninstallBatchFile(bool bPackageDataInsideApk, string PackageName, string ApkName, ProjectParams Params, bool bIsPC, UnrealTargetPlatform Target)
	{
	...
	private string[] GenerateSymbolizeBatchFile(ProjectParams Params, string PackageName, string ApkName, DeploymentContext SC, UnrealArch Architecture, bool bIsPC)
	{
		...
		if (!bIsPC)
		{
			...
			BatchLines = new string[] {
				...
				"fi",
				"cd \"`dirname \"$0\"`\"",
				"$NDKROOT/ndk-stack -sym " + GetFinalSymbolizedSODirectory(ApkName, SC, Architecture) + " -dump \"$1\" > " + Params.ShortProjectName + "_SymbolizedCallStackOutput.txt",
				"exit 0",
				};
		}
		else
		{
			...
			BatchLines = new string[] {
						...
						"setlocal",
						"",
						"%NDKROOT%\\ndk-stack.cmd -sym "+GetFinalSymbolizedSODirectory(ApkName, SC, Architecture)+" -dump \"%1\" > "+ Params.ShortProjectName+"_SymbolizedCallStackOutput.txt",
						"",
						"goto:eof",
						"",
						"",
	...
	public override void GetFilesToArchive(ProjectParams Params, DeploymentContext SC)
	{
		...
		UnrealArchitectures Architectures = GetDeploymentArchitectures(Params, SC);
		bool bMakeSeparateApks = UnrealBuildTool.AndroidExports.ShouldMakeSeparateApks();
		IAndroidDeploy Deploy = UnrealBuildTool.AndroidExports.CreateDeploymentHandler(Params.RawProjectPath, Params.ForcePackageData);
		bool bPackageDataInsideApk = Deploy.GetPackageDataInsideApk();
		...
		foreach (UnrealArch Architecture in Architectures.Architectures)
		{
			...
			if (!Deploy.bNoObbs && !bPackageDataInsideApk && !FileExists(ObbName))
			{
	...
	*/
	...
	private void DeployAndroidFileServer(ProjectParams Params, DeploymentContext SC, string AFSToken)
	{
		...
		foreach (string DeviceName in Params.DeviceNames.Select(DeviceName => GetSerialNumber(DeviceName)))
		{
			...
			if (!Params.Prebuilt && !Deploy.bFromMSBuild)
			{
				string CookFlavor = SC.FinalCookPlatform.IndexOf("_") > 0 ? SC.FinalCookPlatform.Substring(SC.FinalCookPlatform.IndexOf("_")) : "";
				string SOName = GetSONameWithoutArchitecture(Params, SC.StageExecutables[0]);
				Deploy.SetAndroidPluginData(GetDeploymentArchitectures(Params, SC), CollectPluginDataPaths(SC));
				Deploy.PrepForUATPackageOrDeploy(Params.RawProjectPath, Params.ShortProjectName, SC.ProjectRoot, SOName, SC.LocalRoot + "/Engine", Params.Distribution, CookFlavor, SC.StageTargets[0].Receipt.Configuration, !SC.Archive, false, SC.Archive);
			}
    ...
    */
	...
	public override void StripAllSymbols(FileReference SourceFile, FileReference TargetFile)
	{
		AndroidExports.StripSymbols(SourceFile, TargetFile, Log.Logger, true);
	}
}
