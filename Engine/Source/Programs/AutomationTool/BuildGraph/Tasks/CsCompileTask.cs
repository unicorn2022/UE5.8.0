...
namespace AutomationTool.Tasks
{
	...
	public class CsCompileTask : BgTaskImpl
	{
		...
		static void FindBuildProductsAndReferences(HashSet<FileReference> projectFiles, Dictionary<string, string> initialProperties, out HashSet<FileReference> outBuildProducts, out HashSet<FileReference> outReferences)
		{
			...
			Dictionary<FileReference, IDotnetProjectInfo> fileToProjectInfo = [];
			foreach (FileReference projectFile in projectFiles)
			{
				...
				foreach (KeyValuePair<FileReference, IDotnetProjectInfo> pair in fileToProjectInfo)
				{
					IDotnetProjectInfo projectInfo = pair.Value;
					...
					{
						projectInfo.AddReferencedAssemblyAndSupportFiles(reference.Key, outReferences);
					}
				}
			}
		...
		static void ReadProjectsRecursively(FileReference file, Dictionary<string, string> initialProperties, Dictionary<FileReference, IDotnetProjectInfo> fileToProjectInfo)
		{
			...
			if (!fileToProjectInfo.ContainsKey(file))
			{
				...
				if (!DotnetProjectInfo.TryRead(file, initialProperties, out projectInfo))
				{
