...
namespace AutomationTool.Tasks
{
	...
	public class StripTaskParameters
	{
		...
		/// <summary>
		/// Whether all symbols should be stripped. Not relevant to all platforms.
		/// </summary>
		[TaskParameter(Optional = true)]
	...
	public class StripTask : BgTaskImpl
	{
		...
		public override Task ExecuteAsync(JobContext job, HashSet<FileReference> buildProducts, Dictionary<string, HashSet<FileReference>> tagNameToFileSet)
		{
			...
			for (int idx = 0; idx < sourceFiles.Length; idx++)
			{
				...
				if (_parameters.StripAll)
				{
					targetPlatform.StripAllSymbols(sourceFiles[idx], targetFiles[idx]);
				}
				else
				{
