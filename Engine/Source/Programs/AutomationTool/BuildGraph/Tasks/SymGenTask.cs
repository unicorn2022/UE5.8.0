...
namespace BuildGraph.Tasks
{
	...
	public class SymGenTask : CustomTask
	{
		...
		public override void Execute(JobContext job, HashSet<FileReference> buildProducts, Dictionary<string, HashSet<FileReference>> tagNameToFileSet)
		{
			...
			string radSymDymperExecuable = Unreal.RootDirectory + @"\Engine\Extras\rad\Binaries\Win64\radbin.exe";
			...
			Parallel.ForEach(symbolSourceFiles, (sourceFile) =>
			{
					...
					sourceFile.GetExtension() == ".pdb")
				{
					...
					dumpSymsArgs = $"--breakpad {sourceFile.FullName} -out:{radSymbolTemp}";
				}
				else
				{
					string extraOptions = "";
				...
				if (result.ExitCode == 0)
				{
					...
					{
						// Use streaming approach to avoid 2GB string limitation
						// Read directly from the process output without materializing it as a string
						try
						{
							using (StringBuilderReader reader = result.GetOutputReader())
							{
								if (reader != null)
								{
									using (StreamWriter writer = new StreamWriter(symbolFileName))
									{
										ProcessSymbols(reader, writer);
										symbolFiles.Add(new FileReference(symbolFileName));
									}
								}
							}
				...
				{
					if (BuildHostPlatform.Current.Platform != UnrealTargetPlatform.Mac && !bUseRadSym)
					{
		...
		static bool ProcessSymbols(StringBuilderReader reader, TextWriter writer)
		{
			char[] fieldSeparator = { ' ' };
			...
			while (reader.ReadLine(out ReadOnlySpan<char> lineSpan))
			{
				if (lineSpan.Contains(" = ", StringComparison.InvariantCulture))
				{
					Logger.LogInformation("{Text}", lineSpan.ToString());
					continue;
				}
				// Ignore any output from symbol dump before MODULE, these may included erroneous warnings, etc
				if (!bSawModule)
				{
					if (!lineSpan.StartsWith("MODULE", StringComparison.InvariantCulture))
					{
						continue;
					}
				...
				if (lineSpan.StartsWith("FILE", StringComparison.InvariantCulture))
				{
					// Convert to string only for FILE lines that need processing
					string line = lineSpan.ToString();
					string[] fields = line.Split(fieldSeparator, 3);

					// Check we got all the expected fields
					if (fields.Length == 3)
					{
						...
						if (File.Exists(fileName) && fileName.StartsWith(rootDirectory, StringComparison.OrdinalIgnoreCase))
						{
							...
							writer.WriteLine(String.Format("FILE {0} {1}", fields[1], fileName.Substring(rootDirectory.Length + 1).Replace('\\', '/')));
							continue;
						}
					}
				}
