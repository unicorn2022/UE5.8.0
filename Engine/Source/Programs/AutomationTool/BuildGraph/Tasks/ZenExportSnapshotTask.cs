...
#nullable enable
		...
		private static bool TryExportOplogCommand(string app, string commandLine)
		{
			...
			int[] waitDurationWebSeconds = [60, 300, 900, 3600];
			int attemptLimit = waitDurationWebSeconds.Length;
			...
			while (attempt < attemptLimit)
			{
				...
				// 502 and 504 are the old exit codes for retries the new exit codes for retries that also work on mac and linux are 69, 92 and 93 see zen cli documentation of exit codes for more details
				bool isGatewayError = exitCode is 502 or 504 or 69 or 92 or 93 or 84;
		...
		private static bool IsValidCharacter(char character) => character <= 0x7f && IsValidCharacter((byte)character);

		private static bool IsValidCharacter(byte character)
		{
		...
		{
			// Originally matched StringId.Sanitize, but with a MaxLength of 256 instead of 64
			const int MaxLength = 256;
			StringBuilder result = new StringBuilder();
			for (int idx = 0; idx < inString.Length && result.Length < MaxLength; idx++)
			{
				char character = (char)inString[idx];
				if (character >= 'A' && character <= 'Z')
				{
