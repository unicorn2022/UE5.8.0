...
namespace AutomationTool.Tasks
{
	...
	public static partial class StandardTasks
	{
		...
		/// <param name="scope"> Where to display the report</param>
		/// <param name="placement">Where to show the report</param>
		/// <param name="text">Text to be displayed</param>
		/// <returns>Creates a Horde report file, which will be displayed on the dashboard with any job running this task.</returns>
		public static async Task<FileSet> HordeCreateReportAsync(string name, string scope, string placement, string text)
		{
			HordeCreateReportTaskParameters parameters = new()
			{
				Name = name,
				Scope = scope,
				Placement = placement,
			...
			return await ExecuteAsync(new HordeCreateReportTask(parameters));
		}
	}
}