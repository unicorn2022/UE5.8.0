...
#pragma warning restore CA2254
	...
	class BgScriptNode : BgNodeDef
	{
		...
		/// Constructor
		/// </summary>
		public BgScriptNode(string name, IReadOnlyList<BgNodeOutput> inputs, IReadOnlyList<BgNodeOutput> optionalInputs, IReadOnlyList<string> outputNames, IReadOnlyList<BgNodeDef> inputDependencies, IReadOnlyList<BgNodeDef> optionalInputDependencies, IReadOnlyList<BgNodeDef> orderDependencies, IReadOnlyList<FileReference> requiredTokens, IReadOnlyList<string> ignoreModified)
			: base(name, inputs, optionalInputs, outputNames, inputDependencies, optionalInputDependencies, orderDependencies, requiredTokens, ignoreModified)
		{
		}
	}
	...
	public class BgScriptReader
	{
		...
		async Task ReadNodeAsync(BgScriptElement element)
		{
			...
			if (await EvaluateConditionAsync(element) && TryReadObjectName(element, out name))
			{
				...
				string[] optionalRequiresNames = ReadListAttribute(element, "OptionalRequires");
				...
				bool allowRetry = ReadBooleanAttribute(element, "AllowRetry", true);
				...
				// Resolve all the optional inputs we depend on
				HashSet<BgNodeOutput> optionalInputs = ResolveInputReferences(element, optionalRequiresNames);

				// If an input is both required and optional the required taking precedence
				HashSet<BgNodeOutput> duplicateInputs = new(inputs);
				duplicateInputs.IntersectWith(optionalInputs);
				foreach (BgNodeOutput duplicateInput in duplicateInputs)
				{
					LogWarning(element, "Optional input '{Tag}' is also a required input. The required input takes precedence", duplicateInput.TagName);
				}
				optionalInputs.ExceptWith(duplicateInputs);

				// Gather all the optional input dependencies, and check they're all upstream of the current node
				HashSet<BgNodeDef> optionalInputDependencies = [];
				foreach (BgNodeDef optionalInputDependency in optionalInputs.Select(x => x.ProducingNode).Distinct())
				{
					optionalInputDependencies.Add(optionalInputDependency);
				}
				...
				// Intentionally only traverses InputDependencies (required), not OptionalInputDependencies.
				// A required dependency's own optional deps don't need to succeed for it to succeed, so they
				// don't belong in the required chain. Execution ordering for optional dependents is handled
				// separately via OrderDependencies below.
				...
				// Recursively include all their optional dependencies too
				foreach (BgNodeDef optionalInputDependency in optionalInputDependencies.ToArray())
				{
					// A node that optionally depends on another must still respect its resource locks
					requiredTokens.UnionWith(optionalInputDependency.RequiredTokens);
					optionalInputDependencies.UnionWith(optionalInputDependency.InputDependencies);
					optionalInputDependencies.UnionWith(optionalInputDependency.OptionalInputDependencies);
				}
				...
				if (CheckNameIsUnique(element, name))
				{
					...
					BgScriptNode newNode = new(name, inputs.ToArray(), optionalInputs.ToArray(), validOutputNames.ToArray(), inputDependencies.ToArray(), optionalInputDependencies.ToArray(), orderDependencies.ToArray(), requiredTokens.ToArray(), ignoreModified.ToArray())
					{
						RunEarly = runEarly,
						AllowRetry = allowRetry,
						NotifyOnWarnings = notifyOnWarnings
					};
					foreach ((string key, string value) in annotations)
					{
						BgAnnotationDef? annotationDef = newNode.Annotations.FirstOrDefault(a => a.Name.Equals(key, StringComparison.OrdinalIgnoreCase));
						if (annotationDef == null)
						{
							annotationDef = new BgAnnotationDef(key, value);
							newNode.Annotations.Add(annotationDef);
						}
						else
						{
							annotationDef.Value = value;
						}
					}
		...
		async Task ReadAnnotationAsync(BgScriptElement element)
		{
			if (await EvaluateConditionAsync(element))
			{
				...
				foreach (BgNodeDef node in nodes)
				{
					foreach ((string key, string value) in annotations)
					{
						BgAnnotationDef? annotationDef = node.Annotations.FirstOrDefault(a => a.Name.Equals(key, StringComparison.OrdinalIgnoreCase));
						if (annotationDef == null)
						{
							annotationDef = new BgAnnotationDef(key, value);
							node.Annotations.Add(annotationDef);
						}
						else
						{
							annotationDef.Value = value;
		...
		/// <summary>
		/// Outputs a warning message to the log, referencing the file and line number of the element that caused it.
		/// </summary>
		/// <param name="element">The script element causing the error</param>
		/// <param name="format">Standard String.Format()-style format string</param>
		/// <param name="args">Optional arguments</param>
