...
namespace AutomationTool
{
	...
	public static class BgScriptWriter
	{
		...
		public static void Write(this BgGraphDef graph, FileReference file, FileReference? schemaFile)
		{
			...
			using (XmlWriter writer = XmlWriter.Create(file.FullName, settings))
			{
				...
				foreach (BgLabelDef label in graph.Labels)
				{
					...
					includedNodes.ExceptWith(label.IncludedNodes.SelectMany(x => x.OptionalInputDependencies));
					...
					excludedNodes.UnionWith(label.IncludedNodes.SelectMany(x => x.OptionalInputDependencies));
					...
					excludedNodes.ExceptWith(excludedNodes.ToArray().SelectMany(x => x.OptionalInputDependencies));
		...
		public static void Write(this BgNodeDef node, XmlWriter writer)
		{
			...
			string[] optionalRequireNames = node.OptionalInputs.Select(x => x.TagName).ToArray();
			if (optionalRequireNames.Length > 0)
			{
				writer.WriteAttributeString("OptionalRequires", String.Join(";", optionalRequireNames));
			}
			...
			string[] afterNames = node.GetDirectOrderDependencies().Except(node.InputDependencies).Except(node.OptionalInputDependencies).Select(x => x.Name).ToArray();
			...
			if (!node.AllowRetry)
			{
				writer.WriteAttributeString("AllowRetry", node.AllowRetry.ToString());
			}
