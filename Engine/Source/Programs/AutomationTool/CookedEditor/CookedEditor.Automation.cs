...
public class ModifyStageContext
{
	...
	private void AddCookedUFSFilesToList(List<FileReference> FileList, string Extension, DeploymentContext SC)
	{
		...
		FileList.AddRange(UFSFilesToStage.Where(x => x.FullName.EndsWith(Extension, StringComparison.OrdinalIgnoreCase)));
	}
	...
	private void AddUFSFilesToList(List<FileReference> FileList, string Extension, DeploymentContext SC)
	{
		...
		{
			if (Pair.Key.Name.EndsWith(Extension, StringComparison.OrdinalIgnoreCase))
			{
	...
	private void UnUFSFiles(DeploymentContext SC)
	{
		if (bStageUAT)
		{
			...
			NonUFSFilesToStage = NonUFSFilesToStage.Where(x => x.GetFileName().IndexOf("BinaryConfig", StringComparison.OrdinalIgnoreCase) == -1).ToList();
		}
	}
