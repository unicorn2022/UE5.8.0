...
namespace EpicGames.SmartlingLocalization
{
	...
	public abstract class SmartlingLocalizationProvider : LocalizationProvider
	{
			...
			: base(InArgs)
		{
			...
			Client = new HttpClient() { Timeout = TimeSpan.FromSeconds(1300) };
		...
		// {
		// 	// @TODOLocalization: Throw an exception if the config data is null/invalid 
		// }
		...
		private async Task<List<string>> DownloadLatestPOFile(string EpicLocale, string Platform, ProjectImportExportInfo ProjectImportInfo)
		{
			...
			int CurrentTries = 1;
			// The base for which all the exponential back off will be derived from.
			int InitialTimeOut = 250;
			...
			while (true)
			{
				...
				try
				{
					...
					if (DownloadResponse.StatusCode == HttpStatusCode.Unauthorized)
					{
						...
						if (CurrentTries > MaxTries)
						{
							...
							Logs.Add($"[WARNING] Failed in downloading: '{ExportFile.FullName}' ({EpicLocale}) exhausted all retries after {DownloadStopWatch.ElapsedMilliseconds / 1000}.");
							return Logs;
						}
				...
				catch (Exception Ex)
				{
					...
					{
						DownloadStopWatch.Stop();
						Logs.Add($"[WARNING] Failed in downloading: '{ExportFile.FullName}' ({EpicLocale}) exhausted all retries in {DownloadStopWatch.ElapsedMilliseconds / 1000.0:F2} seconds. - {Ex}");
						return Logs;
					}
		...
		private async Task<List<string>> UploadLatestPOFile(string EpicLocale, string Platform, ProjectImportExportInfo ProjectExportInfo)
		{
			...
			while (true)
			{
				try
				{
					...
					if (UploadResponse.StatusCode == HttpStatusCode.Unauthorized)
					{
						...
						if (CurrentTries > MaxTries)
						{
							Logs.Add($"[WARNING] Failed in uploading: '{FileToUpload.FullName}' ({EpicLocale}). Exhausted all retries.");
							return Logs;
						}
				...
				{
					Logs.Add($"[WARNING] Failed in uploading: '{FileToUpload.FullName}' ({EpicLocale}) - {Ex}");
					return Logs;
				}
			}
			...
			{
				Logs.Add($"[FAILED] Failed in uploading: '{FileToUpload.FullName}' ({EpicLocale})");
