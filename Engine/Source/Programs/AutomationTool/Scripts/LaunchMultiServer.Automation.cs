...
Example running 4 clients locally connecting to single proxy server w/ a non-local cluster of 3:
	...
	public class LaunchMultiServer : BuildCommand, IProjectParamsHelpers
	{
		...
		// Stats port counter: each process gets a unique -statsPort starting at this value
		private int NextStatsPort = 10100;
		...
		[CommandLine, Help("DirectClientCount=##", "Specify how many clients to launch in client=direct mode.  If 0 (default), will launch one for each server.")]
		public int DirectClientCount = 0;

		[CommandLine, Help("BotCount=##", "Specify how many client bots to launch. Client bots are launched with -bot it is up to the game to specify the implementation.")]
		...
		[CommandLine, Help("ProxyBotCount=##", "Specify how many bots to launch PER SERVER in client=proxy mode. Prefer -BotCount=XX.")]
		...
		[CommandLine, Help("AttachToProxies", "Force Visual Studio to attach to the proxies when launched (requires VS be launched or it will hang)")]
		public bool AttachToProxies = false;
		...
		[CommandLine, Help("EnableStatsPort", "Enable the -statsPort argument on all launched processes")]
		public bool EnableStatsPort = false;
		...
		public string GenerateRandomSessionString(int Length)
		{
			const string Chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
			string RandString = RandomNumberGenerator.GetString(Chars, Length);
			return RandString;
		}
		...
		public override ExitCode Execute()
		{
			...
			string[] AllGameServerAddresses = LocalGameServerAddresses.Concat(NonLocalServerAddresses).ToArray();
			...
			foreach (string ClientValue in ClientType)
			{
				...
				{
					int MaxDirectClientCount = 0;
					foreach (var GameServerAddressWithPortRange in AllGameServerAddresses)
					{
						...
						if (bValidAddress)
						{
							MaxDirectClientCount += (EndPort - StartPort) + 1;
						}
					}

					if (DirectClientCount != 0)
					{
						if (DirectClientCount > MaxDirectClientCount)
						{
							Logger.LogWarning($"Specified DirectClientCount=({DirectClientCount}) but only a maximum of {MaxDirectClientCount} addresses to connect to.");
							DirectClientCount = MaxDirectClientCount;
						}
					}
					else
					{
                    ...
                    Logger.LogInformation($"Starting {DirectClientCount} direct client instances connecting to MultiServer instances");
					StartProcessesForClients(ProcessLaunchType.DirectClient, AllGameServerAddresses, CommonArgs, 1, DirectClientCount);

					if (BotCount > 0)
					{
						// NumBotsPerServer = Ceil(BotCount / ServerCount);
						int ServerCount = CountServers(AllGameServerAddresses);
						int NumBotsPerServer = (BotCount + ServerCount - 1) / ServerCount;

						Logger.LogInformation($"Starting {BotCount} bots connecting to {string.Join(',', AllGameServerAddresses)}. Roughly {NumBotsPerServer} per server.");
						StartProcessesForClients(ProcessLaunchType.BotClient, AllGameServerAddresses, $"{CommonArgs} -nullrhi -bot -nosound -unattended -nosplash", NumBotsPerServer, BotCount);
					}
				}
				else if (ClientValue.ToLower() == "proxy")
				{
					Logger.LogInformation($"Starting {ProxyServerCount} Proxy Servers each with {ProxyClientCount} Clients");
					...
					if (ProxyBotCount > 0)
					{
						Logger.LogInformation($"Starting {ProxyBotCount} bots for each proxy address: {string.Join(',', LocalProxyAddresses)}.");
						StartProcessesForClients(ProcessLaunchType.BotClient, LocalProxyAddresses, $"{CommonArgs} -nullrhi -bot -nosound -unattended -nosplash", ProxyBotCount);
					}

					if (BotCount > 0)
					{
						// NumBotsPerProxy = Ceil(BotCount / ProxyServerCount);
						int NumBotsPerProxy = (BotCount + ProxyServerCount - 1) / ProxyServerCount;
						Logger.LogInformation($"Starting {BotCount} bots connecting to {string.Join(',', LocalProxyAddresses)}. Roughly {NumBotsPerProxy} per proxy.");
						StartProcessesForClients(ProcessLaunchType.BotClient, LocalProxyAddresses, $"{CommonArgs} -nullrhi -bot -nosound -unattended -nosplash", NumBotsPerProxy, BotCount);
					}
				}
				else
				{
		...
		virtual protected string[] StartProcessesForDedicatedServers(int NumLocalServers, string[] NonLocalServers, string AdditionalServerArguments)
		{
			...
			for (int ServerId = 1; ServerId <= NumLocalServers; ++ServerId)
			{
				...
				ServerArgs += $" -port={ServerGamePort} -log={ProjectFullPath.GetFileNameWithoutAnyExtensions()}-Server-{ServerId}.log";
        ...
        /// <param name="MaxClientCount">The maximum number of clients to launch.  Zero for no limit (one for each server address or proxy server port)</param>
        virtual protected void StartProcessesForClients(ProcessLaunchType ClientType, string[] GameServerAddresses, string AdditionalClientArguments, int ClientsPerServer = 1, int MaxClientCount = 0)
		{
			int TotalClientCount = 0;
			for (int ServerAddressIdx= 0; ServerAddressIdx < GameServerAddresses.Length; ++ServerAddressIdx)
			{
				string GameServerAddressWithPortRange = GameServerAddresses[ServerAddressIdx];
				string GameServerBaseAddress;
				int StartPort, EndPort;
				...
				for (int Port = StartPort; Port <= EndPort; ++Port)
				{
					...
					ClientArgs += NoTimeouts ? " -notimeouts" : "";
					ClientArgs += $" {GameServerBaseAddress}:{Port}";
					ClientArgs += $" -windowed";

					// Potentially launch multiple clients per server address
					for (int ClientInstanceNum = 0; ClientInstanceNum < ClientsPerServer; ++ClientInstanceNum)
					{
						int ClientId = (ServerAddressIdx + 1 + ClientInstanceNum);

						++TotalClientCount;
						string InstanceArgs = ClientArgs + $" -log={ProjectFullPath.GetFileNameWithoutAnyExtensions()}-{ClientType}-{TotalClientCount}.log";
						InstanceArgs += $" -SaveWinPos={TotalClientCount}";

						string InstanceDetails = (ClientsPerServer > 1) ? $"(Srv {ServerAddressIdx + 1})" : string.Empty;
						string AppTitle = $"{ClientType} {ClientId} {InstanceDetails}";
						ExecuteApp(ClientType, AppTitle, InstanceArgs);

						if (MaxClientCount != 0 && TotalClientCount >= MaxClientCount)
						{
							return;
						}
					}
		...
		virtual protected void StartProcessesForProxyServer(string[] LocalProxyServerAddresses, string[] AllGameServerAddresses, string AdditionalProxyArguments)
		{
			...
			CommonProxyServerArgs += " -ini:Engine:[ConsoleVariables]:net.proxy.Enabled=1";
			...
			for (int ProxyIdx = 1; ProxyIdx <= LocalProxyServerAddresses.Length; ++ProxyIdx)
			{
				...
				ProxyServerArgs += $" -log={ProjectFullPath.GetFileNameWithoutAnyExtensions()}-Proxy-{ProxyIdx}.log";
		...
		protected void ExecuteApp(ProcessLaunchType LaunchType, string AppTitle, string AdditionalArguments)
		{
			...
			(
				...
				Client: LaunchType == ProcessLaunchType.DirectClient || LaunchType == ProcessLaunchType.ProxyClient || LaunchType == ProcessLaunchType.BotClient
			);
		...
		/// <summary>
		/// Counts the number of servers specified in the passed-in string array. We account for port ranges.
		/// </summary>
		/// <param name="ServerAddresses">The server addresses (IpAddress:StartPort-EndPort)</param>
		/// <returns>The number of servers specified (taking into account port ranges)</returns>
		protected int CountServers(string[] ServerAddresses)
		{
			int TotalCount = 0;
			foreach (string AddressToParse in ServerAddresses)
			{
				string BaseAddress;
				int StartPort, EndPort;
				ParseAddressAndPortRange(out BaseAddress, out StartPort, out EndPort, AddressToParse);

				TotalCount += EndPort - StartPort + 1;
			}
		...
		{
			return (AttachToServers && (LaunchType == ProcessLaunchType.Server)) ||
				 (AttachToProxies && (LaunchType == ProcessLaunchType.ProxyServer)) ||
