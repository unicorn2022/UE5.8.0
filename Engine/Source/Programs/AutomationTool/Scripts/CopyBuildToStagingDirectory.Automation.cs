	...
	public partial class Project : CommandUtils
	{
		...
		public struct OrderFile : IComparable<OrderFile>
		{
			...
			public int CompareTo(OrderFile Other)
			{
				...
				if (compare == 0)
				{
					return File.GetFileNameWithoutExtension().CompareTo(File.GetFileNameWithoutExtension());
				}
				return compare;
			}
		...
		private static bool IsLocRes(string Extension)
		{
			return Extension == ".locres";
		}

		private static bool IsLocMeta(string Extension)
		{
			return Extension == ".locmeta";
		}
		...
		private static string StripPackageExtension(string FullPath)
		{
			...
			if (IsBulkDataExtension(Extension) || IsOptionalSegment(SecondExtension) || IsMemoryMapped(SecondExtension) || IsLocRes(SecondExtension) || IsLocMeta(SecondExtension))
			{
				...
				foreach (var PathStr in ExtraDirs)
				{
					...
					if (RelativePath != null)
					{
						...
						if (bUFS)
						{
							...
							Files.RemoveAll(x => x.HasExtension(".uasset") || x.HasExtension(".umap") || x.HasExtension(".po") || x.HasExtension(".archive") || (SC.DedicatedServer && x.HasExtension(".mp4")));
		...
		{
			ConcurrentBag<ZenCookedFile> ZenCookedFiles = new ConcurrentBag<ZenCookedFile>();
			...
			ConfigHierarchy gameIni = ConfigCache.ReadHierarchy(
				...
				DirectoryReference.FromFile(SC.RawProjectPath),
				SC.StageTargetPlatform.IniPlatformType);
			IReadOnlyList<OplogEntry> entries;
			try
			{
			...
			{
				throw new AutomationException(String.Format("Failed reading oplog from Zen at {0} for oplog {1}.{2}: {3}.",
					SocketHostNameAndPort, ProjectId, OplogId, Ex.Message));
			}

			// Attempt to read chunk assignment from oplog.
			SC.PackageStoreData.ChunkAssignment = entries.OfType<ChunkAssignmentOp>().FirstOrDefault();

			Parallel.ForEach(entries, entry =>
			{
				switch (entry)
				{
					case PackageOplogEntry pkg:
						foreach (PackageFileInfo pf in pkg.PackageFiles)
						{
							ZenCookedFiles.Add(new ZenCookedFile {
								Filename = FileReference.Combine(SC.PlatformCookDir, pf.Filename).FullName,
								ChunkId  = pf.ChunkId,
							});
						}
						...
						{
							ZenCookedFiles.Add(new ZenCookedFile {
								Filename = FileReference.Combine(SC.PlatformCookDir, bf.Filename).FullName,
								ChunkId  = bf.ChunkId,
							});
						}
						foreach (OplogFileRecord file in pkg.Files)
						{
							string RelativeFilename = SubstituteOplogPath(file.ClientPath, SC.ShortProjectName);
							ZenCookedFiles.Add(new ZenCookedFile {
								Filename = FileReference.Combine(SC.PlatformCookDir, RelativeFilename).FullName,
								ChunkId  = file.ChunkId,
							});
						}
						break;

					case EndCookOplogEntry endCook:
						foreach (OplogFileRecord file in endCook.Files)
						{
							string RelativeFilename = file.ClientPath;
							bool bReportFile = false;
							...
							{
								RelativeFilename = SubstituteOplogPath(RelativeFilename, SC.ShortProjectName);
								bReportFile = true;
							}
							else if (RelativeFilename.EndsWith(".locmeta") || RelativeFilename.EndsWith(".locres"))
							{
								// Record all cooked localization files.
								// Some of the files in the zen oplog come from non-cooked locations.
								// We currently do not have a way to distinguish them.
								//
								// TODO: Find out whether the file is from the cooked location by using
								// file.ServerPath;
								//
								// (See FManifestGenerator::AddSingleFile for the construction of that serverpath).
								// That path is relative to the UnrealEngine basedir (e.g. parent of Engine folder) of the
								// executable that ran the cooker. We don't currently know our UnrealEngine basedir. We
								// can't use the current executing assembly basedir (Unreal.RootDirectory), because the
								// basedir when running the cooker might have been a junction
								// L:\build -> d:\UnrealEngineRoot. We need L:\build here to match what the cooker saw,
								// in case the cooked output directory is under e.g. L:\CookedOutput. We should pass the
								// UnrealEngine basedir in as a ForceCookedRootDir argument to the AutomationTool, and
								// pass it in place of Unreal.RootDirectory in the construction of the DeploymentContext.
								// We would then be able to use SC.LocalRoot to find the UnrealEngine basedir that was
								// used by the cooker.
								RelativeFilename = SubstituteOplogPath(RelativeFilename, SC.ShortProjectName);
								FileReference CookedFile = FileReference.Combine(SC.PlatformCookDir, RelativeFilename);
								if (CookedFile.IsUnderDirectory(SC.PlatformCookDir) && FileReference.Exists(CookedFile))
								{
									bReportFile = true;
								}
							}
			...
			return ZenCookedFiles.ToList();
		}

		/// <summary>
		/// Replace the <c>/{engine}/</c> and <c>/{project}/</c> placeholders in an oplog clientpath
		/// with the concrete <c>Engine/</c> and project-name folders.
		/// </summary>
		private static string SubstituteOplogPath(string clientPath, string shortProjectName)
		{
			return clientPath
				.Replace("/{engine}/",  "Engine/")
				.Replace("/{project}/", shortProjectName + "/");
		}

		private static void LoadPackageStoreData(ProjectParams Params, DeploymentContext SC, bool bLoadCookedFiles = false)
		{
		...
		{
			// bStagingIsUsingPakFiles is only relevant to bShouldMaterializeZenFile, which we are ignoring. Arbitrarily set it to true.
			bool bStagingIsUsingPakFiles = true;
			return GetStagingTypeForCookedFile(File, bStagingIsUsingPakFiles, out bool bShouldMaterializeZenFile);
		}

		private static StagedFileType? GetStagingTypeForCookedFile(FileReference File, bool bStagingIsUsingPakFiles,
			out bool bShouldMaterializeZenFile)
		{
			bShouldMaterializeZenFile = false;
			...
			if (File.HasExtension(".locres") || File.HasExtension(".locmeta"))
			{
				// Localization functions such as StageLocalizationForCookedGameFeaturePlugins currently only
				// look on disk for the localization files. We therefore always materialize localization files.
				// This can be changed to not materialize them if we change all of the Localization functions to look
				// in ZenCookedFiles as well as on disk.
				bShouldMaterializeZenFile = true;
			}

			StagedFileType Result = StagedFileType.UFS;
			// StagedFileType.NonUFS is currently unused for cooked files, but if we ever add it then we will need this
			// condition to materialize it for inclusion in the output staged directory as a loose file.
			if (Result == StagedFileType.NonUFS && bStagingIsUsingPakFiles)
			{
				bShouldMaterializeZenFile = true;
			}
		...
		private static FileReference ZenCookedFileToFileReference(ProjectParams Params, DeploymentContext SC, ZenCookedFile CookedFile, HttpClient Client, List<Task> TaskList, HashSet<string> TemporaryNonUFSFiles)
		{
			...
			{
				bool bUsePak = Params.UsePak(SC.StageTargetPlatform);
				StagedFileType? CookedFileStageType = GetStagingTypeForCookedFile(CookedFileReference, bUsePak, out bool bShouldMaterializeZenFile);
				if (CookedFileStageType.HasValue)
				{
					if (bShouldMaterializeZenFile)
					{
					...
					if (CookedFileStageType.Value == StagedFileType.NonUFS && bUsePak)
					{
		...
		private static List<FileReference> DiscoverCookedFiles(ProjectParams Params, DeploymentContext SC, bool bAlwaysFullyScanCookDir)
		{
			...
			LoadPackageStoreData(Params, SC, true);
		...
		public static void CreateStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			else
			{
				...
				// move Engine/Source/ThirdParty/Licenses/NOTICES.txt file to the root of the stage for necessary third party library attribution
				// this file needs to be treated as a UFS file for casing, but NonUFS for being put into the .pak file
				FileReference ThirdPartyNoticesFile = FileReference.Combine(SC.EngineRoot, "Source", "ThirdParty", "Licenses", "NOTICES.txt");
				if (FileReference.Exists(ThirdPartyNoticesFile))
				{
					StagedFileReference StagedThirdPartyNoticesFile = new StagedFileReference("NOTICES.txt");
					if (SC.StageTargetPlatform.DeployLowerCaseFilenames(StagedFileType.SystemNonUFS))
					{
						StagedThirdPartyNoticesFile = StagedThirdPartyNoticesFile.ToLowerInvariant();
					}
					SC.StageFile(StagedFileType.SystemNonUFS, ThirdPartyNoticesFile, StagedThirdPartyNoticesFile);
				}
				...
				if (!Params.CookOnTheFly && !Params.SkipCookOnTheFly) // only stage the UFS files if we are not using cook on the fly
				{
					...
					// Stage ICU internationalization data from Engine. The folder name is sourced from the
					// target platform's DataDrivenPlatformInfo.ini 
					var DDPI = UnrealBuildTool.DataDrivenPlatformInfo.GetDataDrivenInfoForPlatform(SC.StageTargetPlatform.IniPlatformType);
					var ICUDataVersion = DDPI?.ICUDataVersion ?? "icudt64l";
					...
					if (ShouldCreateBinaryConfig(Params, SC) && !Params.SkipPak)
					{
						...
						if (PlatformGameConfig.TryGetValues("/Script/UnrealEd.ProjectPackagingSettings", "ExtraBinaryConfigWithScalabilityOverride", out IReadOnlyList<string> ScalabilityOverrideValues))
						{
							foreach (string ScalabilityOverride in ScalabilityOverrideValues)
							{
								if(!string.IsNullOrEmpty(ScalabilityOverride))
								{
									FileReference OutputExtraBinary = FileReference.Combine(SC.ProjectRoot, "Intermediate", "Config", TargetPlatformName, String.Format("BinaryConfig{0}.ini", ScalabilityOverride));
									String UnrealPakParamsExtraBinary = String.Format("MakeBinaryConfig -Project=\"{0}\" -Platform={1} -OutputFile=\"{2}\" -StagedPluginsFile=\"{3}\" -ScalabilityIniPlatformOverride={4}",
										SC.RawProjectPath, ConfigHierarchy.GetIniPlatformName(ThisPlatform.IniPlatformType), OutputExtraBinary.FullName, ConfigCacheInputFile.FullName, ScalabilityOverride);
									if (!string.IsNullOrEmpty(SC.CustomConfig))
									{
										UnrealPakParamsExtraBinary += $" -CustomConfig={SC.CustomConfig}";
									}

									RunUnrealPak(Params.RawProjectPath, UnrealPakParamsExtraBinary, "MakeBinaryConfig");
									SC.StageFile(StagedFileType.UFS, OutputExtraBinary, StagedFileReference.Combine(SC.RelativeProjectRootForStage, "Config", OutputExtraBinary.GetFileName()));
								}
							}
						}
					}
		...
		static Nullable<bool> ShouldStageConfigFile(DeploymentContext SC, DirectoryReference ConfigDir, FileReference ConfigFile, string PlatformExtensionName)
		{
			...
			if (DirectoryIdx == -1)
			{
				...
				// special dynamic layer file
				const string PluginOverridePrefix = "pluginoverride";
				if (NormalizedPath.StartsWith(PluginOverridePrefix))
				{
					return true;
				}
		...
		private struct PakFileRules
		{
			...
			// Weather this rule should generate a completely new chunk 
			public bool bGenerateNewChunk;
			
			// Filter assets out of GeneratedNewChunk which do not pass the AllowedChunkFilter (only used when bGenerateNewChunk = true)
			public List<Regex> NewChunkAllowedChunkFilters;

			// Generated chunk name (only valid for bGenerateNewChunk = true)
			public string GeneratedChunkName;
		...
		/// <summary>
		/// Returns true if the given pak chunk name matches the required "pakchunk<N>" format.
		/// </summary>
		private static bool IsValidPakChunkName(string ChunkName)
		{
			return new Regex("^pakchunk\\d+", RegexOptions.IgnoreCase).IsMatch(ChunkName);
		}
		...
		private static List<PakFileRules> GetPakFileRules(ProjectParams Params, DeploymentContext SC)
		{
			...
			foreach (string SectionName in PakRulesConfig.SectionNames)
			{
				...
				PakRulesConfig.TryGetValue(SectionName, "bGenerateNewChunk", out PakRules.bGenerateNewChunk);
				PakRulesConfig.TryGetValue(SectionName, "GeneratedChunkName", out PakRules.GeneratedChunkName);
				if (!string.IsNullOrEmpty(PakRules.GeneratedChunkName) && !IsValidPakChunkName(PakRules.GeneratedChunkName))
				{
					Logger.LogError("Error in PakFileRules {Arg0}, GeneratedChunkName '{Arg1}' does not start with 'pakchunk<N>'.", PakRules.Name, PakRules.GeneratedChunkName);
				}
				...
				if (PakString != null && PakString.Length > 0)
				{
					...
					foreach (string OverridePakName in PakRules.OverridePaks)
					{
						if (!IsValidPakChunkName(OverridePakName))
						{
							Logger.LogError("Error in PakFileRules {Arg0}, OverridePaks entry '{Arg1}' does not start with 'pakchunk<N>'.", PakRules.Name, OverridePakName);
						}
					}
				}
				else
				...
				else if (PakRules.OverridePaks == null && !PakRules.bOnDemand && !PakRules.bGenerateNewChunk)
				{
				...
				IReadOnlyList<string> NewChunkAllowedChunkFilters;
				if (PakRulesConfig.TryGetValues(SectionName, "NewChunkAllowedChunkFilters", out NewChunkAllowedChunkFilters))
				{
					PakRules.NewChunkAllowedChunkFilters = new List<Regex>();
					foreach (string Filter in NewChunkAllowedChunkFilters)
					{
						try
						{
							PakRules.NewChunkAllowedChunkFilters.Add(new Regex(Filter));
						}
						catch (System.Text.RegularExpressions.RegexParseException Ex)
						{
							Logger.LogError("Failed to parse allowed chunk filter, reason '{Arg1}'", Ex.Message);
			...
			// If there are no pre-existing pak rules for an 'OnDemand' container we can check the OnDemandSettings to see if we should create some
			if (RulesList.FindIndex(x => x.bOnDemand == true) == -1)
			{
				if (Params.OnDemandSettings.ShouldCreateDefaultOnDemandPakRule())
				{
					RulesList.AddRange(CreateIoStoreOnDemandPakRules());
				}
				else if (Params.OnDemandSettings.ShouldCreateAssetTypeOnDemandPakRule())
				{
					Dictionary<string, List<string>> EnabledAssets = Params.OnDemandSettings.GetEnabledAssetList();
					RulesList.AddRange(CreateAssetTypeOnDemandPakRules(EnabledAssets));
				}
			}
		...
		private static List<PakFileRules> CreateAssetTypeOnDemandPakRules(Dictionary<string, List<string>> AssetTypes)
		{
			List<PakFileRules> RulesList = new List<PakFileRules>();
			...
			PakRules.Name = "BulkDataOnDemand";
			PakRules.bOnDemand = true;
			PakRules.bUseExactFilePathMatch = true;
			PakRules.ExactFileMatches = new HashSet<string>();
			foreach (KeyValuePair<string, List<string>> EnabledAsset in AssetTypes)
			{
				foreach(string Package in EnabledAsset.Value)
				{
					//PakRules.ExactFileMatches.Add(Package + ".uptnl");
					PakRules.ExactFileMatches.Add(Package + ".ubulk");
				}
			}
		...
		private static void CreatePakUsingStagingManifest(ProjectParams Params, DeploymentContext SC)
		{
			...
			Parallel.ForEach(UnrealPakResponseFile, StagingFile =>
			{
				...
				if (Params.OnDemandSettings.ApplyRulesForStagingManifest() && MatchingRule.HasValue && MatchingRule.Value.bOnDemand)
				{
		...
		private static bool CopyExistingContainer(FileReference InUtocFile, FileReference OutputLocation)
		{
			...
			FileReference UMetaFile = InUtocFile.ChangeExtension(".umeta");
			if (FileReference.Exists(UMetaFile))
			{
				if (!InternalUtils.SafeCopyFile(UMetaFile.FullName, OutputLocation.ChangeExtension(".umeta").FullName))
				{
					Logger.LogWarning("Failed to copy umeta {Src} to {Dst}", UMetaFile.FullName, OutputLocation.ChangeExtension(".umeta").FullName);
				}
			}
		...
		private static void CreatePaks(ProjectParams Params, DeploymentContext SC, List<CreatePakParams> PakParamsList, EncryptionAndSigning.CryptoSettings CryptoSettings)
		{
			...
			bool bGenerateContainerMetadata = false; // do we want container metadata to be generated (can be used to reverse lookup package names)
			PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bGenerateContainerMetadata", out bGenerateContainerMetadata);
			if (bGenerateContainerMetadata) 
			{
				CommonAdditionalArgs += " -ContainerMeta";
			}
			...
			foreach (CreatePakParams PakParams in PakParamsList)
			{
				...
				if (!PakParams.bOnDemand && !PakParams.bStageLoose && SC.StageTargetPlatform != SC.CookSourcePlatform && !Params.IgnorePaksFromDifferentCookSource)
				{
					...
					if (FileExists_NoExceptions(SourceOutputLocation.FullName))
					{
						...
						{
							Logger.LogInformation("Copying source pak from {SourceOutputLocation} to {OutputLocation} instead of creating new pak", SourceOutputLocation.FullName, OutputLocation.FullName);
							...
							if (FileReference.Exists(InSigFile))
							{
								...
								Logger.LogInformation("Copying pak sig from {InSigFile} to {OutSigFile}", InSigFile.FullName, OutSigFile.FullName);

								if (!InternalUtils.SafeCopyFile(InSigFile.FullName, OutSigFile.FullName))
								{
									Logger.LogInformation("Failed to copy pak sig {InSigFile} to {InSigFile2}, creating new pak", InSigFile.FullName, InSigFile.FullName);
									bCopiedExistingPak = false;
								}
							}
					...
					if (!bCopiedExistingPak)
					{
						Logger.LogInformation("Failed to copy source pak from {SourceOutputLocation} to {OutputLocation}, creating new pak", SourceOutputLocation.FullName, OutputLocation.FullName);
					}
					...
					if (bCopiedExistingPak && !bCopiedExistingGlobalUtoc)
					{
						...
						// Also look for the optional global on-demand TOC
						string GlobalOnDemandFileName = "global.uondemandtoc";
						FileReference InFile = FileReference.Combine(SourceOutputLocation.Directory, GlobalOnDemandFileName);
						if (FileExists_NoExceptions(InFile.FullName))
						{
							FileReference OutFile = FileReference.Combine(OutputLocation.Directory, GlobalOnDemandFileName);
							Logger.LogInformation("Copying global on-demand TOC from {InFile} to {OutFile}", InFile, OutFile);
							if (!InternalUtils.SafeCopyFile(InFile.FullName, OutFile.FullName))
							{
								Logger.LogInformation("Failed to copy global on-demand TOC from {InFile} to {OutFile}", InFile, OutFile);
							}
						}
					}
				}
			...
			if (IoStoreCommands.Count > 0)
			{
				...
				{
					if (CompressionMinPercentSaved > 0)
					{
				...
				bool bDeduplicateChunks = false;
				if (PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bDeduplicateChunks", out bDeduplicateChunks) && bDeduplicateChunks)
				{
					AdditionalArgs += " -DeduplicateChunks";
				}

				bool bPlaceShadersAtEnd = false;
				if (PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bPlaceShadersAtEnd", out bPlaceShadersAtEnd) && bPlaceShadersAtEnd)
				{
					AdditionalArgs += " -PlaceShadersAtEnd";
				}

				bool bUsePatchInPlaceLayout = false;
				if (PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bUsePatchInPlaceLayout", out bUsePatchInPlaceLayout) && bUsePatchInPlaceLayout)
				{
					AdditionalArgs += " -UsePatchInPlaceLayout";
					
					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceMaximumTotalGrowthMB", out int PatchInPlaceMaximumTotalGrowthMB))
					{
						AdditionalArgs += $" -patchInPlaceMaximumTotalGrowth={PatchInPlaceMaximumTotalGrowthMB}MB";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceMaximumPerContainerGrowthMB", out int PatchInPlaceMaximumPerContainerGrowthMB))
					{
						AdditionalArgs += $" -patchInPlaceMaximumPerContainerGrowth={PatchInPlaceMaximumPerContainerGrowthMB}MB";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceParetoFrontCoarseSamples", out int PatchInPlaceParetoFrontCoarseSamples))
					{
						AdditionalArgs += $" -patchInPlaceParetoFrontCoarseSamples={PatchInPlaceParetoFrontCoarseSamples}";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceParetoFrontFineSamples", out int PatchInPlaceParetoFrontFineSamples))
					{
						AdditionalArgs += $" -patchInPlaceParetoFrontFineSamples={PatchInPlaceParetoFrontFineSamples}";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceReadGranularity", out int PatchInPlaceReadGranularity))
					{
						AdditionalArgs += $" -patchInPlaceReadGranularity={PatchInPlaceReadGranularity}";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceMaxChainSize", out int PatchInPlaceMaxChainSize))
					{
						AdditionalArgs += $" -patchInPlaceMaxChainSize={PatchInPlaceMaxChainSize}";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceMaxChainLength", out int PatchInPlaceMaxChainLength))
					{
						AdditionalArgs += $" -patchInPlaceMaxChainLength={PatchInPlaceMaxChainLength}";
					}

					if (PlatformGameConfig.GetInt32("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceParetoFrontHeuristic", out int PatchInPlaceParetoFrontHeuristic))
					{
						AdditionalArgs += $" -patchInPlaceParetoFrontHeuristic={PatchInPlaceParetoFrontHeuristic}";
					}

					if (PlatformGameConfig.GetString("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceReferenceContainerPathOverride", out string PatchInPlaceReferenceContainerPathOverride) && PatchInPlaceReferenceContainerPathOverride.Length != 0)
					{
						AdditionalArgs += $" -patchInPlaceReferenceContainerPathOverride=\"{PatchInPlaceReferenceContainerPathOverride}\"";

						if (PlatformGameConfig.GetString("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceReferenceContainerCryptoOverride", out string PatchInPlaceReferenceContainerCryptoOverride) && PatchInPlaceReferenceContainerCryptoOverride.Length != 0)
						{
							AdditionalArgs += $" -patchInPlaceReferenceContainerCryptoOverride=\"{PatchInPlaceReferenceContainerCryptoOverride}\"";
						}

						if (PlatformGameConfig.GetString("/Script/UnrealEd.ProjectPackagingSettings", "PatchInPlaceReferenceAdditionalContainersPath", out string PatchInPlaceReferenceAdditionalContainersPath) && PatchInPlaceReferenceAdditionalContainersPath.Length != 0)
						{
							AdditionalArgs += $" -patchInPlaceReferenceAdditionalContainersPath=\"{PatchInPlaceReferenceAdditionalContainersPath}\"";
						}
					}
				}
				...
				List<string> PerContainerAlphaSortClusterPackageLists;
				if (PlatformGameConfig.GetArray("/Script/UnrealEd.ProjectPackagingSettings", "PerContainerAlphaSortClusterPackageLists", out PerContainerAlphaSortClusterPackageLists))
				{
					AdditionalArgs += string.Format(" -PerContainerAlphaSortClusterPackageLists={0}", string.Join(',', PerContainerAlphaSortClusterPackageLists));
				}

				List<string> PerContainerFallbackOrdering;
				if (PlatformGameConfig.GetArray("/Script/UnrealEd.ProjectPackagingSettings", "PerContainerFallbackOrdering", out PerContainerFallbackOrdering))
				{
					AdditionalArgs += string.Format(" -PerContainerFallbackOrdering={0}", string.Join(',', PerContainerFallbackOrdering));
				}

				AdditionalArgs += $" -platform={ConfigHierarchy.GetIniPlatformName(SC.StageTargetPlatform.IniPlatformType)}";

				bool bGenerateIoContainerMetadata = false; // do we want container metadata to be generated (can be used to reverse lookup package names)
				PlatformGameConfig.GetBool("/Script/UnrealEd.ProjectPackagingSettings", "bGenerateContainerMetadata", out bGenerateIoContainerMetadata );
				if (bGenerateIoContainerMetadata )
				{
					AdditionalArgs += " -ContainerMeta";
				}
			...
			if (!string.IsNullOrWhiteSpace(Params.Upload))
			{
				...
				if (Params.Upload.ToLower() == "localzen")
				{
					...
					UploadArgs.Append(string.Format(" -GlobalContainerPath \"{0}\"", PakPath));
					...
					UploadArgs.Append(" -StreamOnDemand");
		...
		private static void RunIoStore(ProjectParams Params, DeploymentContext SC, string CommandsFileName, List<OrderFile> OrderFiles, string AdditionalArgs)
		{
			...
			string CommandletParams = $"{Params.RawProjectPath} ";
			if (Params.HasDLCName)
			{
		...
		private static byte[] GetChunkFromPackageStore(DeploymentContext SC, string Filename)
		{
			if (SC.PackageStoreData != null && SC.PackageStoreData.ZenServerStore != null && SC.PackageStoreData.ZenCookedFiles != null)
			{
				...
				if (ZenFile != null)
				{
					...
					{
						throw new AutomationException(String.Format("Failed reading chunk {0} for '{1}' from Zen with status code {2} ({3}). Ensure that cooking was successful.",
							Request.RequestUri.PathAndQuery, Filename, (int)HttpGetResult.StatusCode, HttpGetResult.StatusCode));
					}
...
#nullable enable
		/// <summary>
		/// Build <see cref="ChunkDefinition"/>s from the <c>ChunkAssignment</c> op cached on
		/// <c>SC.PackageStoreData.ChunkAssignment</c> by <see cref="ReadZenCookedFilesFromZenServer"/>.
		/// Returns <c>null</c> if the op is absent (cook didn't emit it, or the oplog wasn't
		/// read with cooked-files loading enabled).
		/// </summary>
		private static List<ChunkDefinition>? TryReadChunkAssignmentOp(
			DeploymentContext SC,
			EncryptionAndSigning.CryptoSettings PakCryptoSettings)
		{
			try
			{
				ChunkAssignmentOp? op = SC.PackageStoreData?.ChunkAssignment as ChunkAssignmentOp;
				if (op == null)
				{
					Logger.LogInformation("ChunkAssignment op not cached on PackageStoreData");
					return null;
				}

				var result = new List<ChunkDefinition>(op.Assignments.ContainerChunks.Count);
				foreach (ContainerChunk chunk in op.Assignments.ContainerChunks)
				{
					var cd = new ChunkDefinition(chunk.Name ?? $"pakchunk{chunk.ContainerChunkId}")
					{
						Manifest = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase),
					};

					if (!string.IsNullOrEmpty(chunk.EncryptionKeyGuid))
					{
						cd.RequestedEncryptionKeyGuid = chunk.EncryptionKeyGuid;
						if (PakCryptoSettings.SecondaryEncryptionKeys != null)
						{
							...
							{
								if (string.Compare(Key.Guid, chunk.EncryptionKeyGuid, StringComparison.OrdinalIgnoreCase) == 0)
								{
									cd.EncryptionKeyGuid = chunk.EncryptionKeyGuid;
									break;
								}
							}
						}
					...
					{
						// FilePath/UfsPath is resolved post-parse via the IoChunkId -> packagedata
						// /bulkdata filename lookup performed by ChunkAssignmentOp.ResolveFilenames.
						// Loose files (no IoChunkId) already carry an explicit FilePath.
						string? relativePath = file.FilePath ?? file.UfsPath;
						if (!string.IsNullOrEmpty(relativePath))
						{
							string absolutePath = FileReference.Combine(SC.PlatformCookDir, relativePath).FullName;
							cd.Manifest.Add(absolutePath);
						}
					}

					Logger.LogInformation("ReadChunkAssignmentOp: chunk ({Name}) has {Count} files.",
						cd.ChunkName, cd.Manifest.Count);
					result.Add(cd);
				}
				return result;
			}
			...
			{
				// FileReference.Combine rejects invalid path characters with ArgumentException
				// (and its ArgumentNullException subclass for null inputs). A malformed
				// cooked_path in the op shouldn't blow up staging
				Logger.LogWarning("ChunkAssignment op contained an invalid path; Reason: {Msg}", ex.Message);
				return null;
			}
		}
#nullable disable
		...
		private static void CreatePaksUsingChunkManifests(ProjectParams Params, DeploymentContext SC)
		{
...

			{
				...
				List<ChunkDefinition> opChunkDefinitions = TryReadChunkAssignmentOp(SC, PakCryptoSettings);
				if (opChunkDefinitions != null)
				{
					// ---- Op path (preferred) ----
					Logger.LogInformation("Reading chunk assignments from oplog op 'ChunkAssignment'.");
					ChunkDefinitions = opChunkDefinitions;
				}
				else
				{
					// ---- Legacy .txt path ----
				...
				foreach (ChunkDefinition Chunk in ChunkDefinitions)
				{
					...
					{
						string FixedName = FileName.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);

						List<ChunkDefinition> Chunks;
						if (!FileNameToChunks.TryGetValue(FixedName, out Chunks))
						{
							Chunks = new List<ChunkDefinition>();
							FileNameToChunks.Add(FixedName, Chunks);
						}
						Chunks.Add(Chunk);
					}
				}
				...
				Parallel.ForEach(StagingManifestResponseFile, StagingFile =>
				{
					...
					foreach (ChunkDefinition Chunk in PakList)
					{
						...
						if (MatchingRule.HasValue && MatchingRule.Value.bGenerateNewChunk)
						{
							if (!string.IsNullOrEmpty(MatchingRule.Value.GeneratedChunkName))
							{
								bool bMatchesFilter = true;
								if (MatchingRule.Value.NewChunkAllowedChunkFilters != null)
								{
									bMatchesFilter = MatchingRule.Value.NewChunkAllowedChunkFilters.Any(Filter => Filter.IsMatch(TargetChunk.ChunkName));
								}
								if (bMatchesFilter)
								{
									string ChunkName = MatchingRule.Value.GeneratedChunkName;
							...
							{
								Logger.LogError("Unable to generate new chunk for rule {Arg0}. GeneratedChunkName is not set.", MatchingRule.Value.Name);
							}
			...
			else
			{
				...
				else if (SC.PackageStoreData?.ChunkAssignment == null)
				{
					// throw when not using the json manifest.
		...
		private static bool DoesChunkPakManifestExist(ProjectParams Params, DeploymentContext SC)
		{
			...
			if (SC.PackageStoreData?.ChunkAssignment != null)
			{
				return true;
			}
			...
			foreach (var StagePlatform in PlatformsToStage)
			{
				...
				foreach (var TargetAndConfig in TargetAndConfigPairs)
				{
					...
					foreach (UnrealTargetPlatform ReceiptPlatform in SubPlatformsToStage)
					{
						...
						// we need to synthesize the receipt if we are using an externally-created staged build
						if (Params.PreStaged)
						{
							SingleTargetProperties TargetProperties = Params.ProjectTargets.FirstOrDefault( T => T.TargetName == Target );
							if (TargetProperties == null)
							{
								throw new AutomationException("Cannot find pre-staged target properties for {0}", Target);
							}

							FileReference DefaultReceiptPath = TargetReceipt.GetDefaultPath(Params.RawProjectPath.Directory, Target, ReceiptPlatform, Config, Architecture);
							FileReference DefaultExeName = DefaultReceiptPath.ChangeExtension( Platform.GetExeExtension(ReceiptPlatform) );
							TargetReceipt MockReceipt = new(
								Params.RawProjectPath,
								Target,
								TargetProperties.Rules.Type,
								ReceiptPlatform,
								Config,
								new BuildVersion(),
								Architecture,
								...
								TargetBuildEnvironment.UniqueIfNeeded);

							MockReceipt.Launch = DefaultExeName;
							MockReceipt.LaunchCmd = DefaultExeName;
							MockReceipt.AddBuildProduct(DefaultExeName, BuildProductType.Executable);

							if (!string.IsNullOrEmpty(TargetProperties.Rules.CustomConfig))
							{
								MockReceipt.AdditionalProperties.Add(new ReceiptProperty("CustomConfig", TargetProperties.Rules.CustomConfig));
							}

							TargetsToStage.Add(new StageTarget { Receipt = MockReceipt, RequireFilesExist = true });
							continue;
						}
		...
		/// <summary>
		/// Runs OplogChunkAssigner (if requested) and any additional commandlets before staging manifests are created.
		/// Both read from cook output rather than staging output, so they can safely run before CreateStagingManifest.
		/// </summary>
		private static void RunPreStagingCommandlets(ProjectParams Params, DeploymentContext DeployContext)
		{
			// Only run if we are intending on running the pak step.
			if (Params.RunChunkAssigner)
			{
				LoadPackageStoreData(Params, DeployContext);
				if (DeployContext.PackageStoreData != null && DeployContext.PackageStoreData.ZenServerStore != null)
				{
					Logger.LogInformation("Running OplogChunkAssigner for {CookPlatform}...", DeployContext.CookPlatform);
					OplogChunkAssigner.RunFromStagingContext(Params, DeployContext);
				}
				else if (!Params.SkipPak)
				{
					Logger.LogError("Failed to find PackageStoreData and PackageStoreData.ZenServerStore while attempting to run chunk assigner.");
				}
			}
		}
		...
		{
			if ((ShouldCreatePak(Params) || (Params.Stage && !Params.SkipStage)) && !Params.PreStaged)
			{
				...
				if (!Params.NoClient)
				{
					...
					foreach (var SC in DeployContextList)
					{
						...
						RunPreStagingCommandlets(Params, SC);
