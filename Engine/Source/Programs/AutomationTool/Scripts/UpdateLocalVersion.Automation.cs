...
public class UpdateLocalVersion : BuildCommand
{
	public override void ExecuteBuild()
	{
		...
		int? CompatibleChangelistOverride = ParseParamNullableInt("compatiblecl");
  		// If override is not provided, try to read compatible CL value from p4 if enabled
		if (!CompatibleChangelistOverride.HasValue && P4Enabled)
		{
			CompatibleChangelistOverride = P4Env.CodeChangelist;
		}
