		...
		if (!Preview)
		{
			...
			try
			{
				...
				using (ZipArchive Zip = ZipFile.OpenRead(TmpFile))
				{
					...
					{
						string OutputFileName = Path.Combine(Unreal.RootDirectory.FullName, Entry.FullName);

						if (Entry.FullName.Replace('\\', '/').StartsWith(UATDirectory, StringComparison.OrdinalIgnoreCase))
						{
