	...
	public partial class Project : CommandUtils
	{
		...
		private static IProcessResult RunDedicatedServer(ProjectParams Params, string ServerLogFile, string AdditionalCommandLine)
		{
			...
			if (bCooked)
			{
				...
				// account for app bundles placed outside Binaries/Mac on macOS
				if(ServerPlatformDesc.Type == UnrealTargetPlatform.Mac)
				{
					DirectoryReference BaseStageDir = new DirectoryReference(Params.BaseStageDirectory);
					DirectoryReference StageSubDir = new FileReference(ServerApp).Directory;

					while (StageSubDir.IsUnderDirectory(BaseStageDir) && StageSubDir.ParentDirectory != BaseStageDir)
					{
					    StageSubDir = StageSubDir.ParentDirectory;
					}
					ServerApp = FileReference.Combine(StageSubDir, 
													  $"{Exes[0].GetFileNameWithoutExtension()}.app/Contents/MacOS/{Exes[0].GetFileNameWithoutExtension()}").FullName;
				}
			}
