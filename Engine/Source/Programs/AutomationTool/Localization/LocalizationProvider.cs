...
namespace EpicGames.Localization
{
	public class ProjectImportExportInfo
	{
		...
		public void CalculateSplitPlatformNames(string RootWorkingDirectory)
		{
			...
			List<string> NewSplitPlatformNames = new List<string>();
			if (bUseCultureDirectory)
			{
				DirectoryReference PlatformSourceDirectory = new DirectoryReference(CommandUtils.CombinePaths(RootWorkingDirectory, DestinationPath, PlatformLocalizationFolderName));
	...
	public class ProjectInfo
	{
		...
		public List<string> GetConfigFilesToRun(List<string> RequestedLocalizationStepNames)
		{
			...
			foreach (ProjectStepInfo LocalizationStep in LocalizationSteps)
			{
		...
		public static LocalizationProvider GetLocalizationProvider(string InLocalizationProviderId, LocalizationProvider.LocalizationProviderArgs InLocalizationProviderArgs)
		{
			...
			if (CachedLocalizationProviderTypes == null)
			{
				...
				foreach (Assembly Dll in ScriptManager.AllScriptAssemblies)
				{
					...
					foreach (Type PotentialLocalizationNodeType in AllTypes)
					{
						if (PotentialLocalizationNodeType != typeof(LocalizationProvider) && !PotentialLocalizationNodeType.IsAbstract && !PotentialLocalizationNodeType.IsInterface && typeof(LocalizationProvider).IsAssignableFrom(PotentialLocalizationNodeType))
						{
							...
							MethodInfo Method = PotentialLocalizationNodeType.GetMethod("StaticGetLocalizationProviderId");
							if (Method != null)
							{
								string LocalizationProviderId = null;

								try
								{
									LocalizationProviderId = Method.Invoke(null, null) as string;
								}
								catch(Exception ex)
								{
									Logger.LogWarning("Type '{Name}' threw when calling its StaticGetLocalizationProviderId method. Exception: {Message}. (Assembly={Assembly}, AssemblyQualifiedName={AssemblyQualifiedName})", PotentialLocalizationNodeType.FullName, ex.Message, PotentialLocalizationNodeType.Assembly.FullName, PotentialLocalizationNodeType.AssemblyQualifiedName);
								}

								if (!String.IsNullOrEmpty(LocalizationProviderId))
								{
									if(CachedLocalizationProviderTypes.TryAdd(LocalizationProviderId, PotentialLocalizationNodeType))
									{
										Logger.LogInformation("Registered '{Name}' in the localization provider cache using id '{Id}'. (Assembly={Assembly}, AssemblyQualifiedName={AssemblyQualifiedName})", PotentialLocalizationNodeType.FullName, LocalizationProviderId, PotentialLocalizationNodeType.Assembly.FullName, PotentialLocalizationNodeType.AssemblyQualifiedName);
									}
									else
									{
										Logger.LogWarning("Could not add '{Name}' in the localization provider cache using id '{Id}'. (Assembly={Assembly}, AssemblyQualifiedName={AssemblyQualifiedName})", PotentialLocalizationNodeType.FullName, LocalizationProviderId, PotentialLocalizationNodeType.Assembly.FullName, PotentialLocalizationNodeType.AssemblyQualifiedName);
									}
								}
								else
								{
									Logger.LogWarning("{Name}.StaticGetLocalizationProviderId method returned an empty ID. (Assembly={Assembly}, AssemblyQualifiedName={AssemblyQualifiedName})", PotentialLocalizationNodeType.FullName, PotentialLocalizationNodeType.Assembly.FullName, PotentialLocalizationNodeType.AssemblyQualifiedName);
								}
							}
							else
							{
								Logger.LogWarning("Type '{Name}' derives from LocalizationProvider but is missing its StaticGetLocalizationProviderId method. (Assembly={Assembly}, AssemblyQualifiedName={AssemblyQualifiedName})", PotentialLocalizationNodeType.FullName, PotentialLocalizationNodeType.Assembly.FullName, PotentialLocalizationNodeType.AssemblyQualifiedName);
							}
						}
					}
				}
