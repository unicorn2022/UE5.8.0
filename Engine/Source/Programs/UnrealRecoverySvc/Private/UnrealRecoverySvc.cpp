...
INT32_MAIN_INT32_ARGC_TCHAR_ARGV()
{
	...
	ServerLoopInitArgs.GetServerConfigFunc = [&EditorProcessId]() -> const UConcertServerConfig*
	{
		...
		if (EditorProcessId)
		{
			UE_LOGF(LogSyncServer, Display, "Watching Editor process %d", EditorProcessId);
		}
		else
		{
			UE_LOGF(LogSyncServer, Error, "Invalid -EditorPID argument. Cannot continue!");
			return nullptr;
		}
	...
	FTSTicker::GetCoreTicker().AddTicker(TEXT("CheckEditorHealth"), 1.0f, [&EditorProcessId](float)
	{
		...
		{
			UE_LOGF(LogSyncServer, Warning, "Editor process %d lost! Requesting exit.", EditorProcessId);
