...
FWindowsCpuUtilizationMonitor::FWindowsCpuUtilizationMonitor()
{
	...
	{
		UE_LOGF(LogCpuUtilizationMonitor, Warning, "PdhOpenQuery failed. Error code: %d", PdhStatus);
		return;
	}
	...
	for (int32 CoreIdx = 0; CoreIdx < NumCores; ++CoreIdx)
	{
		...
		if (PdhStatus != ERROR_SUCCESS)
		{
			UE_LOGF(LogCpuUtilizationMonitor, Warning, "PdhAddEnglishCounter failed. Error code: %d", PdhStatus);
			return;
		}
	}
	...
	if (PdhStatus != ERROR_SUCCESS)
	{
		UE_LOGF(LogCpuUtilizationMonitor, Warning, "PdhCollectQueryData failed. Error code: %d", PdhStatus);
		return;
	}
...
FWindowsCpuUtilizationMonitor::~FWindowsCpuUtilizationMonitor()
{
	if (QueryHandle != INVALID_HANDLE_VALUE)
	{
		...
		if (PdhStatus != ERROR_SUCCESS)
		{
			UE_LOGF(LogCpuUtilizationMonitor, Warning, "PdhCloseQuery failed. Error code: %d", PdhStatus);
		}
	}
}
...
bool FWindowsCpuUtilizationMonitor::QueryUpdatedUtilization()
{
	...
	for (int32 HandleIdx = 0; HandleIdx < NumHandles; ++HandleIdx)
	{
		...
		if (PdhStatus != ERROR_SUCCESS)
		{
			UE_LOGF(LogCpuUtilizationMonitor, Warning, "PdhGetFormattedCounterValue failed. Error code: %d", PdhStatus);
			return false;
		}
