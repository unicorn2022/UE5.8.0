...
FScopedNvApi::FScopedNvApi()
{
	...
	else
	{
		...
		else if (!Statics::bErrorLoggedOnce)
		{
			...
			UE_LOGF(LogSwitchboard, Error, "NvAPI_Initialize failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
		}
	}

	if (Statics::InstanceCount == 1)
	{
		UE_LOGF(LogSwitchboard, VeryVerbose, "FScopedNvApi InstanceCount incremented to 1");
	}
...
FScopedNvApi::~FScopedNvApi()
{
	...
	{
		UE_LOGF(LogSwitchboard, VeryVerbose, "FScopedNvApi InstanceCount decremented to 0");
	...
	if (bIsNvApiInitialized)
	{
		...
		else
		{
			...
			NvAPI_ShortString ErrorString;
			NvAPI_GetErrorMessage(Result, ErrorString);
			UE_LOGF(LogSwitchboard, Error, "NvAPI_Unload unexpectedly failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
		}
	}
	...
	for (NvU32 GSyncIdx = 0; GSyncIdx < GSyncCount; GSyncIdx++)
	{
		...
		// gather info first with null data pointers, just to get the count and subsequently allocate necessary memory.
		{
			...
			if (Result != NVAPI_OK)
			{
				NvAPI_ShortString ErrorString;
				NvAPI_GetErrorMessage(Result, ErrorString);
				UE_LOGF(LogSwitchboard, Warning, "NvAPI_GSync_GetTopology failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
				continue;
			}
		}
		...
		// Sync Status Parameters
		{
			...
			if (Result != NVAPI_OK)
			{
				NvAPI_ShortString ErrorString;
				NvAPI_GetErrorMessage(Result, ErrorString);
				UE_LOGF(LogSwitchboard, Warning, "NvAPI_GSync_GetStatusParameters failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
				continue;
			}
		...
		// Sync Control Parameters
		{
			...
			if (Result != NVAPI_OK)
			{
				NvAPI_ShortString ErrorString;
				NvAPI_GetErrorMessage(Result, ErrorString);
				UE_LOGF(LogSwitchboard, Warning, "NvAPI_GSync_GetControlParameters failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
				continue;
			}
...
void FScopedNvApi::FillOutDriverVersion(FSyncStatus& SyncStatus) const
{
	...
	if (Result != NVAPI_OK)
	{
		NvAPI_ShortString ErrorString;
		NvAPI_GetErrorMessage(Result, ErrorString);
		UE_LOGF(LogSwitchboard, Warning, "NvAPI_SYS_GetDriverAndBranchVersion failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
		return;
	}
...
void FScopedNvApi::FillOutPhysicalGpuStats(FSyncStatus & SyncStatus, bool bGetUtilizations, bool bGetClocks, bool bGetTemperatures)
{
	...
	for (NvU32 PhysicalGpuIdx = 0; PhysicalGpuIdx < PhysicalGpuCount; ++PhysicalGpuIdx)
	{
		...
		else if (NvResult == NVAPI_HANDLE_INVALIDATED)
		{
			...
			UE_LOGF(LogSwitchboard, Warning, "Physical GPU handles were invalidated, recaching them.");
			CacheGpuHandles();
			return;
		}
		else
		{
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_GPU_GetDynamicPstatesInfoEx failed. Error code: %d", NvResult);
		}
		...
		{
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_GPU_GetAllClockFrequencies failed. Error code: %d", NvResult);
		}
		...
		{
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_GPU_GetThermalSettings failed. Error code: %d", NvResult);
		}
	}
}
...
bool FScopedNvApi::CacheGpuHandles()
{
	...
	if (NvResult != NVAPI_OK)
	{
		...
		UE_LOGF(LogSwitchboard, Warning, "NvAPI_EnumPhysicalGPUs failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
		return false;
	}
	...
	Algo::Sort(Statics::PhysicalGpuHandles, [](const NvPhysicalGpuHandle& Lhs, const NvPhysicalGpuHandle& Rhs) -> bool {
		...
		if (LhsBusResult != NVAPI_OK || RhsBusResult != NVAPI_OK)
		{
			...
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_GPU_GetBusId failed. Errors: %ls, %ls", ANSI_TO_TCHAR(LhsErrorString), ANSI_TO_TCHAR(RhsErrorString));
			return false;
		}
		...
		if (LhsSlotResult != NVAPI_OK || RhsSlotResult != NVAPI_OK)
		{
			...
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_GPU_GetBusSlotId failed. Errors: %ls, %ls", ANSI_TO_TCHAR(LhsErrorString), ANSI_TO_TCHAR(RhsErrorString));
			return false;
		}
...
void FScopedNvApi::FillOutMosaicTopologies(TArray<FMosaicTopo>& MosaicTopos)
{
	...
	// count how many grids
	{
		...
		if (Result != NVAPI_OK)
		{
			NvAPI_ShortString ErrorString;
			NvAPI_GetErrorMessage(Result, ErrorString);
			UE_LOGF(LogSwitchboard, Warning, "NvAPI_Mosaic_EnumDisplayGrids failed. Error: %ls", ANSI_TO_TCHAR(ErrorString));
			return;
		}
	}
