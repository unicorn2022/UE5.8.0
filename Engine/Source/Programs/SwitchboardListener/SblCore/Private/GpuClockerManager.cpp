...
bool FGpuClockerManager::LockGpuClocksForPid(uint32 Pid)
{
	...
	UE_LOGF(LogSwitchboard, Display, "Locking Gpu Clocks");
	...
	if (!bLocked)
	{
		UE_LOGF(LogSwitchboard, Warning, "Unable to lock Gpu clocks as requested in Switchboard settings for process with id %d. Please run SwitchboardListenerHelper "
...
void FGpuClockerManager::PidEnded(uint32 Pid)
{
	...
	{
		UE_LOGF(LogSwitchboard, Display, "Unlocking Gpu Clocks");
...
bool FGpuClockerManager::LockGpuClocksUsingSBLHelper(uint32 Pid)
{
	...
	if (!bSentMessage)
	{
		UE_LOGF(LogSwitchboard, Error, "Failed to send message to SBLHelper server to request gpu clock locking");
	}
