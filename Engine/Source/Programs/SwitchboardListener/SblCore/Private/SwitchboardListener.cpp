...
struct FRunningProcess
{
	...
	bool CreatePipes()
	{
		...
		{
			UE_LOGF(LogSwitchboard, Error, "Could not create pipe to read process output!");
			return false;
		}
		...
		if (!bCombinedStdoutStderr)
		{
			...
			{
				UE_LOGF(LogSwitchboard, Error, "Could not create pipe to read process stderr!");
				return false;
			}
		}
		...
		{
			UE_LOGF(LogSwitchboard, Error, "Could not create pipe to write process input!");
			return false;
		}
	...
	, CachedMosaicTopos(MakeShared<TArray<FMosaicTopo>>())
{
	...
	{
		UE_LOGF(LogSwitchboard, Warning, "Defaulting to: -ip=%ls", *DefaultIp.ToString());
	}

	const uint16 DefaultPort = 2980;
	if (!Options.Port.IsSet())
	{
		UE_LOGF(LogSwitchboard, Warning, "Defaulting to: -port=%u", DefaultPort);
	}
...
bool FSwitchboardListener::ParseIncomingMessage(const FString& InMessage, const FIPv4Endpoint& InEndpoint, const FConnectionRef& Connection)
{
	...
	if (CreateTaskResult.Status == ECreateTaskStatus::Success)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSwitchboard, Display, "Received %ls command", CreateTaskResult.Task->GetCommandName());
			}
	...
	else if (CreateTaskResult.Status == ECreateTaskStatus::Error_Unauthenticated)
	{
		...
		UE_LOGF(LogSwitchboard, Error, "%ls", *UnauthenticatedError);
	...
	else
	{
		...
		UE_LOGF(LogSwitchboard, Error, "%ls", *ParseError);
...
bool FSwitchboardListener::RunScheduledTask(const FSwitchboardTask& InTask)
{
	switch (InTask.Type)
	{
		...
		case ESwitchboardTaskType::StartLiveLogStream:
		{
			const FSwitchboardStartLiveLogStreamTask& Task = static_cast<const FSwitchboardStartLiveLogStreamTask&>(InTask);
			return Task_StartLiveLogStream(Task);
		}
		case ESwitchboardTaskType::StopLiveLogStream:
		{
			const FSwitchboardStopLiveLogStreamTask& Task = static_cast<const FSwitchboardStopLiveLogStreamTask&>(InTask);
			return Task_StopLiveLogStream(Task);
		}
		default:
		{
...
bool FSwitchboardListener::Task_StartProcess(const FSwitchboardStartTask& InRunTask)
{
	...
	if (!NewProcess->Handle.IsValid())
	{
		...
		UE_LOGF(LogSwitchboard, Error, "%ls", *ErrorMsg);
	...
	UE_LOGF(LogSwitchboard, Display, "Started process %d: %ls %ls", NewProcess->PID, *InRunTask.Command, *InRunTask.Arguments);
...
bool FSwitchboardListener::KillProcessNow(FRunningProcess* InProcess, float SoftKillTimeout)
{
	...
	UE_LOGF(LogSwitchboard, Display, "Killing app with PID %d", InProcess->PID);
...
bool FSwitchboardListener::Task_ReceiveFileFromClient(const FSwitchboardReceiveFileFromClientTask& InReceiveFileFromClientTask)
{
	...
	UE_LOGF(LogSwitchboard, Display, "Writing %d bytes to %ls", DecodedFileContent.Num(), *Destination);
...
bool FSwitchboardListener::Task_RedeployListener(const FSwitchboardRedeployListenerTask& InRedeployListenerTask)
{
	...
	if (RedeployStatus.State != FRedeployStatus::EState::None)
	{
		...
		UE_LOGF(LogSwitchboard, Error, "Redeploy listener: %ls", *ErrorMsg);
	...
	UE_LOGF(LogSwitchboard, Display, "Successfully validated new listener checksum");
	...
	UE_LOGF(LogSwitchboard, Display, "Writing new listener to \"%ls\" (%d bytes)", *TempNewListenerPath, DecodedFileContent.Num());
	...
	UE_LOGF(LogSwitchboard, Display, "Redeploy complete; shutting down");
...
void FSwitchboardListener::RollbackRedeploy()
{
	...
	UE_LOGF(LogSwitchboard, Warning, "Rolling back redeploy...");

	if ((uint8)RedeployStatus.State >= (uint8)FRedeployStatus::EState::NewListenerRenamed)
	{
		UE_LOGF(LogSwitchboard, Warning, "Moving new listener back to temporary location");
	...
	{
		UE_LOGF(LogSwitchboard, Warning, "Moving this listener back to original location");
	...
	{
		UE_LOGF(LogSwitchboard, Warning, "Terminating temporary listener");
	...
	{
		UE_LOGF(LogSwitchboard, Warning, "Removing temporary listener");
...
FRunningProcess* FSwitchboardListener::FindOrStartFlipModeMonitorForUUID(const FGuid& UUID)
{
	...
	{
		UE_LOGF(LogSwitchboard, Error, "Could not create pipe to read MonitorProcess output!");
		return nullptr;
	}
	...
	// Log success
	UE_LOGF(LogSwitchboard, Display, "Started FlipMode monitor %d: %ls %ls", MonitorProcess->PID, *MonitorProcess->Path, *Arguments);
...
bool FSwitchboardListener::Task_SetInactiveTimeout(const FSwitchboardSetInactiveTimeoutTask& InTimeoutTask)
{
	...
	{
		UE_LOGF(LogSwitchboard, Display, "Changing client %ls inactive timeout from %.0f to %.0f seconds",
...
bool FSwitchboardListener::Task_FreeListenerBinary(const FSwitchboardFreeListenerBinaryTask& InFreeListenerBinaryTask)
{
	...
	if (!IFileManager::Get().Move(*MovedListenerPath, *OriginalThisExePath, true))
	{
		...
		UE_LOGF(LogSwitchboard, Error, "Free listener binary: %ls", *ErrorMsg);
...
bool FSwitchboardListener::Task_StartLiveLogStream(const FSwitchboardStartLiveLogStreamTask& InStartLiveLogStreamTask)
{
	SWITCHBOARD_TRACE_CPUPROFILER_EVENT_SCOPE(FSwitchboardListener::Task_StartLiveLogStream);

	const FString& LogFilePath = InStartLiveLogStreamTask.LogFilePath;
	const FIPv4Endpoint& ClientEndpoint = InStartLiveLogStreamTask.Recipient;

	// Check if the given log file exists
	if (!FPaths::FileExists(LogFilePath))
	{
		const FString ErrorMsg = FString::Printf(TEXT("Log file does not exist: %s"), *LogFilePath);

		UE_LOGF(LogSwitchboard, Error, "Start live log stream: %ls", *ErrorMsg);

		SendMessage(CreateTaskDeclinedMessage(InStartLiveLogStreamTask, ErrorMsg, {}), ClientEndpoint);

		return false;
	}

	// Check if stream already exists for this file and client
	{
		UE::TUniqueLock<UE::FMutex> Lock(LiveLogStreamsMutex);
		for (const FLiveLogStream& Stream : ActiveLogStreams)
		{
			if (Stream.LogFilePath == LogFilePath && Stream.ClientEndpoint == ClientEndpoint)
			{
				const FString ErrorMsg = TEXT("Live log stream already active for this file and client");
		...
		// Add new stream
		FLiveLogStream NewStream(LogFilePath, ClientEndpoint, InStartLiveLogStreamTask.bTailOnly, InStartLiveLogStreamTask.ProcessUUID, InStartLiveLogStreamTask.StartOffset);

		if (InStartLiveLogStreamTask.ProcessUUID.IsValid())
		{
			UE_LOGF(LogSwitchboard, Log, "Starting live log stream for %ls with associated process UUID: %ls", *LogFilePath, *InStartLiveLogStreamTask.ProcessUUID.ToString());
		}
		else
		{
			UE_LOGF(LogSwitchboard, Log, "Starting live log stream for %ls with no associated process UUID", *LogFilePath);
		}

		// Handle start position based on StartOffset parameter
		if (InStartLiveLogStreamTask.StartOffset >= 0)
		{
			// Use explicit start offset if provided
			NewStream.LastFilePosition = InStartLiveLogStreamTask.StartOffset;
			UE_LOGF(LogSwitchboard, Log, "Starting live log stream from byte offset: %lld", NewStream.LastFilePosition);
		}
		else if (!NewStream.bTailOnly)
		{
			// Start from the beginning
			NewStream.LastFilePosition = 0;
		}
		else
		{
			// Start from end of file for tail-only mode
			TUniquePtr<IFileHandle> FileHandle(FPlatformFileManager::Get().GetPlatformFile().OpenRead(*LogFilePath));
			if (FileHandle)
			{
				NewStream.LastFilePosition = FileHandle->Size();
			}
		}

		ActiveLogStreams.Add(MoveTemp(NewStream));
	}

	// Send success response
	SendMessage(CreateCommandAcceptedMessage(InStartLiveLogStreamTask.TaskID), ClientEndpoint);
	UE_LOGF(LogSwitchboard, Log, "Started live log stream for %ls", *LogFilePath);

	return true;
}

bool FSwitchboardListener::Task_StopLiveLogStream(const FSwitchboardStopLiveLogStreamTask& InStopLiveLogStreamTask)
{
	SWITCHBOARD_TRACE_CPUPROFILER_EVENT_SCOPE(FSwitchboardListener::Task_StopLiveLogStream);

	const FString& LogFilePath = InStopLiveLogStreamTask.LogFilePath;
	const FIPv4Endpoint& ClientEndpoint = InStopLiveLogStreamTask.Recipient;

	bool bStreamFound = false;
	{
		UE::TUniqueLock<UE::FMutex> Lock(LiveLogStreamsMutex);
		for (int32 StreamIdx = ActiveLogStreams.Num() - 1; StreamIdx >= 0; --StreamIdx)
		{
			const FLiveLogStream& Stream = ActiveLogStreams[StreamIdx];
			if (Stream.LogFilePath == LogFilePath && Stream.ClientEndpoint == ClientEndpoint)
			{
				ActiveLogStreams.RemoveAt(StreamIdx);
				bStreamFound = true;
				break;
			}
		}
	...
	if (bStreamFound)
	{
		SendMessage(CreateCommandAcceptedMessage(InStopLiveLogStreamTask.TaskID), ClientEndpoint);
		UE_LOGF(LogSwitchboard, Log, "Stopped live log stream for %ls", *LogFilePath);

		return true;
	}
	else
	{
		const FString ErrorMsg = TEXT("No active live log stream found for this file and client");
		SendMessage(CreateTaskDeclinedMessage(InStopLiveLogStreamTask, ErrorMsg, {}), ClientEndpoint);

		return false;
	}
}

void FSwitchboardListener::HandleLiveLogStreams()
{
	SWITCHBOARD_TRACE_CPUPROFILER_EVENT_SCOPE(FSwitchboardListener::HandleLiveLogStreams);

	const double CurrentTime = FPlatformTime::Seconds();
	constexpr double StreamUpdateInterval = 0.1; // 100 ms between updates

	UE::TUniqueLock<UE::FMutex> Lock(LiveLogStreamsMutex);

	// Collect indices of streams whose associated processes have ended. 
	// If they have, then the stream are removed, effectively stopping them.

	TArray<int32> StreamsToRemove;

	for (int32 StreamIdx = 0; StreamIdx < ActiveLogStreams.Num(); ++StreamIdx)
	{
		FLiveLogStream& Stream = ActiveLogStreams[StreamIdx];

		// Check if the associated process is still running (if we have a valid UUID)

		if (Stream.AssociatedProcessUUID.IsValid())
		{
			bool bProcessStillRunning = false;

			for (const TSharedPtr<FRunningProcess>& Process : RunningProcesses)
			{
				if (Process && Process->UUID == Stream.AssociatedProcessUUID)
				{
					bProcessStillRunning = true;
					break;
				}
			}

			if (!bProcessStillRunning)
			{
				UE_LOGF(LogSwitchboard, Log, "Stopping live log stream for %ls because the associated Unreal process has ended", *Stream.LogFilePath);
				StreamsToRemove.Add(StreamIdx);
				continue;
			}
		}
	}

	// Remove streams whose processes have ended (in reverse order to preserve indices)

	for (int32 StreamIdx = StreamsToRemove.Num() - 1; StreamIdx >= 0; --StreamIdx)
	{
		const FLiveLogStream& StreamToRemove = ActiveLogStreams[StreamsToRemove[StreamIdx]];

		// Notify Switchboard that the stream has terminated
		const FString TerminationMessage = CreateLiveLogStreamTerminatedMessage(StreamToRemove.LogFilePath, TEXT("process terminated"));
		SendMessage(TerminationMessage, StreamToRemove.ClientEndpoint);

		ActiveLogStreams.RemoveAt(StreamsToRemove[StreamIdx]);
	}

	// Process remaining active streams
	for (FLiveLogStream& Stream : ActiveLogStreams)
	{
		// Rate limiting: only update every StreamUpdateInterval seconds per stream
		if (CurrentTime - Stream.LastUpdateTime < StreamUpdateInterval)
		{
			continue;
		}
		...
		if (!PlatformFile.FileExists(*Stream.LogFilePath))
		{
			UE_LOGF(LogSwitchboard, Warning, "Live log file does not exist: %ls", *Stream.LogFilePath);
			continue;
		}

		// Use shared read mode to allow reading while Unreal is writing to the file (otherwise open will fail)

		TUniquePtr<IFileHandle> FileHandle(PlatformFile.OpenRead(*Stream.LogFilePath, true));

		if (!FileHandle)
		{
			UE_LOGF(LogSwitchboard, Warning, "Live log file cannot be opened for reading: %ls", *Stream.LogFilePath);
			continue;
		}

		const int64 CurrentFileSize = FileHandle->Size();
		if (CurrentFileSize <= Stream.LastFilePosition)
		{
			// Nothing to send.
			continue;
		}

		// Calculate how much new content to read (limit to 32KB per update)

		constexpr int64 MaxReadSize = 32768;
		const int64 AvailableBytes = CurrentFileSize - Stream.LastFilePosition;
		const int64 ReadSize = FMath::Min(AvailableBytes, MaxReadSize);

		TArray<uint8> Buffer;
		Buffer.SetNumUninitialized(ReadSize);

		if (FileHandle->Seek(Stream.LastFilePosition) && FileHandle->Read(Buffer.GetData(), ReadSize))
		{
			Stream.LastFilePosition += ReadSize;

			TSharedPtr<FJsonObject> JsonObject = MakeShareable(new FJsonObject);
			JsonObject->SetStringField(TEXT("command"), TEXT("live log data"));
			JsonObject->SetStringField(TEXT("log_file_path"), Stream.LogFilePath);
			JsonObject->SetNumberField(TEXT("file_position"), Stream.LastFilePosition - ReadSize);
			JsonObject->SetNumberField(TEXT("content_size"), ReadSize);

			// Compress if enabled and chunk is large enough to benefit from compression

			const int64 CompressionThreshold = 1024; // 1KB threshold

			if (ReadSize >= CompressionThreshold)
			{
				// Try to compress the data
				const int32 CompressedUpperBound = FCompression::CompressMemoryBound(NAME_Zlib, ReadSize);
				TArray<uint8> CompressedData;
				CompressedData.SetNumUninitialized(CompressedUpperBound);

				int32 CompressedSize = CompressedUpperBound;
				if (FCompression::CompressMemory(NAME_Zlib, CompressedData.GetData(), CompressedSize, Buffer.GetData(), ReadSize, COMPRESS_BiasSpeed))
				{
					// Only use compression if it actually saves space
					if (CompressedSize < ReadSize)
					{
						CompressedData.SetNum(CompressedSize);
						FString CompressedBase64Content = FBase64::Encode(CompressedData.GetData(), CompressedSize);
						JsonObject->SetStringField(TEXT("content_compressed_base64"), CompressedBase64Content);
						JsonObject->SetNumberField(TEXT("uncompressed_size"), ReadSize);
					}
					else
					{
						// Compression didn't help, use uncompressed
						FString Base64Content = FBase64::Encode(Buffer.GetData(), ReadSize);
						JsonObject->SetStringField(TEXT("content_base64"), Base64Content);
					}
				}
				else
				{
			...
			{
				// No compression (either disabled or chunk too small)
				FString Base64Content = FBase64::Encode(Buffer.GetData(), ReadSize);
				JsonObject->SetStringField(TEXT("content_base64"), Base64Content);
			}
			...
			SendMessage(OutputString, Stream.ClientEndpoint);
		}
	}
}
...
void FSwitchboardListener::CleanUpDisconnectedSockets()
{
	...
	for (const TPair<FIPv4Endpoint, FConnectionRef>& Connection : ConnectionsByEndpoint)
	{
		...
		{
			UE_LOGF(LogSwitchboard, Warning, "Client %ls has been inactive for more than %.1fs -- closing connection", *Client.ToString(), ClientTimeout);
	...
	while (!DisconnectTasks.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSwitchboard, Warning, "Client %ls disconnected before redeploy completed", *Client.ToString());
			RollbackRedeploy();
		}

		UE_LOGF(LogSwitchboard, Display, "Client %ls disconnecting", *Client.ToString());
	...
	for (auto Iter = Processes.CreateIterator(); Iter; ++Iter)
	{
		...
		if (Process->Handle.IsValid())
		{
			...
			if (!FPlatformProcess::IsProcRunning(Process->Handle))
			{
				...
				UE_LOGF(LogSwitchboard, Display, "Process %d (%ls) exited with returncode: %d",
				...
				if (ReturnCode != 0)
				{
					if (Process->Stdout.Num() > 0)
					{
						...
						UE_LOGF(LogSwitchboard, Display, "Stdout:\n%.*ls", StdoutConv.Length(), StdoutConv.Get());
					}

					if (Process->Stderr.Num() > 0)
					{
						FUTF8ToTCHAR StderrConv(reinterpret_cast<const ANSICHAR*>(Process->Stderr.GetData()), Process->Stderr.Num());
						UE_LOGF(LogSwitchboard, Display, "Stderr:\n%.*ls", StderrConv.Length(), StderrConv.Get());
					}
				}
...
bool FSwitchboardListener::SendMessage(const FString& InMessage, const FIPv4Endpoint& InEndpoint)
{
	...
	if (!ConnectionsByEndpoint.Contains(InEndpoint))
	{
		...
		UE_LOGF(LogSwitchboard, Verbose, "Trying to send message to disconnected client %ls", *InEndpoint.ToString());
		return false;
	}

	FConnectionRef Connection = ConnectionsByEndpoint[InEndpoint];

	UE_LOGF(LogSwitchboardProtocol, Verbose, "Sending message to client '%ls' : %ls", *InEndpoint.ToString(), *InMessage);
