...
struct FSwitchboardStartLiveLogStreamTask : public FSwitchboardTask
{
	FSwitchboardStartLiveLogStreamTask(const FGuid& InTaskID, const FIPv4Endpoint& InEndpoint, const FString& InLogFilePath, bool bInTailOnly = false, const FGuid& InProcessUUID = FGuid(), int64 InStartOffset = -1)
		: FSwitchboardTask{ ESwitchboardTaskType::StartLiveLogStream, InTaskID, InEndpoint }
		, LogFilePath(InLogFilePath)
		, bTailOnly(bInTailOnly)
		, ProcessUUID(InProcessUUID)
	...
	bool bTailOnly;
	FGuid ProcessUUID;
	int64 StartOffset; // -1 means use default behavior (0 or file size based on bTailOnly)

	//~ Begin FSwitchboardTask interface
	static constexpr const TCHAR* CommandName = TEXT("start live log stream");
	virtual const TCHAR* GetCommandName() const override { return CommandName; };

	virtual uint32 GetEquivalenceHash() const override
	{
		uint32 Hash = HashCombine(FSwitchboardTask::GetEquivalenceHash(), GetTypeHash(LogFilePath));
		return HashCombine(Hash, GetTypeHash(bTailOnly));
	}
	//~ End FSwitchboardTask interface
};

struct FSwitchboardStopLiveLogStreamTask : public FSwitchboardTask
{
	FSwitchboardStopLiveLogStreamTask(const FGuid& InTaskID, const FIPv4Endpoint& InEndpoint, const FString& InLogFilePath)
		: FSwitchboardTask{ ESwitchboardTaskType::StopLiveLogStream, InTaskID, InEndpoint }
		, LogFilePath(InLogFilePath)
	{}
	...
	//~ Begin FSwitchboardTask interface
	static constexpr const TCHAR* CommandName = TEXT("stop live log stream");
	virtual const TCHAR* GetCommandName() const override { return CommandName; };

	virtual uint32 GetEquivalenceHash() const override
	{
		return HashCombine(FSwitchboardTask::GetEquivalenceHash(), GetTypeHash(LogFilePath));
	}
	//~ End FSwitchboardTask interface
};
