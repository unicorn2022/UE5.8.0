...
FString CreateLiveLogStreamTerminatedMessage(const FString& InLogFilePath, const FString& InReason)
{
	return CreateMessage(TEXT("live log stream terminated"), true, {
		{ TEXT("log_file_path"), InLogFilePath },
		{ TEXT("reason"), InReason }
	});
}
...
TSharedPtr<FJsonValue> TryGetCommandRequiredField(const TSharedPtr<FJsonObject>& CommandObj, const FString& FieldName)
{
	...
	if (!Field)
	{
		UE_LOGF(LogSwitchboardProtocol, Error, "\"%ls\" command missing required field \"%ls\"",
...
FCreateTaskResult CreateTaskFromCommand(const FString& InCommand, const FIPv4Endpoint& InEndpoint, bool bAuthenticated)
{
	...
	else if (CommandName == FSwitchboardStartLiveLogStreamTask::CommandName)
	{
		TSharedPtr<FJsonValue> LogFilePathField = TryGetCommandRequiredField(JsonData, TEXT("log_file_path"));

		if (LogFilePathField.IsValid())
		{
			// Optional tail_only field

			bool bTailOnly = false; // Default to send from the beginning

			TSharedPtr<FJsonValue> TailOnlyField = JsonData->TryGetField(TEXT("tail_only"));

			if (TailOnlyField.IsValid())
			{
				bTailOnly = TailOnlyField->AsBool();
			}

			// Optional process_uuid field (currently there always is one)

			FGuid ProcessUUID;
			TSharedPtr<FJsonValue> ProcessUUIDField = JsonData->TryGetField(TEXT("process_uuid"));
			if (ProcessUUIDField.IsValid())
			{
				FGuid::Parse(ProcessUUIDField->AsString(), ProcessUUID);
			}

			// Optional start_offset field for resuming log streaming

			int64 StartOffset = -1; // -1 means use default behavior
			TSharedPtr<FJsonValue> StartOffsetField = JsonData->TryGetField(TEXT("start_offset"));
			if (StartOffsetField.IsValid())
			{
				StartOffset = (int64)StartOffsetField->AsNumber();
			}

			Result.Status = ECreateTaskStatus::Success;
			Result.Task = MakeUnique<FSwitchboardStartLiveLogStreamTask>(MessageID, InEndpoint, LogFilePathField->AsString(), bTailOnly, ProcessUUID, StartOffset);
			return Result;
		}
	}
	else if (CommandName == FSwitchboardStopLiveLogStreamTask::CommandName)
	{
		TSharedPtr<FJsonValue> LogFilePathField = TryGetCommandRequiredField(JsonData, TEXT("log_file_path"));
		...
		{
			Result.Status = ECreateTaskStatus::Success;
			Result.Task = MakeUnique<FSwitchboardStopLiveLogStreamTask>(MessageID, InEndpoint, LogFilePathField->AsString());
			return Result;
		}
	}
