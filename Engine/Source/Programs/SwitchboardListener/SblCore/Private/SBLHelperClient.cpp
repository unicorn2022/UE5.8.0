...
bool FSBLHelperClient::Connect(const FConnectionParams& InConnectionParams)
{
	...
	// If the hello packet arrived, ServerVersion must have been populated, which we verify below.
	{
		...
		{
			UE_LOGF(LogSBLHelperClient, Error, "Invalid SBLHelperService version '%ls' at endpoint %ls", 
				*ServerVersion,
		...
		{
			UE_LOGF(LogSBLHelperClient, Error, "Incompatible SBLHelperService version '%ls' at endpoint %ls. Minimum required is %d.%d.%d", 
				*ServerVersion,
				*ConnectionParams.Endpoint.ToString(),
				CompatibleVersion[0],
				CompatibleVersion[1],
	...
	// Ok, we're good to go.
	UE_LOGF(LogSBLHelperClient, Display, "Connected to SBLHelperService version '%ls' at endpoint %ls",
		*ServerVersion,
		*ConnectionParams.Endpoint.ToString()
	);
...
void FSBLHelperClient::ParseMessage(const FString& Message)
{
	...
	{
		UE_LOGF(LogSBLHelperClient, Error, "Unhandled command '%ls'", *Cmd);
	}
}
...
TArray<FString> FSBLHelperClient::ReceiveMessages(const float TimeoutSeconds)
{
	...
	do // Execute at least once
	{
		...
		while (Socket->HasPendingData(PendingDataSize))
		{
			...
			{
				UE_LOGF(LogSBLHelperClient, Error, "Error while receiving data via endpoint %ls", 
					*ConnectionParams.Endpoint.ToString()
				);
...
bool FSBLHelperClient::SendMessage(const FString& Message)
{
	...
	UE_LOGF(LogSBLHelperClient, Verbose, "Sending message: %ls", *Message);
