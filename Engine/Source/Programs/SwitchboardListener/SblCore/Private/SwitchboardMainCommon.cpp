...
FProcHandle CheckRedeploy(const FSwitchboardCommandLineOptions& Options)
{
	...
	{
		UE_LOGF(LogSwitchboard, Error, "Couldn't get handle to redeploy parent process; you'll need to remove the old executable manually");
		return FProcHandle();
	}
	...
	while (ProcessEnumerator.MoveNext())
	{
		...
		if (bIsCurrentProc)
		{
			...
			{
				UE_LOGF(LogSwitchboard, Error, "Redeploy expected parent PID of %u, but found %u; you'll need to remove the old executable manually",
	...
	check(false);
	UE_LOGF(LogSwitchboard, Error, "Failed to enumerate our own PID!");
...
bool HandleRedeploy(FProcHandle& RedeployParentProc, uint32 RedeployParentPid)
{
	...
	{
		UE_LOGF(LogSwitchboard, Fatal, "Error opening redeploy IPC semaphore");
		return false;
	}
	...
	{
		UE_LOGF(LogSwitchboard, Display, "Waiting for previous listener to shut down...");
		...
		if (bStillRunning)
		{
			UE_LOGF(LogSwitchboard, Warning, "Forcibly terminating previous listener");
		...
		if (bStillRunning)
		{
			UE_LOGF(LogSwitchboard, Error,
				"Unable to shut down previous listener at %ls; you'll need to remove the old executable manually",
				*OldLauncherPath);
			return false;
		}
	}
	...
	{
		UE_LOGF(LogSwitchboard, Error,
			"Unable to remove previous listener at %ls; you may need to remove the old executable manually",
			*OldLauncherPath);
		return false;
	}
...
bool SwitchboardListenerMainInit(const FSwitchboardCommandLineOptions& InOptions)
{
	...
	UE_LOGF(LogSwitchboard, Display, "SwitchboardListener %u.%u.%u", SBLISTENER_VERSION_MAJOR, SBLISTENER_VERSION_MINOR, SBLISTENER_VERSION_PATCH);

	if (InitResult != 0)
	{
		UE_LOGF(LogSwitchboard, Fatal, "Could not initialize engine, Error code: %d", InitResult);
		RequestEngineExit(TEXT("Engine init failure"));
		return false;
	}

	UE_LOGF(LogSwitchboard, Display, "Successfully initialized engine.");

	if (!InitSocketSystem())
	{
		UE_LOGF(LogSwitchboard, Fatal, "Could not initialize socket system!");
		RequestEngineExit(TEXT("Socket init failure"));
		return false;
	}

	UE_LOGF(LogSwitchboard, Display, "Successfully initialized socket system.");

	if (!FMsQuicRuntimeModule::InitRuntime())
	{
		UE_LOGF(LogSwitchboard, Fatal, "Could not initialize MsQuic runtime.");
	...
	if (RedeployParentProc.IsValid())
	{
		UE_LOGF(LogSwitchboard, Display, "Performing redeploy");
