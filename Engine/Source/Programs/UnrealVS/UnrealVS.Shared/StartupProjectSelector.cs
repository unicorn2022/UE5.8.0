...
namespace UnrealVS
{
	public class StartupProjectSelector
	{
		...
		public StartupProjectSelector()
		{
			...
			UnrealVSPackage.Instance.OnSolutionClosing +=
				delegate
				{
					...
					_CachedStartupProjectNames = null;
				};
		...
		private void UpdateStartupProjectList(params Project[] DirtyProjects)
		{
			...
			_CachedStartupProjectNames = null; // Invalidate: list changed
		...
		private void UpdateStartupProjectCombo()
		{
			...
			_CachedStartupProjectComboText = _bIsSolutionOpened ? MakeStartupProjectComboText() : string.Empty;
		}
		...
		void StartupProjectComboHandler(object Sender, EventArgs Args)
		{
			...
			{
				Marshal.GetNativeVariantForObject(_CachedStartupProjectComboText, OleArgs.OutValue);
			}
			...
			else if (OleArgs.InValue != null)
			{
				...
				// Support partial/substring input: try exact match first, then prefix, then contains.
				ProjectReference ProjectRefMatch =
					_CachedStartupProjects.FirstOrDefault(ProjRef => string.Equals(ProjRef.Name, InputString, StringComparison.OrdinalIgnoreCase))
					?? _CachedStartupProjects.FirstOrDefault(ProjRef => ProjRef.Name.StartsWith(InputString, StringComparison.OrdinalIgnoreCase))
					?? _CachedStartupProjects.FirstOrDefault(ProjRef => ProjRef.Name.IndexOf(InputString, StringComparison.OrdinalIgnoreCase) >= 0);
				...
				if (ProjectRefMatch != null && ProjectRefMatch.Project != null)
				{
					...
					// set_StartupProject fires the proper VS events and marks the solution dirty for .suo persistence.
		...
		void StartupProjectComboListHandler(object Sender, EventArgs Args)
		{
			...
			// Rebuild names array only when the project list has changed, not on every dropdown open
			if (_CachedStartupProjectNames == null)
			{
				_CachedStartupProjectNames = (from ProjRef in _CachedStartupProjects select ProjRef.Name).ToArray();
			}
			Marshal.GetNativeVariantForObject(_CachedStartupProjectNames, OleArgs.OutValue);
		}
		...
		/// Cached names array for the combo list handler, null means dirty/needs rebuild
		string[] _CachedStartupProjectNames = null;

		/// Cached display text for the startup project combo, avoids repeated COM calls on every VS poll
		string _CachedStartupProjectComboText = string.Empty;
	}
}
