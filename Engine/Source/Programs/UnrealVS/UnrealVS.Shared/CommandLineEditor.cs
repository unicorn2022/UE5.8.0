...
namespace UnrealVS
{
	internal class CommandLineEditor : IDisposable
	{
		...
		private void UpdateCommandLineCombo()
		{
			...
			// Mark dirty: MakeCommandLineComboText() will run lazily when VS next polls OutValue.
			// Avoids 5+ COM calls on every OnStartupProjectChanged event (e.g. hover over combo items).
			_CachedCommandLineTextDirty = true;
		}
			...
			try
			{
				...
				else if (OleArgs.OutValue != IntPtr.Zero)
				{
					...
					else
					{
						...
						{
							if (_CachedCommandLineTextDirty)
							{
								string NewText = MakeCommandLineComboText();
								if (NewText != CachedCommandLineText)
								{
									CachedCommandLineText = NewText;
									CommitCommandLineToMRU(CachedCommandLineText);
								}
								_CachedCommandLineTextDirty = false;
							}
							TextToDisplay = CachedCommandLineText;
						}
					}
		...
		/// Cached result of MakeCommandLineComboText(). Lazily computed: set dirty on state change,
		/// recomputed only when VS polls OutValue. Avoids expensive COM calls on every project change event.
		private string CachedCommandLineText = string.Empty;
		private bool _CachedCommandLineTextDirty = true;
