...
namespace UnrealVS
{
	class FileBrowser
	{
		...
		void FileBrowserButtonHandler(object Sender, EventArgs Args)
		{
			...
			// IsVisibleChanged only fires when visibility actually changes; if the window
			// was already visible, focus the filter box explicitly every time.
			BrowserControl?.HandleShow();
		}
