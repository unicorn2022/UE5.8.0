...
#pragma warning restore VSTHRD010 // Invoke single-threaded types on Main thread
		...
		/// <summary>
		/// Get the project selected in the solution explorer by accessing the current selection and converting to a Project if possible.
		/// </summary>
		/// <returns></returns>
		public static Project GetCurrentlySelectedProject()
		{
			ThreadHelper.ThrowIfNotOnUIThread();
			...
			try
			{
				UnrealVSPackage.Instance.SelectionManager.GetCurrentSelection(out HierarchyPtr, out uint ProjectItemId, out _, out SelectionContainerPtr);
				if (HierarchyPtr == IntPtr.Zero)
				{
					return null;
				}

				object SelectedHierarchyObj = Marshal.GetObjectForIUnknown(HierarchyPtr);
				if (!(SelectedHierarchyObj is IVsHierarchy SelectedHierarchy))
				{
					return null;
				}

				return HierarchyObjectToProject(SelectedHierarchy);
			}
			finally
			{
				if (HierarchyPtr != IntPtr.Zero)
		...
		private static readonly string BuildModuleSuffix = ".Build.cs";

		public static bool GetSelectedModule(out Project ModuleProject, out string ModuleName)
		{
			ThreadHelper.ThrowIfNotOnUIThread();
			...
			try
			{
				...
				// Walk over the children and look for a .Build.cs file. If one exists, then this is a Module folder.
				SelectedHierarchy.GetProperty(ProjectItemId, (int)__VSHPROPID.VSHPROPID_FirstChild, out object FirstChildIdObj);

				uint GetItemID(object IdObject)
				{
					if (IdObject is null)
					{
						return VSConstants.VSITEMID_NIL;
					}
					if (IdObject is int IntId)
					{
						return (uint)IntId;
					}
					if (IdObject is uint UintId)
					{
						return UintId;
					}
					return VSConstants.VSITEMID_NIL;
				...
				if (FirstChildIdObj != null)
				{
					uint ChildObjectID = GetItemID(FirstChildIdObj);

					while (ChildObjectID != VSConstants.VSITEMID_NIL)
					{
						SelectedHierarchy.GetProperty(ChildObjectID, (int)__VSHPROPID.VSHPROPID_Name, out object ObjectNameObj);

						if (ObjectNameObj is string ObjectName && ObjectName.EndsWith(BuildModuleSuffix, StringComparison.InvariantCultureIgnoreCase))
						{
							SelectedHierarchy.GetProperty(ProjectItemId, (int)__VSHPROPID.VSHPROPID_ExtObject, out object ProjectObject);

							if (ProjectObject is ProjectItem SelectedItem)
							{
								ModuleName = ObjectName.Substring(0, ObjectName.Length - BuildModuleSuffix.Length);
								ModuleProject = SelectedItem.ContainingProject;
								return true;
							}
						}

						SelectedHierarchy.GetProperty(ChildObjectID, (int)__VSHPROPID.VSHPROPID_NextSibling, out object SiblingIdObj);

						ChildObjectID = GetItemID(SiblingIdObj);
					}
				}
		...
		public static bool SelectProjectInSolutionExplorer(Project Project)
		{
			...
			// Build path to the project in solution explorer: "SolutionName\\FolderName\\...\\ProjectName"
			// Use UIHierarchy.GetItem() for direct O(depth) lookup instead of walking the entire tree.
			var PathParts = new List<string> { Project.Name };
			var ParentItem = Project.ParentProjectItem;
			while (ParentItem != null)
			{
				var ParentProject = ParentItem.ContainingProject;
				if (ParentProject == null) break;
				PathParts.Insert(0, ParentProject.Name);
				ParentItem = ParentProject.ParentProjectItem;
			}
			string SolutionName = Path.GetFileNameWithoutExtension(UnrealVSPackage.Instance.DTE.Solution.FullName);
			if (string.IsNullOrEmpty(SolutionName))
			{
				return false;
			}
			PathParts.Insert(0, SolutionName);
			string ItemPath = string.Join("\\", PathParts);

			UIHierarchy SolutionExplorer = UnrealVSPackage.Instance.DTE2.ToolWindows.SolutionExplorer;
			try
			{
