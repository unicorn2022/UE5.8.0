...
namespace UnsyncUI
{
	...
	public class HelloAnnouncement
	{
	...
	class HelloResponse
	{
		public List<HelloAnnouncement> announcements { get; set; }
	}
	...
	public class UnsyncQueryUtil
	{
		...
		public List<HelloAnnouncement> Hello()
		{
			string serverAddress = Config.server?.address;
			if (string.IsNullOrWhiteSpace(serverAddress))
			{
				return new List<HelloAnnouncement>();
			}

			string scheme = serverAddress.EndsWith(":443") || serverAddress.Contains(":443/") ? "https" : "http";
			string uriString = serverAddress.Contains("://") ? serverAddress : $"{scheme}://{serverAddress}";
			if (!Uri.TryCreate(uriString, UriKind.Absolute, out Uri baseUri))
			{
				return new List<HelloAnnouncement>();
			}
			string url = new Uri(baseUri, "/api/v1/hello").ToString();
			string responseJson = RunCommand($"query http-get {url}");
			HelloResponse response = JsonSerializer.Deserialize<HelloResponse>(responseJson);
			return response?.announcements ?? new List<HelloAnnouncement>();
		}
	}
	...
	public class UnsyncDirectoryEnumerator : IDirectoryEnumerator
	{
		...
		private Task<IEnumerable<DirectoryEntry>> EnumerateEntries(string path, CancellationToken token, EntryFilter filter)
		{
			var tcs = new TaskCompletionSource<IEnumerable<DirectoryEntry>>();
			Task.Run(async () =>
			{
				...
				try
				{
					...
					foreach (var entry in Entries)
					{
							...
							&& entry.FullPath.StartsWith(requiredPrefix))
						{
							...
							DateTime mtime = DateTime.FromFileTime((long)Math.Min(entry.MTime, (UInt64)DateTime.MaxValue.ToFileTime()));
		...
		public Task<IEnumerable<DirectoryEntry>> EnumerateDirectories(string path, CancellationToken token)
		{
			EntryFilter filter;
		...
		public Task<IEnumerable<DirectoryEntry>> EnumerateFiles(string path, CancellationToken token)
		{
			EntryFilter filter;
