...
namespace UnsyncUI
{
	...
	public sealed class BuildModel : BaseModel
	{
		...
		public string Timestamp => DirEntry.MTime == DateTime.MinValue ? "" : DirEntry.MTime.ToString("yyyy-MM-dd HH:mm:ss");

		public Config.Directory RootDir { get; }
		public string Path => DirEntry.Path;
		...
		internal BuildModel(DirectoryEntry dirEntry, Config.Directory rootDir, Config.BuildTemplate template, Config config)
		{
			AppConfig = config;
			DirEntry = dirEntry;
			RootDir = rootDir;
		...
		private async Task EnumeratePlatforms(IDirectoryEnumerator dirEnum, Config.Directory currentDir, string path, Config.BuildTemplate template, CancellationTokenSource cancellationToken)
		{
			...
			try
			{
				...
				{
					if (System.IO.Path.GetFileName(childDir.Path) == ".unsync")
					{
						List<Config.BuildTemplate> fileGroups = null;
						if (currentDir.FileGroups?.Any() == true)
						{
							var dirEntries = await dirEnum.EnumerateFiles(path, cancellationToken.Token);
							var files = DirectoryEntry.ToPaths(dirEntries);
					...
					else
					{
						...
						{
							await EnumeratePlatforms(dirEnum, subDir, childDir.Path, template, cancellationToken);
						}
					}
				}
			}
	...
	public sealed class ProjectModel : TabModel
	{
		...
		{
			if (!System.IO.Directory.Exists(FinalDstPath))
			{
				try
				{
					System.IO.Directory.CreateDirectory(FinalDstPath);
				}
				catch (Exception)
				{
				}
