...
namespace UnsyncUI
{
	public sealed class Config
	{
		...
		public sealed class Project
		{
			...
			private Task EnumerateBuilds(IDirectoryEnumerator dirEnum, ITargetBlock<BuildModel> pipe, Directory d, DirectoryEntry dirEntry, BuildTemplate template, CancellationToken cancelToken)
			{
				return Task.Run(async () =>
				{
					if (!d.Parse(dirEntry.Path, ref template))
						return;

					if (template.IsStreamClFound)
					{
						pipe.Post(new BuildModel(dirEntry, d, template, AppConfig));
					}
					else
					{
						var jobs = new List<Task>();
						foreach (var childDir in await dirEnum.EnumerateDirectories(dirEntry.Path, cancelToken))
						{
		...
		public BuildModel CreateBuildModel(string path, DateTime timestamp, Config.Directory rootDir)
		{
			var dirEntry = new DirectoryEntry { Path = path, MTime = timestamp };
			return new BuildModel(dirEntry, rootDir, default(BuildTemplate), this);
		}

		public BuildModel CreateBuildModel(string path, Config.Directory rootDir)
		{
			return CreateBuildModel(path, DateTime.MinValue, rootDir);
		}
