...
namespace UnsyncUI
{
	...
	public struct DirectoryEntry
	{
		...
		public static IEnumerable<string> ToPaths(IEnumerable<DirectoryEntry> dirEntries)
		{
			return dirEntries.Select(entry => entry.Path);
		}
	}
	...
	static class AsyncIO
    {
        ...
        public static Task<IEnumerable<DirectoryEntry>> EnumerateDirectoriesAsync(string path, CancellationToken token)
        {
            ...
            Task.Run(() =>
			{
				...
				try
				{
					...
					try
					{
						var dirs = dirInfo.EnumerateDirectories("*", enumOptions);
						var dirEntries = dirs.Select(d => new DirectoryEntry { Path = d.FullName, MTime = d.LastWriteTime});
						tcs.TrySetResult(dirEntries);
					}
					catch (Exception ex)
					{
        ...
        public static Task<IEnumerable<DirectoryEntry>> EnumerateFilesAsync(string path, CancellationToken token)
        {
			var tcs = new TaskCompletionSource<IEnumerable<DirectoryEntry>>();
            Task.Run(() =>
			{
				...
				try
				{
					...
					try
					{
						...
						var dirs = dirInfo.EnumerateFiles("*", enumOptions);
						var dirEntries = dirs.Select(d => new DirectoryEntry { Path = d.FullName, MTime = d.LastWriteTime });
    ...
    public sealed class AsyncProcess
    {
		...
		public async Task<string> RunAndGetOutput(bool ReadStdErr = true)
		{
			...
			await foreach (var str in RunAsync(cancellationToken, ReadStdErr))
			{
				result += str;
			}
			return result;
