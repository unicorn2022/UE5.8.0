...
namespace UnsyncUI
{
    ...
    {
		private static readonly ConsoleColorConverter _colorConverter = new ConsoleColorConverter();
		...
		private LogModel _currentLogModel;
        ...
        public JobControl()
        {
			...
			_logDocument = LogRichTextBox.Document;
			LogRichTextBox.Loaded += OnLogRichTextBoxLoaded;
			DataContextChanged += OnDataContextChanged;
        }

		private void OnLogRichTextBoxLoaded(object sender, RoutedEventArgs e)
		{
			if (_scrollViewer != null)
				_scrollViewer.ScrollChanged -= OnScrollChanged;

			_scrollViewer = FindScrollViewer(LogRichTextBox);
			if (_scrollViewer != null)
				_scrollViewer.ScrollChanged += OnScrollChanged;
		}
		...
		private static Brush GetBrush(ConsoleColor color)
			=> (Brush)_colorConverter.Convert(color, typeof(Brush), null, CultureInfo.InvariantCulture);

		private void OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
		{
			if (e.OldValue is JobModel oldModel)
			{
				oldModel.LogLines.CollectionChanged -= OnLogLinesChanged;
				if (_currentLogModel != null)
					_currentLogModel.PropertyChanged -= OnCurrentLogModelChanged;
			}

			_logDocument.Blocks.Clear();
			_currentLogModel = null;
			_currentRun = null;

			if (e.NewValue is JobModel newModel)
			{
				// Populate existing lines (e.g. when DataContext switches to a completed job)
				foreach (var model in newModel.LogLines)
					AppendLogLine(model);

				newModel.LogLines.CollectionChanged += OnLogLinesChanged;
				_autoScroll = true;
				UpdateFollowLink();
			}
		}

		private void AppendLogLine(LogModel model)
		{
			if (_currentLogModel != null)
				_currentLogModel.PropertyChanged -= OnCurrentLogModelChanged;

			var run = new Run(model.Text ?? "");
			run.Foreground = GetBrush(model.Color);
			var para = new Paragraph(run) { Margin = new Thickness(0) };
			_logDocument.Blocks.Add(para);

			_currentRun = run;
			_currentLogModel = model;
			_currentLogModel.PropertyChanged += OnCurrentLogModelChanged;
		}

		private void OnCurrentLogModelChanged(object sender, PropertyChangedEventArgs e)
		{
			var model = (LogModel)sender;
			if (_currentRun == null)
				return;

			if (e.PropertyName == nameof(LogModel.Text))
				_currentRun.Text = model.Text ?? "";
			else if (e.PropertyName == nameof(LogModel.Color))
				_currentRun.Foreground = GetBrush(model.Color);
		}
		...
		private void OnLogLinesChanged(object sender, NotifyCollectionChangedEventArgs e)
		{
			...
			{
				foreach (LogModel model in e.NewItems)
					AppendLogLine(model);
			}
		...
		private void OnScrollChanged(object sender, ScrollChangedEventArgs e)
		{
			...
			// ExtentHeightChange != 0 means content grew, not a user scroll
		...
		private bool IsAtBottom()
		{
			...
			return _scrollViewer.VerticalOffset >= _scrollViewer.ScrollableHeight - 2.0;
		}

		private void UpdateFollowLink()
		{
			FollowLink.Visibility = _autoScroll
    ...
    public sealed class JobModel : BaseModel
    {
        ...
        private async void RunAsync()
        {
			...
			try
			{
				...
				if (App.Current.UnsyncSupportsStdout)
				{
					args.Add("--stdout");
				}
				if (DryRun)
