...
namespace CruncherSharp
{
	public class SymbolMemberInfo
	{
			...
			get
			{
				switch (Category)
				{
					...
					default:
						...
						uint Alignment = (uint)Size;
						if (Count > 0)
							Alignment = (uint)(Size / Count);

						if (Alignment > 16) // impossible, ignore
						{
							Alignment = 1;
						}

						if (PaddingBefore >= 4 && Alignment < 8)
						{
							Alignment = 8;
						}
						else if (PaddingBefore >= 2 && Alignment < 4)
						{
							Alignment = 4;
						}
						else if (PaddingBefore > 0 && Alignment < 2)
						{
							Alignment = 2;
						}
		...
		public bool IsExapandable => (Category == MemberCategory.Base || Category == MemberCategory.UDT) && (TypeInfo != null && TypeInfo.Members.Count > 0);
