...
namespace PDBReader
{
	...
	public unsafe class TypeFullInfo
	{
		...
		private TypeFunction[] _functions;
		...
		{
			return new TypeFullInfo(basic) { _members = new TypeMember[0], _functions = new TypeFunction[0] };
		}

		internal static TypeFullInfo MakeStructure(TypeBasicInfo basic, DataMemberInfo[] dataMembers, FunctionMemberInfo[] functionMembers)
		{
			...
			var functions = functionMembers.Select(x =>
			{
				return new TypeFunction
				{
					...
					IsPure = x.IsPure,
					IsIntro = x.IsIntro,
					ReturnType = Marshal.PtrToStringAnsi(x.ReturnType) ?? "",
					Parameters = Marshal.PtrToStringAnsi(x.Parameters) ?? "",
				};
			}).ToArray();

			return new TypeFullInfo(basic) { _members = members, _functions = functions };
		}
		...
		public IEnumerable<TypeFunction> Functions
		{
			get
			{
				return _functions;
	...
	public struct  TypeFunction
	{
		...
		public bool IsPure { get; internal set; }
		public bool IsIntro { get; internal set; }
	...
	public unsafe class PDB : IDisposable
	{
		...
		{ 
			for (uint i = _tpiStream.TypeIndexBegin; i < _tpiStream.TypeIndexEnd; ++i)
			{
				...
				if (kind != TypeRecordKind.Structure && kind != TypeRecordKind.Class && kind != TypeRecordKind.Union && kind != TypeRecordKind.Enum)
				{
					continue;
				}

				var flags = (*record).Class.Flags; // Class, union and enum have fwdref at the same offset
				if ((flags & ClassFlags.FwdRef) == ClassFlags.FwdRef)
				{
					continue;
				}
				// Structs/classes/unions require HasUniqueName to filter anonymous types; enums don't need it
				if (kind != TypeRecordKind.Enum && (flags & ClassFlags.HasUniqueName) != ClassFlags.HasUniqueName)
				{
					continue;
				}
			...
			for (uint i = _tpiStream.TypeIndexBegin; i < _tpiStream.TypeIndexEnd; ++i)
			{
				...
				else if (kind == TypeRecordKind.Enum)
				{
					// fwdref bit is at the same struct offset as LF_CLASS
					var flags = (*record).Class.Flags;
					if ((flags & ClassFlags.FwdRef) == ClassFlags.FwdRef)
					{
						var basicInfo = GetBasicTypeInfoInternal(i);
		...
		{
			return GetFullTypeInfo(info, loadFunctions: true);
		}

		public TypeFullInfo GetFullTypeInfo(TypeBasicInfo info, bool loadFunctions)
		{
			if (info.IsPrimitive)
			{
			...
			unsafe
			{
				...
				if (!loadFunctions)
				{
					// Fast path: only load data members, skip function counting and reading
					...
					return TypeFullInfo.MakeStructure(info, memberInfos, new FunctionMemberInfo[0]);
				}
				else
				{
					// Full path: load both data members and functions
					PDBReader.CountNumFields(_handle, fieldListRecord, out StructFieldCounts counts);
					var memberInfos = new DataMemberInfo[counts.NumDataMembers];
					var functionInfos = new FunctionMemberInfo[counts.NumFunctionMembers];
					PDBReader.ReadDataMembers(_handle, fieldListRecord, memberInfos);
					PDBReader.ReadFunctionMembers(_handle, fieldListRecord, functionInfos);
					return TypeFullInfo.MakeStructure(info, memberInfos, functionInfos);
				}
			}
		}

		public FunctionSignature GetFunctionSignature(uint typeIndex)
		{
			var returnTypeInfo = GetBasicTypeInfo(typeIndex);
