...
namespace CruncherSharp
{
    public class SymbolInfo
    {
		...
		public List<SymbolInfo> UsedBy { get; set; }
		...
		public void SortFunctions()
		{
			...
			Functions.Sort((a, b) =>
			{
				...
				int categoryOrder = GetFunctionCategoryOrder(a.Category).CompareTo(GetFunctionCategoryOrder(b.Category));
				if (categoryOrder != 0)
		...
		private int GetFunctionCategoryOrder(SymbolFunctionInfo.FunctionCategory category)
		{
			switch (category)
			{
				...
				case SymbolFunctionInfo.FunctionCategory.Destructor:
					return 1;
				case SymbolFunctionInfo.FunctionCategory.Function:
					return 2;
		...
		public int NumDerivedClasses()
		{
			if (DerivedClasses == null)
			{
				return 0;
			}
			int DerivedClassesCound = DerivedClasses.Count;
			foreach (var derivedClass in DerivedClasses)
			{
				DerivedClassesCound += derivedClass.NumDerivedClasses();
			}
			return DerivedClassesCound;

		}

		public int CountUnusedNonOverridingVirtuals()
		{
			int count = Functions != null ? Functions.Count(f => f.UnusedVirtual) : 0;
			return count;
		}

		public int VTableWasted => 8 * CountUnusedNonOverridingVirtuals() * (1 + NumDerivedClasses());

		public int CountVirtualFunctions()
		{
			return CountVirtualFunctions(new System.Collections.Generic.HashSet<SymbolInfo>());
		}

		private int CountVirtualFunctions(System.Collections.Generic.HashSet<SymbolInfo> visited)
		{
			if (!visited.Add(this)) return 0;
			// Count all virtual slots introduced by this class (overrides reuse a parent slot, so only !IsOverride adds a new entry)
			int count = Functions != null ? Functions.Count(f => f.Virtual && !f.IsOverride) : 0;
			foreach (var member in Members)
			{
				if (member.Category == SymbolMemberInfo.MemberCategory.Base && member.TypeInfo != null)
					count += member.TypeInfo.CountVirtualFunctions(visited);
			}
			return count;
		}

		public int VTableCost => 8 * CountVirtualFunctions() * (1 + NumDerivedClasses());
        ...
        public void CheckOverride()
        {
            ...
            {
				// Check if this function is overloaded by derived classes
                if (function.Virtual &&
        ...
        private void CheckMasking(SymbolFunctionInfo func)
        {
			...
			foreach (var function in Functions)
            {
                if (function.Virtual == false &&
					string.Compare(function.Name, func.Name, StringComparison.Ordinal) == 0 &&
					string.Compare(function.Parameters, func.Parameters, StringComparison.Ordinal) == 0)
                {
