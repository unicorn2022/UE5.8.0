...
def _build_channel(channel_name:str, channel_dir:Path, cleaner:"_Cleaner") -> dict[str]:
    ...
    if channel._name:
        ...
        _log.print("Name override:", channel_name)
...
#-------------------------------------------------------------------------------
def _get_sources_inline(add_func) -> None:
...
#-------------------------------------------------------------------------------
def _get_sources_platform(add_func) -> None:
    # !! todo !!
    ...
    # platforms_dir = item / "Platforms"
    # if not platforms_dir.is_dir():
    #     return

    # for item in (x for x in platforms_dir.iterdir() if x.is_dir()):
    #     candidate = item / "Extras/ushell/channel"
    #     if candidate.is_dir():
    #         name = item.name.lower()
    #         add_func(candidate.parent, f"$platform.{name}.")
    pass

#-------------------------------------------------------------------------------
def _get_sources_user(add_func) -> None:
    from_home = Path.home() / ".ushell/channels"
    if from_home.is_dir():
        add_func(from_home, "$home.")

    from_env = os.getenv("FLOW_CHANNELS_DIR")
    if not from_env:
        return
...
def _get_sources() -> _Sources:
    ...
    def add_source(source_dir:Path, group_name:str="") -> int:
        count = 0
        for item in source_dir.glob("*"):
            ...
            if candidate.is_file():
                ...
                count += add_source(item, item.name + ".")

        return count

    _get_sources_inline(add_source)
    _get_sources_platform(add_source)
    _get_sources_user(add_source)
