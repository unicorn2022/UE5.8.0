...
class Sync(_SyncBase):
    ...
    def _main_impl(self):
        ...
        try:
            ...
            if not self.args.dryrun:
                self._record_cl(version_cl)

    def _record_cl(self, version_cl):
        self._update_build_version(version_cl)
        self._update_ugs_state(version_cl)

    def _update_build_version(self, version_cl):
        build_ver_path = self._local_root + "Engine/Build/Build.version"
        if not os.path.isfile(build_ver_path):
            return
    ...
    def _update_ugs_state(self, version_cl):
        state_path = Path(self._local_root) / ".ugs/state.json"
        if not state_path.is_file():
            return

        import json
        with open(state_path, "r") as x:
            data = json.load(x)

        data["LastSyncChangeNumber"] = version_cl

        with open(state_path, "w") as x:
            json.dump(data, x, indent="\t")
            x.write("\n")
