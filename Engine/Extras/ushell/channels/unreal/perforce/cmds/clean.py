...
class _Collector(object):
    ...
    def pin_needle(self, needle):
        needle = needle.strip()
        if needle:
            self._pinned_needles.append(needle)
        ...
        for dir in self._careful_dirs:
            for item in (x for x in dir.rglob("*") if x.is_file()):
                ...
                if key not in self._pinned_files:
                    if not any(needle in key for needle in self._pinned_needles):
...
class Clean(flow.cmd.Cmd):
    ...
    dryrun     = flow.cmd.Opt(False, "Do nothing except reports statistics (this is slow...)")
    ...
    pinneedles = flow.cmd.Opt("site-packages", "If 'needle' is in deletee path then keep (comma-separated)")
    ...
    def main(self):
        ...
        print("Adding pin needles...")
        for i, needle in enumerate(self.args.pinneedles.split(",")):
            if needle:
                print(f"{i:02}", needle)
                collector.pin_needle(needle)
