...
UPROJ_OPT_STR = "Prefer opening active project's .uproject ('--uproj=no' to disable,"
" 'USHELL_UPROJ=yes/no' environment variable to change the default behaviour)."
...
class _Base(unrealcmd.MultiPlatformCmd):
    ...
    def _get_sln_path(self):
        ...
        candidates = []
        def gather(candidate_dir):
            for ext in (".slnx", ".sln"):
                candidates.append(candidate_dir / (primary_name + ext))

        if project := ue_context.get_project():
            gather(project.get_dir())

        engine = ue_context.get_engine()
        gather(engine.get_dir().parent)
...
#-------------------------------------------------------------------------------
class OpenVSCode(unrealcmd.MultiPlatformCmd):
    """ Opens a Visual Studio Code workspace """

    def _get_wokspace_path(self):
        ue_context = self.get_unreal_context()
        engine = ue_context.get_engine()
        engine_dir_name = engine.get_dir().parent.stem

        primary_name = None
        try:
            config = engine.get_ubt().get_configuration()
            config.get_value("ProjectFileGenerator", "PrimaryProjectName")
        except Exception:
            primary_name = "UE5"
        ...
        if project := ue_context.get_project():
            ...
            # could be name of the project itself
            possible_names.append(project_name)
            # if bPrimaryProjectNameFromFolder - could append engine directory name
            possible_names.append(f"{project_name}_{engine_dir_name}")

        # could be just "UE5"
        possible_names.append(primary_name)
        # if UE_NAME_PROJECT_AFTER_FOLDER:
        possible_names.append(f"{primary_name}_{engine_dir_name}")
        # if bPrimaryProjectNameFromFolder:
        possible_names.append(engine_dir_name)

        workspace_dir = engine.get_dir().parent
        if ue_context.get_type() == unreal.ContextType.FOREIGN:
            workspace_dir = ue_context.get_project().get_dir()

        for workspace_name in possible_names:
            workspace_path = workspace_dir / f"{workspace_name}.code-workspace"
            if workspace_path.is_file():
                return workspace_path
        # no project is generated - open as a folder
        return workspace_dir

    def main(self):
        self.print_info("Opening workspace in VSCode")
        workspace_path = self._get_wokspace_path()
        if not os.path.exists(workspace_path):
            self.print_error("Workspace not found")
            return False
        print("Path:", workspace_path)

        code_executable = "code.cmd" if os.name == "nt" else "code"

        args = (
            workspace_path,
        )

        cmd = self.get_exec_context().create_runnable(code_executable, *args)
        result = cmd.launch()
        return True
...
#-------------------------------------------------------------------------------
class File(_Base):
    """ Finds a suitable VS instance and opens the given file """
    filepath    = unrealcmd.Arg(Path, "Path of the file to open")
    line        = unrealcmd.Arg(-1, "Optional line number to open")
    focus       = unrealcmd.Opt(False, "Bring VS into the foreground (unreliably)")

    def main(self):
        if not self.args.filepath.is_file():
            self.print_error(f"Invalid file path '{self.args.filepath}'")
            return False

        import vs.dte
        vs_instances = list(vs.dte.running())
        if not vs_instances:
            self.print_error("No VS instances appear to be running")
            return False

        target_vs = None
        if len(vs_instances) > 1:
            try:
                sln_path = self._get_sln_path()
                print("Using:", sln_path)
            ...
            if sln_path.is_file():
                for instance in vs_instances:
                    ...
                    if candidate and sln_path.samefile(candidate):
                        target_vs = instance
                        break
        else:
            target_vs = vs_instances[0]

        if not target_vs:
            self.print_error("No suitable VS instance found from a total of " + str(len(vs_instances)))
            return False

        file_path = self.args.filepath.absolute()
        if not target_vs.open_file(str(file_path), self.args.line):
            self.print_error("Something went wrong asking VS to open the file")
            return False
...
#-------------------------------------------------------------------------------
class OpenVS(_Base):
    """ Opens a solution in Visual Studio. """
    uproj = unrealcmd.Opt((None, "yes", "no"), UPROJ_OPT_STR)

    def main(self):
        if os.name != "nt":
            self.print_error("Opening a solution in Visual Studio is only supported on Windows")
            return False

        self.print_info("Opening solution in Visual Studio")

        use_uproj = slnutils.use_uproj(self.args)

        if use_uproj:
            sln_path = self._get_uproject_path()
            if sln_path:
                sln_path = sln_path.parent
        else:
            sln_path = self._get_sln_path()
        ...
        if use_uproj:
            if not sln_path:
                self.print_error("Project path not found")
                return False
        else:
            if not os.path.isfile(sln_path):
                self.print_error("Project file not found")
        ...
        run_args = ("cmd.exe", "/d", "/c", "start devenv", sln_path)
        shell_open = self.get_exec_context().create_runnable(*run_args)
        return shell_open.run()


#-------------------------------------------------------------------------------
class OpenRider(_Base):
    """ Opens a solution in Rider. """
    uproj = unrealcmd.Opt((None, "yes", "no"), UPROJ_OPT_STR)

    def main(self):
        if os.name != "nt":
            self.print_error("Opening a solution in Rider is only supported on Windows")
            return False

        self.print_info("Opening solution in Rider")
        ...
        run_args = ("cmd.exe", "/d", "/c", slnutils.find_rider_bin(), sln_path)

        try:
            print("Launching Rider:", *run_args)
...
class OpenXcode(_Base):
    """ Opens a workspace in Xcode. """
    platform = unrealcmd.Arg("Mac", "Platform to open (default = Mac)")

    platform_to_workspace = {
        "mac": "UE5 (Mac).xcworkspace",
        "ios": "UE5 (IOS).xcworkspace",
        "tvos":"UE5 (TVOS).xcworkspace",
        "visionos": "UE5 (VisionOS).xcworkspace"
    }

    def main(self):
        if sys.platform != "darwin":
            self.print_error("Opening a solution in Xcode is only supported on Mac")
            return False

        self.print_info("Opening workspace in Xcode")

        platform = self.args.platform.lower()
        if platform not in self.platform_to_workspace:
            self.print_error("Unsupported platform")
            return False

        workspace_name = self.platform_to_workspace[platform]
        workspace_path = self.get_unreal_context().get_branch().get_dir() / Path(workspace_name)

        print("Path:", workspace_path)
        if not os.path.isdir(workspace_path):
            self.print_error("Workspace dir not found")
            return False

        run_args = ("open", workspace_path)

        try:
            print("Launching Xcode:", *run_args)
            proc = subprocess.Popen(run_args)
            return True
        except Exception as e:
            print("!! Failed launching Xcode:", str(e))
            return False
