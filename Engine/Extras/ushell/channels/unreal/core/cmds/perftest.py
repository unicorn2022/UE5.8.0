...
class PerfTestBaseCmd(unrealcmd.Cmd):
    ...
    def _get_asset_name(self, object_path) -> str:
        # Remove spaces and quotes from the start and end.
        s = object_path.strip(' \'"')
        if not s:
            return ''

        # Remove any sub-objects by splitting on the first :.
        s = s.split(':', 1)[0]

        # Get the last segment
        basename = s.rsplit('/', 1)[-1]

        # Use the asset name rather than package name.
        if '.' in basename:
...
class StaticCamera(PerfTestBaseCmd):
    ...
    Static Camera Test.
    """
    MapName  = unrealcmd.Opt("",  "Static Camera Test Map Name")
    def main(self):
        args = (None,)
        if self.args.MapName != "":
    ...
    def complete_MapName(self, prefix):
        return [self._get_asset_name(x) for x in self._get_available_static_cameratests()]
