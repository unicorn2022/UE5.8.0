...
class GetBuild(unrealcmd.Cmd):
    ...
    If your search only matches one build that build is downloaded for you, to
    add filtering you can use --match as follows:

        * --match 8v93f39cvn3        - A known build id.
        * --match 1337               - Searches for the build closest to a changelist
        * --match job=v99f;step=39f9 - Advanced searching by specify key values

    If you find multiple builds that match you will be given the option to pick
    one, where the * indicates the build closest to your current commit."""

    actions             = unrealcmd.Arg([str], "The platforms and build types to download data for")
    ...
    wildcard            = unrealcmd.Opt("", "Windows style wildcard (using * and ?) to match file paths to include (';' separated)")
    sync                = unrealcmd.Opt(False, "Use P4 to sync to the specified changelist.")
    buildgroup          = unrealcmd.Opt("", "The build group to filter, this is frequently globally unique and thus allows you to uniquely specify a build without any other options")
    config              = unrealcmd.Opt("", "The Configuration to filter")
    nobins              = unrealcmd.Opt(False, "Add an exclusion for executable files (exe/dll/pdb/etc...)")
    ...
    _build_types = ("staged", "packaged")

    def complete_actions(self, prefix):
        for platform in unrealcmd.Cmd.complete_platform(self, prefix):
            yield platform
            if self.is_server_platform(platform):
                yield platform + "-server"
        yield from self._build_types
        yield from self.complete_config
    ...
    complete_config = ("development", "test", "shipping")
    ...
    def lookup_host(self):
        ...
        # if host is a semicolon separated list we parse out the first entry as zen cli does not yet support a list of hosts
    ...
    def convert_platform(self, platform):
        # strip suffix to map platform name separately
        def strip_suffix(platform):
            for suffix in ["-server", "-client"]:
                if platform.endswith(suffix):
                    return platform[0:-len(suffix)], suffix
            return platform, ""

        platform, suffix = strip_suffix(platform)
        ...
        try:
            ...
            return converted_platform + suffix
        except ValueError:
            # unknown platform, just assume the name is valid
    ...
    def add_query_eq(self, key, value):
        self._query[key] = {"$eq": value}

    def add_query_neq(self, key, value):
        self._query[key] = {"$neq": value}

    def add_query_in(self, key, value):
        self._query[key] = {"$in": value}

    def build_query(self, search_request, build_type, branch, platforms, preflight, buildgroup, config):
        ...
        # Parse match string into query filters
        for statement in search_request.split(";"):
            if statement.find("!=") != -1:
                k, v = statement.split("!=", 1)
                self.add_query_neq(k, v)
            elif statement.find("~=") != -1:
                k, v = statement.split("~=", 1)
                self.add_query_in(k, v.split(","))
            elif statement.find("=") != -1:
                k, v = statement.split("=", 1)
                self.add_query_eq(k, v)

        if search_changelist:   self.add_query_eq("commit", int(search_changelist))
        if build_id:            self.add_query_eq("buildid", str(build_id))
        if build_type:          self.add_query_eq("type", str(build_type))
        if buildgroup:          self.add_query_eq("buildgroup", str(buildgroup))
        if config:              self.add_query_eq("configuration", str(config))

        # ignore the defaulted branch filtering when specifying build group as
        # that uniquely identifies the branch anyway this allows us to download
        # a build from any branch if you know its build group
        if branch and not buildgroup:
            self.add_query_eq("branch", str(branch.lower()))

        if not preflight:
            self.add_query_eq("ispreflight", "false")
        ...
        if platforms:
            ...
            for platform in platforms:
                ...
                # if not explicitly specified, add -client suffix for every platform
                if not p.endswith("-server") and not p.endswith("-client"):
                    platform_filters.append(f"{p}-client")
                # if not explicitly specified, add -server suffix for server platform
                if not p.endswith("-server") and self.is_server_platform(p):
                    platform_filters.append(f"{p}-server")

            self.add_query_in("platform", platform_filters)
    ...
    def search_build(self, host, override_host, namespace, request):
        ...
        return [{
            ...
            "config": r["metadata"].get("Configuration", "All")
        } for r in results]
    ...
    def _show_select_build_prompt(self, branch_cl, builds):
        ...
        def abbreviate(x):
            ...
            x = "".join(c for c in x if c not in vowels_plus)
            return x[:7]
        ...
        def list_available(closest_cl, available_builds):
            ...
            for build in available_builds:
                ...
                yield "%10s%s %s %s" % (cl, middle, name, config)

        available = list(list_available(closest_cl, available_builds))
        available.sort(reverse=True)

        for line in fzf.run(available, sort=False):
            segments = line.split()
            build_name = segments[-2]
            config_name = segments[-1]
            commit = segments[0].rstrip("*")
            build_key = build_name + commit + config_name
    ...
    def run_zen(self, args, include_passthrough_args = True):
        override = os.environ.get("UE_ZEN_BINARY_PATH")
        if override:
            from pathlib import Path
            bin_path = Path(override)
            self.print_info(f"Using zen binary from UE_ZEN_BINARY_PATH: {bin_path}")
        else:
            ue_context = self.get_unreal_context()
            engine = ue_context.get_engine()
        ...
        global_args = os.environ.get("UE_ZEN_ARGS", "").split()
        if global_args:
            self.print_info(f"Using extra zen args from UE_ZEN_ARGS: {' '.join(global_args)}")
        if include_passthrough_args:
            args = (*global_args, *args, *self.args.runargs)
        else:
    ...
    def preprocess_actions(self):
        non_platform_actions = set(self._build_types) | set(self.complete_config)
        self.args.buildtype = next((x for x in self.args.actions if x in self._build_types), "staged")
        self.args.platforms = [x for x in self.args.actions if x not in non_platform_actions]
        if not self.args.config:
            self.args.config = next((x for x in self.args.actions if x in self.complete_config), "")

    def _print_build_targets(self):
        self.print_info("Dataset requests")

        print("Type:", self.args.buildtype)

        for name in self.args.platforms:
            print("Build:", name)

        if match := self.args.match:
    ...
    def main(self):
        ...
        project_path = None
        if not project:
            if not (project := ue_context.get_project()):
                raise EnvironmentError("There is no active project set")
            project_path = project.get_dir()
            project = project.get_name()

        self.preprocess_actions()
        self._print_build_targets()
        ...
        self.args.excludes = getattr(self.args, "exclude-wildcard", "")

        for name in ("wildcard", "excludes"):
            if "," in getattr(self.args, name):
                parts = getattr(self.args, name).split(",")
                setattr(self.args, name, ";".join(parts))

        if self.args.nobins:
            parts = self.args.excludes.split(";")
            parts = (*parts, "*.exe", "*.dll", "*.self", "*.nss", "*.pdb", "*.map")
            self.args.excludes = ";".join(parts)
        ...
        self.print_info("Querying build list")
        self.build_query(self.args.match, build_type, branch, platforms_to_download, self.args.preflight, self.args.buildgroup, self.args.config)
        ...
        for build in builds:
            ...
            config_str = str(build.get("config", ""))
            build_key = build_name + commit_str + config_str
        ...
        if found_builds == None or len(found_builds) == 0:
            self.print_error(f"No builds found for namespace '{namespace}'")
            print("Request = ", end="")
            import pprint
        ...
        if len(found_builds) == 1:
            ...
            self.print_info("Build selection")
        ...
        # If dest ends with '...' replace the suffix with the build's buildgroup
        if dest.endswith("..."):
            buildgroup = selected_build.get("buildgroup", "")
            if not buildgroup:
                self.print_error("Dest uses '...' suffix but the selected build has no buildgroup")
                return False
            dest = dest[:-3] + buildgroup
        ...
        for bucket, build_id, build_platform in builds_to_download:
            ...
            args = (
                ...
                "--build-id=" + build_id,
                f"--wildcard={self.args.wildcard}" if self.args.wildcard else "",
                "--exclude-wildcard=" + self.args.excludes
            )

            result |= self.run_zen(args)

        if result == 0 and self.args.sync:
            (result, err) = run_cmd(f"_p4 sync {selected_build["commit"]}")
            if result == False:
                self.print_error(f"P4 sync failed with error: {err}. Aborting.")
                return False
