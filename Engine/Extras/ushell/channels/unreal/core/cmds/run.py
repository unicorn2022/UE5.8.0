...
class Target(_ProgramTarget):
    """ Runs a named target on the host platform. """
...
class _Runtime(_RunCmd, _Attachable):
    ...
    def _main_impl(self, target_type):
        ...
        # Detect the user doing -basedir= things themselves.
        user_basedir = (x for x in self.args.runargs if x.startswith("-basedir="))
        user_basedir = next(user_basedir, None)
        if user_basedir:
            if self.args.datadir:
                self.print_warning("Ignoring --datadir as it conflicts with -basedir=")
                self.args.datadir = ""
            stage_dir = Path(user_basedir[9:]).absolute().parent.parent.parent
        ...
        # Stick with directories the user asked for.
        if user_basedir:
            print(stage_dir)
            stage_dir = None
        ...
        if not self.args.cooked:
            projectstore_marker = (stage_dir / "ue.projectstore") if stage_dir else Path()
