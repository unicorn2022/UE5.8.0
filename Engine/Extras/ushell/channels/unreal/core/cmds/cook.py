...
class _Cook(unrealcmd.MultiPlatformCmd):
    ...
    def _cook(self, cook_form, exec_context=None):
        ...
        if not build:
            self.print_error(f"No {variant.name.lower()} editor build found (check .target receipt in Binaries/)")
            return False

        cmd = build.get_binary_path()
        if cmd is None:
            self.print_error(f"Unable to get 'Launch' path from .target receipt in Binaries/")
            return False
...
class _Runtime(_Cook):
    """ If the given platform is known the environment for the platform will be
    derived and details printed. Otherwise the runtime target will be used for a
    best-effort guess. """

    platform = unrealcmd.Arg("", "The platform to cook data for")

    def _main_unplatformed(self, target):
        cook_form = self.args.platform
        if target == unreal.TargetType.CLIENT:
            cook_form += "Client"

        self.print_info("Platform")
        print("Given:", self.args.platform)
        print("Form:", cook_form)
    ...
    def main(self, target):
        ...
        try:
            ...
            return self._main_unplatformed(target)
