...
#-------------------------------------------------------------------------------
def _unescape_ini_quoted_string(value):
    """Unescape a UE INI quoted string value, matching FParse::QuotedString."""
    result = []
    i = 0
    while i < len(value):
        ch = value[i]
...
def _read_ini_value(ini_path, section, key):
    """Read a value from a UE-style INI file given a section and key name.
    ...
    with open(ini_path, "r") as f:
        for line in f:
            line = line.strip()
            if line.startswith("["):
                in_section = line.lower() == section_header.lower()
                continue
            if in_section:
                eq_index = line.find("=")
                if eq_index > 0 and line[:eq_index].lower() == key.lower():
                    raw = line[eq_index + 1:]
                    if raw.startswith('"') and raw.endswith('"'):
                        return _unescape_ini_quoted_string(raw[1:-1])
                    return raw
    return None

def _get_zen_host_auth():
    """Retrieve the Zen host auth data from the platform credential store.

    Matches the storage locations used by FPlatformMisc::GetStoredValue with
    store ID "Epic Games", section "Zen", key "HostAuth":
      - Windows: HKCU\\Software\\Epic Games\\Zen\\HostAuth (registry)
      - macOS:   ~/Library/Application Support/Epic/Epic Games/KeyValueStore.ini [Zen] HostAuth
      - Linux:   ~/.config/Epic/Epic Games/KeyValueStore.ini [Zen] HostAuth
    Returns the parsed auth dict or None if not found.
    """
    try:
        auth_json = None
        if os.name == "nt":
            import winreg
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Epic Games\Zen") as key:
                auth_json, _ = winreg.QueryValueEx(key, "HostAuth")
        else:
            ...
            if sys.platform == "darwin":
                app_settings_dir = os.path.join(os.path.expanduser("~"),
                    "Library", "Application Support", "Epic")
            else:
                app_settings_dir = os.path.join(os.path.expanduser("~"),
                    ".config", "Epic")

            ini_path = os.path.join(app_settings_dir, "Epic Games", "KeyValueStore.ini")
            if os.path.isfile(ini_path):
                auth_json = _read_ini_value(ini_path, "Zen", "HostAuth")
...
class ZenBaseCmd(unrealcmd.Cmd):
    ...
    def perform_target_creation(self, snapshot, projectid = None, oplogid = None):
        ...
        host_auth = _get_zen_host_auth()
        if host_auth:
            zenserver_data["hostauth"] = host_auth
        zenserver_data["projectid"] = target_projectid
        zenserver_data["oplogid"] = target_oplogid
        projectstore = {
            "zenserver" : zenserver_data
        }
