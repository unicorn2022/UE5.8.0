...
class PlatformBase(unreal.Platform):
    ...
    def _launch(self, exec_context, stage_dir, binary_path, args):
        ...
        joined_args = " ".join(x.replace("\"", "\\\"") for x in args)

        # Check the HOST platform (not the target Android platform)
        if unreal.Platform.get_host() == "Mac":
             # Mac requires escaping the single quote
            args = "\\'" + joined_args + "\\'"
        else:
            # Windows/Linux use standard single quotes
            args = "'" + joined_args + "'"
