...
class Platform(object):
    ...
    def _get_version_json(self, *, key=None, use_autosdk=True):
        ...
        # Nothing like a good bit of unstructuring.
        key = key or self.json_version_key
        ...
        def read_sdk_json(path):
            ...
            for candidate in (key, "MainVersion"):
                if version := sdk_info.get(candidate, None):
                    return version

            # ...or traverse up the platform forest
            for upward_key in ("ParentSDKFile", "IncludeSDKFile"):
                if not (next_path := sdk_info.get(upward_key, None)):
                    continue

                if next_path.startswith("$(EngineDir)"):
                    engine_dir = self._context.get_engine().get_dir()
                    next_path = engine_dir / next_path[13:]
                else:
                    next_path = path.parent / next_path
                return read_sdk_json(next_path)
