...
struct Handle
{
            ...
            ~Handle()               { if (_handle != nullptr || _handle != INVALID_HANDLE_VALUE) CloseHandle(_handle); }
...
bool Dte::open_file(const wchar_t* path, int line) const
{
    ...
    BStr path_str(path);
    BStr view_kind(L"{7651A703-06E5-11D1-8EBD-00A0C90F26EA}"); // from vsViewKindTextView

    VARIANT_BOOL is_open = FALSE;
    item_ops->IsFileOpen(path_str, view_kind, &is_open);

    ComPtr<Document> document;
    _dte->get_ActiveDocument(document);

    ComPtr<Window> window;
    if (FAILED(item_ops->OpenFile(path_str, view_kind, window)))
        return false;

    if (line < 0)
        return true;

    if (is_open == VARIANT_FALSE)
    {
        // File open is async. A crude attempt to wait for it without having to
        // implement event subscription.

        for (int i = 0; i++ < 16; ::Sleep(250))
        {
            ComPtr<Document> candidate;
            if (FAILED(_dte->get_ActiveDocument(candidate)))
                break;

            if (candidate.get() != document.get())
                break;
        }
...
//------------------------------------------------------------------------------
const wchar_t* Dte::get_caption() const
{
    ...
    BSTR out;
    return main_window->get_Caption(&out) ? out : nullptr;
}
...
bool Dte::activate() const
{
    ...
    uintptr_t hwnd = 0;
    main_window->get_HWnd((long*)&hwnd);
...
//------------------------------------------------------------------------------
static int suspend_resume(bool is_suspend, unsigned pid)
{
    HANDLE handle = OpenProcess(PROCESS_SUSPEND_RESUME, FALSE, pid);
    if (handle == nullptr)
        return -1;

    int ret = is_suspend ? NtSuspendProcess(handle) : NtResumeProcess(handle);
...
static PyObject* suspend_resume(bool is_suspend, PyObject** args, int nargs)
{
    ...
    int ok = suspend_resume(is_suspend, pid);
...
//------------------------------------------------------------------------------
static PyObject* run_to_int3(PyObject* self, PyObject** args, Py_ssize_t nargs)
{
    if (nargs < 1)
        Py_RETURN_FALSE;
    ...
    suspend_resume(false, pid);

    int count = 2;
    for (
        DEBUG_EVENT event;
        WaitForDebugEvent(&event, 0x49e) == TRUE;
        ContinueDebugEvent(event.dwProcessId, event.dwThreadId, DBG_CONTINUE))
    {
        ...
        // There are two breakpoints, one from the attach and one from the loader.
        if (--count > 0)
            continue;

        suspend_resume(true, pid);
        break;
    }
...
//------------------------------------------------------------------------------
static PyObject* get_proc_cwd(PyObject* self, PyObject** args, Py_ssize_t nargs)
{
    if (nargs != 1 || !PyLong_Check(args[0]))
        Py_RETURN_NONE;

    unsigned int pid = PyLong_AsLong(args[0]);
    Handle proc = OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ, FALSE, pid);

    PROCESS_BASIC_INFORMATION pbi;
    NtQueryInformationProcess(proc, ProcessBasicInformation, &pbi, sizeof(pbi), nullptr);

    auto read_mem = [&] (auto addr, auto* out, int length=-1)
    {
        length = (length < 0) ? sizeof(*out) : length;
        return (ReadProcessMemory(proc, (void*)addr, out, length, nullptr) != FALSE);
    };

    uintptr_t upp_address;
    read_mem(uintptr_t(pbi.PebBaseAddress) + 0x20, &upp_address);

    UNICODE_STRING cwd_str;
    if (!read_mem(upp_address + 0x38, &cwd_str))
        Py_RETURN_NONE;

    enum { MaxSize = 512 };
    char buf[MaxSize];
    int length = cwd_str.Length;
    if (length > MaxSize)
        length = MaxSize;
    if (!read_mem(cwd_str.Buffer, buf, length))
        Py_RETURN_NONE;

    auto* cwd = (wchar_t*)buf;
    length = length / sizeof(wchar_t);
    length -= (cwd[length - 1] == '\\');
    return PyUnicode_FromWideChar(cwd, length);
}
...
static PyMethodDef dte_module_methods[] = {
    ...
    { "run_to_int3",     PyCFunction(run_to_int3),     METH_FASTCALL, "Run until NT's implicit int3s then suspends"},
    { "get_proc_cwd",    PyCFunction(get_proc_cwd),    METH_FASTCALL, "Returns CWD of a process by ID"},
    {},
};
