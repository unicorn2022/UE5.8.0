...
function generateDockerfile {
	...
	args=(
		...
		# This configures the BuildGraph flags that we use during builds:
		# 
		# - Disables Perforce logic in UAT (required if IsBuildMachine == 1)
		# - Disables building the Engine for AArch64
		# - Enables support for client and dedicated server targets
		# 
		--opt buildgraph-args='-NoP4 -set:WithLinuxArm64=false -set:WithClient=true -set:WithServer=true'
		
		# This configures the environment variables that we support securely passing from the host system into the container during builds:
		# 
		# - UBA related: IsBuildMachine, UE_HORDE_URL, UE_HORDE_TOKEN
		# - DDC related: UE_Cloud* variables
		# 
		--opt build-secret-vars='["IsBuildMachine", "UE_HORDE_URL","UE_HORDE_TOKEN","UE_CloudDataCacheAccessToken","UE_CloudDataCacheHost","UE_CloudDataCacheHttpVersion","UE_CloudDataCacheWriteOnly","UE_CloudPublishDescriptorHost","UE_CloudPublishDescriptorHttpVersion","UE_CloudPublishHost","UE_CloudPublishHttpVersion"]'
		
		# This automatically sets the Build ID so we don't need to specify it manually (requires ue4-docker version 0.0.119 or newer)
		--opt automatic-build-id=true
