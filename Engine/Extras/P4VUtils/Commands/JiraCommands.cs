...
namespace P4VUtils.Commands
{
	...
	class OpenJiraCommand : Command
	{
		...
		public override async Task<int> Execute(string[] args, IReadOnlyDictionary<string, string> configValues, ILogger logger)
		{
			...
			MatchCollection matches = Regex.Matches(description.Description, GetJiraRegexPattern(configValues), RegexOptions.Multiline);

			if(matches.Count == 0)
			{
				string message = $"Description for {changeNumber} does not contain any valid jira issue tags {GetJiraRegexHelpText(configValues)}";
		...
		private static string GetJiraRegexPattern(IReadOnlyDictionary<string, string> ConfigValues)
		{
			string? JiraRegexPattern;
			if (ConfigValues.TryGetValue("JiraRegexPattern", out JiraRegexPattern))
			{
				return JiraRegexPattern;
			}

			return "configure-jira-regex-pattern-in-p4vutils.ini";
		}

		private static string GetJiraRegexHelpText(IReadOnlyDictionary<string, string> ConfigValues)
		{
			string? JiraRegexHelp;
			if (ConfigValues.TryGetValue("JiraRegexHelp", out JiraRegexHelp))
			{
				return JiraRegexHelp;
			}
