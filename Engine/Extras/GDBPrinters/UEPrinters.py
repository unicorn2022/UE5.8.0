...
# ------------------------------------------------------------------------------
# ContainerPrinterHelper - used to wrap another container but just adjust the results slightly
#
class ContainerPrinterHelper:
	def __init__(self, container):
		self.Container = gdb.default_visualizer(container)

	def to_string(self):
		return self.Container.to_string()

	def children(self):
		return self.Container.children()

	def display_hint(self):
		return self.Container.display_hint()
...
class TBitArrayPrinter:
	...
	def get_value(self, index):
		return (self.Data[index//32] >> (index % 32) & 1) != 0
...
# ------------------------------------------------------------------------------
# TSparseSet - Printer for only sparse type maps
#
class TSparseSetPrinter(ContainerPrinterHelper):
	def __init__(self, val):
...
# ------------------------------------------------------------------------------
# TCompactSet - Printer for only compact type maps
#
class TCompactSetPrinter(TArrayPrinter):
	def __init__(self, val):
		self.Value = val
		self.ArrayNum = self.Value['NumElements']
		self.ArrayMax = self.Value['MaxElements']
		self.Data = get_allocation_data(self.Value['Elements'], self.Value.type.template_argument(0))

# ------------------------------------------------------------------------------
# TSet - Resolve to either sparse or compact depending on the definition of the type by determing what properties are available
#
class TSetPrinter(ContainerPrinterHelper):
	def __init__(self, val):
		if gdb.types.has_field(val.type, "NumElements"):
			self.Container = TCompactSetPrinter(val)
		else:
			self.Container = TSparseSetPrinter(val)

# ------------------------------------------------------------------------------
# TMap - Printer for all formats of maps
#

class TMapPrinter(ContainerPrinterHelper):
	def __init__(self, val):
...
def add_container_printers(pp):
	...
	pp.add_printer("TCompactSet", '^TCompactSet<.+>$', TCompactSetPrinter)

	if (True): # useful to remove all map matching if you need to debug set printer or below
		pp.add_printer("TMapBase", '^TMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TSortableMapBase", '^TSortableMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TMap", '^TMap<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TSparseMapBase", '^TSparseMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TSortableSparseMapBase", '^TSortableSparseMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TSparseMap", '^TSparseMap<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TCompactMapBase", '^TCompactMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TSortableCompactMapBase", '^TSortableCompactMapBase<.+,.+,.+,.+>$', TMapPrinter)
		pp.add_printer("TCompactMap", '^TCompactMap<.+,.+,.+,.+>$', TMapPrinter)
