...
class EdgeBotImpl extends PerforceStatefulBot {
	...
	get displayName() {
		return `${this.sourceNode.displayName} -> ${this.targetBranch.displayName}`
	}
	get incognitoMode() {
	...
	private async analyzeIntegrationError(errors: string[]) {
		...
		for (const err of errors) {
			...
			if (match) {
				if (paths.length < RoboArgs.exclusiveLockOpenedsToRun) {
	...
	performMerge(info: ChangeInfo, target: MergeAction, additionalDescription: string): Promise<EdgeIntegrationDetails> {
		...
		if (this.incognitoMode) {
			for (const line of info.description.split('\n')) {
				...
				if (jiraTagMatch) {
					const ueJiras = Array.from(jiraTagMatch[1].matchAll(UE_JIRA_REGEX)).map(m => m[1])

					if (ueJiras.length > 0) {
						description += `#jira ${ueJiras.join(" ")}\n`
					}
				}
				else {
					description += line + "\n"
		...
		const thisBotMergeCommands = target.furtherMerges
			...
			.map(target => (
				...
				target.mergeMode === 'null'   ? '!' :
				target.mergeMode === 'manual' ? '~' : '') + target.branchName)
		...
		for (const other of target.furtherMerges) {
			...
			if (other.otherBot) {
				const otherPrefix = other.mergeMode === 'skip'   ? '-' :
				                    other.mergeMode === 'null'   ? '!' :
				                    other.mergeMode === 'manual' ? '~' : ''
				description += `#ROBOMERGE[${other.otherBot}]: ${otherPrefix}${other.branchName}\n`
			}
		}
	...
	pause(message: string, owner: string) {
		...
		this.eventTriggers.reportPause({message, owner, 
			nodeOrEdgeName: this.displayName,
			sourceBranchUpperName: this.sourceBranch.upperName, 
	...
	private async SkipIntegration(pending: PendingChange, edgeServerAddress: string|undefined, msg: string, reason?: AlreadyIntegrated['reason']) {
		const event: AlreadyIntegrated = {change: pending.change, action: pending.action, reason}
	...
	private async integrate(info: ChangeInfo, target: MergeAction) : Promise<EdgeIntegrationDetails> {
		...
		const doVirtualMerge = 
		    ...
		    || (!info.userRequest && !info.forceCreateAShelf && target.mergeMode !== 'manual')
		...
		const [mode, results] = await this.p4.integrate(info.targetWorkspace, source, changenum, integTarget, {edgeServerAddress, virtual: doVirtualMerge, nullMerge: target.mergeMode === 'null'})
		...
		switch (mode) {
			...
			case 'no_files':
				return await this.SkipIntegration(
					pending,
					edgeServerAddress,
					`Change ${to_integrate} has no files in ${this.sourceBranch.name} that map to ${this.targetBranch.name}`,
					'no_files'
				)
		}
	...
	async _resolveChangelist(pending: PendingChange, submitRetries  = 3) : Promise<EdgeIntegrationDetails> {
		...
		let result = await this.p4.resolve(
			...
			pending.newCl,
			(pending.change.forceStompChanges || pending.action.mergeMode === 'manual') ? 'normal' : pending.action.mergeMode,
			false, // detail === 'quick',
			edgeServerAddress
		)
		...
		if (pending.action.mergeMode === 'manual' || pending.change.forceCreateAShelf) {
			// the user requested manual merge (~BranchName, #manual flag, or forceCreateAShelf)
		...
		if (result.hasConflict()) {
			...
			if (failure.additionalInfo.describeResult.entries.length > RoboArgs.maxStompFileCount) {
				failure.additionalInfo.limitedInfoMode = true
				failure.additionalInfo.describeResult = undefined
			}
		...
		else
		{
			...
			if (this.options.approval && !pending.action.flags.has('#filter_approved#')) {
			...
			else if (pending.action.flags.has('#filter_blocked#')) {

				const blockedMessage = [...pending.action.flags].filter(flag => flag.startsWith('#filter_reason_')).map(flag => flag.slice('#filter_reason_'.length))?.[0]
										|| "Blocked by filter" 

				// still notify as a 'failure' to trigger normal blockage mechanism
				failure = { kind: 'Blocked', description: blockedMessage, details: blockedMessage }
			}
			else {
	...
	public forceSetLastClWithContext(value: number, culprit: string, reason: string, unblock?: boolean, isSkip?: boolean) {
		...
		// trigger events
		this.sourceNode.onForcedLastCl(this.displayName, this.targetBranch.upperName, value, prevValue, culprit, reason, isSkip)
