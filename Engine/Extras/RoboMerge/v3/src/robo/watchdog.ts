...
class Watchdog {
	...
	constructor() {
		...
		this.statusServer = new RoboServer(args.externalUrl,
			...
			() => this.watchdogLogger.getLogTail(), () => this.getLastCrash(), () => this.stopBot(), () => this.startBot(), args.behindLoadBalancer
		)
	...
	startServer() {
		...
		if (!args.noTLS) {
			try {
				...
				console.log(err)
				this.statusServer.open(args.watchdogPort, 'http').then(() => 
					this.watchdogLogger.warn(`HTTP web server opened on port ${args.watchdogPort}`)
				)
				return
			}
		} 
		...
		const certFiles = {
			key: args.noTLS ? "" : fs.readFileSync(`${Session.VAULT_PATH}/${tlsKeyFilename}`, 'ascii'),
			cert: args.noTLS ? "" : fs.readFileSync('./certs/robomerge.pem', 'ascii')
		}
		// Removed ca: [fs.readFileSync('./certs/thwate.cer')] - JR 10/4

		this.statusServer.open(args.watchdogPort, protocol, certFiles).then(() => 
