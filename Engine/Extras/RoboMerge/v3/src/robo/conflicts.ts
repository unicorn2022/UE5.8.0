...
import { AlreadyIntegrated, Blockage, Branch, BranchArg, branchesMatch, ExclusiveLockInfo } from './branch-interfaces';
...
function buildDetailsSnapshot(blockage: Blockage): { snapshot: string[], truncated: boolean } {
	let entries: string[]
	if (blockage.failure.kind === 'Merge conflict' && blockage.failure.details) {
		entries = blockage.failure.details.split('\n').map(s => s.trim()).filter(Boolean).sort()
	}
	else if (blockage.failure.kind === 'Exclusive check-out') {
		const lockInfo = blockage.failure.additionalInfo as ExclusiveLockInfo | undefined
		entries = (lockInfo?.exclusiveFiles ?? [])
			.map(f => `${f.user.toLowerCase()}:${f.depotPath}`)
			.sort()
	}
	else {
		// Syntax error and all other kinds: snapshot the description text for change detection
		entries = blockage.failure.description ? [blockage.failure.description] : []
	}
	const truncated = entries.length > MAX_SNAPSHOT_ENTRIES
	return { snapshot: entries.slice(0, MAX_SNAPSHOT_ENTRIES), truncated }
}
...
function makePersistentConflict(blockage: Blockage): PersistentConflict {
	...
	const result: PersistentConflict = {
		...
		disableStompReason: blockage.failure.additionalInfo?.disableStompReason,
		time: blockage.time,
		nagCount: 0,
		ugsIssue: -1
	}
	...
	const { snapshot, truncated } = buildDetailsSnapshot(blockage)
	if (snapshot.length > 0) {
		result.detailsSnapshot = snapshot
		if (truncated) {
			result.detailsSnapshotTruncated = true
		}
	}
...
export class Conflicts {
	...
	updateBlockage(blockage: Blockage) {
		...
		for (let index = 0; index != this.conflicts.length; ++index) {
			...
			if (conflictMatches(conflict, replacement.sourceCl, replacement.targetBranchName)) {
				...
				// Detect if failure circumstances changed before replacing
				const oldConflict = conflict
				const kindChanged = oldConflict.kind !== replacement.kind
				// Only compare when both snapshots are present — avoids false positives after
				// a service restart or BSON-shrink that cleared the persisted snapshot
				const snapshotChanged = oldConflict.detailsSnapshot && replacement.detailsSnapshot
					? JSON.stringify(oldConflict.detailsSnapshot) !== JSON.stringify(replacement.detailsSnapshot)
					: false
				...
				if (kindChanged || snapshotChanged) {
					this.eventTriggers.reportBlockageStatusChanged({ previousConflict: oldConflict, newBlockage: blockage })
				}
				return
			}
		}
	...
	checkForResolvedConflicts(nodeCL: number, edges: Map<string, EdgeBot>) {
		...
		for (const conflict of this.conflicts) {
			...
			if (!conflict.targetBranchName ) {
				...
				else if (conflict.kind == 'Syntax error' && [...edges.values()].some(edgeBot => edgeBot.lastCl < conflict.cl)) {
					remainingConflicts.push(conflict)
					continue
				}
			}
	...
	// If a forced CL is the same as a conflict CL, the conflict is skipped.
	// Otherwise we'll notify that the branch has been forced to a CL, unless the
	// caller flagged this as a skip (in which case the conflict was already gone).
	onForcedLastCl(forced: ForcedCl) {
		this.eventTriggers.reportForcedLastCl(forced)
		if (!this.notifyConflictSkipped(forced) && !forced.isSkip) {
	...
	private notifyConflictSkipped(forced: ForcedCl) {
		const isForward = forced.forcedCl > forced.previousCl
		const isBackward = forced.forcedCl < forced.previousCl

		const remainingConflicts: PersistentConflict[] = []
		const skippedToReport: PersistentConflict[] = []
		const cancelledToReport: PersistentConflict[] = []

		for (const conflict of this.conflicts) {
			const onSameEdge = conflict.blockedBranchName === forced.sourceBranchUpperName &&
				conflict.targetBranchName === forced.targetBranchUpperName
			let resolution: Resolution = Resolution.DUNNO

			if (conflict.cl === forced.forcedCl) {
				resolution = Resolution.SKIPPED
				conflict.resolvingReason = forced.reason
				skippedToReport.push(conflict)
			}
			else if (onSameEdge) {
				if (isForward && conflict.cl < forced.forcedCl) {
					resolution = Resolution.CANCELLED
					conflict.resolvingReason = 'Bot position was advanced past this CL'
					cancelledToReport.push(conflict)
				}
				else if (isBackward && conflict.cl > forced.forcedCl) {
					resolution = Resolution.CANCELLED
					conflict.resolvingReason = 'Bot position was reset to an earlier CL'
					cancelledToReport.push(conflict)
				}
			}
			if (resolution != Resolution.DUNNO) {
		...
		if (skippedToReport.length === 0 && cancelledToReport.length === 0) {
			return false
		}
		...
		for (const unblock of skippedToReport) {
			this.conflictLogger.info('Reporting unblocked (skipped) ' + unblock.cl)
			this.reportUnblocked(unblock)
		}
		for (const unblock of cancelledToReport) {
			this.conflictLogger.info('Reporting unblocked (cancelled) ' + unblock.cl)
			this.reportUnblocked(unblock)
		}

		// Only suppress reportNonSkipLastClChange when a conflict was explicitly skipped
		return skippedToReport.length > 0
	}
	...
	async applyStatus(conflictsForStatus: Partial<ConflictStatusFields>[], slackMessages?: SlackMessages) {
		for (const conflict of this.conflicts) {
			...
			conflictObj.disableStompReason = conflict.disableStompReason
