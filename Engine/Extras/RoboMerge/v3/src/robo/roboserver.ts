...
export class RoboServer {
	...
	constructor(externalUrl: string, sendMessage: (msg: string, args?: any[]) => any, getLogTail: Function | null, 
		getLastCrash: Function, stopBot: Function, startBot: Function, behindLoadBalancer?: boolean) {
		this.server = new WebServer(this.roboserverLogger, behindLoadBalancer)
		this.functions = {
...
class RoboWebApp implements AppInterface {
	...
	getTrackedChanges() {
		...
		return this.sendMessage('trackChange', [this.getQueryFromSecure(), tagsObj])
	}
	...
	// Example usage:
	// curl -X POST -k -v --cookie "auth=AUTH_COOKIE_FROM_BROWSER" https://robomerge.companyname.net/api/test/directmessage/user
...
// Raw operation parameters for building Slack interactive button values
export interface BlockageNodeOpParams {
	bot: string
	branch: string
	cl: number
	// The robomerge instance that should handle this action (its externalUrl)
	host: string
	// For acknowledge and skip (the edge/target branch name)
	edge?: string
	// For createShelf, stomp, unlock (the target branch name — same value as edge)
	target?: string
	// For createShelf (the target branch stream, used for workspace lookup)
	targetStream?: string
}
...
export interface BlockageNodeOpUrls {
	...
	stompUrl?: string
	unlockUrl?: string
	// Raw params for Slack interactive buttons
	params?: BlockageNodeOpParams
}
