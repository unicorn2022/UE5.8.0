...
import { Change, P4_FORCE, PerforceContext, Workspace } from '../common/perforce';
...
export class AutoBranchUpdater implements Bot {
	...
	constructor(private graphBot: GraphBot, parentLogger: ContextualLogger, private readonly previewMode: boolean) {
		...
		this.prevLastCl = AutoBranchUpdater.initialCl
	...
	async _tryReloadBranchDefs() {
		...
		try {
			...
			postToRobomergeAlerts(`ERROR: failed to reload branch specs file ${this.graphBot.filename}`)
			return
		}
		...
		if (!result.branchGraphDef) {
			const slackMessages = this.graphBot.getSlackMessages()
			let changes: Change[] = []
			if (slackMessages && this.prevLastCl != this.lastCl) {
				changes = await this.p4.changes(this.filePath, {since: this.prevLastCl+1, until: this.lastCl})
			}
			let errText = `Failed to parse/validate branch specs file for ${botname}`
			if (changes.length > 0) {
				const uniqueUsers = changes.reduce((map, change) => {
					if (!map.has(change.user)) {
						map.set(change.user, {
							changes: [],
							emailPromise: this.p4.getEmail(change.user)
						})
					}
					map.get(change.user)!.changes.push(change)
					return map
				}, new Map<string, { changes: Change[], emailPromise: Promise<any> }>())

				await Promise.all(
					Array.from(uniqueUsers.values())
						 .map(userInfo => userInfo.emailPromise
						 .then(email => email ? slackMessages!.addUserToChannel(email, RoboArgs.slackAlertChannel) : undefined)))

				const userMessages = await Promise.all(
					Array.from(uniqueUsers.entries()).map(async ([username, userInfo]) => {
						const pingTarget = await slackMessages!.getPingTarget(userInfo.emailPromise, username)
						const changeList = userInfo.changes.map(change => `<${this.p4.swarmURL}/changes/${change.change}|CL ${change.change}>`).join(', ')
						return `${pingTarget} (${changeList})`
					})
				)
				errText += '\nPossibly ' + userMessages.join(', ')
			}
			const validationErrorText = result.validationErrors.join('\n')
			const file: SlackFile = {
				content: validationErrorText,
				channels: RoboArgs.slackAlertChannel,
				filename: "branchspecerrors.txt"
			}
			postToRobomergeAlerts(errText, [file])
			this.abuLogger.error(`${errText}\n${validationErrorText}`.trim())
			return
		}
