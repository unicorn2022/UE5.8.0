...
import { notificationsInit, postToRobomergeAlerts } from './notifications'
import { setProcessAlertCallback } from '../common/helper'
...
const COMMAND_LINE_ARGS: {[param: string]: Arg<any>} = {
	...
	behindLoadBalancer: {
		match: /^(-behindLoadBalancer)$/,
		parse: str => str === "false" ? false : true,
		env: 'ROBO_BEHIND_LOADBALANCER',
		dflt: false
	},
	...
	maxStompFileCount: {
		match: /^-maxStompFileCount=([0-9]+)$/,
		env: 'ROBO_MAX_STOMP_FILE_COUNT',
		parse: (str: string) => {
			return parseInt(str)
		},
		dflt: 20000
	...
	maxReconsiderFiles: {
		match: /^-maxReconsiderFiles=([0-9]+)$/,
		env: 'ROBO_MAX_RECONSIDER_FILES',
		parse: (str: string) => {
			return parseInt(str)
		},
		dflt: 50000
	...
	slackAlertChannel: {
		match: /^-slackAlertChannel=(.+)$/,
		env: 'ROBO_SLACK_ALERT_CHANNEL',
		dflt: 'C044ZUSS71S' // #dt-robomerge-alert-ext
	},
	mongoDB_URI: {
		match: /^-mongoDB_URI=(.+)$/,
	...
	alwaysPersistToFile: {
		match: /^(-alwaysPersistToFile)$/,
		parse: str => str === "false" ? false : true,
		env: 'ROBO_ALWAYS_PERSIST_TO_FILE',
		dflt: false
	},
	persistenceDir: {
	...
	webserverPort: {
		match: /^-webServerPort=(\d+)$/,
		env: 'ROBO_WEB_SERVER_PORT',
		parse: (str: string) => {
...
setProcessAlertCallback((msg) => postToRobomergeAlerts(msg))
...
if (RoboArgs.useMongo) {
	...
	mongoose.connection.on('connected', () => {
		roboStartupLogger.info(`MongoDB Connected`)
		doDBPersistedMessagesHousekeeping()
	})
	mongoose.connection.on('error', (err: any) => {
		roboStartupLogger.error(`MongoDB Connection Error: ${err}`)
		process.exit(1);
	})
}
...
async function init(logger: ContextualLogger) {
	...
	if (args.branchSpecsRootPath) {
		...
		// Ensure we have a workspace for branch specs and that it points to the correct root path
		const workspace = await p4.find_workspace_by_name(args.branchSpecsWorkspace, {includeUnloaded: true})
		if (workspace.length === 1) {
			if (!args.devMode) {
				await p4.deleteWorkspace(args.branchSpecsWorkspace)
			}
		}
		if (!args.devMode || workspace.length === 0) {
	...
	if (args.persistenceBackupFrequency > 0 && !args.previewOnly) {
		const workspace = await p4.find_workspace_by_name(args.persistenceBackupWorkspace, {includeUnloaded: true})
		...
		else if (workspace[0].IsUnloaded) {
			await p4.reloadWorkspace(args.persistenceBackupWorkspace)
		}
	} else {
...
if (!args.noIPC) {
	...
	const sendMessage = (name: string, args?: any[]) => ipc.handle({name:name, args:args}).catch(err => roboStartupLogger.error(`IPC error! ${err.toString()}\n${err.stack}`));
	...
	const ws = new RoboServer(args.externalUrl, sendMessage, () => roboStartupLogger.getLogTail(),
		...
		args.behindLoadBalancer);

	if (args.noTLS) {
		roboStartupLogger.info(`Server will start in self-non-secure mode...`)
	} else {
		roboStartupLogger.info(`Server will start in self-secure mode...`)
	}
	...
	const port = args.webserverPort != -1 ? args.webserverPort : args.noTLS ? 8877 : 4433
...
async function main(logger: ContextualLogger) {
	while (true) {
		try {
			await initializePerforce(logger, RoboArgs.vault, args.devMode);
			break;
		}
		catch (err) {
	...
	const sendMessage = (name: string, args?: any[]) => ipc.handle({name, args}).catch((err: any) => logger.error(`IPC error! ${err.toString()}\n${err.stack}`))
	if (RoboArgs.hasSocketMode) {
		logger.info(`Starting Slack Socket Mode (externalUrl='${args.externalUrl}', botToken=${RoboArgs.vault.slackTokens?.bot ? 'present' : 'MISSING'})`)
		const socket = new SlackSocket(RoboArgs.vault.slackAppToken!, args.externalUrl, {
			sendMessage,
			botToken: RoboArgs.vault.slackTokens?.bot,
			slackDomain: args.slackDomain,
			logger
		})
		socket.start()
	}
	else {
		logger.info('Slack Socket Mode disabled (slackAppToken not present in vault)')
	}
}
