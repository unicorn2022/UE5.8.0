...
export class Gate {
	...
	applyStatus(outStatus: { [key: string]: any }) {
		...
		if (mostRecentGate) {
			...
			outStatus.lastGoodCLPath = this.context.options.lastGoodCLPath
	...
	async setGateCl(value: number, culprit: string, reason: string) {
		...
		const workspaceQueryResult = await p4.find_workspace_by_name(workspace.name, {includeUnloaded: true})
		if (workspaceQueryResult.length === 1) {
			await p4.deleteWorkspace(workspace.name)
		}
...
// log stats here for now, but if it works out, use the type in the event
	...
	private loadFromPersistence() {
		...
		if (saved) {
			...
			if (saved.current) {
				...
				if (this.lastCl < saved.current.cl) {
					this.reportCatchingUp()
				}
			}
	...
	setEdgeLastCl(value: number) {
		this.eventContext.edgeLastCl = value
	}
