...
import { changelistFilters } from './targets'
...
const FILTER_MODES = ['AllowOrBlock', 'AllowOrSkip'] as const
export type FilterModeType = typeof FILTER_MODES[number]

function toFilterMode(value: string): FilterModeType {
  if ((FILTER_MODES as readonly string[]).includes(value)) {
    return value as FilterModeType;
  }
...
export class BranchDefs {
	...
	static async parseAndValidate(logger: ContextualLogger, branchSpecsText: string, isPreviewing: boolean = false): Promise<ParseResult> {
		...
		if (branchGraph.edges) {
			...
			if (edge.changelistFilter) {
				try {
					await import(`./changelistfilters/${edge.changelistFilter.name}`)
					const changelistFilter = changelistFilters.get(edge.changelistFilter.name)
					if (changelistFilter) {
						const [cfErrors, cfWarnings] = await changelistFilter.validate(branchGraph.branches, edge, isPreviewing, logger)
						cfErrors.forEach(err => validationErrors.push(`Custom filter '${edge.changelistFilter!.name}' for edge ${edge.from}->${edge.to}: ${err}`))
						cfWarnings.forEach(warn => validationWarnings.push(`Custom filter '${edge.changelistFilter!.name}' for edge ${edge.from}->${edge.to}: ${warn}`))
					}
					else {
						validationErrors.push(`Edge ${edge.from}->${edge.to} specifies custom filter '${edge.changelistFilter.name}' but it is not found after import`)
					}
					try {
						toFilterMode(edge.changelistFilter.mode)
					}
					catch {
						validationErrors.push(`Edge ${edge.from}->${edge.to} specifies invalid filter mode '${edge.changelistFilter.mode}'`)
					}
				}
				catch {
					validationErrors.push(`Edge ${edge.from}->${edge.to} specifies custom filter '${edge.changelistFilter.name}' but unable to import it`)
				}
			}
