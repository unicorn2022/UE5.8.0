...
import { BranchGraphInterface, ChangeInfo, PendingChange, Target, TargetInfo } from './branch-interfaces';
...
export function getIntegrationOwner(arg0: Branch | PendingChange, sourceBranch?: Branch, overriddenOwner?: string) {
	...
	if (pending) {
		if (pending.action.mergeMode === 'manual' ||
...
export class DescriptionParser {
	...
	override(other: DescriptionParser) {
		...
		this.hasDefaultBotDirective = other.hasDefaultBotDirective
	}
	...
	append(other: DescriptionParser) {
		...
		this.hasDefaultBotDirective = this.hasDefaultBotDirective || other.hasDefaultBotDirective
	}
			...
			for (const command of commands) {
				...
				if (!command.bot && !isFlag(command.param)) {
					if (this.isDefaultBot) {
						...
						this.hasDefaultBotDirective = true
						continue
					}
				}
...
export type ChangelistFilterResult = {
	result: 'allow' | 'skip' | 'block',
	message?: string
}
export class ChangelistFilter {

	static async shouldFilter(_info: ChangeInfo, _action: MergeAction, _edgeProperties: EdgeOptions, _logger: ContextualLogger) : Promise<ChangelistFilterResult> {
		return {result: 'block'}
	}

	// Validation function returns a tuple of an array each for errors and warnings
	static async validate(_branches: NodeOptions[], _edge: EdgeProperties, _isPreviewing: boolean, _logger: ContextualLogger) : Promise<[string[],string[]]> {
		return [[],[]]
	}
}

export const changelistFilters = new Map<string, typeof ChangelistFilter>()
...
class RequestedIntegrations {
	...
	parse(commandArguments: Readonly<string[]>, cl: number, logger: ContextualLogger) {
		for (const arg of commandArguments) {
			...
			case '~':
				this.integrations.push([arg.substring(1), 'manual'])
				break
...
// note: this allows multiple mentions of same branch, with flag of last mention taking priority. Could be an error instead
	...
	// handle multiple routes to manual targets
	const manualTargets: Branch[] = [...requestedMerges]
		.filter(([_, mergeMode]) => mergeMode === 'manual')
		.map(([branchName, _]) => branchGraph.getBranch(branchName)!)
		.filter(Boolean)
	...
	for (const [initialEdge, furtherEdges] of integrations) {
		...
		for (const manualTarget of manualTargets) {
			if (furtherEdgeBranches.indexOf(manualTarget) < 0
				&&
				[...furtherEdgeBranches, targetBranch]
					.map(b => b.forcedDownstream)
					.filter(ds => (ds || []).map(b => branchGraph.getBranch(b)).indexOf(manualTarget) >= 0)
					.length > 0
				) {
				furtherEdgeBranches.push(manualTarget)
			}
		}
...
export function computeTargets(
	...
	for (const [direct, indirectTargets] of merges.entries()) {
		const mergeMode : MergeMode = targets.get(direct) || (flags.has('manual') ? 'manual' : 'normal')
		if (mergeMode === 'skip') {
			continue
		}
