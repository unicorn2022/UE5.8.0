...
import { changelistFilters, computeTargets, descriptionParsers, parseDescription, processOtherBotTargets, getIntegrationOwner, getNodeBotFullName, getNodeBotFullNameForLogging } from './targets';
...
type InfoResult = { info?: ChangeInfo, errors: any[], defer?: boolean }
...
export class NodeBot extends PerforceStatefulBot implements NodeBotInterface {
	...
	private deferredCl: number | null = null
	...
	private async getChanges(startingCl: number) {
		...
		try {
			if (startingCl <= 0) {
				...
				return await this.p4.changes(path, {since: startingCl})
			}
		}
		catch (err) {
	...
	simplified: (n²/2 + n/2 + 3)/2 = (n² + n + 6)/4
		...
		if (changes.length === 0) {
			...
			if (!this.isAvailable) {
				...
				await Promise.all(Array.from(availableEdgesAtCL.entries()).map(async ([minCL,edgesAtCL]) => this._processListOfChanges(edgesAtCL, minCL, changes)));
			}
		}
		this.sendNagNotifications()
		return true
	...
	async processQueuedChange(fromQueue: QueuedChange) {
		...
		if (fromQueue.targetBranchName) {
			...
			if (!specifiedTargetBranch) {
				await this.sendErrorNotificationToUser(
					fromQueue.who,
					fromQueue.cl,
		...
		if (typeof changeResult === 'string') {
			const clUnknown = /Change \d+ unknown/i.test(changeResult)

			let reconsiderSubject: string
			...
			if (clUnknown) {
				reconsiderSubject = `Reconsider Request Failed -- CL Does Not Exist`
				reconsiderErrorMessage += `\n\nChange ${fromQueue.cl} does not exist in Perforce.`
			}
			else {
				reconsiderSubject = `Reconsider Request Failed -- Error Querying Perforce`
		...
		// Check if the change exceeds the maximum file limit for reconsider requests
		if (RoboArgs.maxReconsiderFiles > 0) {
			try {
				const sizeResult = await this.p4.sizes(`${this.branch.rootPath}@=${change.change}`, true)
				const fileCount = sizeResult[0].fileCount

				if (fileCount > RoboArgs.maxReconsiderFiles) {
					const limitExceededMessage = `Reconsider request for CL ${fromQueue.cl} on ${this.fullName} exceeds the maximum file limit.\n\n` +
						`This change contains ${fileCount} file(s), but the maximum allowed for reconsider is ${RoboArgs.maxReconsiderFiles}.\n\n` +
						`Please perform this merge manually instead of using the reconsider feature.`

					await this.sendErrorNotificationToUser(
						fromQueue.who,
						fromQueue.cl,
						`Reconsider Request Rejected -- Too Many Files`,
						limitExceededMessage
					)

					this.nodeBotLogger.warn(`${this.fullName} - Reconsider request for CL ${fromQueue.cl} rejected: ${fileCount} files exceeds limit of ${RoboArgs.maxReconsiderFiles}`)
					return
				}
			}
			catch (err) {
				const errorMessage = `Reconsider request for CL ${fromQueue.cl} on ${this.fullName} could not be processed.\n\n` +
					`Failed to check file count: ${err}\n\n` +
					`Please contact your Robomerge administrator or perform this merge manually.`

				await this.sendErrorNotificationToUser(
					fromQueue.who,
					fromQueue.cl,
					`Reconsider Request Failed -- Unable to Verify File Count`,
					errorMessage
				)

				this.nodeBotLogger.error(`${this.fullName} - Failed to check file count for CL ${fromQueue.cl}: ${err}`)
				return
			}
		}
		...
		const mergeResult = await this._processAndMergeCl(new Map<string, EdgeBot>(this.edges), change, true, fromQueue.who, fromQueue.workspace, specifiedTargetBranch)
		if (mergeResult.errors.length > 0 && change.isUserRequest) {
			await this.sendErrorNotificationToUser(
				fromQueue.who,
				fromQueue.cl,
				`Reconsider Request Failed -- No Matching Targets`,
				`While processing manually queued change ${fromQueue.cl} on ${this.fullName}: no targets were found to integrate to.\n\n` +
				`If you specified a #robomerge directive without a bot qualifier, try qualifying it explicitly: ` +
				`#robomerge[${this.graphBotName}] <target>`
			)
		}
		return
	}
	...
	pause(message: string, owner: string) {
		...
		super.pause(message, owner)
		this.eventTriggers.reportPause({ 
			message,
			owner, 
			nodeOrEdgeName: this.displayName,
			sourceBranchUpperName: this.branch.upperName, 
			targetBranchUpperName: this.branch.upperName
		})
	}
	...
	unpause(requester: string) {
		...
		super.unpause(requester)
		this.eventTriggers.reportUnpause({
			owner: requester, 
			nodeOrEdgeName: this.displayName,
			sourceBranchUpperName: this.branch.upperName, 
			targetBranchUpperName: this.branch.upperName
	...
	onForcedLastCl(nodeOrEdgeName: string, targetBranchUpperName: string, forcedCl: number, previousCl: number, culprit: string, reason: string, isSkip?: boolean) {
		this.conflicts.onForcedLastCl({nodeOrEdgeName, sourceBranchUpperName: this.branch.upperName, targetBranchUpperName, forcedCl, previousCl, culprit, reason, isSkip})
	}
	...
	{
		try {
			// Check for limited info mode first
			if (mergeConflictInfo.limitedInfoMode) {
				return await this._evaluateConflictForStompLimitedMode(changeCl, mergeConflictInfo)
			}

			if (!mergeConflictInfo.describeResult) {
				return this.stompFailureResult(mergeConflictInfo.disableStompReason || "DescribeResult missing for unknown reason")
			}
			...
			if (!remainingAllBinary) {
				...
				returnResult.requestResult = 'Invalid'
			}
				...
				returnResult.requestResult = 'Valid'
	...
	private async _evaluateConflictForStompLimitedMode(changeCl: number, mergeConflictInfo: MergeConflictAdditionalInformation): Promise<StompVerification> {
		try {
			// Get resolve result to check for conflicts
			const resolveResult = Object.assign(new ResolveResultAdditionalInfo(), mergeConflictInfo.resolveResult)
			...
			if (!mergeConflictInfo.client) {
				return this.stompFailureResult(`Client workspace information missing for CL ${changeCl}`)
			}
			...
			let remainingAllBinary = true
			for (let i = 0; i < remainingFiles.length && remainingAllBinary; i += BATCH_SIZE) {
				const batch = remainingFiles.slice(i, i + BATCH_SIZE)			
				const clientFilePaths = batch.map(f => f.clientFile)
				const whereResults = await this.p4.where(mergeConflictInfo.client!, clientFilePaths)
				const depotPaths = whereResults.map(r => r.depotFile).filter(f => f) as string[]
				const fstatResults = await this.p4.fstat(mergeConflictInfo.client!, depotPaths, true)
				
				// Check each result directly
				for (const fstatResult of fstatResults) {
					const stompable = fstatResult.headType.startsWith('binary') || isStompableNonBinary(fstatResult.depotFile)
					if (!stompable) {
						remainingAllBinary = false
						break
					}
			...
			return {
				success: true,
				requestResult: remainingAllBinary ? 'LimitedInfo' : 'Invalid',
				remainingAllBinary,
				message: remainingAllBinary
					? "Valid Request (Limited Information Mode - Changelist exceeds file count limit)"
					: "Invalid Request: Not all remaining files are binary",
				svFiles: [],  // Empty - no detailed file info
				nonBinaryFilesResolved: false,  // Unknown in limited mode
				swarmURL: this.p4.swarmURL
			}
		} catch (error) {
			return this.stompFailureResult(`Encountered error processing ${changeCl} in limited mode: ${error}`)
		}
	}
	...
	async stompChanges(owner: string, changeCl: number, targetBranchName: string): Promise<OperationResult> {
		...
		if (verifyResult.requestResult === 'LimitedInfo') {
			this.nodeBotLogger.warn(`Stomp in limited mode: CL ${changeCl} -> ${targetBranchName}`)
			description = `\nStomp Operation performed without detailed revision tracking due to large changelist size.\n` +
						 `Authors of stomped changes were not automatically identified. Please verify affected changes manually.`
		}
		else {
			// Normal mode: build #fyi from stomped revisions
			const fyiSet = new Set<string>([owner.toLowerCase()])
	...
	async verifyUnlock(changeCl: number, targetBranchName: string) : Promise<UnlockVerification> {
		...
		if (lockedFiles.length > RoboArgs.exclusiveLockOpenedsToRun) {
			lockedFiles = [...lockedFiles.slice(0,RoboArgs.exclusiveLockOpenedsToRun), ...(await this.p4.getOpenedDetails(lockedFiles.slice(RoboArgs.exclusiveLockOpenedsToRun)))]
		}
	...
	async applyStatus(status: BranchStatus) {
		...
		if (this.deferredCl !== null) {
			status.deferred_cl = this.deferredCl
		}
	...
	getBlockageUrls(blockage: Blockage, hasSocketMode = false, skipReason?: string) {
		...
		return NodeBot.getBlockageUrls(blockage, externalUrl, botname, branchname, cl, disallowSkip, hasSocketMode, skipReason)
	}

	static getBlockageUrls(blockage: Blockage, externalUrl: string, botname: string, branchname: string, cl: string, disallowSkip: boolean, hasSocketMode = false, skipReason?: string) {
		const actionBranchName = blockage.action ? blockage.action.branch.name : undefined
		let urls : BlockageNodeOpUrls = {
			...
			params: hasSocketMode ? {
				bot: botname,
				branch: branchname,
				cl: parseInt(cl, 10),
				host: externalUrl,
				edge: actionBranchName,
				target: actionBranchName,
				targetStream: blockage.action ? blockage.action.branch.stream : undefined
			} : undefined
		}
	...
	onAlreadyIntegrated(event: AlreadyIntegrated) {
		...
		if (event.change.userRequest && this.slackMessages) {
			let message: string
			const swarmLink = `<${this.p4.swarmURL}/changes/${event.change.cl}|Change ${event.change.cl}>`
			if (event.reason === 'no_files') {
				message = `${swarmLink} contains no files in ${event.change.branch.name} that map to ${event.action.branch.name}. Verify the changelist is from the correct stream.`
			}
			else {
				message = `${swarmLink} was not necessary in ${event.action.branch.name}`
			}
			let dm: SlackMessage = {
				text: message,
...
`)
	...
	async notifyShelfRequester(pendingChange: PendingChange) {
		...
		let showStillBlockedMessage = pendingChange.change.forceCreateAShelf && pendingChange.action.mergeMode !== 'manual'
	...
	private async sendErrorNotificationToUser(who: string, cl: number, emailSubject: string, messageText: string) {
		if (this.slackMessages) {
			...
			let emailAddress = await this.findEmail(who)
			this.slackMessages.postDM(emailAddress, cl, this.branch, dm)
		}
		else {
			this.sendErrorEmail(
				new Recipients(who),
	...
	private async _processListOfChanges(availableEdges: Map<string, EdgeBot>, minCL: number, changes: Change[]) {
		...
		for (let changeIndex = 0; changeIndex < changes.length; ++changeIndex) {
			...
			if (changeResult.deferred) {
				this.deferredCl = change.change
				return
			}

			// Exit immediately on syntax errors or merge conflicts
			if (this.isBlocked) {
				this.deferredCl = null
				return
			}
	...
	private async _processAndMergeCl(
		...
		const isManualChange = numTargets > 0 ? result.info.targets![0].mergeMode === 'manual' : false;
	...
	private async _createChangeInfo(
		...
		// parse the change
		const info : ChangeInfo = await this.parseChange(change, optTargetBranch)

		if (info.defer) {
			return { errors: [], defer: true }
		}
...
<tr>
	...
	private async parseChange(change: Change, optTargetBranch?: Branch | null): Promise<ChangeInfo> {
		...
		if (commandOverride) {
			...
			// When commandOverride comes from the reconsider box (a user request), the user is
			// explicitly targeting this bot, so bare #robomerge directives should be accepted
			// regardless of whether this is the default bot for the source node.
			const parsedOverride = parseDescription(commandOverride, {lineEnd: '|', ...parseDescCommonParams, isDefaultBot: parseDescCommonParams.isDefaultBot || !!change.isUserRequest})
		...
		// For non-default bots: if default bot directives were skipped, gate on default bot state
		if (!this.branch.isDefaultBot && parsedLines.hasDefaultBotDirective && !commandOverride) {
			const state = this._checkDefaultBotState(change.change)
			if (state === 'no-default-bot') {
				info.errors = [`Default bot #ROBOMERGE directive found but this node has no default bot — use #ROBOMERGE[${this.graphBotName}] to target this bot explicitly`]
				return info
			}
			if (state === 'defer') {
				info.defer = true
				...
				if (computedMergeActions.length > 0) {
					...
					for (const action of computedMergeActions) {
						...
						if (edgeProperties) {
							...
							const authorIsExcluded = !info.userRequest && edgeProperties.excludeAuthors && edgeProperties.excludeAuthors.indexOf(info.author) >= 0
							...
							if (edgeProperties.excludeDescriptions && !info.userRequest) {
							...
							if (edgeProperties.changelistFilter) {
								const changelistFilter = changelistFilters.get(edgeProperties.changelistFilter.name)
								if (changelistFilter) {
									const filterResult = await changelistFilter.shouldFilter(info, action, edgeProperties, this.nodeBotLogger)
									switch(filterResult.result) {
										case 'allow':
											action.flags.add('#filter_approved#')
											break
										case 'block':
											action.flags.add('#filter_blocked#')
											break
										case 'skip':
											continue
									}
									if (filterResult.message) {
										action.flags.add(`#filter_reason_${filterResult.message}`)
									}
								}
							}
						}
...
// recreate target list, excluding 
	...
	private _checkDefaultBotState(cl: number): 'defer' | 'no-default-bot' | 'proceed' {
		// Find default bots monitoring the same stream as this non-default bot.
		// We cannot rely on 'general'-flagged edges because a default bot with flowsTo:[]
		// creates no edges at all, so we iterate all known branch graphs instead.
		const myStream = this.branch.stream
		const seenGraphs = new Set<BranchGraphInterface>()
		let foundDefaultBot = false

		for (const branchGraph of this.ubergraph.graph.branchGraphAliases.values()) {
			if (branchGraph === this.branchGraph) continue     // skip our own bot
			if (seenGraphs.has(branchGraph)) continue          // aliases may point to same graph
			seenGraphs.add(branchGraph)

			for (const branch of branchGraph.branches) {
				if (branch.stream !== myStream || !branch.isDefaultBot) continue

				foundDefaultBot = true
				const defaultNodeBot = branch.bot
				if (!defaultNodeBot) continue

				// Default bot hasn't processed this CL yet (including if blocked on a syntax error) — defer
				if (defaultNodeBot.lastCl < cl) {
					this.nodeBotLogger.info(`CL ${cl}: deferring — default bot (${branchGraph.botname}) has not yet processed this CL (lastCl=${defaultNodeBot.lastCl})`)
					return 'defer'
				}
			}
		}

		return foundDefaultBot ? 'proceed' : 'no-default-bot'
	}
	...
	private async handleSyntaxError(originalCommitMessage: string, change: ChangeInfo) {
		...
		if (change.userRequest) {
			...
			if (!existingBlockage) {
				const ownerEmail = this.p4.getEmail(owner)
				const triager = this.getBranchProperty("triager")
				const triagerEmail = triager && this.p4.getEmail(triager)
				this.conflicts.onBlockage({action: null, change, failure: {kind: 'Syntax error', description: message}, owner,
					ownerEmail, triager, triagerEmail, time: blockageOccurred})
			}
	...
	async handleMergeFailure(failure: Failure, pending: PendingChange, postIntegrate?: boolean) {
		...
		if (pending.change.userRequest) {
			...
			const clLink = `<${this.p4.swarmURL}/changes/${pending.change.cl}|Change ${pending.change.cl}>`
			let message = `While reconsidering ${clLink}`
	...
	findOrCreateBlockage(failure: Failure, pending: PendingChange, shortMessage: string, approval?: {settings: ApprovalOptions, shelfCl: number}) {
		...
		const ownerEmail = this.p4.getEmail(owner)
		const triager = this.getBranchProperty({targetBranchName: pending.action.branch.upperName, property: "triager"})
		const triagerEmail = triager && this.p4.getEmail(triager)
		const blockage: Blockage = {change: pending.change, action: pending.action, failure, owner, ownerEmail, triager, triagerEmail, time: blockageOccurred, approval}
	...
	async reportApprovalRequired(approval: ApprovalOptions, pending: PendingChange) {
		if (this.slackMessages) {
			...
			const opts: SlackMessage = {
				...
				cl: pending.change.cl,
				target: pending.action.branch.upperName
			}

			this.slackMessages.postNonConflictMessage(opts)
		}
	}
	private getBranchProperty(property: string): any
	private getBranchProperty(args: {targetBranchName?: string, property: string}): any
	private getBranchProperty(property_or_args: string | {targetBranchName?: string, property: string}) {
		const isArgs = typeof(property_or_args) !== 'string'
		const {targetBranchName, property} = (isArgs ? property_or_args : {targetBranchName: undefined, property: property_or_args})
		...
		if (targetBranchName) {
			const targetBranch = this.branchGraph.getBranch(targetBranchName)
			if (targetBranch && targetBranch.config[nodeKey] !== undefined)
			{
				prop = targetBranch.config[nodeKey]
			}

			const edgeProperties = this.branch.edgeProperties.get(targetBranchName)
	...
	private async sendNagNotifications() {
		...
		for (const conflict of this.conflicts.getConflicts()) {
			...
			const getNagProperty = (property: string) => {
				return this.getBranchProperty({targetBranchName: conflict.targetBranchName, property})
			}
			...
			if (nagWhenBlocked) {
				...
				if (this.slackMessages) {
					...
					if (nagDelay > nagWait && !giveLeeway) {
						...
						naggedSomeone = true
						const triager = getNagProperty("triager")
						this.eventTriggers.nagBlockage({
							sourceBranch: this.branch,
							conflict,
							triager,
							triagerEmail: triager && this.findEmail(triager),
							ownerEmail: this.findEmail(conflict.owner)
						})
					}
				}
