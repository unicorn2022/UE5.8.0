...
function isBotVisibleToUser(botName: string, graph: Graph, userTags: Set<string>, logger: ContextualLogger) {
    const bg = graph.branchGraphAliases.get(botName.toUpperCase())
    return bg ? Status.includeBranch(bg.config.visibility, userTags, logger) : false
}

function buildReconsiderInfo(node: Node, graph: Graph, userTags: Set<string>, botFilter: string[], logger: ContextualLogger) {
    const byBot = new Map<string, {node: string, targets: {edgeName: string, stream: string}[]}>()
    for (const edge of graph.getEdgesBySource(node)) {
        const nodeBot = edge.sourceAnnotation as NodeBotInterface
        const botName = edge.bot as string
        if (botFilter.length > 0 && !botFilter.includes(botName)) continue

        // Use the authoritative branchGraphAliases registry to check source bot visibility
        if (!isBotVisibleToUser(botName, graph, userTags, logger)) continue

        // Also check the target stream is visible: it must appear as a source in at least
        // one visible bot. If the target has no outgoing edges (leaf node) we allow it
        // through since the source bot visibility check above is sufficient.
        const targetOutEdges = [...graph.getEdgesBySource(edge.target)]
        if (targetOutEdges.length > 0) {
            const targetVisible = targetOutEdges.some(te =>
                isBotVisibleToUser(te.bot as string, graph, userTags, logger))
            if (!targetVisible) continue
        }

        if (!byBot.has(botName)) {
            byBot.set(botName, {node: nodeBot.branch.name, targets: []})
        }
        byBot.get(botName)!.targets.push({
            edgeName: edge.targetName,
            stream: edge.target.stream as string
        })
    }
    if (byBot.size === 0) return undefined
    return [...byBot.entries()].map(([bot, info]) => ({bot, node: info.node, targets: info.targets}))
}
...
export async function trackChange(robo: RoboMerge, logger: ContextualLogger, initialCL: TrackedCL, userTags: Set<string>, filters: TrackFilters) {
    ...
    const getMoreDescEntries = async (p4: PerforceContext, clToConsider: TrackedCL, desc: DescribeResult) => {
        ...
        if (desc.path.endsWith("/...")) {
            ...
            const filesPromises = [p4.files(rootDir, FILES_PER_DIR).catch(() => { return []})]
    ...
    const considerCL = async (clToConsider: TrackedCL) => {
        ...
        if (changeToConsider.node) {
            ...
            for (let edge of edges) {
                ...
                if (PerforceContext.sameServerID(clToConsider.serverId, edge.target.serverID)) {
                    sameServerTargets.push(edge.target)
                }
                else {
        ...
        for (let i=0; i < changeToConsider.desc.entries.length; i++) {
            ...
            if (entry.action != 'move/delete') {
                for (let i=externalServerTargets.length-1; i>=0; i--) {
                    ...
                    if (externalTarget.stream.startsWith('//')) {
                        ...
                        const changes = await externalP4.changes(targetPath)
                ...
                if (runIntegrated) {
                    ...
                    if (integrated.length > 0) {
                        for (let integ of integrated) {
                            if (getStreamFromPath(integ.fromFile) === getStreamFromPath(integ.toFile)) {
                                continue  // same-stream integration — backout/revert, not a forward merge
                            }
        ...
        // Fallback for same-server targets not found via p4 integrated (e.g. conflict resolutions
        // where the user converted integrated files to edits, leaving no p4 integration records).
        sameServerTargets = sameServerTargets.filter(target =>
            !changeToConsider.destCLs.some(cl => { const n = changes.get(cl)?.node; return n && typeof n !== 'string' && n.stream === target.stream })
        )
        if (sameServerTargets.length > 0) {
            for (const entry of changeToConsider.desc.entries) {
                if (entry.action != 'move/delete') {
                    const srcStream = getStreamFromPath(entry.depotFile)
                    if (srcStream) {
                        for (let i=sameServerTargets.length-1; i>=0; i--) {
                            const targetStream = sameServerTargets[i].stream
                            if (targetStream.startsWith('//')) {
                                const targetPath = entry.depotFile.replace(srcStream, targetStream)
                                const targetCLs = await p4.changes(targetPath, {since: clToConsider.cl})
                                for (const targetCL of targetCLs) {
                                    if (targetCL.desc.includes(clToConsider.cl.toString())) {
                                        const newCL = {cl: targetCL.change, serverId: clToConsider.serverId}
                                        if (!changeToConsider.destCLs.some(cl => compareTrackedCL(cl, newCL))) {
                                            changeToConsider.destCLs.push(newCL)
                                            const existingChange = changes.get(newCL)
                                            if (existingChange) {
                                                existingChange.sourceCL = clToConsider
                                            }
                                            else {
                                                await gatherCLInfo(newCL, {parentCL: clToConsider})
                                            }
                                        }
                                        sameServerTargets.splice(i, 1)
                                        break
                                    }
                                }
                            }
        ...
        if (includeInResults) {
            ...
            const reconsiderInfo = changeToConsider.node ? buildReconsiderInfo(changeToConsider.node, graph, userTags, botFilter, logger) : undefined
            data.changes[`${clToConsider.cl}`] = {streamDisplayName, swarmLink, mergeMethod: changeToConsider.mergeMethod, sourceCL: changeToConsider.sourceCL?.cl, ...(reconsiderInfo && {reconsiderInfo})}
        }
    }
