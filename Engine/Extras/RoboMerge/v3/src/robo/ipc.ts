...
import { IPCControls, EdgeBotInterface, NodeBotInterface, ReconsiderArgs } from './bot-interfaces';
...
export class IPC {
	...
	private async trackChange(queryObj: any, tagsObj: any): Promise<OperationReturnType> {
		...
		this.ipcLogger.info(`${queryObj.who} initiated track changes for CL# ${trackedCL}`)
	...
	private logError(msg: string) {
		if (RoboArgs.isRunningFunctionalTests) {
	...
	private async doOperation(botname: string, nodeName: string, operation: string, query: Query, edgeName?: string)
		...
		case 'set_last_cl':
			...
			let prevCl = generalOpTarget.forceSetLastClWithContext(cl, query.who, query.reason, true, query.isSkip === 'true')
		...
		case 'reconsider':
			const argMatch = query.cl.match(/^\s*(\d+)\s*(.*)/)
			if (argMatch) {
			...
			// Parse #stomp from the command text. This flag is only valid in the reconsider
			// box — it is not in FLAGMAP so it cannot appear in a CL description, and it
			// is stripped here before the text reaches the description parser, so it is
			// never propagated to child CLs.
			const segments = (argMatch![2] || '').split('|')
			const reconsiderArgs: Partial<ReconsiderArgs> = {}
			const commandSegments: string[] = []
			for (const segment of segments) {
				const segTokens = segment.trim().split(/\s+/).filter(Boolean)
				const stompIdx = segTokens.findIndex(t => t.toLowerCase() === '#stomp')
				if (stompIdx !== -1) {
					if (stompIdx > 0 && /^#robomerge/i.test(segTokens[stompIdx - 1])) {
						return {statusCode: 400, message: '#stomp must not appear as an argument to a #robomerge command'}
					}
					segTokens.splice(stompIdx, 1)
					reconsiderArgs.additionalFlags = ['stomp']
				}
				const remaining = segTokens.join(' ')
				if (remaining) {
					commandSegments.push(remaining)
				}
			}
			const commandText = commandSegments.join(' | ')
			if (commandText) {
				reconsiderArgs.commandOverride = commandText
			}

			try {
				generalOpTarget.reconsider(query.who, cl, reconsiderArgs)
			}
			catch (err) {
				throw err
			}
			...
			if (stompVerification.success) {
				return {
					...
					message: JSON.stringify({
						...
						requestResult: stompVerification.requestResult,
