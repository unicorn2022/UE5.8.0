...
export class PerforceContext {
	...
	static async initServer(logger: ContextualLogger, serverID: string, serverDetails?: any) {
		...
		if (resp && resp.User) {
			...
			let serverConfig: ServerConfig = {
				...
				serverAddress,
				serverVersion,
				multiServerEnvironment: serversOutput.some(server => server['Services'] == 'edge-server'),
	 ...
	 */
	changes(path_in: string, opts?: ChangesOpts): Promise<Change[]> {
		opts = opts || {}
		const rangeStr = opts.since ? (opts.until ? `${opts.since},${opts.until}` : `${opts.since},now`) : (opts.until ? `${opts.until}` : '')
		const range = rangeStr ? '@' + rangeStr : ''
		const path = path_in + range
		const args = ['changes', '-l',
			(opts.status ? `-s${opts.status}` : '-ssubmitted'),
			...(opts.limit ? [`-m${opts.limit}`] : []),
			path];

		return this.execAndParse(null, args, {quiet: opts.quiet||true}, {
	...
	async getWorkspaceEdgeServer(workspaceName: string): Promise<EdgeServer | null | false> {
		...
		if (serverIdLine) {
			...
			if (!address) {
				this.logger.warn(`No address record for edge server '${serverId}' (workspace '${workspaceName}') — cannot use this workspace`)
				return false
			}
	...
	async deleteWorkspace(workspaceName: string, edgeServerAddress?: string) {
		let result
		try {
			await this.execAndParse(null, ['client', '-d', workspaceName], {serverAddress: edgeServerAddress});
		}
		catch (reason) {
			if (!isExecP4Error(reason)) {
				throw reason
			...
			// delete succeeds with an error because ... sigh
			if (!output.trim().startsWith('Client') ||
				!output.trim().endsWith('deleted.')) {
				throw err
			}
			result = output;			
		}
	...
	async integrate(roboWorkspace: RoboWorkspace, source: IntegrationSource, dest_changelist: number, target: IntegrationTarget, opts?: IntegrateOpts)
		...
		// If we're doing a null merge then the branches and deletes need to be scheduled for resolve
		if (opts.nullMerge) {
			cmdList.push('-Rb')
			cmdList.push('-Rd')
		}
	...
	async revertFiles(files: string[], client: string, opts?: string[]) {
		...
		try {
			const results = await this.execAndParse(null, args, {serverAddress: edgeServer ? edgeServer.address : undefined})
	...
	getGroup(groupname: string) {
		// Verify it is actually a group, then return the details of it
		return this.execAndParse(null, ['groups', '-v', groupname], {quiet:true})
					.then(groups => {
						if (groups.length == 0) {
							return null
						}
						return this.execAndParseArray(null, ['group', '-o', groupname], {quiet:true})
					})
	}
	...
	async fstat(roboWorkspace: RoboWorkspace, depotPath: string|string[], quiet?: boolean) {
		if (typeof depotPath === "string") {
			depotPath = [depotPath]
		}
		try {
	...
	static _execP4(logger: ContextualLogger, roboWorkspace: RoboWorkspace, args: string[], optsIn?: ExecOpts) {
		...
		if (workspace && workspace.directory && !opts.noCwd) {
			if (!fs.existsSync(workspace.directory)) {
