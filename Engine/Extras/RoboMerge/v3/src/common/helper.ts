...
type ProcessAlertFn = (message: string) => Promise<void>
let _processAlertFn: ProcessAlertFn | null = null

export function setProcessAlertCallback(fn: ProcessAlertFn) {
	_processAlertFn = fn
}
...
process.on('unhandledRejection', async (reason, promise) => {
	...
	if (_processAlertFn) {
		...
		await _processAlertFn(`@here Robomerge process crashed (unhandled rejection): ${msg}\n\`\`\`${stack}\`\`\``)
	}
...
process.on('uncaughtException', async (err: Error) => {
	...
	if (_processAlertFn) {
		await _processAlertFn(`@here Robomerge process crashed (uncaught exception): ${err.message}\n\`\`\`${err.stack}\`\`\``)
	}
