...
export class WebServer {
	...
	public secure = false
	protected behindLoadBalancer = false

	temp_debug_id: number

	constructor(parentLogger: ContextualLogger, behindLoadBalancer?: boolean) {
		this.serverLogger = parentLogger.createChild('WebServer')
		this.behindLoadBalancer = behindLoadBalancer || false
	}
	...
	open(optPort?: number, protocol?: string, certFiles?: CertFiles) {
		...
		const isSelfSecure = protocol.toLowerCase() !== 'http'
		this.secure = this.behindLoadBalancer || isSelfSecure
		const port = optPort || (this.secure ? 443 : 80)


		if (this.secure && isSelfSecure) {
		...
		if (this.behindLoadBalancer) {
			this.serverLogger.info(`Web server secure behind loadbalancer, running as http but presuming secure.`)
		} else if (isSelfSecure) {
			this.serverLogger.info(`Web server using local installed certs for https.`)
		} else {
			this.serverLogger.warn(`Web server may be unsecure`)
		}
...
// 	this.handleCustomRequest(makeApp(req.path, req.cookies), response, req, prop, match),
	...
	private async onRequest(req: http.IncomingMessage, response: http.ServerResponse) {
		...
		for (const handler of this.handlers) {
			...
			if (match) {
				try {
					await handler.handler(response, {url: url, cookies: req.headers.cookie as string, reqData, headers: req.headers as Record<string, string | string[] | undefined>}, match)
				}
				catch (err) {
