...
export async function chooseBestWorkspaceForUser(p4: PerforceContext, user: string, stream?: string) {
	...
	if (workspaces.length > 0) {
		...
		let targetWorkspace: ClientSpec | undefined
		...
		if (stream) {
			...
			for (let def of workspaces) {
				const defStream = def.Stream ? def.Stream.toLowerCase() : ''
				const matchlen = def.Stream ? matchPrefix(defStream, branch_stream) : 0
				if (matchlen == branch_stream.length && matchlen == defStream.length) {
					if (await p4.getWorkspaceEdgeServer(def.client) !== false) {
						return def
					}
				}
				else if (matchlen > target_match || !targetWorkspace) {
					// better prefix match, or no reachable default found yet
					if (await p4.getWorkspaceEdgeServer(def.client) !== false) {
