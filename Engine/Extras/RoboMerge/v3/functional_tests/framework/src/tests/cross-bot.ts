...
function createStreams(p4: Perforce, test: FunctionalTest) {
	if (streamsPromise) {
		...
		return streamsPromise.then(
			() => Promise.all(['Main', 'Dev', 'RES', 'FNM', 'Plus', 'R26', 'R25', 'R25Dev'].map(s => {
...
export class CrossBotTest extends FunctionalTest {
	...
	async run() {
		...
		// ~dev: cross-bot manual merge — bot2 should shelf at Dev instead of auto-merging
		await P4Util.editFile(releaseClient, 'test9.txt', 'Initial content\n\nMergeable line')
		await P4Util.submit(releaseClient, `Edit\n#robomerge[${crossBot2!.botName}] ~dev`)
...
export class CrossBotTest2 extends FunctionalTest {
	...
	async verify() {
		await Promise.all([
			...
			this.checkHeadRevision('Dev', 'test9.txt', 1, DEPOT_NAME)  // ~dev: shelved, not submitted
		])

		// Confirm a pending/shelved CL exists in Dev workspace (from ~dev cross-bot manual)
		const devClient = this.getClient('Dev', 'testuser1', DEPOT_NAME)
		const pendingChanges = await devClient.changes(1, true)
		if (pendingChanges.length === 0) {
			throw new Error('Expected a shelved CL in Dev workspace from cross-bot ~dev but found none')
		}
	}
