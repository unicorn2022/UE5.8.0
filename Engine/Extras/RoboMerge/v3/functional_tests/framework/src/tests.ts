...
import { Approval, ApprovalNonBlocking } from './tests/approval'
...
import { SyntaxErrDeferDefaultBot, SyntaxErrDeferNonDefaultBot } from './tests/syntax-error-deferred-multibot'
import { ManualEdgeMerges } from './tests/manual-edge-merges'
...
async function go() {
	...
	const availableTests: FunctionalTest[] = [
		...
		new Approval(p4),
		new ApprovalNonBlocking(p4),  //55

		new SyntaxErrDeferDefaultBot(p4),
		new SyntaxErrDeferNonDefaultBot(p4),
		new ManualEdgeMerges(p4)
