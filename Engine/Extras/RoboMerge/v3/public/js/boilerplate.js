...
// Hash preservation for authentication flow
function preserveHashForAuth() {
	if (window.location.hash) {
		try {
			sessionStorage.setItem('robomerge_auth_hash', window.location.hash);
		} catch (e) {
			console.warn('Failed to preserve hash in sessionStorage:', e);
		}
	}
}
...
function makeClMenuButton(cl, swarmURL, options = {}) {
	// Handle non-numeric CLs - display as bold text without menu
	if (typeof(cl) !== "number") {
		return `<span style="font-weight:bolder">${cl}</span>`
	}

	// Extract options
	const alias = options.alias || cl
	const botname = options.botname || ''
	const nodename = options.nodename || ''
	const edgename = options.edgename || ''
	const allowSetLastCL = options.allowSetLastCL || false
	const allowSetGateCL = options.allowSetGateCL || false
	const customClass = options.customClass || ''
	const prefix = options.prefix || ''
	const tooltip = options.tooltip || ''
	const jobLink = options.jobLink || ''
	const fileHistoryLink = options.fileHistoryLink || ''

	// If no swarmURL, return plain text (no menu functionality)
	if (!swarmURL) {
		return `<span>${alias}</span>`
	}

	// Build data attributes for button
	let dataAttrs = `data-cl="${cl}" data-swarm-url="${swarmURL}" data-botname="${botname}" data-nodename="${nodename}" data-edgename="${edgename}"`

	if (jobLink) {
		dataAttrs += ` data-job-link="${jobLink}"`
	}

	// Store operation function and args if provided (for Set Last CL or Set Gate CL functionality)
	// Note: These will be retrieved from data attributes in event handlers
	if ((allowSetLastCL || allowSetGateCL) && options.operationFunction && options.operationArgs) {
		// Store a reference ID that can be used to retrieve the function from a global map
		const opId = `cl-op-${cl}-${Date.now()}`
		if (!window.clMenuOperations) {
			window.clMenuOperations = {}
		}
		window.clMenuOperations[opId] = {
			operationFunction: options.operationFunction,
			operationArgs: options.operationArgs
		}
		dataAttrs += ` data-operation-id="${opId}"`
	}

	// Build menu items
	let menuItems = ''

	// If there's a job link, add it as the first option
	if (jobLink) {
		menuItems += `<a class="dropdown-item" href="${jobLink}" target="_blank">
			<i class="fas fa-external-link-alt"></i> Open CIS Job
		</a>`
		menuItems += `<div class="dropdown-divider"></div>`
	}

	// Copy CL to Clipboard
	menuItems += `<a class="dropdown-item" href="#" data-action="copy-cl">
		<i class="fas fa-copy"></i> Copy CL to Clipboard
	</a>`

	// Open CL in Swarm
	menuItems += `<a class="dropdown-item" href="${swarmURL}/changes/${cl}" target="_blank">
		<i class="fas fa-external-link-alt"></i> Open CL in Swarm
	</a>`

	// Track Changes
	menuItems += `<a class="dropdown-item" href="#" data-action="track-changes">
		<i class="fas fa-search"></i> Track Changes
	</a>`

	// If there's a file history link (for gate CLs from file paths), add it
	if (fileHistoryLink) {
		menuItems += `<a class="dropdown-item" href="${fileHistoryLink}" target="_blank">
			<i class="fas fa-history"></i> View Gate File History
		</a>`
	}

	// Conditional: Set Last CL option
	if (allowSetLastCL) {
		menuItems += `<div class="dropdown-divider"></div>`
		menuItems += `<a class="dropdown-item" href="#" data-action="set-last-cl">
			<i class="fas fa-pencil-alt"></i> Set Last CL...
		</a>`
	}

	// Conditional: Set Gate CL option
	if (allowSetGateCL) {
		menuItems += `<div class="dropdown-divider"></div>`
		menuItems += `<a class="dropdown-item" href="#" data-action="set-gate-cl">
			<i class="fas fa-pencil-alt"></i> Set Gate CL...
		</a>`
	}

	// Build complete HTML structure
	const buttonClasses = customClass ? `btn btn-sm btn-secondary-alt ${customClass}` : 'btn btn-sm btn-secondary-alt'
	const wrapperClasses = customClass === 'last-good-cl' ? 'btn-group cl-menu-button gate-cl' : 'btn-group cl-menu-button'
...
function generateRobomergeFooter() {
	...
	trackChangeButton.click(function() {
		promptForAsync({
			...
			if (err) {
				console.error('Error in track change prompt:', err);
			}
		});
	})
...
function createPauseDivs(data, conflict) {
	...
	// Display deferral info if applicable
	if (data.deferred_cl) {
		const cl = data.deferred_cl
		divs.push($('<div class="info-block deferred">')
			.append(
				$('<p>').html(
					`<h6>Deferred on CL ${makeClMenuButton(cl, data.swarmURL)}</h6>` +
					`This bot is waiting for CL ${cl} to be evaluated by another bot ` +
					`before proceeding.<br>If this persists, check whether CL ${cl} ` +
					`has a syntax error.`
				)
			)
		)
	...
	if (data.is_blocked) {
		...
		divs.push($('<div class="info-block conflict">')
			.append(
				$('<p>').html('<h6>Blocked on change ' + makeClMenuButton(changelist, data.swarmURL) + `</h6> ${info}`)
			)
		)
	}
...
function postRenderNameCell_Node(nameCell, nodeData, includeCollapseControls, conflict=null) {
	...
	if (includeCollapseControls && nodeData.edges && Object.keys(nodeData.edges).length !== 0) {
		...
		collapseLink.attr('data-target', '#' + getNodeEdgesRowId(nodeData.bot, nodeData.def.name))
	...
	// Append Pause Info
	if (!nodeData.is_available || nodeData.deferred_cl) {
...
function preRenderStatusCell_Edge(nodeData, edgeData) {
	...
	if (edgeData.is_blocked) {
		...
		if (nodeData.is_paused || nodeData.is_blocked || nodeData.retry_cl || nodeData.deferred_cl) {
			const nodeLabel = nodeData.is_paused ? 'PAUSED' : (nodeData.deferred_cl && !nodeData.is_blocked) ? 'DEFERRED' : 'BLOCKED'
			const nodeColor = (nodeData.deferred_cl && !nodeData.is_blocked && !nodeData.is_paused) ? 'LIGHTBLUE' : 'ORANGE'
			$('<div>')
				.text('NODE ' + nodeLabel)
				.addClass('important-status')
				.css('color', nodeColor)
...
function renderActionsCell_Common(actionCell, data, operationFunction, operationArgs, conflict=null) {
	...
	if (data.is_blocked) {
		...
		if (conflict) {
			...
			}, conflict.disableStompReason ? conflict.disableStompReason : `Use ${conflict.cl} to stomp binary changes in ${conflict.target}`);
			if (conflict.disableStompReason) {
	...
	if (!data.is_paused) {
		...
		const pauseOption = createActionOption(`Pause ${dataDisplayName}`, function() {
			promptForAsync({
				...
				if (err) {
					console.error('Error in pause prompt:', err);
				}
			});
	...
	if (optEdgeName || window.allowNodeReconsiders) {
		...
		const reconsiderOption = createActionOption(`Reconsider...`, function() {
			showReconsiderDialog(`Enter the CL to reconsider (should be a CL from ${dataDisplayName}):`, '').then(data => {
				// Ensure they entered a CL
				if (isNaN(parseInt(data.cl))) {
				...
				if (err) {
					console.error('Error in reconsider prompt:', err);
				}
			});
...
function renderLastChangeCell_Common(botname, nodename, lastCl, swarmURL, operationFunction, operationArgs, catchupText=null, edgename=null) {
	...
	$(makeClMenuButton(lastCl, swarmURL, {
		botname: botname,
		nodename: nodename,
		edgename: edgename,
		allowSetLastCL: true,
...
function postRenderLastChangeCell_Edge(botname, lastChangeCell, edgeData, operationFunction, operationArgs) {
	if (edgeData.lastGoodCL) {
		...
		// Determine if gate is settable (only if lastGoodCLPath is a string, not a number)
		const isGateSettable = typeof edgeData.lastGoodCLPath === 'string'

		// Build file history link if lastGoodCLPath is a string (file path)
		let fileHistoryLink = ''
		if (typeof edgeData.lastGoodCLPath === 'string') {
			// Remove leading '//' from depot path for Swarm URL, same as makeSwarmFileLink
			const pathForSwarm = edgeData.lastGoodCLPath.startsWith('//')
				? edgeData.lastGoodCLPath.slice(2)
				: edgeData.lastGoodCLPath
			fileHistoryLink = `${edgeData.swarmURL}/files/${encodeURI(pathForSwarm)}#commits`
		}

		// Use CL menu button with custom styling to maintain green gate appearance
		$(makeClMenuButton(edgeData.lastGoodCL, edgeData.swarmURL, {
			botname: botname,
			nodename: edgeData.display_name,
			edgename: edgeData.displayName,
			allowSetGateCL: isGateSettable && !edgeData.lastGoodCLJobLink,
			operationFunction: operationFunction,
			operationArgs: operationArgs,
			customClass: 'last-good-cl',
			prefix: '\u{2713} ',
			tooltip: tooltip,
			jobLink: edgeData.lastGoodCLJobLink || '',
			fileHistoryLink: fileHistoryLink
		})).appendTo(lastChangeCell)
	}
}
...
if (!onLoginPage) {
	...
	// Preserve hash before potential auth redirect
	preserveHashForAuth();
}

/*********************************
 * CL MENU BUTTON EVENT HANDLERS *
 *********************************/

// Global event handlers for CL menu button actions using jQuery delegation
$(document).on('click', '.cl-menu-button .dropdown-item[data-action="copy-cl"]', function(e) {
	e.preventDefault();
	const button = $(this).closest('.btn-group').find('button');
	const cl = button.data('cl');

	if (navigator.clipboard && navigator.clipboard.writeText) {
		navigator.clipboard.writeText(cl.toString()).catch(() => {
			// Fallback: show CL in alert
			alert(`Failed to copy. CL number: ${cl}`);
		});
	} else {
...
$(document).on('click', '.cl-menu-button .dropdown-item[data-action="track-changes"]', function(e) {
	e.preventDefault();
	const button = $(this).closest('.btn-group').find('button');
	const cl = button.data('cl');

	// Track changes page URL
	let trackUrl = `/trackchange/${cl}`;

	// Open in new tab
	window.open(trackUrl, '_blank');
});

$(document).on('click', '.cl-menu-button .dropdown-item[data-action="set-last-cl"]', function(e) {
	e.preventDefault();
	const button = $(this).closest('.btn-group').find('button');
	const cl = button.data('cl');
	const botname = button.data('botname');
	const nodename = button.data('nodename');
	const edgename = button.data('edgename');
	const opId = button.data('operation-id');

	// Retrieve the operation function and args from global map
	if (!window.clMenuOperations || !window.clMenuOperations[opId]) {
		displayErrorMessage('Set Last CL operation not available');
		return;
	}

	const { operationFunction, operationArgs } = window.clMenuOperations[opId];

	// Use async modal dialog
	promptForAsync({
		cl: {prompt: 'Enter CL', default: cl}
	}).then(data => {
		if (data) {
			data.reason = "manually set through Robomerge homepage";

			// Call the operation function with the arguments
		...
		if (err) {
			console.error('Error in set last CL prompt:', err);
		}
	});
});

$(document).on('click', '.cl-menu-button .dropdown-item[data-action="set-gate-cl"]', function(e) {
	e.preventDefault();
	const button = $(this).closest('.btn-group').find('button');
	const cl = button.data('cl');
	const botname = button.data('botname');
	...
	// Retrieve the operation function and args from global map
	if (!window.clMenuOperations || !window.clMenuOperations[opId]) {
		displayErrorMessage('Set Gate CL operation not available');
		return;
	}
	...
	promptForAsync({
		...
		if (data) {
			...
			operationFunction(...operationArgs, "/set_gate_cl?" + toQuery(data), function(success) {
				if (success) {
					updateBranchList(botname);
					displaySuccessfulMessage(`Successfully set gate for ${edgename} to changelist ${data.cl}`);
				} else {
					displayErrorMessage(`Error setting gate for ${edgename} to changelist ${data.cl}, please check logs.`);
					$('#helpButton').trigger('click');
				}
			});
		}
		...
		if (err) {
			console.error('Error in set gate CL prompt:', err);
		}
	});
});
