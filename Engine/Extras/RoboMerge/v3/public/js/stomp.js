    ...
    getBranch(requestedBotName, requestedBranchName, function(data) {
        try {
            ...
            $('.changelist').each(function() {
                $(this).html(makeClMenuButton(requestedBranchCl, requestedNode.swarmURL))
            })
            ...
            verifyOperation.done(function(success) {
                ...
                if (stompJson.requestResult !== 'Invalid') {
                    $('#afterVerificationResultText').html('Stomp Verification Complete')

                    if (stompJson.requestResult === 'LimitedInfo') {
                        $('#afterVerificationResultText').append(
                            $('<h4 class="text-center text-warning">').text(
                                "Limited Information Mode - Review Warnings Before Proceeding"
                            )
                        )
                    } else {
                        $('#afterVerificationResultText').append(
...
function visualizeChangelist(changelist, changelistData, swarmURL) {
    ...
    if (robomergeUser && changelistData.author.toLowerCase() === robomergeUser.userName.toLowerCase()) {
        changelistDiv.append($('<h3>').html(`${makeClMenuButton(changelist, swarmURL)} by <strong>you</strong>`))
    } else {
        changelistDiv.append($('<h3>').html(`${makeClMenuButton(changelist, swarmURL)} by ${changelistData.author}`))
    }
...
function displayLimitedInfoModeWarning(requestedBranchCl, stompJson) {
    const linkToSwarm = makeClMenuButton(requestedBranchCl, stompJson.swarmURL, { alias: `${requestedBranchCl}` })

    $('#limitedInfoModeWarning').removeClass('initiallyHidden')
    $('#limitedInfoClMessage').html(`CL ${linkToSwarm} exceeds the analysis limit.`)
}

function visualizeStompVerification(requestedBranchCl, stompJson) {
    // Check for limited info mode
    if (stompJson.requestResult === 'LimitedInfo') {
        displayLimitedInfoModeWarning(requestedBranchCl, stompJson)
        return
    }
    ...
    if (!stompJson.remainingAllBinary) {
        ...
        $('#resultVisualization').append(
            $('<h5 class="centered-text">').html(`Robomerge cannot proceed with the stomp request using CL ${linkToSwarm}. The following changelists contain unstompable changes due to the presence of text files:`)
        )
    } 
    ...
    if (stompJson.requestResult !== 'Invalid') {
        ...
        // Check if we have any changelists to display
        const sortedKeys = Object.keys(clDict).sort()

        if (sortedKeys.length > 0) {
            // Display valid request with changelists
            $('#resultVisualization').append(
                $('<h5 class="centered-text">').html(
                    `The following changelists will be affected after stomping changes with files from CL ${linkToSwarm}:`
                )
            )

            // Display each changelist
            sortedKeys.forEach(function(key) {
                $('#resultVisualization').append(visualizeChangelist(key, clDict[key], stompJson.swarmURL))
            })
        } else {
            // No changelists to display
            $('#resultVisualization').append(
                $('<h5 class="centered-text">').html(
                    `Unable to calculate which, if any, changelists will be affected by stomping changes with files from CL ${linkToSwarm}`
                )
            )
        }
    }
