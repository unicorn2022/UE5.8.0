...
class Graph {
    ...
    addBranch(branchStatus, group = null, title = null) {
        ...
        if (branch.aliases && branch.aliases.length !== 0) {
            const displayAliases = branch.aliases.map(a => a === branch.upperName ? branchStatus.display_name : a);
            tooltip += ` (${displayAliases.join(', ')})`;
        }
        const info = new Info(`_${branchStatus.bot}_${branch.upperName.replace(/[^\w]+/g, '_')}`, tooltip, branchStatus.display_name, [branchStatus]);
        info.title = title
            ...
            if (streams.length > 1) {
                ...
                for (const branchStatus of streams) {
                    ...
                    if (branchStatus.def.aliases && branchStatus.def.aliases.length !== 0) {
                        const displayAliases = branchStatus.def.aliases.map(a => a === branchStatus.def.upperName ? branchStatus.display_name : a);
                        tooltip += ` (${displayAliases.join(', ')})`;
                    }
