...
function showFlowGraph(data, args, addlegend) {
    ...
    renderGraph(lines.join('\n'))
        ...
        // scale graph to 70% of default size, but never smaller than MIN_GRAPH_HEIGHT
        // (single-node graphs render tiny without a floor)
        const MIN_GRAPH_HEIGHT = 60;
        const height = Math.max(Math.round(parseInt(svgEl.attr('height')) * .7), MIN_GRAPH_HEIGHT);
